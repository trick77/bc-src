...
use bcrust_core::funcs::distance;
use bcrust_core::mining::primitives::{
    get_exp_factor_diff, get_new_block_count, get_new_pre_exp_diff,
};
use bcrust_core::protos::miner::{MinerRequest, MinerResponse, MinerResponseResult};
use bcrust_core::protos::miner_grpc::{Miner, MinerServer};
                ...
                loop {
                    {
                        ...
                        let different_work_id =
                            *work_id_current.lock().unwrap() != p.get_work_id().to_string();
                    ...
                    let new_block_count = get_new_block_count(
                        p.get_last_previous_block().get_blockchain_headers(),
                        p.get_new_block_headers(),
                    );
                    let new_pre_exp_diff =
                        get_new_pre_exp_diff(ts, last_previous_block, new_block_count);

                    let threshold =
                        get_exp_factor_diff(new_pre_exp_diff, last_previous_block.get_height());

                    let nonce_hash =
                        blake2b(64, &[], nonce.to_string().as_bytes()).as_bytes()[32..64].to_hex();

                    let in_str = format!("{}{}{}{}", miner, merkle_root, nonce_hash, ts);

                    let result_candidate =
                        blake2b(64, &[], in_str.as_bytes()).as_bytes()[32..64].to_hex();
...
fn main() {
    ...
    let _server = server.build().expect("server 1");
