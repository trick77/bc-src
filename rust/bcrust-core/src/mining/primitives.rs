...
use crate::funcs::xor_hashes;
use crate::mining::traits::RoveredBlockLike;
use crate::protos::core::{BcBlock, BlockchainHeader, BlockchainHeaders};
...
pub fn block_hash(msg: &RoveredBlockLike) -> String {
    ...
    blake2b(64, &[], payload.as_bytes()).as_bytes()[32..64].to_hex()
}
...
pub fn get_children_block_hashes(blockchain_headers: &BlockchainHeaders) -> Vec<String> {
    ...
    headers_list.iter().map(|h| block_hash(h)).collect()
}
...
pub fn get_new_block_count(
    previous_headers: &BlockchainHeaders,
    current_headers: &BlockchainHeaders,
) -> u8 {
...
pub fn get_diff(
    ...
    let elapsed_time = BigInt::from(current_block_time)
        - BigInt::from(previous_block_time)
        - BigInt::from(stale_cost);
    let elapsed_time_bonus =
        elapsed_time.clone() + (elapsed_time.clone() - (7 * new_block_count as u64));
...
pub fn get_new_pre_exp_diff(
    current_timestamp: u64,
    last_previous_block: &BcBlock,
    new_block_count: u8,
) -> String {
    get_diff(
        current_timestamp,
...
mod tests {
    ...
    const CORRECT_HASH_EMPTY_STRING_B: &str =
        "d25e1031afee585313896444934eb04b903a685b1448b755d56f701afe9be2ce";

    #[test]
    fn block_hash_blockchain_header_test() {
        assert_eq!(
            block_hash(&BlockchainHeader::new()),
            CORRECT_HASH_EMPTY_STRING_B.to_string()
        );
    }
    ...
    fn get_children_block_hashes_test() {
        ...
        assert_eq!(
            get_children_block_hashes(&headers),
            vec![CORRECT_HASH_EMPTY_STRING_B.to_string()]
        )
    }

    #[test]
    fn get_children_root_hash_test() {
        let hash_a = blake2b(64, &[], "a".as_bytes()).as_bytes()[32..64].to_hex();
        let hash_b = blake2b(64, &[], "b".as_bytes()).as_bytes()[32..64].to_hex();
        let hashes = &[hash_a, hash_b];

        const CORRECT_A_B_CHILDREN_ROOT: &str =
            "930af0a9ddc0507cc4b3b719f434109d2a894286119b98f4ff77ee7ff358735e";
        assert_eq!(
            get_children_root_hash(hashes),
            CORRECT_A_B_CHILDREN_ROOT.to_string()
        )
    }
    ...
    fn get_exp_factor_diff_test() {
        assert_eq!(
            get_exp_factor_diff(String::from("1024"), 1),
            "1024".to_string()
        );
        assert_eq!(
            get_exp_factor_diff(String::from("290112262029012"), 66000000 * 3),
            "290112262029014".to_string()
        )
    }
