...
const BC_MINIMUM_DIFFICULTY = !isNaN(process.env.BC_MINIMUM_DIFFICULTY) ? BigInt(process.env.BC_MINIMUM_DIFFICULTY) : BigInt(288112262029662);
const BC_STAT_SAMPLE_COUNT = min(3, Math.floor(BC_MINER_WORKERS / 2));

const knownWork = new LRUCache({
    ...
    worker.on('message', data => {
      if (data.type === 'getWork') {
        ...
        if (knownWork.has(data.workId)) {
          ...
          knownWork.del(data.workId);
          return;
        } else {
          knownWork.set(data.workId, true);
        }
        if (radianPerformance) {
          data.data.radianPerformance = radianPerformance;
        }
        if (!data.sampleData) {
          process.send({
            type: 'solution',
  ...
  setInterval(() => {
    if (stats.length >= 3) {
      ...
      if (formattedMetric !== undefined && formattedMetric > 0) {
        ...
        radianPerformance = `${formattedMetric} ${sizeUnit}RAD/s`;
        if (addr) {
          console.table([{
