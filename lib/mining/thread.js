...
const BC_MINIMUM_DIFFICULTY = !isNaN(process.env.BC_MINIMUM_DIFFICULTY) ? BigInt(process.env.BC_MINIMUM_DIFFICULTY) : BigInt(298112262029662);
...
if (cluster.isMaster) {
  ...
  setInterval(() => {
    if (stats.length >= 5) {
      ...
      const coreCountAdjustment = distancePerRadianSecond * workerLimit;
      ...
      if (Number(formattedMetric) > 1000) {
        formattedMetric = Number(formattedMetric) / 1000;
        sizeUnit = 'm';
      }

      if (Number(formattedMetric) > 100000) {
    ...
    } else if (stats.length > 0 && lastStat !== stats.length) {
      lastStat = stats.length;
      globalLog.info('\n\n  ' + 'sampling PoD performance in radians <- ' + stats.length + '/5\n');
    }
  ...
  const main = () => {
    process.on('message', ({
      ...
      if (sampleData) {
        ...
        difficulty = BC_MINIMUM_DIFFICULTY;
      }
