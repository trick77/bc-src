...
function getDiff(currentBlockTime, previousBlockTime, previousDifficulty, minimalDifficulty, newBlockCount, newestChildHeader) {
  ...
  let elapsedTimeBonus = elapsedTime + (elapsedTime - BigInt(10) + BigInt(newBlockCount));
...
const prepareChildBlockHeadersMap = async (previousBlock, newChildBlock, newChildHeaders, persistence) => {
  ...
  Object.keys(newBlockchainHeaders.toObject()).map(async listKey => {
    ...
    if (newlyAssignedBlocks.length === 0) {
      ...
      if (persistence) {
        const diskBlock = await persistence.getBlockByHash(headerFromPreviousBlock.getHash(), chain);
        if (diskBlock) {
          const confirmations = headerFromPreviousBlock.getBlockchainConfirmationsInParentCount();
          logger.info(`loaded disk block ${diskBlock.getHeight()}`);
          diskBlock.setBlockchainConfirmationsInParentCount(confirmations);
          diskBlock.setTimestamp(lastHeaderFromPreviousBlock.getTimestamp());
          headerFromPreviousBlock = diskBlock;
        }
      }
 ...
 */
const prepareNewBlock = exports.prepareNewBlock = async (currentTimestamp, lastPreviousBlock, newChildHeaders, blockWhichTriggeredMining, newTransactions, minerAddress, unfinishedBlock, persistence) => {
  ...
  const newMerkleRoot = createMerkleRoot(concatAll([blockHashes, newTransactions.map(tx => tx.getHash()), [finalDifficulty, minerAddress, newHeight, GENESIS_DATA.version, GENESIS_DATA.schemaVersion, 2, // NRG Grant
