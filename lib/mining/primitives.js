...
exports.childrenHighestBlockProto = childrenHighestBlockProto;
 ...
 *
 *      PART 4: Create Overline Block Hash  [COMPLETE]
 *
 */
...
const BC_INDEX_WORK = process.env.BC_INDEX_WORK === 'true';
...
let WS = false;
let HEADER_COLUMN = false;
if (BC_INDEX_WORK) {
  WS = require('fs').createWriteStream('mining_index.csv');
}

const minerLogMetrics = msg => {
  if (BC_INDEX_WORK) {
    // create header column
    if (!HEADER_COLUMN) {
      HEADER_COLUMN = true;
      WS.write(Object.keys(msg).join(',') + '\n');
    }
    WS.write(Object.values(msg).join(',') + '\n');
  }
};
...
const BC_MINIMUM_DIFFICULTY_AT = exports.BC_MINIMUM_DIFFICULTY_AT = !isNaN(process.env.BC_MINIMUM_DIFFICULTY_AT) ? BigInt(process.env.BC_MINIMUM_DIFFICULTY_AT) : BigInt(306612262029662);
const BC_MINIMUM_DIFFICULTY_OT = exports.BC_MINIMUM_DIFFICULTY_OT = !isNaN(process.env.BC_MINIMUM_DIFFICULTY_OT) ? BigInt(process.env.BC_MINIMUM_DIFFICULTY_OT) : BigInt(300612262029662);
const BC_MINIMUM_DIFFICULTY_OR = exports.BC_MINIMUM_DIFFICULTY_OR = !isNaN(process.env.BC_MINIMUM_DIFFICULTY_OR) ? BigInt(process.env.BC_MINIMUM_DIFFICULTY_OR) : BigInt(308612262029662);
...
function getDiff(currentBlockTime, previousBlockTime, previousDifficulty, minimalDifficulty, newBlockCount, newestChildHeader) {
  ...
  // BT PERIOD
  const BT_A = 1587132412;
  const BT_B = 1590157894;
  const BT_C = 1590283479;
  const BT_D = 1590337524;
  const BT_E = 1591666381;
  const BT_F = 1596864102;
  const BT_G = 1597558249;
  const BT_H = 1591666380;
  // AT PERIOD
  const AT_A = 1625431234;
  const AT_B = 1640266692;
  const AT_C = 1642981050;
  const AT_D = 1644536250;
  // WIRELESS TRANSMITION MINIMUM
  const WIREMIN_A = 1647116144;
  const WIREMIN_B = 1647126841;
  const WIREMIN_C = 1654509179;
  const WIREMIN_D = 1654635758;

  let bigBcMinimumDifficulty = BC_MINIMUM_DIFFICULTY;
  if (previousBlockTime > WIREMIN_A) {
    bigBcMinimumDifficulty = BC_MINIMUM_DIFFICULTY_AT;
  }
  ...
  if (previousBlockTime === BT_A || previousBlockTime === 1587132449) {
    return 298678954710059;
  }
  if (previousBlockTime === BT_B || previousBlockTime === 1590132950) {
    return 291678954710059;
  }
  if (previousBlockTime === BT_C) {
    return 291678954710059;
  }
  if (previousBlockTime === BT_D) {
    return 291678954710059;
  }
  if (previousBlockTime === BT_E) {
    return 291678954710059;
  }
  if (previousBlockTime === BT_F) {
    return 291678954710059;
  }
  if (previousBlockTime === BT_G) {
    return 291678954710059;
  }
  if (previousBlockTime === AT_A) {
  ...
  if (staleCost > big0) {
    ...
    // update for OT mine
    if (previousBlockTime <= BT_H) {
      elapsedTime = elapsedTimeHold;
    }
  }

  if (previousBlockTime > AT_B && elapsedTime > BigInt(36) && BigInt(newBlockCount) > BigInt(1)) {
    if (previousBlockTime > AT_D && elapsedTime > BigInt(86) && BigInt(newBlockCount) > BigInt(1)) {
      elapsedTime = elapsedTime * BigInt(newBlockCount);
    } else if (previousBlockTime < AT_C) {
      elapsedTime = elapsedTime * BigInt(newBlockCount);
    }
  } else {
    debug(`previous block time is not above AT_B`);
  }

  let w = 0;
  let elapsedTimeBonus = elapsedTime + (elapsedTime - BigInt(staleWeight) + BigInt(newBlockCount));

  if (previousBlockTime >= 1646773191 && elapsedTime > BigInt(86)) {
    w = elapsedTimeBonus; // original elapsted time
    elapsedTimeBonus = elapsedTime * BigInt(newBlockCount);
  }

  debug('time bonus  ' + Number(elapsedTimeBonus));
  elapsedTime = elapsedTimeBonus;

  if (elapsedTime > BigInt(16155)) {
    elapsedTime = BigInt(16155);
    elapsedTimeBonus = BigInt(16155);
  }
  ...
  // x < -99 ? -99 : x
  if (previousBlockTime >= 1659273394) {
    z = x;
  } else if (BigInt(x) < bigMinus99) {
    z = x;
    if (previousBlockTime < 1647022731) {
      x = bigMinus99;
    }
    if (previousBlockTime > WIREMIN_A) {
      x = bigMinus99;
  ...
  if (previousBlockTime > AT_D) {
    if (x < big0) {
      if (previousBlockTime >= WIREMIN_A) {
        n = BigInt(1615500) - elapsedTimeBonus; // <-- AT
      } else if (previousBlockTime >= 1647022731) {
        if (previousBlockTime >= 1647039583) {
          n = BigInt(161550000) - elapsedTimeBonus; // <-- AT
        } else {
          n = BigInt(1615500) - elapsedTimeBonus; // <-- AT
        }
        if (n < 0) {
      ...
      if (previousBlockTime >= WIREMIN_A) {
        n = BigInt(1215500) - elapsedTimeBonus; // <-- AT
      } else if (previousBlockTime >= 1647039583) {
        n = BigInt(161550000) - elapsedTimeBonus; // <-- AT
      } else {
  ...
  if (BC_INDEX_WORK) {
    minerLogMetrics({
      newBlockCount,
      bigChildHeaderTime,
      bigChildHeaderTimeBound,
      currentBlockTime,
      previousBlockTime,
      newBlockCount,
      bigPreviousDifficulty,
      staleCost,
      bigChildHeaderTime,
      elapsedTime,
      elapsedTimeBonus,
    ...
    console.log('----------------------------- bigChildHeaderTimeBound: ' + bigChildHeaderTimeBound);
    ...
    console.log('----------------------------- bigPreviousDifficulty: ' + bigPreviousDifficulty);
    console.log('----------------------------- staleCost: ' + staleCost);
    console.log('----------------------------- bigChildHeaderTime: ' + bigChildHeaderTime);
    console.log('----------------------------- elapsedTime: ' + elapsedTime);
    console.log('----------------------------- elapsedTimeBonus: ' + elapsedTimeBonus);
    console.log('----------------------------- w: ' + w);
    console.log('----------------------------- m: ' + m);
    console.log('----------------------------- n: ' + n);
    console.log('----------------------------- x: ' + x);
  ...
  // move to oRouter threshold
  if (x < bigMinimalDifficulty && previousBlockTime > WIREMIN_D) {
    return BC_MINIMUM_DIFFICULTY_OR;
  }

  if (x < bigMinimalDifficulty && previousBlockTime > WIREMIN_C) {
    return BC_MINIMUM_DIFFICULTY_OT;
  }

  if (x < bigMinimalDifficulty && previousBlockTime > WIREMIN_B) {
    return BC_MINIMUM_DIFFICULTY_AT;
  }

  // original difficulty minimum during CPU age
...
function childrenHighestBlockProto(block) {
  const allHeaders = blockchainMapToList(block.getBlockchainHeaders());
  const sets = {};
  ...
  allHeaders.map(h => {
    ...
    if (!sets[blockchain]) {
      ...
      sets[blockchain].push(h);
    }
  });

  const highest = Object.values(sets).reduce((all, headers) => {
...
const prepareNewBlock = exports.prepareNewBlock = async (currentTimestamp, lastPreviousBlock, newChildHeaders, blockWhichTriggeredMining, newTransactions, minerAddress, unfinishedBlock, persistence, nrgGrant) => {
  ...
  let childHeaders = blockchainMapToList(childBlockHeaders);
  const prevChildHeaders = blockchainMapToList(lastPreviousBlock.getBlockchainHeaders());

  const highestPrevChildHeaders = childrenHighestBlockProto(lastPreviousBlock);
  const mergedHeaders = highestPrevChildHeaders.reduce((all, prevHeader) => {
    const inNewBlock = childHeaders.reduce((has, newHeader) => {
      if (has) return true;
      if (newHeader.getBlockchain() === prevHeader.getBlockchain() && newHeader.getHeight() >= prevHeader.getHeight()) {
        return true;
      }
      return has;
    }, false);
    ...
    if (!inNewBlock) {
      highestPrevChildHeaders.map(header => {
        if (header.getBlockchain() === prevHeader.getBlockchain()) {
          header.setBlockchainConfirmationsInParentCount(prevHeader.getBlockchainConfirmationsInParentCount() + 1);
          childHeaders.push(header);
          all.push(header.getBlockchain());
        }
      });
    }
  ...
  if (mergedHeaders.length > 0) {
    logger.info(`merged headers: ${mergedHeaders.join(',')}`);
  }

  childHeaders = childHeaders.sort((a, b) => {
  ...
  const newChildHeadersMap = groupBy(block => block.getBlockchain(), childHeaders);
  const newBlockchainHeaders = new BlockchainHeaders();
  // construct new BlockchainHeaders from newChildHeaders
  toPairs(newChildHeadersMap).forEach(([chain, blocks]) => {
    newBlockchainHeaders[chainToSet(chain)](blocks.map(block => {
      const c = block.getBlockchainConfirmationsInParentCount ? block.getBlockchainConfirmationsInParentCount() : 1;
      return copyHeader(block, c);
    }));
  });

  const blockHashes = getChildrenBlocksHashes(blockchainMapToList(newBlockchainHeaders));
  const newChainRoot = getChildrenRootHash(blockHashes);
  const newBlockCount = parseInt(lastPreviousBlock.getHeight(), 10) === 1 ? 1 : getNewBlockCount(lastPreviousBlock.getBlockchainHeaders(), newBlockchainHeaders);
  ...
  newBlock.setSchemaVersion(2); // merge mining support enabled
  ...
  newBlock.setBlockchainHeaders(newBlockchainHeaders);
