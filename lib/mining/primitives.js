...
const prepareChildBlockHeadersMap = async (previousBlock, newChildBlock, newChildHeaders, persistence) => {
  ...
  Object.keys(newBlockchainHeaders.toObject()).forEach(async listKey => {
    ...
    if (newlyAssignedBlocks.length === 0) {
      ...
      if (persistence) {
        ...
        if (diskBlock !== null && diskBlock.getHash) {
          const confirmations = lastHeaderFromPreviousBlock.getBlockchainConfirmationsInParentCount() + 1;
          logger.info(`loaded disk block ${diskBlock.getHeight()}, confirmations: ${confirmations}`);
          diskBlock = copyHeader(diskBlock, confirmations);
          logger.info(Object.keys(diskBlock));
          logger.info('--------');
          logger.info(Object.keys(headerFromPreviousBlock));
  ...
  logger.info(`prepareChildBlockHeadersMap: previous BC block: ${previousBlock.getHeight()} final headers: ${inspect(Object.values(newBlockchainHeaders.toObject()), { depth: 3 })}`);
