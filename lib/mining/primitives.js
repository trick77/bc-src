...
const BC_MINIMUM_DIFFICULTY = exports.BC_MINIMUM_DIFFICULTY = !isNaN(process.env.BC_MINIMUM_DIFFICULTY) ? BigInt(process.env.BC_MINIMUM_DIFFICULTY) : BigInt(298112262029662);
...
function getDiff(currentBlockTime, previousBlockTime, previousDifficulty, minimalDifficulty, newBlockCount, newestChildHeader) {
  ...
  const bigBcMinimumDifficulty = BigInt(BC_MINIMUM_DIFFICULTY);
  ...
  if (newestChildBlock.timestamp === undefined) {
    if (newestChildBlock.toObject) {
      ...
      console.log(`using default child cost`);
  ...
  const bigChildHeaderTimeBound = bigChildHeaderTime + bigTargetTimeWindow * BigInt(2);
  ...
  const elapsedTimeBonus = elapsedTime + (elapsedTime - BigInt(10) + BigInt(newBlockCount)); // <-- AT
...
function mine(currentTimestamp, work, miner, merkleRoot, threshold, difficultyCalculator, emitter, sampleData = false) {
  ...
  let difficultyBN = new BN(difficulty).lt(new BN(BC_MINIMUM_DIFFICULTY)) ? new BN(BC_MINIMUM_DIFFICULTY) : new BN(difficulty);
  let result = 0;
  ...
  const inc = Fiber(e => {
    ...
    while (true) {
      ...
      nowms = ts.now();
      now = nowms / 1000 << 0;
      const nonce = String(Math.abs(Random.engines.nativeMath()));
      ...
      if (iterations > 100000 && sampleData) {
        res = {
          iterations,
          timeDiff: nowms - tsStart
        };
...
function prepareNewBlock(currentTimestamp, lastPreviousBlock, newChildHeaders, blockWhichTriggeredMining, newTransactions, minerAddress, unfinishedBlock) {
  ...
  const newMerkleRoot = createMerkleRoot(concatAll([blockHashes, newTransactions.map(tx => tx.getHash()), [finalDifficulty, minerAddress, newHeight, GENESIS_DATA.version, GENESIS_DATA.schemaVersion, GENESIS_DATA.nrgGrant, GENESIS_DATA.blockchainFingerprintsRoot]]));
