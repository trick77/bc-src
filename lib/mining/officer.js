...
class MiningOfficer {
  ...
  constructor(pubsub, persistence, workerPool, txPendingPool, chainState, engineEmitter, options) {
    ...
    this._lastMinerCallTimes = [];
    this._lastMinerCall = false;
  ...
  async newRoveredBlock(rovers, block, blockCache, areRoversSynced, blockLruCache, emblemPerformance = 0) {
    ...
    if (BC_MINER_MUTEX) {
      ...
      if (bcmm !== 'open') {
        this._logger.info('[] <- [] ' + 'ROVER-BLOCKED BY MUTEX ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 64) + ' <- EMB MINING POWER +' + emblemPerformance + '% OL (' + radPower + ' RAD/s)');
        return;
      }
    ...
    if (!BC_MINER_BOOT && block && synced && synced !== 'complete') {
      this._logger.info('[] <- [] ' + 'ROVER ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 64) + ' <- EMB MINING POWER +' + emblemPerformance + '% OL (' + radPower + ' RAD/s)');
      return;
    } else if (BC_MINER_BOOT !== false && block && synced && synced !== 'complete') {
      this._logger.info('[] <- [] ' + 'ROVER (BCMB: TRUE)' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 64) + ' <- EMB MINING POWER +' + emblemPerformance + '% OL (' + radPower + ' RAD/s)');
      await this._persistence.put(`${BC_SUPER_COLLIDER}.sync.initialsync`, 'complete');
      synced = 'complete';
    } else if (block) {
      this._logger.info('[] <- [] MINER EDGE <- ' + 'ROVER ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 64) + ' <- EMB MINING POWER +' + emblemPerformance + '% OL (' + radPower + ' RAD/s)');
    }
  ...
  async startMining(rovers, block, givenWorkId) {
    ...
    //try {
    let indexrebuilt = await this.persistence.get(`${BC_SUPER_COLLIDER}.indexrebuilt`);
    ...
    if (!indexrebuilt || indexrebuilt !== 'complete') {
      this._logger.info(`[] -> [] MINING yielded until index rebuilt`);
      return;
    }
    ...
    if (mbs.length > 0) {
      debug(`requesting from rovers ${mbs.join(',')}`);
      ...
      if (rmraw) {
        ...
        if (diff >= 135) {
          ...
          for (let rov of s) {
            ...
            const highest = min(sr.highest, sr.lowest + 80);
          ...
          let roversCalled = 0;
          if (mbs.indexOf('neo') > -1 && diff > 20) {
            roversCalled++;
            const rov = 'neo';
            const sr = missingBlocks[rov];
            const highest = min(sr.highest, sr.lowest + 30);
            await this.persistence.put(`${rov}.range.lowest.height`, sr.lowest);
            await this.persistence.put(`${rov}.range.highest.height`, highest);
            this._logger.info(`requesting ${rov} rover search for ${sr.lowest} -> ${highest}`);
            this.pubsub.publish('rover.request', {
          ...
          if (mbs.indexOf('lsk') > -1 && diff > 20) {
            roversCalled++;
            const rov = 'lsk';
            const sr = missingBlocks[rov];
            const highest = sr.lowest + LSK_MAX_FETCH_BLOCKS + 9;
            await this.persistence.put(`${rov}.range.lowest.height`, sr.lowest);
            await this.persistence.put(`${rov}.range.highest.height`, highest);
            this._logger.info(`requesting ${rov} rover search for ${sr.lowest} -> ${highest}`);
            this.pubsub.publish('rover.request', {
          ...
          if (mbs.indexOf('wav') > -1 && diff > 30) {
            roversCalled++;
            const rov = 'wav';
            const sr = missingBlocks[rov];
            const highest = min(sr.highest, sr.lowest + 20);
            await this.persistence.put(`${rov}.range.lowest.height`, sr.lowest);
            await this.persistence.put(`${rov}.range.highest.height`, highest);
            this._logger.info(`requesting ${rov} rover search for ${sr.lowest} -> ${highest}`);
            this.pubsub.publish('rover.request', {
          ...
          if (mbs.indexOf('eth') > -1 && diff > 20) {
            const rov = 'eth';
            const sr = missingBlocks[rov];
            const highest = min(sr.highest, sr.lowest + ETH_MAX_FETCH_BLOCKS + 10);
            await this.persistence.put(`${rov}.range.lowest.height`, sr.lowest);
            await this.persistence.put(`${rov}.range.highest.height`, highest);
            this._logger.info(`requesting ${rov} rover search for ${sr.lowest} -> ${highest}`);
            this.pubsub.publish('rover.request', {
    ...
    if (bestChain.length < 1) {
      ...
      if (synced && synced === 'pending') {
        this._logger.info(`mining work has pending sync status before building work`);
        return;
      }
    ...
    if (BC_GRPC_MINER) {
      ...
      let avg = false;
      const workSubmissionMs = Date.now();
      let elapsed = 0;
      if (!this._lastMinerCall) {
        this._lastMinerCall = workSubmissionMs;
      } else {
        elapsed = workSubmissionMs - this._lastMinerCall;
        this._lastMinerCall = workSubmissionMs;
        this._lastMinerCallTimes.push(elapsed);
      }

      if (this._lastMinerCallTimes.length > 50) {
        const average = this._lastMinerCallTimes.reduce((a, b) => a + b) / this._lastMinerCallTimes.length;
        avg = average;
        this._lastMinerCallTimes.shift();
      }

      this._logger.info(`[] -> [] OVERLINE MINER -> ${this._minerKey} with work id: ${workId} ${elapsed}ms avg: ${avg}, total: ${this._lastMinerCallTimes.length}`);
      ...
      this._rpc.miner.mine(minerRequest, (err, response) => {
        ...
        if (response.getResult() === MinerResponseResult.CANCELED) {
          debug(`mining restarted for ${workId} because of new work`);
          return;
        }
  ...
  async _handleWorkerFinishedMessage(solution) {
    ...
    const synced = await this._persistence.get(`${BC_SUPER_COLLIDER}.sync.initialsync`);
    if (synced && synced === 'pending') {
      this._logger.info(`mining work has pending sync status before processing ${workId}`);
    }
    ...
    const { nonce, distance, timestamp, difficulty, iterations, timeDiff } = solution;
    const schemaVersion = unfinishedBlock.getSchemaVersion();
    ...
    this._logger.info(' schema version: ' + schemaVersion);
    ...
    if (BC_MINER_BOOT) {
      ...
      if (reorgBlock && reorgBlock.getHeight && !BC_MINER_BOOT && parseInt(reorgBlock.getHeight(), 10) > parseInt(unfinishedBlock.getHeight(), 10) + 1) {
        if (synced === 'pending') {
          ...
          const waypoint = await this._persistence.processPeerExpiration();
          if (waypoint && waypoint.getAddress) {
            this._logger.info(`multiverse state is edge is local: ${reorgBlock.getHeight()}, setting sync status to 'pending'`);
          } else {
            return;
          }
        }
