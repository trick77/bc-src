...
class MiningOfficer {
  ...
  async newRoveredBlock(rovers, block, blockCache, areRoversSynced, blockLruCache, emblemPerformance = 0) {
    ...
    if (BC_LOW_POWER_MODE || this._relayMode) {
      this._logger.info(`[] -> [] NOT MINING "BC_LOW_POWER_MODE=TRUE"`);
      return true;
    }
  ...
  async startMining(rovers, block, givenWorkId) {
    ...
    //try {
    const lastPreviousBlock = await this.persistence.get(`${BC_SUPER_COLLIDER}.block.last.utxoSaved`);
    ...
    if (!lastPreviousBlock) {
      const msg = `${BC_SUPER_COLLIDER}.block.last.utxoSaved not available on disk`;
  ...
  async _handleWorkerFinishedMessage(solution) {
    ...
    const multichainLatest = await this._persistence.get(`${BC_SUPER_COLLIDER}.block.last.utxoSaved`);
    ...
    let latestBlock = await this._persistence.get(`${BC_SUPER_COLLIDER}.block.last.utxoSaved`);
    ...
    if (BC_MINER_BOOT) {
      ...
      const reorgBlock = await this._persistence.get(`${BC_SUPER_COLLIDER}.block.reorgto`);
      if (reorgBlock && reorgBlock.getHeight && !BC_MINER_BOOT && parseInt(reorgBlock.getHeight(), 10) > parseInt(unfinishedBlock.getHeight(), 10)) {
