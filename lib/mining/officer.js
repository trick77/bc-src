...
class MiningOfficer {
  ...
  async startMining(rovers, block, givenWorkId) {
    ...
    if (!bestChain) {
      ...
      //let lb = await this._persistence.getBlockByHeight(lastPreviousBlock.getHeight())

      //if (!lb) {
      //  lb = await this._persistence.getBlockByHeight(lastPreviousBlock.getHeight() - 1)
      //}

      //if (!lb) {
      //  lb = await this._persistence.getBlockByHeight(lastPreviousBlock.getHeight() - 2)
      //}

      //if (!lb) {
      //  lb = await this._persistence.getBlockByHeight(lastPreviousBlock.getHeight() - 3)
      //}

      //if (lb) {
      //  await this._persistence.delBlock(lastPreviousBlock.getHash(), 0, BC_SUPER_COLLIDER)
      //  await this._persistence.del(`${BC_SUPER_COLLIDER}.block.latest`)
      //  await this._persistence.put(`${BC_SUPER_COLLIDER}.sync.edge`, lb.getHeight())
      //  await this._persistence.put(`${BC_SUPER_COLLIDER}.block.latest`, lb, { sync: true })
      //  this._logger.info(`attempting to restart on previous branch ${lastPreviousBlock.getHash()} to height ${lb.getHeight()}`)
      //  return
      //}
      return;
    }
  ...
  async _handleWorkerFinishedMessage(solution) {
    ...
    if (BC_MINER_BOOT) {
      ...
      if (reorgBlock && reorgBlock.getHeight && !BC_MINER_BOOT && parseInt(reorgBlock.getHeight(), 10) > parseInt(unfinishedBlock.getHeight(), 10) + 1) {
        this._logger.info(`solution is stale, multiverse state is changing to new edge: ${reorgBlock.getHeight()}, setting sync status to 'pending'`);
        await this._persistence.processPeerExpiration();
        await this._persistence.put(`${BC_SUPER_COLLIDER}.sync.initialsync`, 'pending');
      } else {
