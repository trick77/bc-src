...
const BC_NETWORK_AGING_RATE = exports.BC_NETWORK_AGING_RATE = process.env.BC_NETWORK_AGING_RATE ? parseInt(process.env.BC_NETWORK_AGING_RATE) : 1800000;
...
class MiningOfficer {
  ...
  async startMining(rovers, block, blockCache) {
    ...
    try {
      ...
      if (lastPreviousBlock && lastPreviousBlock.getHash) {
        this._logger.info(`latest edge: ${lastPreviousBlock.getHeight()} ${lastPreviousBlock.getHash()}`);
      }
      ...
      if (bestChain && bestChain[0] && bestChain[0].getHash && bestChain[0].getHash() === this._highestBestChainHash) {
        this._logger.info(`[] -> [] MINER collision active: ${bestChain.length}, rover search requests: ${Object.keys(missingBlocks).length}`);
        this._highestBestChainHash = "hal";
        return;
      }
      ...
      if (this._relayMode) {
        this._logger.info(`In relay mode - not mining. colliding blocks: ${bestChain.length}, missing links: ${Object.keys(missingBlocks).length}`);
        return true;
      }
