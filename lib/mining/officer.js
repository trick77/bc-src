...
class MiningOfficer {
  ...
  async newRoveredBlock(rovers, block, blockCache, areRoversSynced, blockLruCache, emblemPerformance = 0) {
    ...
    if (!BC_MINER_BOOT && block && synced && synced !== 'complete') {
      ...
      this._logger.info('[] <- [] MINING EDGE <- ' + 'ROVER ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 48) + ' <- EMB MINING POWER +' + emblemPerformance + '% OL (' + radPower + ' RAD/s)');
    }
