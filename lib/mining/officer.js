...
class MiningOfficer {
  ...
  async startMining(rovers, block, givenWorkId) {
      ...
      if (mbs.length > 0) {
        ...
        if (rmraw) {
          ...
          if (diff >= 220) {
            ...
            let s = shuffle(mbs).slice(0, 3);
            for (let rov of s) {
              let maxHeight = 512;
              const sr = missingBlocks[rov];
              const highest = max(sr.highest, sr.lowest + maxHeight);
            ...
            this._logger.info(`last rover request delayed for ${220 - diff} seconds...`);
          }
        } else {
          await this.persistence.put(`${BC_SUPER_COLLIDER}.rover.mutex`, ts);
        }
    ...
    if (false || BC_RUST_MINER) {
      ...
      this._logger.info(`[] -> [] MINER NRG -> ${this._minerKey} with work id: ${workId}`);
      ...
      this._rpc.miner.mine(minerRequest, async (err, response) => {
        ...
        // await this.persistence.del(`${BC_SUPER_COLLIDER}.miner.work`)
        if (mbs.length > 0) {
