...
class MiningOfficer {
  ...
  async _handleWorkerFinishedMessage(solution) {
    ...
    if (BC_MINER_BOOT) {
      ...
      this.persistence.putLatestBlock(unfinishedBlock, BC_SUPER_COLLIDER, { context: 'local', saveHeaders: false }).then(() => {
        this._logger.info(`announcing ${unfinishedBlock.getHeight()}}`);
