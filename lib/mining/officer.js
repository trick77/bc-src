...
class MiningOfficer {
  ...
  async newRoveredBlock(rovers, block, blockCache, areRoversSynced, blockLruCache, emblemPerformance = 0) {
    ...
    if (BC_MINER_MUTEX) {
      ...
      if (bcmm !== 'open') {
        this._logger.info('[] <- [] ' + 'ROVER-BLOCKED BY MUTEX ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 32) + ' <- Emblem (EMB) adding +' + emblemPerformance + '% NRG');
        return;
      }
    ...
    if (!BC_MINER_BOOT && block && synced && synced !== 'complete') {
      this._logger.info('[] <- [] ' + 'ROVER ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash() + ' <- Emblem (EMB) adding +' + emblemPerformance + '% NRG');
      if (BC_MINER_MUTEX) {
        await this._persistence.put(`${BC_SUPER_COLLIDER}.miner.mutex`, 'open');
      }
      return;
    } else if (BC_MINER_BOOT && block && synced && synced !== 'complete') {
      this._logger.info('[] <- [] ' + 'ROVER (BCMB: TRUE)' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash() + ' <- Emblem (EMB) adding +' + emblemPerformance + '% NRG');
      await this._persistence.put(`${BC_SUPER_COLLIDER}.sync.initialsync`, 'complete');
      await this._persistence.put(`${BC_SUPER_COLLIDER}.miner.mutex`, 'open');
      synced = 'complete';
    } else if (block) {
      this._logger.info('[] <- [] ' + 'ROVER ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 32) + ' <- Emblem (EMB) adding +' + emblemPerformance + '% NRG');
    }
  ...
  async startMining(rovers, block, givenWorkId, preventRoverRequest = false) {
    ...
    let { bestChain, missingBlocks } = await validRoverChain(this._knownRovers, this.persistence);
    if (!missingBlocks) {
      missingBlocks = {};
    }
    ...
    if (bestChain.length < 1) {
      ...
      this._logger.info(`multichain is at edge of global state`);
      if (!preventRoverRequest) {
        let s = mbs;
        for (let rov of s) {
          let maxHeight = 32;
      ...
      this._logger.info(`colliding edges ending at ${bestChain[0].getBlockchain()} block ${bestChain[0].getHeight()}`);
    }
    ...
    if (false || BC_RUST_MINER || BC_MINER_MUTEX) {
      ...
      this._logger.info(`last previous block distance: ${lastPreviousBlock.getDistance()}`);
      this._logger.info(`last previous block diff: ${lastPreviousBlock.getDifficulty()}`);
      this._logger.info(`last previous block height: ${lastPreviousBlock.getHeight()}`);
      this._logger.info(`current timestamp: ${currentTimestamp}`);
      this._logger.info(`current diff: ${newBlock.getDifficulty()}`);
      this._logger.info(`current workid: ${workId}`);

      this._logger.info(`[] -> [] MINER NRG -> ${this._minerKey}`);

      this._rpc.miner.mine(minerRequest, async (err, response) => {
        this._logger.info(`miner request ${minerRequest.getWorkId()} ended after ${ts.nowSeconds() - minerRequest.getCurrentTimestamp()}`);
        try {
          ...
          if (response.getResult() === MinerResponseResult.CANCELED) {
            ...
            //  return
            //}
            //return this.startMining(rovers, block, workId, true)
          }
    ...
    //  for (let rov of s) {
    //    const sr = missingBlocks[rov]
    //    const highest = min(sr.highest, sr.lowest + 32)
  ...
  async _handleWorkerFinishedMessage(solution) {
    ...
    if (BC_MINER_BOOT) {
      ...
      this._logger.info(`calling putBlock on ${unfinishedBlock.getHeight()}}`);
