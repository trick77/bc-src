...
class MiningOfficer {
  ...
  async newRoveredBlock(rovers, block, blockCache, areRoversSynced, blockLruCache, emblemPerformance = 0) {
    ...
    if (BC_MINER_MUTEX) {
      ...
      if (bcmm !== 'open') {
        this._logger.info('[] <- [] ' + 'ROVER-BLOCKED BY MUTEX ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 32) + ' <- EMBLEM PERF. +' + emblemPerformance + '% NRG');
      }
    }
    ...
    if (!BC_MINER_BOOT && block && synced && synced !== 'complete') {
      this._logger.info('[] <- [] ' + 'ROVER ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash() + ' <- EMBLEM PERF. +' + emblemPerformance + '% NRG');
      return;
    } else if (BC_MINER_BOOT && block && synced && synced !== 'complete') {
      this._logger.info('[] <- [] ' + 'ROVER (BCMB: TRUE)' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash() + ' <- EMBLEM PERF. +' + emblemPerformance + '% NRG');
      await this._persistence.put(`${BC_SUPER_COLLIDER}.sync.initialsync`, 'complete');
      synced = 'complete';
    } else if (block) {
      this._logger.info('[] <- [] ' + 'ROVER ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 32) + ' <- EMBLEM PERF. +' + emblemPerformance + '% NRG');
    }
  ...
  async startMining(rovers, block, blockCache) {
    ...
    try {
      ...
      if (BC_MINER_MUTEX) {
        await this._persistence.get(`${BC_SUPER_COLLIDER}.miner.mutex`, 'closed');
      }
      if (this._chainState) {
  ...
  async _handleWorkerFinishedMessage(solution) {
    ...
    this._persistence.putBlock(unfinishedBlock, 0, 'bc').then(async () => {
      if (BC_MINER_MUTEX) {
        await this._persistence.get(`${BC_SUPER_COLLIDER}.miner.mutex`, 'open');
      }
