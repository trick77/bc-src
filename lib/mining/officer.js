...
const BC_STALE_NETWORK_TIMEOUT = exports.BC_STALE_NETWORK_TIMEOUT = process.env.BC_STALE_NETWORK_TIMEOUT ? parseInt(process.env.BC_STALE_NETWORK_TIMEOUT) : 1200000;
...
class MiningOfficer {
  ...
  async printState() {
    ...
    const values = keys.reduce((all, a, i) => {
      ...
      if (this._collectedBlocks[a] && this._collectedBlocks[a] === 0) {
        silentRovers.push(a);
      }
    ...
    if (this._startupTimeMS && nowMS - BC_STALE_NETWORK_TIMEOUT > this._startupTimeMS && silentRovers.length > 0) {
      this._startupTimeMS = false;
      this._logger.warn(`BC_STALE_NETWORK_TIMEOUT threshold reached. These rovers were unable to connect to their respective networks: ${silentRovers.join(' ')}`);
      for (let r of silentRovers) {
        const requiredBlockCount = Math.floor(ROVER_RESYNC_PERIOD / ROVER_SECONDS_PER_BLOCK[a]);
        this._collectedBlocks[r] = requiredBlockCount + 1;
      }
    } else if (this._startupTimeMS && nowMS - BC_STALE_NETWORK_TIMEOUT > this._startupTimeMS && silentRovers.length === 0) {
      this._startupTimeMS = false;
    }
    return report;
  }
  ...
  async startMining(rovers, block, blockCache) {
    ...
    try {
      ...
      //if (!(new BN(edge)).eq(new BN(lastPreviousBlock.getHeight())) &&
      //edge !== 1 &&
      //!(new BN(edge).eq(lastPreviousBlockHeightY)) &&
      //!(new BN(edge).eq(lastPreviousBlockHeightZ)) &&
      //!(new BN(edge).eq(lastPreviousBlockHeightB)) &&
      //!(new BN(edge).eq(lastPreviousBlockHeightC)) &&
      //!(new BN(edge).eq(lastPreviousBlockHeightD)) &&
      //edge !== 2) {
      //  this._logger.info(`waiting for multiverse edge ${edge} to synchronize with latest block ${lastPreviousBlock.getHeight()}`)
      //  return Promise.resolve(false)
      //}
