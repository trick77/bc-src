...
const { max, mean, merge, all, equals, values } = require('ramda');
...
class MiningOfficer {
  ...
  async newRoveredBlock(rovers, block, blockCache, areRoversSynced, blockLruCache, emblemPerformance = 0) {
    ...
    if (BC_MINER_MUTEX) {
      ...
      // regardless once it moves past set it to open again
    }
        ...
        this._rpc.miner.mine(minerRequest, async (err, response) => {
          if (err) {
            ...
            if (BC_MINER_MUTEX) {
              ...
              await this.startMining(rovers, block, workId);
            }
            return;
          }
          ...
          if (response.getResult() === MinerResponseResult.CANCELED) {
            ...
            if (BC_MINER_MUTEX) {
              return this._rpc.miner.mine(minerRequest, async (err, response) => {
                if (err) {
                  this._logger.error('native mining request failed', err);
                  return;
                }
          ...
          this._logger.debug('response from rust miner', response.toObject());
  ...
  async _handleWorkerFinishedMessage(solution) {
    ...
    if (workId !== undefined) {
      ...
      if (candidates.length > 0) {
        let unfinishedBlockObject = candidates.unshift();

        if (unfinishedBlockObject) {
    ...
    if (!isValidBlock(unfinishedBlock)) {
      this._logger.info(`candidate block is malformed`);
      this._cleanUnfinishedBlock();
      if (BC_MINER_MUTEX) {
        await this._persistence.put(`${BC_SUPER_COLLIDER}.miner.mutex`, 'open');
      }
