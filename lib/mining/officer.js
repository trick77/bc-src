...
class MiningOfficer {
  ...
  async newRoveredBlock(rovers, block, blockCache, areRoversSynced, blockLruCache, emblemPerformance = 0) {
    ...
    if (BC_MINER_MUTEX) {
      ...
      if (bcmm !== 'open') {
        this._logger.info('[] <- [] ' + 'ROVER-BLOCKED BY MUTEX ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 18) + '... <- EMBLEM (EMB) BOOST +' + emblemPerformance + '% OL');
        return;
      }
    ...
    if (!BC_MINER_BOOT && block && synced && synced !== 'complete') {
      this._logger.info('[] <- [] ' + 'ROVER ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 18) + '... <- EMBLEM (EMB) BOOST +' + emblemPerformance + '% OL');
      return;
    } else if (BC_MINER_BOOT && block && synced && synced !== 'complete') {
      this._logger.info('[] <- [] ' + 'ROVER (BCMB: TRUE)' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 18) + '... <- EMBLEM (EMB) BOOST +' + emblemPerformance + '% OL');
      await this._persistence.put(`${BC_SUPER_COLLIDER}.sync.initialsync`, 'complete');
      synced = 'complete';
    } else if (block) {
      this._logger.info('[] <- [] EDGE <- ' + 'ROVER ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 18) + '... <- EMBLEM (EMB) BOOST +' + emblemPerformance + '% OL');
    }
