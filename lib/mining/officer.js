...
class MiningOfficer {
  ...
  async _handleWorkerFinishedMessage(solution) {
    ...
    let latestBlock = await this._persistence.get('bc.block.last.utxoSaved');
    if (!latestBlock) latestBlock = await this._persistence.get(`${BC_SUPER_COLLIDER}.block.latest`);
