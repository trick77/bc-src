  ...
  async startMining(rovers, block, givenWorkId) {
    ...
    if (mbs.length > 0) {
      ...
      if (rmraw) {
        ...
        } else if (mbs.indexOf('lsk') > -1 && diff > 25) {
          const rov = 'lsk';
          const sr = missingBlocks[rov];
          const highest = min(sr.highest, sr.lowest + maxHeight);
          this._logger.info(`requesting ${rov} rover search for ${sr.lowest} -> ${highest}`);
        ...
        } else if (mbs.indexOf('wav') > -1 && diff > 25) {
          const rov = 'wav';
          const sr = missingBlocks[rov];
          const highest = min(sr.highest, sr.lowest + maxHeight);
          this._logger.info(`requesting ${rov} rover search for ${sr.lowest} -> ${highest}`);
        ...
        } else if (mbs.indexOf('neo') > -1 && diff > 25) {
          const rov = 'neo';
          const sr = missingBlocks[rov];
          const highest = min(sr.highest, sr.lowest + maxHeight);
          this._logger.info(`requesting ${rov} rover search for ${sr.lowest} -> ${highest}`);
    ...
    const [newBlock, finalTimestamp, triggerBlock] = await prepareNewBlock(currentTimestamp, lastPreviousBlock, bestChain, block, allTxs, this._minerKey, this._unfinishedBlock, this._persistence);
