...
class MiningOfficer {
  ...
  async startMining(rovers, block, givenWorkId) {
    ...
    if (!lastPreviousBlock) {
      const lbc = await this._persistence.getBlockByHeight(6882912, BC_SUPER_COLLIDER);
      if (lbc) {
    ...
    if (!bestChain) {
      ...
      const newLb = await this._persistence.getBlockByHash(lastPreviousBlock.getPreviousHash());
      ...
      if (newLb) {
        ...
        await this._persistence.put(`${BC_SUPER_COLLIDER}.sync.edge`, newLb.getHeight());
        await this._persistence.putLatestBlock(newLb, BC_SUPER_COLLIDER, { iterateUp: false });
        await this._persistence.put(`${BC_SUPER_COLLIDER}.block.latest`, newLb);
        this._logger.info(`attempting to restart on previous branch ${lastPreviousBlock.getHash()}`);
        return this.startMining(rovers, block, givenWorkId);
      }
    }
