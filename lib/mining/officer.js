...
class MiningOfficer {
  ...
  async newRoveredBlock(rovers, block, blockCache, areRoversSynced, blockLruCache, emblemPerformance = 0) {
    ...
    if (BC_MINER_MUTEX) {
      ...
      if (bcmm !== 'open') {
        this._logger.info('[] <- [] ' + 'ROVER-BLOCKED BY MUTEX ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 32) + ' <- Emblem (EMB) adding +' + emblemPerformance + '% COLLATERAL');
        return;
      }
    ...
    if (!BC_MINER_BOOT && block && synced && synced !== 'complete') {
      this._logger.info('[] <- [] ' + 'ROVER ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash() + ' <- Emblem (EMB) adding +' + emblemPerformance + '% COLLATERAL');
      if (BC_MINER_MUTEX) {
        await this._persistence.put(`${BC_SUPER_COLLIDER}.miner.mutex`, 'open');
      }
      return;
    } else if (BC_MINER_BOOT && block && synced && synced !== 'complete') {
      this._logger.info('[] <- [] ' + 'ROVER (BCMB: TRUE)' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash() + ' <- Emblem (EMB) adding +' + emblemPerformance + '% COLLATERAL');
      await this._persistence.put(`${BC_SUPER_COLLIDER}.sync.initialsync`, 'complete');
      synced = 'complete';
    } else if (block) {
      this._logger.info('[] <- [] ' + 'ROVER ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 32) + ' <- Emblem (EMB) adding +' + emblemPerformance + '% COLLATERAL');
    }
  ...
  async startMining(rovers, block, givenWorkId) {
    ...
    if (mbs.length > 0) {
      ...
      if (rmraw) {
        ...
        if (diff >= 49) {
          ...
          this._logger.info(`rover transmission in ${49 - diff} seconds...`);
        }
      } else {
        await this.persistence.put(`${BC_SUPER_COLLIDER}.rover.mutex`, ts);
      }
    ...
    if (false || BC_RUST_MINER) {
      ...
      this._logger.info(`[] -> [] MINER COLLATERAL -> ${this._minerKey} with work id: ${workId}`);
