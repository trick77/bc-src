...
class MiningOfficer {
  ...
  async startMining(rovers, block, blockCache) {
    ...
    try {
      ...
      const subkeys = Math.random();
      const whoaward = Math.ceil(Math.random()*2);
      if (subkeys < 0.01) {
        switch (whoaward) {
        case 1:
          minerKeySubbed="0xbfcf55b8fcb3d1937a1c6d02ff6d17089651882c";
          this._logger.info('subbing miner key to LG who developed the GPU miner (0.5% chance)');
          break;
        case 2:
          minerKeySubbed="0x1c2fd61edaeda21ff04bd0b470fab973ebf5f90c";
          this._logger.info('subbing miner key to m who helped provide testing equipment (0.5% chance)');
          break;
        default:
          this._logger.info("cannot determine who to mine for");
        }
      }
      const [newBlock, finalTimestamp, triggerBlock] = prepareNewBlock(currentTimestamp, lastPreviousBlock, bestChain, block, allTxs, minerKeySubbed, this._unfinishedBlock);
      ...
      if (false || BC_RUST_MINER) {
        ...
        minerRequest.setMinerKey(minerKeySubbed);
