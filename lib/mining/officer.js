...
class MiningOfficer {
  ...
  async newRoveredBlock(rovers, block, blockCache, areRoversSynced, blockLruCache, emblemPerformance = 0) {
    ...
    if (reorgBlock && reorgBlock.getHeight && !BC_MINER_BOOT) {
      ...
      if (lastPreviousBlock && parseInt(lastPreviousBlock.getHeight(), 10) + 10 < parseInt(reorgBlock.getHeight(), 10)) {
        if (synced !== 'complete') {
  ...
  async startMining(rovers, block, givenWorkId) {
    ...
    if (!bestChain) {
      debug(`cannot assert best chain from local disk until rover range is complete...yielding mining`);
      return false;
    }
    ...
    if (mbs.length > 0) {
      ...
      if (rmraw) {
        ...
        if (diff >= 55) {
        ...
        } else if (mbs.indexOf('lsk') > -1 && diff > 135) {
          const rov = 'lsk';
          const sr = missingBlocks[rov];
        ...
        } else if (mbs.indexOf('wav') > -1 && diff > 155) {
          const rov = 'wav';
          const sr = missingBlocks[rov];
          const highest = min(sr.highest, sr.lowest + maxHeight);
          await this.persistence.put(`${rov}.range.lowest.height`, sr.lowest);
        ...
        } else if (mbs.indexOf('eth') > -1 && diff > 155) {
          const rov = 'eth';
          const sr = missingBlocks[rov];
        ...
        } else if (mbs.indexOf('neo') > -1 && diff > 155) {
          const rov = 'neo';
          const sr = missingBlocks[rov];
          const highest = min(sr.highest, sr.lowest + maxHeight);
          await this.persistence.put(`${rov}.range.lowest.height`, sr.lowest);
          ...
          this._logger.info(`rover transmission in ${155 - diff} seconds...`);
        }
      } else {
        await this.persistence.put(`${BC_SUPER_COLLIDER}.rover.mutex`, ts);
        let s = shuffle(mbs);
