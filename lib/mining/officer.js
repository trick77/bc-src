...
class MiningOfficer {
  ...
  async newRoveredBlock(rovers, block, blockCache, areRoversSynced, blockLruCache, emblemPerformance = 0) {
    ...
    this._emitter.emit('collectBlock', { block });
    this._roverEmitter.emit('collectBlock', { block });
    if (this.pubsub && this.pubsub.publish) {
      this.pubsub.publish('rover.block', block);
    }
    ...
    try {
      ...
      if (bestChain.length < 1) {
        ...
        if (mbs.length > 0) {
          ...
          //if (mbs.indexOf('lsk') > -1) {
          //  s = ['lsk']
          //} else if (mbs.indexOf('neo') > -1) {
          //  s = ['neo']
          //} else if (mbs.indexOf('wav') > -1) {
          //  s = ['wav']
          //} else if (mbs.indexOf('eth') > -1) {
          //  s = ['eth']
          //} else if (mbs.indexOf('btc') > -1) {
          //  s = ['btc']
          //}
        ...
        if (bestChain.length < 2) {
          ...
          for (let rov of s) {
            ...
            this._logger.info(`opening ${s} rover search for ${sr.lowest} -> ${highest}`);
            this.pubsub.publish('rover.request', {
              type: 'rover.request',
              data: {
                rover: rov,
  ...
  async _handleWorkerFinishedMessage(solution) {
    ...
    if (BC_MINER_BOOT) {
      ...
      await this.persistence.putBlock(unfinishedBlock, 0, BC_SUPER_COLLIDER);
