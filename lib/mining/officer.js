...
const { childrenHeightSum, validateRoveredSequences, isValidBlock, validRoverChain } = require('../bc/validation');
...
class MiningOfficer {
  ...
  async newRoveredBlock(rovers, block, blockCache, areRoversSynced, blockLruCache, emblemPerformance = 0) {
    ...
    if (BC_MINER_MUTEX) {
      ...
      if (bcmm !== 'open') {
        this._logger.info('[] <- [] ' + 'ROVER-BLOCKED BY MUTEX ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 32) + ' <- EMBLEM (EMB) BOOST +' + emblemPerformance + '% OVERLINE');
        return;
      }
    ...
    if (!BC_MINER_BOOT && block && synced && synced !== 'complete') {
      this._logger.info('[] <- [] ' + 'ROVER ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 24) + '... <- EMBLEM (EMB) BOOST +' + emblemPerformance + '% OVERLINE');
      return;
    } else if (BC_MINER_BOOT && block && synced && synced !== 'complete') {
      this._logger.info('[] <- [] ' + 'ROVER (BCMB: TRUE)' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 24) + '... <- EMBLEM (EMB) BOOST +' + emblemPerformance + '% OVERLINE');
      await this._persistence.put(`${BC_SUPER_COLLIDER}.sync.initialsync`, 'complete');
      synced = 'complete';
    } else if (block) {
      this._logger.info('[] <- [] EDGE <- ' + 'ROVER ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 32) + '... <- EMBLEM (EMB) BOOST +' + emblemPerformance + '% OVERLINE');
    }
    ...
    if (reorgBlock && reorgBlock.getHeight && !BC_MINER_BOOT) {
      const lastPreviousBlock = await this._persistence.get(`${BC_SUPER_COLLIDER}.block.latest`);
      if (lastPreviousBlock && parseInt(lastPreviousBlock.getHeight(), 10) <= parseInt(reorgBlock.getHeight(), 10)) {
  ...
  async startMining(rovers, block, givenWorkId) {
    ...
    //try {
    const lastMinedBlock = await this.persistence.get(`${BC_SUPER_COLLIDER}.block.mine`);
    let lastPreviousBlock = await this.persistence.get(`${BC_SUPER_COLLIDER}.block.latest`);

    if (lastMinedBlock && parseInt(lastMinedBlock.getHeight(), 10) > parseInt(lastPreviousBlock.getHeight(), 10)) {
      this._logger.info(`miner climbing to previous sequence ${lastMinedBlock.getHeight()}:${lastMinedBlock.getHash().slice(0, 18)}...`);
      await this.persistence.del(`${BC_SUPER_COLLIDER}.block.mine`);
      lastPreviousBlock = lastMinedBlock;
    }
    ...
    if (!bestChain) {
      this._logger.info(`unable to assert best chain from local disk...yielding mining`);
      if (BC_MINER_MUTEX) {
        await this._persistence.put(`${BC_SUPER_COLLIDER}.miner.mutex`, 'open');
      }
      return;
    ...
    if (prevBlockHeightSum >= newBlockChildHeightSum) {
      this._logger.info(`newest block does not increase weight <- weight change: ${prevBlockHeightSum - newBlockChildHeightSum} yielding...`);
      return;
    }
    ...
    this._logger.info(`work to collide: ${workId}, block ${newBlock.getHeight()}, ol grant: ${nrgGrant}, radians: ${newBlock.getDifficulty()}`);
  ...
  async _handleWorkerFinishedMessage(solution) {
    ...
    if (BC_MINER_BOOT) {
      ...
      const reorgBlock = await this._persistence.get(`${BC_SUPER_COLLIDER}.block.reorgto`);
      if (reorgBlock && reorgBlock.getHeight && !BC_MINER_BOOT && parseInt(reorgBlock.getHeight(), 10) > parseInt(unfinishedBlock.getHeight(), 10) + 2) {
        this._logger.info(`solution is stale, multiverse state is changing to new edge: ${reorgBlock.getHeight()}`);
        //if (parseInt(reorgBlock.getHeight(), 10) < parseInt(unfinishedBlock.getHeight(), 10)) {
        //  this._logger.info(`assigning new block height despite multiverse stale state at ${unfinishedBlock.getHeight()}`)
      ...
      await this.persistence.put(`${BC_SUPER_COLLIDER}.block.mine`, unfinishedBlock);
