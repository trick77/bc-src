...
const { childrenHeightSum, validateRoveredSequences, isValidBlock, validRoverChain, VALIDATION_GAP_BC_HEIGHT_INTERVAL } = require('../bc/validation');
...
class MiningOfficer {
  ...
  async newRoveredBlock(rovers, block, blockCache, areRoversSynced, blockLruCache, emblemPerformance = 0) {
    ...
    if (!BC_MINER_BOOT && block && synced && synced !== 'complete') {
      ...
      this._logger.info('[] <- [] MINING EDGE <- ' + 'ROVER ' + block.getBlockchain() + ' block ' + block.getHeight() + ' ' + block.getHash().slice(0, 18) + '... <- EMBLEM (EMB)) BOOST +' + emblemPerformance + '% OL');
    }
    ...
    if (reorgBlock && reorgBlock.getHeight && !BC_MINER_BOOT) {
      ...
      if (lastPreviousBlock && parseInt(lastPreviousBlock.getHeight(), 10) < parseInt(reorgBlock.getHeight(), 10)) {
        this._logger.info(`multiverse edge evaluation is progress: ${reorgBlock.getHeight()}`);
      }
    }
