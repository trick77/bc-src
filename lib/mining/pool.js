...
class WorkerPool {
  ...
  async allRise({ minerKey, emblemPerformance }) {
    ...
    //const pool: ChildProcess = fork(MINER_POOL_WORKER_PATH, [], { execArgv: process.execArgv.filter(opt => opt !== '--inspect') })
  ...
  async _handlePoolMessage(msg) {
    ...
    if (msg === undefined) {
      ...
      if (msg.data !== undefined && msg.workId !== undefined) {
        ...
        if (msg && msg.data && msg.data.radianPerformance) {
          await this.persistence.put(`${BC_SUPER_COLLIDER}.miner.radians`, msg.data.radianPerformance);
        }
      }
