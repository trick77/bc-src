...
class Interpreter {
  ...
  async getScriptEnv(outputScript, inputScript, input, tx, block) {
    try {
      ...
      if (env.OUTPOINT_TX_BLOCK === false) {
        throw new Error(`unable to load referenced block containing tx ${env.OUTPOINT_HASH} of outpoint from block ${block.getHeight()}`);
      }
  ...
  async getBlockForTxHash(txHash, blockchain) {
    ...
    let block = await this.persistence.getBlockByTxHash(txHash);
    if (block && !block.getHash && block.length > 0) {
      // means there are multiple blocks at this height so return the first one found
      block = block[0];
    }
    if (block) {
      return block;
