...
class Interpreter {
  ...
  constructor(persistence) {
    ...
    this.chainState = persistence._chainState;
    this.blockCache = LRUCache({
      max: 200
    });
    this.txCache = LRUCache({
      max: 10000
    });
    this.scriptCache = LRUCache({
      max: 10000
    });
  }
  ...
  async getScriptMarkedTxs(script, env) {
    ...
    if (markedOperations.length === 0) {
      ...
      let markedTxs = await this.persistence.getMarkedTxsForMatchedTx(env.OUTPOINT_HASH, env.OUTPOINT_INDEX);
      return markedTxs;
      // }
    }
  }
  ...
  async getScriptEnv(outputScript, inputScript, input, tx, block) {
    try {
      ...
      if (env.OUTPOINT_TX_BLOCK === false) throw new Error('unable to load referenced block containing tx of outpoint');
      ...
      // if (!this.latestBlock) {
      //   this.latestBlock = await this.persistence.get(`${blockchain}.block.latest`)
      // } else {
      //   const cs = this.chainState.getLatestBlockHash(blockchain)
      //   if (cs && cs !== this.latestBlock.getHash()) {
      //     this.latestBlock = await this.persistence.get(`${blockchain}.block.latest`)
      //   } else if (!cs) {
      //     this.latestBlock = await this.persistence.get(`${blockchain}.block.latest`)
      //   }
      ...
      if (Validator.includesCallbackOpcode(outputScript) === true && env.SCRIPT.indexOf('OP_MONOID') < 0 === true) {
        ...
        const [callbackScript, callbackHeight, callbackTx, callbackTxIndex] = await this.persistence.getInitialMakerOrderWithTxAndIndex(outputScript);
        ...
        if (env.CALLBACK_TX !== null && env.CALLBACK_TX !== false) {
          ...
          const numer = parseFloat(internalToHuman(callbackOutput.getValue(), NRG));
          const denom = parseFloat(internalToHuman(env.OUTPOINT_TX.getOutputsList()[env.OUTPOINT_INDEX].getValue(), NRG));
          env.RATIO = numer / denom;
          ...
          if (callbackScript.indexOf('OP_MONOID') === 0) {
            ...
            let makerWithinSettlment = await this.persistence.isTxWithinSettlement(env.OUTPOINT_HASH, env.OUTPOINT_INDEX, true, env.INPUT_TX_BLOCK);
            let takerWithinSettlment = await this.persistence.isTxWithinSettlement(env.OUTPOINT_HASH, env.OUTPOINT_INDEX, false, env.INPUT_TX_BLOCK);
            env.SETTLED = !makerWithinSettlment && !takerWithinSettlment;
        ...
        let makerWithinSettlment = await this.persistence.isTxWithinSettlement(env.OUTPOINT_HASH, env.OUTPOINT_INDEX, true, env.INPUT_TX_BLOCK);
        env.SETTLED = !makerWithinSettlment;
      }
