      ...
      if (Validator.includesCallbackOpcode(outputScript) === true && env.SCRIPT.indexOf('OP_MONOID') < 0 === true) {
        ...
        if (env.CALLBACK_TX !== null && env.CALLBACK_TX !== false) {
          ...
          if (callbackScript.indexOf('OP_MONOID') === 0) {
            ...
            let isWithinSettlement = await this.persistence.isTxWithinSettlement(env.OUTPOINT_HASH, env.OUTPOINT_INDEX, env.INPUT_TX_BLOCK);
        ...
        let isWithinSettlement = await this.persistence.isTxWithinSettlement(env.OUTPOINT_HASH, env.OUTPOINT_INDEX, env.INPUT_TX_BLOCK, true);
        env.SETTLED = !isWithinSettlement;
      }
