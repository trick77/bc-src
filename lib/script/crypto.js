...
module.exports = {
  ...
  verifySignature(msg, signature, pubKey) {
    ...
    debug(`${msg.toString('hex')},${signature.slice(0, 64).toString('hex')},${pubKey.toString('hex')}`);
    return secp256k1.ecdsaVerify(new Uint8Array(signature.slice(0, 64)), new Uint8Array(msg), new Uint8Array(pubKey));
  },
  // see txUtils.pubKeyRecover - that one returns uncompressed version of the public key
  pubKeyFromSignature(msg, signature) {
    const pubKey = secp256k1.ecdsaRecover(new Uint8Array(signature.slice(0, 64)), bufferToInt(signature.slice(64)), new Uint8Array(msg));
