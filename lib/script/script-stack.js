...
const IS_MAKER_CHECK = process.env.IS_MAKER_CHECK === 'true';
...
class ScriptStack {
  ...
  OP_BLAKE2BLPRIV() {
    ...
    var address = pubKeyToAddrHuman(Buffer.from(secp256k1.publicKeyConvert(new Uint8Array(pubKey), false))).toString('hex');
  ...
  OP_MAKERCOLL() {
    ...
    if (!isNaN(makerToAddress)) {
      let addr = BigInt(makerToAddress).toString(16);
      if (addr.length > 40) {
        debug('Incorrect ETH address length');
      } else {
        makerToAddress = '0x' + new Array(40 - addr.length).fill('0').join('') + addr;
      }
    }

    if (!isNaN(makerFromAddress)) {
      let addr = BigInt(makerFromAddress).toString(16);
      if (addr.length > 40) {
        debug('Incorrect ETH address length');
      } else {
        makerFromAddress = '0x' + new Array(40 - addr.length).fill('0').join('') + addr;
      }
    }

    if (!isNaN(takerToAddress)) {
      let addr = BigInt(takerToAddress).toString(16);

      if (addr.length > 40) {
        debug('Incorrect ETH address length');
      } else {
        takerToAddress = '0x' + new Array(40 - addr.length).fill('0').join('') + addr;
      }
    }

    if (!isNaN(takerFromAddress)) {
      let addr = BigInt(takerFromAddress).toString(16);
      if (addr.length > 40) {
        debug('Incorrect ETH address length');
      } else {
        takerFromAddress = '0x' + new Array(40 - addr.length).fill('0').join('') + addr;
      }
    }
    ...
    receivesUnit = new _decimal.Decimal(receivesUnit).mul(this.env.RATIO).div(new _decimal.Decimal(this.env.BASE)).floor().toString();
    sendsUnit = new _decimal.Decimal(sendsUnit).mul(this.env.RATIO).div(new _decimal.Decimal(this.env.BASE)).floor().toString();
    ...
    var takerAmountSent = this.env.MARKED_TXS.reduce((sum, mtx) => {
      ...
      if (mtx.getAddrFrom().toLowerCase() === takerFromAddress.toLowerCase() && // confirm value is coming from maker address
      mtx.getAddrTo().toLowerCase() === makerToAddress.toLowerCase() && // confirm value is being sent to maker address
    ...
    var takerAmountRecieved = this.env.MARKED_TXS.reduce((sum, mtx) => {
      if (mtx.getAddrTo().toLowerCase() === takerToAddress.toLowerCase() && mtx.getAddrFrom().toLowerCase() === makerFromAddress.toLowerCase() && mtx.getToken() === makerFromChain) {
        return sum.add(new BN(mtx.getValue()));
      }
      return sum;
    }, new BN(0));

    debug({ takerAmountSent, takerAmountRecieved });
    debug({ takerAmountSent: takerAmountSent.toString(), takerAmountRecieved: takerAmountRecieved.toString() });
    debug({ sendsUnit, receivesUnit });

    if (takerAmountSent.gte(new BN(receivesUnit))) {
      takerSuccess = true;
    }

    if (takerAmountRecieved.gte(new BN(sendsUnit))) {
      makerSuccess = true;
    }
  ...
  OP_DEPSET() {
    ...
    if (!this.env) {
      ...
      var startBlock = this.env.CALLBACK_TX_BLOCK ? this.env.CALLBACK_TX_BLOCK : this.env.OUTPOINT_TX_BLOCK;
      var currentBlock = this.env.INPUT_TX_BLOCK ? this.env.INPUT_TX_BLOCK : this.env.LATEST_BLOCK;
  ...
  OP_MONADSPLIT() {
    ...
    if (this.env === undefined || this.env === false || this.env.OUTPOINT_TX === false) {
      ...
      debug({ monads });
      for (let m of monads) {
        debug(`monad ${toASM(Buffer.from(m.getOutputScript()), 0x01)} is ${new BN(m.getValue()).toString()}`);
      }
      ...
      debug({ greatestMonad, lowestMonad, sumMonads: sumMonads.toString() });
      ...
      debug({ lowest: lowestMonadValue.toString(), greatest: greatestMonadValue.toString() });
