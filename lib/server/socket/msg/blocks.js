...
const blockGet = (persistence, id) => {
  if (id === 'latest') {
    return persistence.get('bc.block.last.utxoSaved');
  }
  return persistence.getBlockByHeight(id);
};
...
module.exports = {
  get: (server, client, payload) => {
    ...
    blockGet(persistence, payload.data.id).then(block => {
      ...
      logger.debug('requesting block height from server');
      if (!firstBlock) {
        return [];
      }
      ...
      if (err) {
        logger.debug(`Unable to 'get.blocks', err: ${err.toString()}`);
      }
    });
  }
};