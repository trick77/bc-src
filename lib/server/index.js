...
const RPC_PORT = process.env.BC_RPC_PORT && parseInt(process.env.BC_RPC_PORT, 10) || config.server.rpcPort;
...
class Server {
  ...
  run(opts) {
    ...
    this.rpcApp.use(responseTime());
    this.rpcApp.use(bodyParser.json({ limit: '1mb' }));
    ...
    const mapping = {
      ...
      getSpendableOutpoints: GetBalanceRequest,
      getSpendableCollateral: GetSpendableCollateralRequest,
      getUnlockTakerTxParams: GetUnlockTakerTxParamsRequest,
      getOpenOrders: GetBalanceRequest,
      getMatchedOrders: GetBalanceRequest,
    ...
    // FIXME: move to rpc
    this.rpcApp.use('/rpc', cors({ origin: '*' }), (req, res, next) => {
    ...
    this._server = server;
    // Create https server for RPC
    // $FlowFixMe see https://github.com/facebook/flow/issues/5113
    const serverRpc = http.Server(this.rpcApp);
    this._serverRpc = serverRpc;
    ...
    serverRpc.listen(RPC_PORT, () => {
      this._logger.info(`RPC available at http://0.0.0.0:${RPC_PORT}`);
    });
  ...
  initRpc() {
    this.rpcApp.post('/rpc', (req, res, next) => {
  ...
  initUi() {
    ...
    this.app.use('/logs', express.static(logsDir), serveIndex(logsDir, {
      icons: true,
      view: 'details'
    }));

    // Serve static content
    this.app.use(express.static(assetsDir), serveIndex(assetsDir, {
      icons: true
    }));
