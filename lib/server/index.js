...
class Server {
  ...
  async run(opts) {
    ...
    const mapping = {
      ...
      const geores = this.engine.geoDbA.get(ip) ? this.engine.geoDbA.get(ip) : this.engine.geoDbB.get(ip);
  ...
  async _createNgrokTunnel() {
    ...
    this._ngrokTunnelAddress = await ngrok.connect(PORT); // FIXME configure properly
  ...
  _getPeers() {
    ...
    const geo = this._engine.geoDbA.get(ip) ? this._engine.geoDbA.get(ip) : this._engine.geoDbB.get(ip);
    const me = {
      ip,
      city: geo.city,
