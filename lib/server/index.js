...
const { Null, Block, Transaction } = require('@overline/proto/proto/core_pb');
const {
  GetFeedMessagesRequest,
  GetBlocksByRoveredHashRequest,
  TransferRequest,
  GetUtxosRequest,
  GetOutPointRequest,
  ...
  RpcFeedTransactionRequest,
  RpcUpdateFeedTransactionRequest,
  RpcTransaction
...
class Server {
  ...
  async run(opts) {
    ...
    const mapping = {
      getEphemeralFeedMessages: GetFeedMessagesRequest,
      getOriginalMakerOrder: GetOutPointRequest,
      getBlocksByRoveredHash: GetBlocksByRoveredHashRequest,
      ...
      getTakerForMaker: GetOutPointRequest,
      ...
      getOpenOrder: GetOutPointRequest,
      getMatchedOrder: GetOutPointRequest,
      getOpenCallbackOrder: GetOutPointRequest,
      ...
      newFeed: RpcFeedTransactionRequest,
      updateFeed: RpcUpdateFeedTransactionRequest,
      sendTx: Transaction,
      getCurrentWork: Null,
      getSettings: Null,
      getSyncStatus: Null,
  ...
  initWebSocket(server) {
    ...
    this.engine.pubsub.subscribe('feed.message', '<server>', data => {
      this._wsBroadcast({ type: 'feed.message', data: data.toObject() });
    });

    // TODO: this should be optional and off by default
    this.engine.pubsub.subscribe('sync.status', '<server>', data => {
      this._wsBroadcast({ type: 'sync.status', data: data });
    });

    this.engine.pubsub.subscribe('tx.evm', '<server>', data => {
      this._wsBroadcast({ type: 'tx.evm', data: data.toObject() });
    });
