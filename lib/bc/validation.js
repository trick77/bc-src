...
const getCoinbaseMissingBlockHeight = exports.getCoinbaseMissingBlockHeight = async (block, persistence, txHandler, txPendingPool, calledFrom) => {
  ...
  for (let b of blocksToAdd) {
    ...
    let validBlock = await isCoinbaseValid(b, persistence, txHandler, txPendingPool);
    if (validBlock === false) {
...
const validateCoinbase = exports.validateCoinbase = async (block, persistence, engine, txHandler, txPendingPool, calledFrom) => {
  ...
  if (engine.evalBlocks.length === 0) {
    ...
    let res = await updateUTXO(block, persistence, txHandler, txPendingPool, 'validate');
    ...
    return new Promise((resolve, reject) => {
      engine._utxoEventEmitter.once(`utxoUpdate.${block.getHash()}.${date}.${rand}`, async () => {
        let res = await updateUTXO(block, persistence, txHandler, txPendingPool, 'engine');
        let index = engine.indexBlocks.indexOf(`${block.getHash()}${date}${rand}`);
        engine.evalBlocks.splice(index, 1);
        engine.indexBlocks.splice(index, 1);
        let data = engine.evalBlocks[0];
...
const updateUTXO = exports.updateUTXO = async (block, persistence, txHandler, txPendingPool, calledFrom) => {
  if (!block || block.getTxsList === undefined) return false;
  ...
  if (isUpdating) {
    ...
    if (hash.length == 2) {
      ...
      if (blockSaved) {
        ...
        let oldSaved = await isCoinbaseValid(blockS, persistence, txHandler, txPendingPool);
  ...
  let updateUTXOsResult = await persistence.removeUTXOsFrom(block.getHeight());
  if (updateUTXOsResult.result) {
    for (const tx of updateUTXOsResult.tryAddingNewTxs) {
      txPendingPool.tryAddingNewTx(tx);
    }
...
const isCoinbaseValid = exports.isCoinbaseValid = async (block, persistence, txHandler, txPendingPool) => {
  ...
  let updateUTXOsResult = await persistence.updateUTXOs(block);
  if (updateUTXOsResult.result) {
