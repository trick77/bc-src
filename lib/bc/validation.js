...
const updateUTXO = exports.updateUTXO = async (block, persistence, txHandler, txPendingPool, utxoManager, calledFrom, parentBlock) => {
  ...
  if (higherBlock) {
    ...
    logger.info(`higher block ${higherBlock}`);
    return false;
  }
  // LDL
  //TD: debugUTXO(`check from ${block.getHeight() - 1}`)
  logger.info(`check from ${block.getHeight() - 1}`);
  let foundBlock = true;
  let prevHeight;
  let prevHash;
  ...
  for (let i = 0; i < 1000; i++) {
    ...
    debugUTXO(`while checking ${block.getHeight() - 1}, ${block.getPreviousHash()} saved state...`);
    if (parseInt(block.getHeight(), 10) === 1) break;
    let alreadySaved = await utxoManager.areUTXOsSavedForBlock(block.getHeight() - 1, block.getPreviousHash());
    ...
    if (alreadySaved) {
      ...
      if (block) {
        debugUTXO(`block ${block.getHeight() - 1}, ${block.getHash()} add to state...`);
        blocksToAdd.unshift(block);
        break;
      } else {
        ...
        //  blocksToAdd.unshift(block)
        //  debugUTXO(`block found by prev hash but not on disk: ${prevHash}`)
        //  //break
        //}
      }
    } else {
