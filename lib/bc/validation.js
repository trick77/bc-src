...
function isValidBlock(newBlock, opts = { fullBlock: true }) {
  if (newBlock === undefined) {
    logger.debug('candidate block is undefined');
    return false;
  }
  if (newBlock.getBlockchainHeaders === undefined) {
    logger.debug('candidate block has incomplete child headers references');
    // return false
  }
  // block soft opening limit of 60006 blocks
  if (new BN(newBlock.getHeight()).lt(new BN(100006))) {
    return true;
  }
  ...
  if (!isDistanceCorrectlyCalculated(newBlock)) {
    logger.debug('failed: isDistanceCorrectlyCalculated');
    return false;
  }
  // DISABLED until OT panelling
...
function blockchainHeadersAreChain(childHeaderList, parentHeaderList, mountBlockHeaderList) {
  ...
  if (!check) {
    logger.info(`child header list length: ${childHeaderList.length} parent header list length: ${parentHeaderList.length}`);
    logger.info('first condition %o %s %s', firstChildHeader.getPreviousHash() === lastParentHeader.getHash(), firstChildHeader.getPreviousHash(), lastParentHeader.getHash());
    logger.info('second condition %o %s %s', childHeaderList.length > 2, firstChildHeader.getPreviousHash(), lastParentHeader.getHash());
    logger.info('third condition %o %s %s', firstChildHeader.getHash() === lastParentHeader.getHash(), firstChildHeader.getHash(), lastParentHeader.getHash());
...
const validateCoinbase = exports.validateCoinbase = async (block, persistence, txHandler, calledFrom) => {
  ...
  while (true) {
    ...
    if (!block) {
      debug(`block not found removing simplex`);
