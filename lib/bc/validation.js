...
function isValidBlock(newBlock, opts = { fullBlock: true }) {
  ...
  if (!isChainRootCorrectlyCalculated(newBlock)) {
    debug('failed: isChainRootCorrectlyCalculated');
    return false;
  }
  ...
  if (!isValidBlockTime(newBlock)) {
    logger.warn(`block ${newBlock.getHeight()} failed: isValidBlockTime`);
    return false;
  }
...
function isValidBlockTime(newBlock) {
  ...
  if (newestHeaderTimestamp + 855000 < bcBlockTimestamp) {
