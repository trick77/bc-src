...
function isDistanceCorrectlyCalculated(newBlock) {
  ...
  logger.info(`isDistanceCorrectlyCalculated validation running on ${newBlock.getHeight()}.${newBlock.getHash()} expected work: ${expectedWork}`);
...
const getCoinbaseMissingBlockHeight = exports.getCoinbaseMissingBlockHeight = async (block, persistence, txHandler, calledFrom) => {
  ...
  logger.warn(`validating ${block.getHeight()}:${block.getHash()} from ${calledFrom}`);
  let date = Date.now();
...
const validateCoinbase = exports.validateCoinbase = async (block, persistence, txHandler, calledFrom) => {
  if (!block || block.getTxsList === undefined) return false;
  logger.info(`validating ${block.getHeight()}:${block.getHash()} from ${calledFrom}`);
  let date = Date.now();
  let alreadySaved = await persistence.areUTXOsSavedForBlock(block.getHeight(), block.getHash());
  debug(`check if utxos are already saved: ${alreadySaved}`);
  if (alreadySaved) return true;
  debug(`loading utxo simplex...`);
  const utxoSimplex = persistence._chainState.getSimplexCache();
  logger.info(`...utxo simplex is ${utxoSimplex} within ${block.getHash()}`);
  if (utxoSimplex) {
    debug(`utxo pruning in progress, yielding request for block ${block.getHeight()}, simplex is ${utxoSimplex}`);
    return false;
  } else {
  ...
  while (true) {
    // LDL
    debug(`while checking ${block.getHeight() - 1}`);
    if (parseInt(block.getHeight(), 10) === 1) break;
    let alreadySaved = await persistence.areUTXOsSavedForBlock(block.getHeight() - 1, block.getPreviousHash());
    if (alreadySaved) break;
    ...
    if (!block) {
      // LDL
      logger.info(`block ${prevHeight} : ${prevHash} not on local disk...`);
      persistence._chainState.delSimplexCache();
      return false;
    }
    blocksToAdd.unshift(block);
...
const validRoverChain = exports.validRoverChain = async (rovers, persistence) => {
  ...
  logger.info(`assembled multiverse from ${newBlockHeadersKeys.length} rovers with ${totalNewBlocks} connected chain blocks`);
  ...
  try {
    ...
    for (const key of newBlockHeadersKeys) {
      ...
      if (edgesFound[chain]) {
        ...
        const opts = await persistence.getBlocksByHeight(height, chain, { asHeader: true });
      ...
      let longestChains = Object.keys(longestChainsTable).reduce((all, chain) => {
        ...
        const scores = Object.keys(longestChainsTable[chain]).reduce((s, mount) => {
          ...
          for (let branch of Object.keys(longestChainsTable[chain][mount])) {
            ...
            debugRover(`p2. processing longest chains for ${chain} @ ${mount}, branch length ${branchOption.length}`);
        ...
        if (scores.length === 1) {
          ...
          for (let mountPoint of Object.keys(longestChainsTable[chain])) {
            for (let branch of longestChainsTable[chain][mountPoint]) {
              debugRover(`sorting available chain branchs for ${chain} - ${mountPoint.slice(0, 16)} ${branch.length}`);
              if (!best) {
                if (branch.length === 1 && branch[0].getHash() === lastHeaderInPreviousBlock.getHash()) {
                  chainList.push(branch);
                  continue;
                }
                best = branch;
              } else {
          ...
          if (!best) {
            debugRover(`unable to determine highest path forward discovered ${chain}`);
    ...
    for (const [i, block] of blocksFromDisk.entries()) {
      ...
      if (nextBlock && nextBlock.getPreviousHash() === block.getHash() && lastHeaderInPreviousBlock.getPreviousHash() === block.getPreviousHash() && lastHeaderInPreviousBlock.getHash() === block.getHash() && lastHeaderInPreviousBlock.getHeight() === block.getHeight()) {
        // debugRover(`- block ${block.getHeight()} HASH: ${block.getHash().slice(0, 8)} PREV: ${block.getPreviousHash()} matches the block already in the chain and there are more blocks to come`)
        continue;
        //return all
      }
      ...
      if (lastHeaderInPreviousBlock.getPreviousHash() === block.getPreviousHash() && lastHeaderInPreviousBlock.getHash() !== block.getHash() && nextBlock && nextBlock.getPreviousHash() === block.getHash()) {
        // debugRover(`+ block ${block.getHeight()} HASH: ${block.getHash().slice(0, 8)} PREV: ${block.getPreviousHash().slice(0, 8)} adding as initial Goldling`)
        if (register[bc] < BC_MAX_NEW_BLOCKS) {
          all.push(block);
        }
        continue;
      ...
      if (lastHeaderInPreviousBlock.getPreviousHash() !== block.getPreviousHash() && lastHeaderInPreviousBlock.getHash() !== block.getHash() && lastHeaderInPreviousBlock.getHeight() === block.getHeight() && nextBlock && nextBlock.getPreviousHash() === block.getHash()) {
        ...
        if (earlyHeaders) {
          debugRover(`+ added ${earlyHeaders.length} with ${block.getHeight()} HASH: ${block.getHash().slice(0, 8)} PREV: ${block.getPreviousHash().slice(0, 8)} adding bridging goldlings`);
          all = earlyHeaders.concat(all);
          continue;
        } else {
          debugRover(`+ unable to find headers block ${block.getHeight()} HASH: ${block.getHash().slice(0, 8)} PREV: ${block.getPreviousHash().slice(0, 8)} adding but cannot bridge goldlings`);
      ...
      if (previousBlock && previousBlock.getPreviousHash() === lastHeaderInPreviousBlock.getPreviousHash() && previousBlock.getHash() === block.getPreviousHash() && previousBlock.getHash() !== lastHeaderInPreviousBlock.getHash()) {
        debugRover(`- block ${block.getHeight()} HASH: ${block.getHash().slice(0, 8)} PREV: ${block.getPreviousHash().slice(0, 8)} NOT ADDING as GOLDLING detected`);
      ...
      if (!nextBlock && !previousBlock && block.getHash() === lastHeaderInPreviousBlock.getHash() && block.getPreviousHash() === lastHeaderInPreviousBlock.getPreviousHash()) {
        //all.push(block)
        debugRover(`- block ${block.getHeight()} HASH: ${block.getHash().slice(0, 8)} PREV: ${block.getPreviousHash().slice(0, 8)} already in chain and one child so not adding`);
        continue;
        //return all
      }
      ...
      if (nextBlock && nextBlock.getPreviousHash() === block.getHash() && block.getHash() === lastHeaderInPreviousBlock.getHash() && block.getPreviousHash() === lastHeaderInPreviousBlock.getPreviousHash()) {
        debugRover(`- block ${block.getHeight()} HASH: ${block.getHash().slice(0, 8)} PREV: ${block.getPreviousHash().slice(0, 8)} already in blockchain and part of upcoming sequence `);
        continue;
        //return all
      }
      ...
      if (previousBlock && block.getPreviousHash() === previousBlock.getHash()) {
        debugRover(`+ block ${block.getHeight()} HASH: ${block.getHash().slice(0, 8)} PREV: ${block.getPreviousHash().slice(0, 8)} extends blockchain sequence`);
      ...
      if (!previousBlock && lastHeaderInPreviousBlock.getHash() === block.getPreviousHash()) {
        ...
        if (nextBlock && nextBlock.getHeight() === block.getHeight() && nextBlock.getPreviousHash() === lastHeaderInPreviousBlock.getHash()) {
          debugRover(`- block ${block.getHeight()} HASH: ${block.getHash().slice(0, 8)} PREV: ${block.getPreviousHash().slice(0, 8)} blocking the for a potential Goldling`);
        } else {
          debugRover(`+ block ${block.getHeight()} HASH: ${block.getHash().slice(0, 8)} PREV: ${block.getPreviousHash().slice(0, 8)} extends last header as new child header block`);
          if (register[bc] < BC_MAX_NEW_BLOCKS) {
            all.push(block);
          }
        }
      ...
      if (!missingBlocks[block.getBlockchain()]) {
        missingBlocks[block.getBlockchain()] = {
          highest: parseInt(block.getHeight(), 10),
          lowest: parseInt(lastHeaderInPreviousBlock.getHeight(), 10) - 4
        };
      } else {
        // only update lowest
        if (missingBlocks[block.getBlockchain()].lowest > parseInt(block.getHeight(), 10)) {
      ...
      debugRover(`unable to process <-- block ${block.getHeight()} HASH: ${block.getHash().slice(0, 8)} PREV: ${block.getPreviousHash().slice(0, 8)}`);
      //return all
    }
    ...
    debugRover(`bestChain length is ${all.length}`);
    debugBestChain(`bestChain length is ${all.length}`);
