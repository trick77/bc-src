...
function isDistanceAboveDifficulty(newBlock) {
  logger.info(`isDistanceAboveDifficulty validation running on ${newBlock.getHeight()}`);
  ...
  logger.info(`receivedDistance ${receivedDistance}, receivedDifficulty ${receivedDifficulty}`);
...
const validRoverChain = exports.validRoverChain = async (rovers, persistence) => {
  ...
  try {
    ...
    for (const key of newBlockHeadersKeys) {
      ...
      if (edgesFound[chain] > 100 || blocksAdded[chain] > BC_MAX_NEW_BLOCKS * 2) {
        savedReads[chain]++;
    ...
    logger.info(`loaded - ${inspect(loaded)}`);
    logger.info(`failed - ${inspect(failed)}`);
    ...
    for (const [i, block] of blocksFromDisk.entries()) {
      ...
      if (previousBlock && previousBlock.getPreviousHash() === lastHeaderInPreviousBlock.getPreviousHash() && previousBlock.getHash() === block.getPreviousHash() && previousBlock.getHash() !== lastHeaderInPreviousBlock.getHash()) {
        ...
        console.log(`- ${block.getBlockchain()} block ${block.getHeight()} HASH: ${block.getHash().slice(0, 8)} PREV: ${block.getPreviousHash().slice(0, 8)} NOT ADDING as GOLDLING detected`);
      ...
      if (!nextBlock && !previousBlock && block.getHash() === lastHeaderInPreviousBlock.getHash() && block.getPreviousHash() === lastHeaderInPreviousBlock.getPreviousHash()) {
        ...
        // here evaluate the latest block height and potentially add it to missing
