  ...
  try {
    ...
    for (const [i, block] of blocksFromDisk.entries()) {
      ...
      if (lastHeaderInPreviousBlock.getPreviousHash() !== block.getPreviousHash() && lastHeaderInPreviousBlock.getHash() !== block.getHash() && lastHeaderInPreviousBlock.getHeight() === block.getHeight() && nextBlock && nextBlock.getPreviousHash() === block.getHash()) {
        ...
        if (earlyHeaders) {
          debug(`+ added ${earlyHeaders.length} with ${block.getHeight()} HASH: ${block.getHash().slice(0, 8)} PREV: ${block.getPreviousHash().slice(0, 8)} adding bridging goldlings`);
          all = earlyHeaders.concat(all);
          continue;
        } else {
          debug(`+ unable to find headers block ${block.getHeight()} HASH: ${block.getHash().slice(0, 8)} PREV: ${block.getPreviousHash().slice(0, 8)} adding but cannot bridge goldlings`);
      ...
      if (previousBlock && previousBlock.getPreviousHash() === lastHeaderInPreviousBlock.getPreviousHash() && previousBlock.getHash() === block.getPreviousHash() && previousBlock.getHash() !== lastHeaderInPreviousBlock.getHash()) {
        debug(`- block ${block.getHeight()} HASH: ${block.getHash().slice(0, 8)} PREV: ${block.getPreviousHash().slice(0, 8)} NOT ADDING as GOLDLING detected`);
        if (goldlings.indexOf(block.getBlockchain()) < 0) {
          goldlings.push(block.getBlockchain());
        }
        //if (all.length < 32) {
        //  debug(`- block ${block.getHeight()} HASH: ${block.getHash().slice(0, 8)} PREV: ${block.getPreviousHash().slice(0, 8)} ADDING to GOLDLING detected`)
        //  all.push(block)
        //}
        continue;
        //return all
    ...
    debug(`${goldlings.length} Goldlings found in the following connected chains: ${goldlings}`);
    for (let g of goldlings) {
      delete missingBlocks[g];
    }
    debug(`bestChain length is ${all.length}`);
