...
const BC_MAX_NEW_BLOCKS = parseInt(process.env.BC_MAX_NEW_BLOCKS, 10) || 40;
...
function isValidBlockTime(newBlock) {
  ...
  if (parseInt(newBlock.getHeight(), 10) < 4700000) {
    return true;
  }
  ...
  if (newestHeaderTimestamp + 2855000 < bcBlockTimestamp) {

    const newest = getNewestHeaderReverse(newBlock);

    if (newest) {
      if (newest.timestamp + 2855000 > bcBlockTimestamp) {
...
const validateCoinbase = exports.validateCoinbase = async (block, persistence, txHandler, calledFrom) => {
  debugCall(`CALL VALIDATE on ${block.getHeight()}:${block.getHash()}`);
  let date = Date.now();
  ...
  if (block.getHeight() > 4739506 && block.getHeight() < 4740006) {
    const cb = await undefined._persistence.get(`lease.db.built`);
    if (!cb) {
      const balances = await persistence._engine.getLeaseBalances();
      logger.info(`index window loaded ${Object.keys(balances).length}`);
      await undefined._persistence.putLeaseDb(balances);
    }
  }

  if (persistence._engine.evalBlocks.length === 0) {
    persistence._engine.evalBlocks.push({ block, date, rand });
    persistence._engine.indexBlocks.push(`${block.getHash()}${date}${rand}`);

    let res = await updateUTXO(block, persistence, txHandler, 'engine');
    let index = persistence._engine.indexBlocks.indexOf(`${block.getHash()}${date}${rand}`);
    persistence._engine.evalBlocks.splice(index, 1);
    persistence._engine.indexBlocks.splice(index, 1);
    let data = persistence._engine.evalBlocks[0];
    if (data) {
      let { block, rand, date } = data;
      persistence._engine._utxoEventEmitter.emit(`utxoUpdate.${block.getHash()}.${date}.${rand}`);
    }
    ...
    return new Promise((resolve, reject) => {
      persistence._engine._utxoEventEmitter.once(`utxoUpdate.${block.getHash()}.${date}.${rand}`, async () => {
        let res = await updateUTXO(block, persistence, txHandler, 'engine');
        let index = persistence._engine.indexBlocks.indexOf(`${block.getHash()}${date}${rand}`);
        persistence._engine.evalBlocks.splice(index, 1);
        persistence._engine.indexBlocks.splice(index, 1);
...
const updateUTXO = exports.updateUTXO = async (block, persistence, txHandler, calledFrom) => {
  if (!block || block.getTxsList === undefined) return false;
  ...
  if (isUpdating) {
    ...
    if (hash.length == 2) {
      ...
      let blockSaved = await persistence.areUTXOsSavedForBlock(blockS.getHeight(), blockS.getHash());
      if (blockSaved) {
        isUpdating = false;
        await persistence.del('updateUTXOs');
      } else {
        debugUTXO(`saving utxo for ${isUpdating}`);
        let oldSaved = await isCoinbaseValid(blockS, persistence, txHandler);
...
const isCoinbaseValid = exports.isCoinbaseValid = async (block, persistence, txHandler) => {
  ...
  if (validated) debugUTXO(`${block.getHeight()} is validated ${validated}`);
  if (!validated) logger.info(`block ${block.getHeight()} cannot be updated`);
...
const validRoverChain = exports.validRoverChain = async (rovers, persistence, collideAtEdge = false, collideAtRoverEdge = false) => {
  ...
  for (const key of newBlockHeadersKeys) {
    ...
    if (edgesFound[chain] > 3000 || blocksAdded[chain] > BC_MAX_NEW_BLOCKS + 5) {
      savedReads[chain]++;
      if (failed[chain].low > height) failed[chain].low = height;
      if (failed[chain].high < height) failed[chain].high = height;
      continue;
