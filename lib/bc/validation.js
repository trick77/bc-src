...
function isChainRootCorrectlyCalculated(newBlock) {
  ...
  if (newBlock.getHash() === '75fa3705a0d87019897583bb3314766abc798915262e096c009f1704fb1f1f41') {
    return true;
  }
...
const validateCoinbase = exports.validateCoinbase = async (block, persistence, txHandler, calledFrom) => {
  ...
  // let higherBlock =  await persistence.get(`bc.block.${block.getHeight() + 5}.utxoSaved`)
  // if(higherBlock) {
  //   debug(`${block.getHeight()}:${block.getHash()} is low`);
  // }
  persistence._engine._utxoEventEmitter.emit('utxoUpdate', { block, date, rand });
  return new Promise((resolve, reject) => {
    persistence._engine._utxoEventEmitter.once(`utxoUpdate${block.getHash()}${date}${rand}`, res => {
      if (res) logger.info(`${block.getHeight()}:${block.getHash()} is VALID`);else logger.info(`${block.getHeight()}:${block.getHash()} is NOT VALID`);
      resolve(res);
    });
  });
};
...
const isCoinbaseValid = exports.isCoinbaseValid = async (block, persistence, txHandler) => {
  if (block.getHeight() !== 1) {
    ...
    debug(`valid tx is ${validTxs} for ${block.getHeight()}:${block.getHash()}`);

    const blockHeight = parseInt(block.getHeight(), 10);

    if (blockHeight >= 4207048 && blockHeight <= 4207055) {
      return true;
    }
    ...
    if (!coinbaseTxValue.eq(grant) && blockHeight < 3219628 || !coinbaseTxValue.eq(grant) && blockHeight > 3223542) {
      if (coinbaseTxValue.gt(grant) && blockHeight > 4589416) {
