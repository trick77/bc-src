...
const BC_MAX_NEW_BLOCKS = parseInt(process.env.BC_MAX_NEW_BLOCKS, 10) || 60;
...
function isValidBlockTime(newBlock) {
  ...
  if (newestHeaderTimestamp + days < bcBlockTimestamp) {
    ...
    if (bcBlockTimestamp + 180000 > now) {
      logger.warn(`block ${newBlock.getHeight()} potentially stale with newest header: ${newestHeaderTimestamp}`);
      return true;
    } else {
...
const validateTxs = exports.validateTxs = async (txHandler, block, parentBlock) => {
  ...
  const validTxs = await txHandler.validateTxs(block, parentBlock);
...
const getCoinbaseMissingBlockHeight = exports.getCoinbaseMissingBlockHeight = async (block, persistence, txHandler, txPendingPool, utxoManager, calledFrom) => {
  ...
  debugUTXO(`preparing index for ${blocksToAdd.length} blocks`);
...
const validateCoinbase = exports.validateCoinbase = async (block, persistence, engine, txHandler, txPendingPool, utxoManager, calledFrom, parentBlock) => {
  debugUTXO(`CALL VALIDATE on ${block.getHeight()}:${block.getHash()} from ${calledFrom}`);
  let date = Date.now();
  let rand = Math.random();
  let alreadySaved = await utxoManager.areUTXOsSavedForBlock(block.getHeight(), block.getHash());
  if (alreadySaved) {
    //debugUTXO(`${block.getHeight()}:${block.getHash()} is SAVED`)
    debugUTXO(`${block.getHeight()}:${block.getHash()} is SAVED`);
    return true;
  }
  ...
  if (engine.evalBlocks.length === 0) {
    ...
    let res = await updateUTXO(block, persistence, txHandler, txPendingPool, utxoManager, calledFrom, parentBlock);
    ...
    if (data) {
      ...
      logger.info(`${block.getHeight()} validated ${res} within ${Date.now() - date} update`);
    } else {
      logger.info(`${block.getHeight()} validated ${res} within ${Date.now() - date} non-update`);
    }
    return res;
  } else {
    engine.evalBlocks.push({ block, date, rand });
    ...
    return new Promise((resolve, reject) => {
      try {
        let timeout = setTimeout(() => {
          engine._utxoEventEmitter.emit(`utxoUpdate.${block.getHash()}.${date}.${rand}`, true);
        }, 30000);
        engine._utxoEventEmitter.once(`utxoUpdate.${block.getHash()}.${date}.${rand}`, async failedUTXO => {
          // TD
          if (failedUTXO) {
            return resolve(`failed utxo update ${block.getHeight()}:${block.getHash()} -> date: ${date} rand: ${rand}`);
          }
          if (utxoManager && utxoManager._activeWaypoint) {
            const currentPeer = utxoManager._activeWaypoint;
            currentPeer.setExpires(Number(new Date()) + 40000);
          ...
          let res = await updateUTXO(block, persistence, txHandler, txPendingPool, utxoManager, 'engine', parentBlock);
          ...
          logger.info(`${block.getHeight()} validated ${res} within ${Date.now() - date} is considered ${res}`);
          resolve(res);
        });
      } catch (err) {}
    });
...
const updateUTXO = exports.updateUTXO = async (block, persistence, txHandler, txPendingPool, utxoManager, calledFrom, parentBlock) => {
  if (!block || block.getTxsList === undefined) {
    // TD
    //debugUTXO(`unable to get block`)
    debugUTXO(`unable to get block`);
    return false;
  }
  //TD: debugUTXO(`${block.getHeight()}:${block.getHash()} from ${calledFrom}`)
  logger.info(`${block.getHeight()}:${block.getHash()} from ${calledFrom}`);
  let date = Date.now();
  let alreadySaved = await utxoManager.areUTXOsSavedForBlock(block.getHeight(), block.getHash());
  // TD
  //debugUTXO(`check if utxos are already saved: ${alreadySaved}`)
  debugUTXO(`check if utxos are already saved: ${alreadySaved}`);
  if (alreadySaved) return true;
  let isUpdating = await persistence.get('updateUTXOs', { sync: true });
  // TD
  //debugUTXO(`isUpdating is ${isUpdating}`)
  logger.info(`isUpdating is ${isUpdating}`);
  if (isUpdating) {
    ...
    if (hash.length == 2) {
      ...
      if (!blockS || !blockS.getHeight()) {
        logger.info(`unable to get block by hash: ${hash[1]}`);
        return false;
      }
      let blockSaved = await utxoManager.areUTXOsSavedForBlock(blockS.getHeight(), blockS.getHash());
      if (blockSaved) {
        isUpdating = false;
        await persistence.del('updateUTXOs', { sync: true });
      } else {
        logger.info(`saving utxo for ${isUpdating}`);
        let oldSaved = await isCoinbaseValid(blockS, persistence, txHandler, txPendingPool, utxoManager, calledFrom, parentBlock);
        logger.info(`saved utxo for ${isUpdating} is ${oldSaved}`);
        await persistence.del('updateUTXOs', { sync: true });
        isUpdating = false;
      }
    } else {
      logger.info(`cannot save utxo for ${block.getHeight()}`);
      return false;
    }
  }
  ...
  // TD: This does not account for the fact that higher up may not be on the right branch
  if (higherBlock) {
    //TD: debugUTXO(`higher Block is ${higherBlock}`);
    logger.info(`higher Block is ${higherBlock}`);
    await persistence.put(`${BC_SUPER_COLLIDER}.unmount`, `${block.getHeight() + 16}:${higherBlock}`, { sync: true });
    // TD
    //debugUTXO(`higher block ${higherBlock} from ${block.getHeight()}:${block.getHash()}`);
    logger.info(`higher block ${higherBlock} from ${block.getHeight()}:${block.getHash()}`);
    return false;
  }
  //TD: debugUTXO(`check from ${block.getHeight() - 1}`)
  logger.info(`checking from ${block.getHeight() - 1} : ${block.getHash()}`);
  let startHeight = block.getHeight();
  let startHash = block.getHash();
  let startBlock = block;
  let blocksToAdd = [block];
  let foundBlock = true;
  let prevHeight = block.getHeight() - 1;
  let prevHash = block.getPreviousHash();
  let lastFoundBlock = false;
  let unlockHeight = false;

  // TD: this entire section should be redone
  for (let i = 0; i < 1000; i++) {
    ...
    //TD
    //debugUTXO(`while checking ${block.getHeight() - 1}, ${block.getPreviousHash()} saved state of ${i}`)
    logger.info(`while checking ${block.getHeight() - 1}, ${block.getPreviousHash()} saved state of ${i}`);
    if (parseInt(block.getHeight(), 10) === 1) break;

    // checking if the previous block is already saved
    let alreadySaved = await utxoManager.areUTXOsSavedForBlock(block.getHeight() - 1, block.getPreviousHash());
    prevHash = block.getPreviousHash();
    prevHeight = block.getHeight() - 1;

    // check if the block is already saved as a UTXO
    if (alreadySaved) {
      block = await persistence.getBlockByHash(prevHash, 'bc');
      if (block) {
        //TD
        //debugUTXO(`block ${block.getHeight() - 1}, ${block.getHash()} add to state after ${i} iterations...`)
        logger.info(`block ${block.getHeight() - 1}, ${block.getHash()} add to state after ${i} iterations...`);
        // TD: unclear why to add this
        blocksToAdd.unshift(block);
        break;
      } else {
        block = await persistence.getBlockByHeight(prevHeight, BC_SUPER_COLLIDER);
        if (block && block.getHash() === prevHash) {
          await persistence.saveBlock(block);
          blocksToAdd.unshift(block);
          //TD
          //debugUTXO(`block found by prev hash but not on disk: ${prevHash} saving after ${i} iterations....`)
          logger.info(`block found by prev hash but not on disk: ${prevHash} saving after ${i} iterations....`);
          break;
        } else {
          //TD
          //debugUTXO(`unable to find block ${prevHeight}:${prevHash}`)
          logger.info(`unable to find block ${prevHeight}:${prevHash}`);
          block = false;
        }
      }
      // if the block is not already saved add it to the
    } else {
      //TD
      //debugUTXO(`block utxos have not been calculated ${prevHeight}:${prevHash}`)
      logger.info(`block utxos have not been calculated ${prevHeight}:${prevHash}`);
      block = await persistence.getBlockByHash(prevHash, 'bc');
    }
    //else {
    //  block = await persistence.getBlockByHash(prevHash, BC_SUPER_COLLIDER)
    //  if (block) {
    //    blocksToAdd.unshift(block)
    //    debugUTXO(`block found by prev hash and unsaved ${prevHash}`)
    //  } else {
    //    block = await persistence.getBlockByHeight(prevHeight, BC_SUPER_COLLIDER)
    //    if (block && block.getHash() === prevHash) {
    //      blocksToAdd.unshift(block)
    ...
    if (!block) {
      //TD
      //debugUTXO(`potential branch found at ${prevHash}`)
      logger.info(`potential branch found at ${prevHash}`);
      const blocks = await persistence.getBlocksByHeight(prevHeight, BC_SUPER_COLLIDER);
      if (blocks && blocks.length > 0) {
        const match = blocks.reduce((all, b) => {
          if (b.getHash() === prevHash) {
            all = `${b.getHeight()}:${b.getHash()}`;
          }
          return all;
        }, false);
        //TD
        //debugUTXO(`blocks found at this level ${blocks.length} with connecting edge ${match} `)
        logger.info(`blocks found at this level ${blocks.length} with connecting edge ${match} `);
        for (const b of blocks) {
          if (block) continue;
          if (prevHash === b.getHash()) {
            block = b;
            //TD: debugUTXO(`reindexing hashed block ${prevHeight}:${prevHash}`)
            logger.info(`reindexing hashed block ${prevHeight}:${prevHash}`);
            await persistence.putBlockHashAtHeight(prevHash, prevHeight, BC_SUPER_COLLIDER);
          }
        }
        if (blocks.length === 0) {
          const b = await persistence.getBlockByHeight(prevHeight, BC_SUPER_COLLIDER);
          if (b && b.getHash() === prevHash) {
            block = b;
            //TD
            //debugUTXO(`updating hashed block ${prevHeight}:${prevHash}`)
            logger.info(`updating hashed block ${prevHeight}:${prevHash}`);
            await persistence.putBlockHashAtHeight(prevHash, prevHeight, BC_SUPER_COLLIDER);
            await persistence.putBlock(b, 0, BC_SUPER_COLLIDER);
          } else if (b && b.getHash) {
            await persistence.putBlockHashAtHeight(b.getHash(), b.getHeight(), BC_SUPER_COLLIDER);
            alreadySaved = await utxoManager.areUTXOsSavedForBlock(b.getHeight(), b.getHash());
            if (alreadySaved) {
              //TD
              //debugUTXO(`already saved block at height: ${b.getHeight()}`)
              logger.info(`already saved block at height: ${b.getHeight()}`);
              break;
            }
            //TD
            //debugUTXO(`found ${blocks.length} solutions at ${prevHeight}:${prevHash} however found one block ${b.getHeight()}: ${b.getHash()}`)
            logger.info(`found ${blocks.length} solutions at ${prevHeight}:${prevHash} however found one block ${b.getHeight()}: ${b.getHash()}`);
            block = b;
          } else {
            debugUTXO(`found ${blocks.length} solutions at ${prevHeight}:${prevHash}`);
          }
        }
      }
    }
    ...
    if (!block) {
      //TD
      //debugUTXO(`no block map searching by height directly <- ${prevHeight}`)
      logger.info(`no block map searching by height directly <- ${prevHeight}`);
      const b = await persistence.getBlockByHeight(prevHeight, BC_SUPER_COLLIDER);

      if (b && b.getHash() === prevHash) {
        // TD
        //debugUTXO(`block found <- ${b.getHeight()}`)
        logger.info(`block found <- ${b.getHeight()}`);
        block = b;
        // create refernece to the missing block
        await persistence.putBlockHashAtHeight(b.getHash(), b.getHeight(), 'bc');
      } else if (b) {
        // TD
        let alreadySaved = await utxoManager.areUTXOsSavedForBlock(b.getHeight(), b.getHash());
        if (alreadySaved) {
          blocksToAdd.unshift(b);
          break;
        }

        // check if there are other blocks at that height
        let parentBlocks = await persistence.getBlocksByHeight(prevHeight + 1, BC_SUPER_COLLIDER);

        // create refernece to the missing block
        await persistence.putBlockHashAtHeight(b.getHash(), b.getHeight(), 'bc');

        if (parentBlocks && parentBlocks.length < 1) {
          const higherBlock = await persistence.getBlockByHeight(prevHeight + 1, BC_SUPER_COLLIDER);
          if (higherBlock && higherBlock.getPreviousHash() !== b.getHash()) {
            block = b;
            //TD
            //debugUTXO(`branch found ${higherBlock.getPreviousHash()} !== ${b.getHash()}`)
            logger.info(`branch found ${higherBlock.getPreviousHash()} !== ${b.getHash()}`);
            prevHeight = b.getHeight();
            prevHash = b.getHash();
            logger.info(`block found higher that references the block at this height from: ${prevHeight} ${prevHash}`);
            if (blocksToAdd.length > 0) {
              logger.info(`replacing previous sequence with match: ${higherBlock.getHeight()} ${higherBlock.getHeight()}`);
              if (blocksToAdd[0].getHeight() === higherBlock.getHeight()) {
                unlockHeight = higherBlock.getHeight();
                blocksToAdd.shift();
                blocksToAdd.unshift(higherBlock);
              }
            }
          } else if (higherBlock && higherBlock.getPreviousHash() === b.getHash()) {
            // TD
            //debugUTXO(`branch found ${higherBlock.getPreviousHash()} matching ${b.getHash()}`)
            logger.info(`branch found ${higherBlock.getPreviousHash()} matching ${b.getHash()}`);
            block = b;
          }
        } else {
          logger.info(`possible parents ${parentBlocks.length} for ${prevHeight}: ${prevHash}`);
        }

        if (!block) {
          logger.info(`${prevHeight} : ${prevHash} <- requires index`);
          await persistence.put(`${BC_SUPER_COLLIDER}.unmount`, `${prevHeight}:${prevHash}`);
          // this is not a feature that can be made
          foundBlock = false;
          break;
        } else {
          logger.info(`${prevHeight} : ${prevHash} unable to determine block at path`);
        }
      }
      // LDL
    }
  ...
  //TD
  //debugUTXO(`total of ${blocksToAdd.length} found from ${startHeight}:${startHash}`)
  logger.info(`total of ${blocksToAdd.length} found from ${startHeight}:${startHash}`);

  const foundBlocks = {};
  blocksToAdd = blocksToAdd.reduce((all, b) => {
    if (foundBlocks[b.getHash()]) {
      return all;
    }
    foundBlocks[b.getHash()] = true;
    all.push(b);
    return all;
  }, []);

  blocksToAdd = blocksToAdd.sort((a, b) => {
  ...
  if (!foundBlock) {
    // TD
    logger.info(`${prevHeight} : ${prevHash} <- undiscovered`);
    //TD
    //debugUTXO(`block not found`)
  ...
  if (isUpdating && isUpdating !== `SAVING.${block.getHash()}`) {
    //TD
    //debugUTXO(`block saving ${isUpdating} not equal to ${block.getHash()}`)
    logger.info(`block saving ${isUpdating} not equal to ${block.getHash()}`);
    return false;
  }

  const originalHeight = block ? block.getHeight() : prevHeight;
  const removeFromHeight = unlockHeight !== false ? unlockHeight : originalHeight;
  logger.info(`blocks in queue: ${blocksToAdd.length}, unlocking from ${removeFromHeight}, start height: ${startHeight}, original discovery height: ${originalHeight}`);
  let updateUTXOsResult = await utxoManager.removeUTXOsFrom(removeFromHeight);
  //let updateUTXOsResult = await utxoManager.removeUTXOsFrom(startHeight)
  //TD
  //debugUTXOResult(updateUTXOsResult)
  logger.info(updateUTXOsResult);
  if (updateUTXOsResult.result) {
    ...
    //TD
    //debugUTXO(`unable to get result from ${block.getHeight()} ${block.getHash()}`)
    logger.info(`unable to get result from ${block.getHeight()} ${block.getHash()}`);
    return false;
  }

  // TD
  //debugUTXO(`check to ${prevHeight} with blocks to add: ${blocksToAdd.length}`)
  logger.info(`check to ${prevHeight} with blocks to add: ${blocksToAdd.length}`);

  let i = 0;
  for (let b of blocksToAdd) {
    //TD
    //debugUTXO(`in for loop parsing ${b.getHeight()}-${b.getHash()}`)
    logger.info(`in for loop parsing ${b.getHeight()}-${b.getHash()}`);
    let validBlock = await isCoinbaseValid(b, persistence, txHandler, txPendingPool, utxoManager, calledFrom, parentBlock);
    let alternativePath = false;
    //TD
    //debugUTXO(`${b.getHeight()} is ${validBlock}`);
    logger.info(`${b.getHeight()} is ${validBlock}`);
    if (blocksToAdd[i + 1] && blocksToAdd[i + 1].getHeight() === b.getHeight()) {
      logger.info(`counter block higher at ${b.getHeight()} is hash ${blocksToAdd[i + 1].getHash()}`);
      alternativePath = true;
    } else if (blocksToAdd[i - 1] && blocksToAdd[i - 1].getHeight() === b.getHeight()) {
      logger.info(`counter block lower at ${b.getHeight()} is hash ${blocksToAdd[i + 1].getHash()}`);
      alternativePath = true;
    }
    if (validBlock === false && !alternativePath) {
...
const isCoinbaseValid = exports.isCoinbaseValid = async (block, persistence, txHandler, txPendingPool, utxoManager, calledFrom = "validation", parentBlock) => {
  if (block.getHeight() !== 1) {
    // ensure we have the utxo set up to date
    let blockTxs = block.getTxsList();
    let validTxs = await validateTxs(txHandler, block, parentBlock);
    ...
    if (!validTxs && blockHeight < 3219628 || !validTxs && blockHeight > 3223542) {
      ...
      const consensusBlock = await persistence.get(`${blockHeight}:${block.getHash()}`);
      // LDL
      if (!consensusBlock) {
  ...
  debugUTXOResult(updateUTXOsResult);
  if (updateUTXOsResult.result) {
    if (utxoManager._activeWaypoint) {
      const currentPeer = utxoManager._activeWaypoint;
      currentPeer.setExpires(Number(new Date()) + 40000);
      await persistence.put(`${BC_SUPER_COLLIDER}.sync.initialpeer`, currentPeer);
    }
  ...
  if (!updateUTXOsResult.result) {
    // TD: terrible
    logger.info(`block ${block.getHeight()} cannot be updated`);
  }
...
const validRoverChain = exports.validRoverChain = async (rovers, persistence, collideAtEdge = false, collideAtRoverEdge = false) => {
  ...
  try {
    ...
    return Promise.resolve({ bestChain: false, missingBlocks: missingBlocks });
  }
};