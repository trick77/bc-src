...
const BC_TETHERED_WORK_BLOCK = parseInt(process.env.BC_TETHERED_WORK_BLOCK, 10) || 8200066;
const BC_SUPER_COLLIDER = process.env.BC_SUPER_COLLIDER ? process.env.BC_SUPER_COLLIDER.toLowerCase() : 'bc';
const BC_MAX_NEW_BLOCKS = parseInt(process.env.BC_MAX_NEW_BLOCKS, 10) || 100;
const {
  COINBASE_TX_ESTIMATE_SIZE,
  getMaxBlockSize,
  getMaxDistanceWithEmblems,
  getNrgGrant,
  getTxsDistanceSum
...
const { getBlockSchema } = require('./util');
...
function isValidBlock(newBlock, opts = { fullBlock: true, tetheredWork: false }) {
  ...
  if (!opts.tetheredWork && !isDistanceAboveDifficulty(newBlock)) {
    logger.info(`block ${newBlock.getHeight()} failed: isDistanceAboveDifficulty`);
    return false;
  }
  if (!opts.tetheredWork && !isDistanceCorrectlyCalculated(newBlock)) {
  ...
  if (parseInt(newBlock.getHeight(), 10) > 665616 && !(parseInt(newBlock.getHeight(), 10) > 7579000 && parseInt(newBlock.getHeight(), 10) < 8059000)) {
...
const validateTxs = exports.validateTxs = async (txHandler, block) => {
  ...
  if (height && height < 2) {
    ...
    return true;
    // more overline weighted txs
  } else if (height === 7630282 && height === "75405be95c40f6cbc1977c2ade9c669b18d232b2621935d07e034f4028763733") {
    return true;
  }
...
const validateCoinbase = exports.validateCoinbase = async (block, persistence, engine, txHandler, txPendingPool, utxoManager, calledFrom) => {
  ...
  // build emblem balance between these blocks
  if (block.getHeight() > 7560066 && block.getHeight() < 7570066) {
    const land = await persistence.get(`lease.db.land`);
    if (!land) {
      const balances = await getLeaseBalances('c');
      logger.info(`index window update loaded ${Object.keys(balances).length}`);
      await persistence.putLeaseDb(balances, 'land');
    }
  }

  // build emblem balance between these blocks
  if (block.getHeight() > 7660066 && block.getHeight() < 7661066) {
    const friend = await persistence.get(`lease.db.friend`);
    if (!friend) {
      const balances = await getLeaseBalances('d');
      logger.info(`index window update loaded ${Object.keys(balances).length}`);
      await persistence.putLeaseDb(balances, 'friend');
    }
  }
  ...
  if (engine.evalBlocks.length === 0) {
    ...
    debugUTXO(`direct open ${block.getHeight()} -> ${rand}`);
    ...
    debugUTXO(`direct action ${block.getHeight()} validated ${res} within ${Math.floor((Date.now() - date) / 1000)}s`);
    return res;
  } else {
    engine.evalBlocks.push({ block, date, rand });
    engine.indexBlocks.push(`${block.getHash()}${date}${rand}`);
    ...
    return new Promise((resolve, reject) => {
      debugUTXO(`promised open ${block.getHeight()} -> ${rand}`);
      engine._utxoEventEmitter.once(`utxoUpdate.${block.getHash()}.${date}.${rand}`, async () => {
        ...
        debugUTXO(`promised ${block.getHeight()} validated ${res} within ${Math.floor((Date.now() - date) / 1000)}s rand: ${rand}`);
        resolve(res);
      });
    });
  }
...
const updateUTXO = exports.updateUTXO = async (block, persistence, txHandler, txPendingPool, utxoManager, calledFrom) => {
  ...
  for (let i = 0; i < 100; i++) {
    ...
    // first check if there is a block at that hash
    block = await persistence.getBlockByHash(prevHash, 'bc');

    // if there is not a block run two more checks
    if (!block) {
      // first see if it was stored directly at thet height
      const blockAtHeight = await persistence.getBlockByHeight(prevHeight, 'bc');
      if (blockAtHeight && blockAtHeight.getHash && blockAtHeight.getHash() === prevHash) {
        block = blockAtHeight;
      } else {
        // second see if the block was stored as a part of a group of options at that height
        const blocksAtHeight = await persistence.getBlocksByHeight(prevHeight, 'bc');
        if (blocksAtHeight && blocksAtHeight.length > 0) {
          // determine if any of the blocks at this height match the block we are looking for
          block = blocksAtHeight.reduce((all, b) => {
            if (!all && b && b.getHash && b.getHash() === prevHash) {
              return b;
            }
            return all;
          }, false);
      ...
      // if a block was discovered save the block so these does not happen in the future
      if (block && block.getHash) {
        await persistence.saveBlock(block);
      }
    }

    // finally set foundblock to false which fails this operation and triggers a search for the segment from a waypoint
    if (!block) {
  ...
  if (block.getHeight() !== 1) {
    ...
    if (givenMinerOutputScript !== newOutputLockScript) {
      if (blockHeight > 5826066) {
        ...
        debugUTXO(`invalid miner script block: ${blockHeight}, block hash: ${block.getHash()}, given miner: ${minerAddress}`);
        return false;
      }
    }

    // 3220463
    if (!coinbaseTxValue.eq(grant) && blockHeight < 3219628 || !coinbaseTxValue.eq(grant) && blockHeight > 4767104) {
      if (coinbaseTxValue.gt(grant) && blockHeight > 7631314) {
        if (blockTxs.length === 1 && blockHeight < 7829881) {
          debugUTXO(`reindex block ${blockHeight} ${block.getHash()}`);
        } else {
...
const validateRequireMountBlock = exports.validateRequireMountBlock = (newBlock, latestBlock) => {
  ...
  if (parseInt(newBlock.getHeight(), 10) - 1 !== parseInt(latestBlock.getHeight(), 10) || newBlock.getHeight() < 3 || newBlock.getHeight() === 4313561 || latestBlock.getHeight() === 4313561 || newBlock.getHeight() < 8090000 && newBlock.getHeight() > 7570000) {
...
function validateSequenceDifficulty(previousBlock, newBlock) {
  ...
  if (new BN(BC_MINIMUM_DIFFICULTY).gt(new BN(finalDifficulty))) {
    debug('difficulty should be equal: false');
    return true;
  }
...
const validRoverChain = exports.validRoverChain = async (rovers, persistence, collideAtEdge = false, collideAtRoverEdge = false) => {
  ...
  const mountFound = {};
  let loaded = {
    btc: { high: 0, low: Infinity },
    eth: { high: 0, low: Infinity },
    wav: { high: 0, low: Infinity },
    lsk: { high: 0, low: Infinity },
    neo: { high: 0, low: Infinity }
  };
  let failed = {
    btc: { high: 0, low: Infinity },
    eth: { high: 0, low: Infinity },
  ...
  for (const key of newBlockHeadersKeys) {
    ...
    if (edgesFound[chain] > 1000 || blocksAdded[chain] > BC_MAX_NEW_BLOCKS + 5) {
      ...
      if (opts && opts.length > 0) {
        ...
        let higherBlocks = await persistence.getBlocksByHeight(parseInt(height) + 1, chain, {
          asHeader: true,
          cached: false
        });
        debugRover(`higherBlocks for ${height} is ${higherBlocks}`);
        if (!higherBlocks || higherBlocks && higherBlocks.length === 0) {
          higherBlocks = await persistence.getBlockByHeight(parseInt(height) + 1, chain, {
            asHeader: true,
            cached: false
          });
  ...
  if (!blocksFromDisk || blocksFromDisk.length < 1) {
    logger.info(`blocks no ready to construct rover challenge`);
    return false;
  }
  ...
  for (const [i, block] of blocksFromDisk.entries()) {
    ...
    if (register[bc] === 1 && lastHeaderInPreviousBlock) {
      if (parseInt(block.getHeight(), 10) === parseInt(lastHeaderInPreviousBlock.getHeight(), 10) + 1 && block.getPreviousHash() !== lastHeaderInPreviousBlock.getHash()) {
        ...
        const prevMountBlock = await persistence.getBlockByHash(block.getPreviousHash(), bc, {
          asHeader: false,
          cached: false
        });
        if (prevMountBlock) {
          ...
          if (prevMountBlock.getHash() !== lastHeaderInPreviousBlock.getPreviousHash()) {
            ...
            const goldlingBlock = await persistence.getBlockByHash(prevMountBlock.getPreviousHash(), bc, {
              asHeader: false,
              cached: false
            });
            if (goldlingBlock) {
...
// gets the correct validation schema for the given schema
const isValidBlockBySchema = exports.isValidBlockBySchema = async (newBlock, opts = { fullBlock: true }) => {

  if (BC_TETHERED_WORK_BLOCK > newBlock.getHeight()) {
    // tethered work height has not been reached
    opts.tetheredWork = false;
    return isValidBlock(newBlock, opts);
  }

  // gets the schema for the block based on
  const schema = getBlockSchema(newBlock);

  // block is not using tethered work
  if (schema === 1) {
    opts.tetheredWork = false;
    return isValidBlock(newBlock, opts);
  }

  // block is using tethered work
  if (schema === 2) {
    opts.tetheredWork = true;
    const validBlockData = isValidBlock(newBlock, opts);
    if (!validateBlockData) {
      return false;
    }
    // now tests are run on the block to determine if the tethered work is valid
    return true;
  }

  throw new Error(`unsupported block schema ${schema}`);
};