...
const validateCoinbase = exports.validateCoinbase = async (block, persistence, txHandler, calledFrom) => {
  ...
  persistence._engine._emitter.emit('utxoUpdate', { block, date, rand });
  return new Promise((resolve, reject) => {
    persistence._engine._emitter.once(`utxoUpdate${date}${rand}`, res => {
      resolve(res);
    });
  });
};
...
const updateUTXO = exports.updateUTXO = async (block, persistence, txHandler, calledFrom) => {
  ...
  while (true && block.getHeight() > 1) {
    ...
    if (!block) {
      // LDL
      logger.info(`${prevHeight} : ${prevHash} cannot be found`);
  ...
  if (utxoSimplex || isUpdating && isUpdating !== `SAVING.${block.getHash()}`) {
    debugUTXO(`utxo pruning in progress, yielding request for block ${block.getHeight()}, simplex is ${utxoSimplex}`);
    return false;
  } else {
    debugUTXO(`restarting save on ${block.getHeight()}.${block.getHash()}`);
    persistence._chainState.setSimplexCache(`Validating ${block.getHeight()}.${block.getHash()}`);
  }
...
const isCoinbaseValid = exports.isCoinbaseValid = async (block, persistence, txHandler) => {
  if (block.getHeight() !== 1) {
    ...
    if (inputs.length !== 0) {
      logger.warn(`invalid root <- required 0 inputs, given ${inputs.length}`);
      return false;
    }
...
function validateSequenceTotalDistance(previousBlock, newBlock) {
  ...
  if (!claimedDistance && new BN(newBlock.getHeight()).gt(new BN(2469110)) && new BN(newBlock.getHeight()).lt(new BN(2499000))) {
...
const validRoverChain = exports.validRoverChain = async (rovers, persistence, collideAtEdge = false) => {
  ...
  for (const key of newBlockHeadersKeys) {
    ...
    if (edgesFound[chain] > 3000 || blocksAdded[chain] > BC_MAX_NEW_BLOCKS + 5) {
      ...
      const opts = await persistence.getBlocksByHeight(height, chain, { asHeader: true, cached: false });
  ...
  for (const [i, block] of blocksFromDisk.entries()) {
    ...
    if (lastHeaderInPreviousBlock.getPreviousHash() !== block.getPreviousHash() && lastHeaderInPreviousBlock.getHash() !== block.getHash() && lastHeaderInPreviousBlock.getHeight() === block.getHeight() && nextBlock && nextBlock.getPreviousHash() === block.getHash()) {
      ...
      if (earlyHeaders && earlyHeaders.length > 0 && finalBlocks.length > 0 && parseInt(earlyHeaders[0].getHeight(), 10) <= parseInt(finalBlocks[0].getHeight(), 10)) {
      ...
      if (latestRoverBlock && latestRoverBlock.getHeight() > block.getHeight()) {
        if (!missingBlocks[roverChain]) {
          missingBlocks[roverChain] = {
            ...
            lowest: parseInt(block.getHeight(), 10) - 4
          };
        } else {
