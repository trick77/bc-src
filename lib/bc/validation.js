...
function isValidBlockTime(newBlock) {
  ...
  if (parseInt(newBlock.getHeight(), 10) < 5900000) {
    return true;
  }
...
const getCoinbaseMissingBlockHeight = exports.getCoinbaseMissingBlockHeight = async (block, persistence, txHandler, txPendingPool, utxoManager, calledFrom) => {
  ...
  for (let b of blocksToAdd) {
    ...
    let validBlock = await isCoinbaseValid(b, persistence, txHandler, txPendingPool, utxoManager);
    if (validBlock === false) {
...
const validateCoinbase = exports.validateCoinbase = async (block, persistence, engine, txHandler, txPendingPool, utxoManager, calledFrom) => {
  ...
  if (engine.evalBlocks.length === 0) {
    ...
    let res = await updateUTXO(block, persistence, txHandler, txPendingPool, utxoManager, 'validate');
  ...
  if (isUpdating) {
    ...
    if (hash.length == 2) {
      ...
      if (blockSaved) {
        ...
        let oldSaved = await isCoinbaseValid(blockS, persistence, txHandler, txPendingPool, utxoManager);
...
const isCoinbaseValid = exports.isCoinbaseValid = async (block, persistence, txHandler, txPendingPool, utxoManager) => {
  ...
  let updateUTXOsResult = await utxoManager.updateUTXOs(block);
  debugUTXOResult(updateUTXOsResult);
  if (updateUTXOsResult.result) {
