        ...
        if (trades) {
          ...
          for (let i = 0; i < trades.length; i++) {
            ...
            if (within && isSettled) {
              ...
              let unsettled = await this._persistence.unsettleTx(hash, chain, height, childHash, markedBlock, txHash, txIndex, isMaker);
        ...
        if (trades) {
          ...
          for (let i = 0; i < trades.length; i++) {
            ...
            if (within && (!isSettled || isSameTx && isDiffHash > block.getHeight() > 2590000 || isSameTx && isDiffHeight && block.getHeight() > 2542197)) {
              let markedBlock = await this.getBlockForMarkedTx(block, chain, hash);
              let settled = await this._persistence.settleTx(hash, chain, height, childHash, markedBlock, txHash, txIndex, isMaker);
