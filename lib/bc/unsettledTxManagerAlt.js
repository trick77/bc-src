...
class UnsettledTxManager {
  ...
  async onNewRoveredBlock(possibleTransactions, blockHash) {
    ...
    for (let j = 0; j < possibleTransactions.length; j++) {
      ...
      const { to, from, tokenType, chain, amount, txId } = this.getDetailsFromPtx(pTx);
      if (amount.toString() !== '0' && tokenType !== 'btc') debug(`${amount.toString()} ${tokenType} from ${from} to ${to}`);
      let trades = await this._persistence.getTradeIndices(from, to, tokenType, amount.toString());
      if (trades) {
        this.sortTrades(trades);
        for (let i = 0; i < trades.length; i++) {
          ...
          debug({ within, type, isSettled });
          if (within && !isSettled && !uniqueTxs[`${txId}`] && !alreadyMarked[`${txHash}.${txOutputIndex}.${type}`]) {
            alreadyMarked[`${txHash}.${txOutputIndex}.${type}`] = true;
            uniqueTxs[`${txId}`] = true;
  ...
  getDetailsFromPtx(pTx) {
    ...
    return { to, from, tokenType, chain, amount, txId };
  }
