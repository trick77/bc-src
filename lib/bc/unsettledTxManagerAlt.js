...
class UnsettledTxManager {
  ...
  async onNewBcBlock(block) {
    try {
      ...
      if (markedBlockTransactions.length == 0) return Promise.resolve(true);else this._logger.info(`${markedBlockTransactions.length} Marked Txs within ${block.getHeight()}:${block.getHash()}`);
      for (let j = 0; j < markedBlockTransactions.length; j++) {
        let mTx = markedBlockTransactions[j];
        const { to, from, chain, amount, height, tokenType, hash } = this.getDetailsFromMtx(mTx);
        let trades = await this._persistence.getTradeIndices(from, to, tokenType, amount.toString());
        if (trades) {
          ...
          for (let i = 0; i < trades.length; i++) {
            ...
            if (markedKey) {
              ...
              if (height !== parseInt(childChainHeight)) isDiffHeight = true;
              // console.log({within,type,isSettled,isDiffHeight,height,childChainHeight:parseInt(childChainHeight)})
            }
            // console.log({within,type,isSettled,isDiffHeight,height})
            if (within && (!isSettled || isDiffHeight && block.getHeight() > 2542197)) {
              let markedBlock = await this.getBlockForMarkedTx(block, chain, hash);
  ...
  async onNewRoveredBlock(possibleTransactions, blockHash) {
    ...
    for (let j = 0; j < possibleTransactions.length; j++) {
      ...
      if (amount.toString() !== '0' && tokenType !== 'btc') debugUTXO(`${amount.toString()} ${tokenType} from ${from} to ${to} within ${blockHash}`);
      let trades = await this._persistence.getTradeIndices(from, to, tokenType, amount.toString());
      if (trades) {
        this.sortTrades(trades);
        for (let i = 0; i < trades.length; i++) {
          ...
          debug({ within, type, isSettled, settledWithinThisBlock });
          if (settledWithinThisBlock || within && !isSettled && !uniqueTxs[`${txId}`] && !alreadyMarked[`${txHash}.${txIndex}.${type}`]) {
