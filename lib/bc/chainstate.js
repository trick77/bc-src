...
const StateRanks = {
  ...
  constructor(dirPath, rovers, opts = {}) {
    ...
    this._utxoCache = LRUCache({
      max: 200000
    });
    this._addressCache = LRUCache({
      max: 200000
    });
  ...
  openBlockRangeRequest(blockchain, highest, lowest, opts = {}) {
    ...
    if (BigInt(highest - lowest) > BigInt(rangeSizeLimit) && blockchain !== BC_SUPER_COLLIDER) {
      this._logger.warn(`WARN: requested range ${requestedRange} out of greater than ${rangeSizeLimit} for ${blockchain} rover`);
      return report;
  ...
  closeBlockRangeRequest(blockchain, roverRange, missingBlocks) {
    ...
    if (missingBlocks.length === 0) {
      ...
      this._logger.info(`rover returning default`);
      return {
