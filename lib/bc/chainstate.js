...
const { Utxo, TransactionOutput } = require('@overline/proto/proto/core_pb');
...
const StateRanks = {
   ...
   *   - RESTART <- Local overline node has encountered a critical error from which it could not recover, cannot mine, check error logs and restart
   ...
   *   - PUT_STATE <- Update to global overline node state
  ...
  async getBlockchainState(blockchain) {
    ...
    if (this._rovers.indexOf(blockchain) < 0) {
      throw new Error(`no "${blockchain}" rovers are streaming to this overline node`);
    }
    ...
    if (requiredMetrics) {
      ...
      if (currentState === 'WAIT') {
        ...
        /*
         * STATE: CLOSED
         * local blockchain state is not available, overline node running as headless SDK only
         */
  ...
  async putLatestBlock(blockchain, height, hash, force = false) {
    ...
    await this._persistence.put(`${blockchain}.block.latest.height`, height);
    // DEBUG
  ...
  async openBlockRangeRequest(blockchain, highest, lowest, opts = {}) {
    ...
    if (this._rovers.indexOf(blockchain) < 0) {
      throw new Error(`no rovers for blockchain ${blockchain} are streaming to this overline node`);
    }
    ...
    if (BigInt(highest - lowest) > BigInt(rangeSizeLimit) && blockchain !== BC_SUPER_COLLIDER) {
      this._logger.debug(` requested range ${requestedRange} out of greater than ${rangeSizeLimit} for ${blockchain} rover`);
    ...
    if (!c) {
      ...
      if (highestHash && mg) {
        ...
        if (highest === currentHighestHeight && currentHighestHash !== highestHash) {
          this._logger.info(`conflict at ${highest} current: ${currentHighestHash} new: ${highestHash}`);
          await this._persistence.put(`${blockchain}.range.highest.hash`, highestHash);
          report.highestHash = highestHash;
      ...
      if (lowestHash && lhh) {
        ...
        if (lowest === currentLowestHeight && currentLowestHash !== lowestHash) {
          this._logger.info(`conflict at height ${lowest} hash: ${currentLowestHash} new: ${lowestHash}`);
          report.lowestHash = lowestHash;
          await this._persistence.put(`${blockchain}.range.lowest.hash`, lowestHash);
  ...
  async closeBlockRangeRequest(blockchain, roverRange, missingBlocks) {
    ...
    if (this._rovers.indexOf(blockchain) < 0) {
      throw new Error(`no rovers for blockchain ${blockchain} are connected to this overline node`);
    }
