...
class Multiverse {
  ...
  async extendMultiverse(newBlock, context = 'peer', updateStorage = true, handleAsNewPeer = false, address = false, replaceLatestBlock = false) {
    ...
    let date = Date.now();
    debugExtend(`extending multiverse on ${newBlock.getHeight()}:${newBlock.getHash()}`);
    if (!newBlock) {
    ...
    if (new BN(newBlock.getHeight()).lte(new BN(latestBlock.getHeight())) && parseInt(newBlock.getHeight(), 10) <= parseInt(latestBlock.getHeight(), 10)) {
      ...
      return Promise.resolve({
        ...
        schedules: validBlockSeq.schedules,
        assertSyncComplete: syncComplete,
        blockSendingAlternative: parseInt(newBlock.getHeight(), 10) === parseInt(latestBlock.getHeight(), 10)
      });
    }
    ...
    if (new BN(purposedBlockChildHeightSum).gt(new BN(latestBlockChildHeightSum)) && new BN(latestBlock.getHeight()).lt(new BN(newBlock.getHeight()))) {
      ...
      if (diff > 19) {
        ...
        if (reorgLowest && parseInt(reorgLowest.getHeight(), 10) > parseInt(latestBlock.getHeight(), 10)) {
          ...
          lowest = max(2, parseInt(reorgLowest.getHeight(), 10) - 8);
          low = lowest;
        } else {
