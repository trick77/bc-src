...
class Multiverse {
  ...
  async addBranch(purposedBranch) {
    ...
    const tasks = purposedBranch.reverse().map(block => this.persistence.putBlock(block, 0));
   ...
   * @returns {Object}
   */
  async extendMultiverse(newBlock, context = 'peer', updateStorage = true, handleAsNewPeer = false, address = false) {
    /*
    ...
    if (latestReorgBlock && parseInt(newBlock.getHeight(), 10) > parseInt(latestReorgBlock.getHeight(), 10)) {
      ...
      await this.persistence.putBlock(newBlock, 0, blockchain, { asHeader: false });
    } else {
      // LDL
    ...
    if (BC_BUILD_GENESIS) {
      if (parseInt(newBlock.getHeight(), 10) > 500) {
        this._logger.warn(`reached maximum build height for BC_BUILD_GENESIS. Exiting...`);
        process.exit();
      }
    }
    ...
    if (blockchain === BC_SUPER_COLLIDER) {
      ...
      if (currentPeer && synced && synced === 'pending' && !peerExpired && latestReorgBlock) {
        ...
        if (edge < parseInt(newBlock.getHeight(), 10)) {
    ...
    if (extendsChainState || BC_NAIVE || newBlock.getPreviousHash() === latestBlock.getHash()) {
      ...
      if (!validCoinbase) {
        this._logger.warn(`invalid coinbase for block ${newBlock.getHeight()}`);
        return Promise.resolve({
          stored: false,
          needsResync: false,
          rangeRequest: false,
    ...
    if (synced === 'reorg' && dlh && dlh > 3) {
      ...
      await this.persistence.reorgBlockchain(BC_SUPER_COLLIDER, { chainState: this._chainState });
      this._logger.warn(`isolated multiverse change request from ${newBlock.getHeight()} requesting range ${dlh} -> ${newBlock.getHeight()}`);
      const opts = {};
      if (address) {
        opts.address = address;
      }
      ...
      await this.persistence.putBlock(newBlock, 0, blockchain);
      if (createdReorg) {
        const rangeRequest = this._chainState.openBlockRangeRequest(BC_SUPER_COLLIDER, parseInt(newBlock.getHeight(), 10), dlh);
    ...
    if (new BN(purposedBlockChildHeightSum).gt(new BN(latestBlockChildHeightSum))) {
      ...
      this._logger.info(`purposed new block is better multiverse ${newBlock.getHeight()} requesting range ${lowest} -> ${highest} purposed prev hash ${newBlock.getPreviousHash().slice(0, 8)} latest block hash ${latestBlock.getHash().slice(0, 8)}`);
      const diff = new BN(newBlock.getHeight()).sub(new BN(latestBlock.getHeight())).toNumber();
      if (diff > 6) {
        this._logger.info(`difference is greater than multiverse threshold: 6, increasing lower bound from ${lowest} to ${max(2, lowest - 90)}`);
        lowest = max(2, lowest - 1);
      }
