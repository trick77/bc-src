...
class Multiverse {
  ...
  async extendMultiverse(newBlock, context = 'peer', updateStorage = true, handleAsNewPeer = false, address = false, replaceLatestBlock = false, connection = false) {
    ...
    if (new BN(newBlock.getHeight()).gt(new BN(latestBlock.getHeight())) && new BN(latestBlock.getTimestamp()).lt(new BN(newBlock.getTimestamp())) && synced === 'complete' && !extendsChainBound && newBlock.getHeight() > 7600092) {
