...
class Multiverse {
  ...
  async validateBlockSequenceInline(blocks, mountBlock) {
    ...
    if (blockchain === BC_SUPER_COLLIDER && new BN(upperBound.getHeight()).lt(new BN(2999000)) && new BN(upperBound.getHeight()).gt(new BN(2711000))) {
      debug(`valid sequence with upper ${upperBound.getHeight()} to lower ${lowerBound.getHeight()} `);
      return { valid: true, schedules: [] };
    }
      ...
      if (lastHeaderLatestBlock.getHash() !== newBlock.getPreviousHash()) {
        ...
        if (!foundAlternative) {
          ...
          if (this._chainState) {
            ...
            await this.persistence.putLatestBlock(foundAlternative, blockchain, { iterateUp: false, context: context });
          }
          return true;
        }
      } else {
  ...
  async extendMultiverse(newBlock, context = 'peer', updateStorage = true, handleAsNewPeer = false, address = false, replaceLatestBlock = false) {
    ...
    debug(`${blockchain} block ${newBlock.getHeight()} extends chain: ${extendsChainState}`);
    ...
    const flagBlock = new BN(newBlock.getHeight()).eq(new BN(641452)) || new BN(newBlock.getHeight()).eq(new BN(641453));
    const validCoinbase = flagBlock ? true : await validateCoinbase(newBlock, this.persistence, this.txHandler, 'bc.multiverse');
    ...
    if (rm) {
      ...
      if (rb) {
        ...
        if (nb) {
          await this.persistence.putLatestBlock(nb, blockchain, { context: context, iterateUp: false });
        }
      }
    }
    ...
    if (blockchain === BC_SUPER_COLLIDER) {
      ...
      if (edge) {
        ...
        if (latestBlockHeight) {
          dlh = latestBlockHeight - 10;
        } else {
          dlh = parseInt(newBlock.getHeight(), 10) - 10;
        }
      }
      ...
      if (synced === 'reorg' && dlh && dlh > 3 || synced === 'reorg' && !dlh) {
        ...
        if (lowestLatestBlock && lowestBlock && latestBlock) {
          if (lowestLatestBlock.getHash() === latestBlock.getHash()) {
            ...
            lowestLatestBlock = await this.persistence.getBlockByHeight(lowest);
          }
        }
        ...
        if (createdReorg) {
          ...
          const rangeRequest = this._chainState.openBlockRangeRequest(BC_SUPER_COLLIDER, parseInt(newBlock.getHeight(), 10), lowest);
      ...
      if (reorgFromBlock) {
        ...
        if (synced && currentPeer && synced === 'pending') {
          ...
          if (!reorgToBlock || !reorgToBlock.getHeight) {
            ...
            // evalutate block
            this._logger.warn(`stale multiverse change request detected, removing...`);
            await this.persistence.put(`${BC_SUPER_COLLIDER}.sync.edge`, parseInt(newBlock.getHeight(), 10));
    ...
    if (!validBlockSeq.valid && !childrenHeightsValid && parseInt(latestBlock.getHeight(), 10) > 1 && parseInt(latestBlock.getHeight(), 10) + 1 === parseInt(newBlock.getHeight(), 10)) {
      ...
      if (new BN(newBlock.getTotalDistance()).lt(new BN(latestBlock.getTotalDistance()))) {
        ...
        this._logger.info(`${blockchain} block ${newBlock.getHeight()} rejected invalid mined block sequence`);
        return Promise.resolve({
          valid: validBlockSeq.valid,
          stored: false,
          needsResync: false,
    ...
    if (extendsChainState || BC_NAIVE || newBlock.getPreviousHash() === latestBlock.getHash()) {
      ...
      if (this._chainState && updateStorage && validCoinbase) {
        ...
        if (context !== 'local' && updateStorage) {
          externalOps = await this.persistence.putBlock(newBlock, 0, blockchain, { context: context });
        } else {
          externalOps = await this.persistence.putBlock(newBlock, 0, blockchain, { context: context });
        }
        ...
        await this.persistence.putLatestBlock(newBlock, blockchain, { context: context });
        if (context !== 'local') {
          for (const rover of this._knownRovers) {
            if (!newBlock.getBlockchainHeaders()[chainToGet(rover)]) {
              continue;
      ...
      if (validCoinbase) {
        this._logger.info(`new block ${newBlock.getHeight()} : ${newBlock.getHash().slice(0, 8)}... extends multiverse`);
        return Promise.resolve({
          valid: true,
          stored: true,
          needsResync: false,
    ...
    if (latestBlock.getHeight() === newBlock.getHeight() && context === 'local') {
      this._logger.info(`multiple blocks submitted by miner for height ${newBlock.getHeight()}`);
      return Promise.resolve({
        stored: false,
        needsResync: false,
        rangeRequest: false,
    ...
    if (new BN(purposedBlockChildHeightSum).gt(new BN(latestBlockChildHeightSum))) {
      ...
      if (latestBlock.getPreviousHash() === newBlock.getPreviousHash()) {
        ...
        if (latestBlockParent) {
          ...
          if (validCoinbase) {
            ...
            if (remount) {
              this._logger.warn(`remount found ${remount}`);
              await this.persistence.del(`${BC_SUPER_COLLIDER}.remount`);
            }
      ...
      if (diff > 9) {
        this._logger.info(`difference is greater than multiverse threshold increasing lower bound from ${lowest} (${low}/${high}) to ${max(2, lowest)} : ${latestBlock.getHeight()}`);
        lowest = max(2, parseInt(latestBlock.getHeight(), 10) - 10);
      }
      ...
      this._logger.info(`mount: ${possibleMount}`);
      ...
      const rr = {
        ...
        lowestHeight: lowest
        // this stops miner
      };this._logger.info(`opening multiverse change request from ${lowest} to ${highest}`);
      await this.persistence.put(`${BC_SUPER_COLLIDER}.sync.edge`, parseInt(newBlock.getHeight(), 10));
    ...
    if (new BN(newBlock.getDifficulty()).gt(new BN(latestBlock.getDifficulty())) || new BN(newBlock.getDifficulty()).eq(new BN(latestBlock.getDifficulty())) && new BN(newBlock.getTotalDistance()).gt(new BN(latestBlock.getTotalDistance()))) {
      this._logger.info(`purposed ${blockchain} block ${newBlock.getHeight()} accepted to multiverse`);
