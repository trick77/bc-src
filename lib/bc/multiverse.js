...
class Multiverse {
  ...
  async extendMultiverse(newBlock, context = 'peer', updateStorage = true, handleAsNewPeer = false, address = false, replaceLatestBlock = false) {
    ...
    if (new BN(newBlock.getDifficulty()).gte(new BN(latestBlock.getDifficulty())) && new BN(newBlock.getTotalDistance()).gt(new BN(latestBlock.getTotalDistance()))) {
      ...
      this._logger.info(`purposed ${blockchain} block ${newBlock.getHeight()}:${newBlock.getHash()} not approved as next in multiverse, difficulty is ${newBlock.getDifficulty()}, distance is ${newBlock.getTotalDistance()}`);
      this._logger.info(`latest block ${latestBlock.getHeight()}:${latestBlock.getHash()} difficulty is ${latestBlock.getDifficulty()}, distance is ${latestBlock.getTotalDistance()}`);
      return Promise.resolve({
        valid: false,
        stored: false,
        needsResync: false,
