...
class Multiverse {
  ...
  async validateBlockSequenceInline(blocks, mountBlock) {
    ...
    if (blockchain === BC_SUPER_COLLIDER && new BN(upperBound.getHeight()).lt(new BN(2739000)) && new BN(upperBound.getHeight()).gt(new BN(2711000))) {
      debug(`valid sequence with upper ${upperBound.getHeight()} to lower ${lowerBound.getHeight()} `);
      return { valid: true, schedules: [] };
    }
    ...
    if (lastHeaderLatestBlock && parseInt(lastHeaderLatestBlock.getHeight(), 10) + 1 === parseInt(newBlock.getHeight(), 10)) {
      ...
      if (lastHeaderLatestBlock.getHash() !== newBlock.getPreviousHash()) {
        ...
        if (!foundAlternative) {
          ...
          await this.persistence.putBlock(foundAlternative, 0, blockchain, { force: true, saveHeaders: true });
          if (this._chainState) {
  ...
  async extendMultiverse(newBlock, context = 'peer', updateStorage = true, handleAsNewPeer = false, address = false, replaceLatestBlock = false) {
      ...
      if (synced === 'reorg' && dlh && dlh > 3) {
        ...
        if (lowestLatestBlock && lowestBlock && latestBlock) {
          if (lowestLatestBlock.getHash() === latestBlock.getHash()) {
            ...
            const lowestBlocks = await this.persistence.getBlocksByHeight(lowest);
            lowestLatestBlock = lowestBlocks[0];
          }
        }
    ...
    if (extendsChainState || BC_NAIVE || newBlock.getPreviousHash() === latestBlock.getHash()) {
      ...
      const isValidSeq = foundRootBlockToMountBranch ? true : await this.validateBlockSequenceInline([newBlock, latestBlock], false);
      if (!isValidSeq.valid) {
        this._logger.warn(`purposed block is not valid sequence from previous block`);
        return Promise.resolve({
          stored: false,
          needsResync: false,
          rangeRequest: false,
