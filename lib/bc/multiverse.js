...
class Multiverse {
  ...
  async extendMultiverse(newBlock, context = 'peer', updateStorage = true, handleAsNewPeer = false, address = false, replaceLatestBlock = false) {
    ...
    if (newBlock.getHeight() > latestBlock.getHeight() && latestBlock.getTimestamp() < newBlock.getTimestamp() && !extendsChainBound && newBlock.getHeight() > 4205532) {
      this._logger.warn(`${blockchain} latest: ${latestBlockHeight} ${newBlock.getHeight()} extends chain: ${extendsChainState}`);
    ...
    this._logger.info(`${blockchain} latest: ${latestBlockHeight} ${newBlock.getHeight()} extends chain: ${extendsChainState}`);
    ...
    // MMP
    if (!newBlock.getBlockchain && new BN(purposedBlockChildHeightSum).gte(new BN(latestBlockChildHeightSum))) {
