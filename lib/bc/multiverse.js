...
class Multiverse {
  ...
  async extendMultiverse(newBlock, context = 'peer', updateStorage = true, handleAsNewPeer = false, address = false, replaceLatestBlock = false) {
    ...
    if (newBlock.getHeight() > latestBlock.getHeight() && latestBlock.getTimestamp() < newBlock.getTimestamp() && !extendsChainBound) {
    ...
    // MMP
    if (!newBlock.getBlockchain && new BN(purposedBlockChildHeightSum).gte(new BN(latestBlockChildHeightSum)) && synced !== 'pending') {
