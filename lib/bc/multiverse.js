  ...
  async extendMultiverse(newBlock, context = 'peer', updateStorage = true, handleAsNewPeer = false, address = false, replaceLatestBlock = false) {
    ...
    if (blockchain === BC_SUPER_COLLIDER) {
      ...
      if (synced === 'reorg' && dlh && dlh > 3) {
        ...
        low = lastBlock ? lastBlock : lowest;

        if (low && !isNaN(low)) {

          if (dlh && new BN(dlh).lt(new BN(low)) && new BN(dlh).gt(new BN(3))) {
            low = dlh;
          }
        ...
        //  })
        //}
      } else if (synced && currentPeer && synced !== 'complete' && latestReorgBlock) {
    ...
    if (new BN(purposedBlockChildHeightSum).gt(new BN(latestBlockChildHeightSum))) {
      ...
      if (createdReorg || !currentPeer) {
        ...
        // LDL
        this._logger.info(`yielding request from ${lowest} to ${highest}`);
        return Promise.resolve({
          stored: true,
          needsResync: false,
          rangeRequest: false,
