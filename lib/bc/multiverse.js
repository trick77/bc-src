...
class Multiverse {
  ...
  constructor(persistence, rovers, chainState, engine) {
    ...
    if (chainState) {
      ...
      this._blockchainManager = new BlockchainManager(BC_CHAIN_ID, BC_EMBLEM_CHAIN_ID, chainState, { rovers, persistence });
    }
  }
  ...
  async extendMultiverse(newBlock, context = 'peer', updateStorage = true, handleAsNewPeer = false, address = false, replaceLatestBlock = false) {
    ...
    if (this._knownBlocksCache.has(newBlock.getHash())) {
      ...
      return Promise.resolve({ valid: true, stored: true, needsResync: false, rangeRequest: false, assertSyncComplete: false });
    }
    ...
    const now = Date.now();
    const nowSecondsBound = Math.floor(now / 1000) + 50;
    ...
    if (extendsChainState) {
      ...
      setImmediate(async () => {
        await this.persistence.put(`${blockchain}.block.latest.hash`, newBlock.getHash(), { sync: true });
        await this.persistence.put(`${blockchain}.block.latest.height`, new BN(newBlock.getHeight()).toNumber(), { sync: true });
      });
    }
    ...
    if (new BN(newBlock.getHeight()).gt(new BN(latestBlock.getHeight())) && new BN(latestBlock.getTimestamp()).lt(new BN(newBlock.getTimestamp())) && !extendsChainBound && newBlock.getHeight() > 4205532) {
    ...
    this._logger.info(`${blockchain} latest: ${latestBlockHeight}:${latestBlockHash.slice(0, 8)} ${newBlock.getHeight()}:${newBlock.getHash().slice(0, 8)}:${newBlock.getPreviousHash().slice(0, 8)} extends chain: ${extendsChainState} took ${Date.now() - date}ms`);
    ...
    if (!newBlock.getBlockchain && new BN(purposedBlockChildHeightSum).gt(new BN(latestBlockChildHeightSum)) && (latestBlock.getHeight() - 30 < newBlock.getHeight() || latestBlock.getTimestamp() + 30 < newBlock.getTimestamp())) {
      ...
      if (!extendsChainState && newBlock.getHeight() === latestBlock.getHeight() && newBlock.getHash() !== latestBlock.getHash() && newBlock.getPreviousHash() === latestBlock.getPreviousHash()) {
        ...
        validCoinbase = await validateCoinbase(newBlock, this.persistence, this.txHandler, 'bc.multiverse');
      }
    }

    if (validCoinbase && currentPeer && extendsChainState) {
      this._logger.info(`open waypoint request detected: ${newBlock.getHeight()} ${newBlock.getHash()}`);
      await this.persistence.put(`${BC_SUPER_COLLIDER}.block.reorgfrom`, newBlock);
    }
    ...
    if (extendsChainState || BC_NAIVE || newBlock.getPreviousHash() === latestBlock.getHash()) {
      ...
      if (updateStorage && validCoinbase && extendsChainState) {
        ...
        if (parseInt(latestBlock.getHeight(), 10) <= parseInt(newBlock.getHeight(), 10)) {
          await this.persistence.putLatestBlock(newBlock, blockchain, { chainState: this._chainState, context: context, saveHeaders: true });
        }
        //if (context !== 'local') {
        for (const rover of this._knownRovers) {
          ...
          if (roveredHeader) {
            ...
            if (!roverBlock) {
              ...
              await this.persistence.putLatestBlock(roveredHeader, blockchain, { chainState: this._chainState, saveHeaders: true });
    ...
    // combat bad miners
    if (new BN(newBlock.getHeight()).lte(new BN(latestBlock.getHeight())) && parseInt(newBlock.getHeight(), 10) <= parseInt(latestBlock.getHeight(), 10)) {
      this._logger.info(`purposed ${blockchain} block ${newBlock.getHeight()} rejected as total height is lower than current block <- new block:  ${newBlock.getHeight()} latestBlock ${latestBlock.getHeight()}:${latestBlock.getHash()}`);
      return Promise.resolve({
        valid: false,
        stored: false,
        needsResync: false,
    ...
    if (new BN(purposedBlockChildHeightSum).gt(new BN(latestBlockChildHeightSum)) && new BN(latestBlock.getHeight()).lt(new BN(newBlock.getHeight()))) {
      ...
      if (dlh && new BN(dlh).lt(new BN(lowest)) && new BN(dlh).gt(new BN(3))) {
        ...
        // lowest = dlh
        debugLowest(`12.lowest set to ${lowest}`);
      }
