...
class Dex {
  ...
  async getMatchedOrders(bcAddress, from, to) {
    ...
    let { utxos } = await this._persistence.getUtxos(ScriptType.TAKER_OUTPUT, { address: bcAddress, from, to });
  ...
  async getMatchedOrder(hash, index) {
    ...
    if (utxoIndex >= 0) {
      ...
      if (utxo) await this.addMakerOrder(matchedOrders, utxo);
      return matchedOrders;
    } else return [];
  }
  ...
  async getOpenOrders(bcAddress, from, to) {
    ...
    if (bcAddress) bcAddress = bcAddress.toLowerCase();
    let openOrders = [];
    let { utxos } = await this._persistence.getUtxos(ScriptType.MAKER_OUTPUT, { address: bcAddress, from, to });
    let { utxos: callbackUtxos } = await this._persistence.getUtxos(ScriptType.TAKER_CALLBACK, { address: bcAddress, from, to });
