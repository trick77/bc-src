...
const debug = require('debug')('bcnode:dex');
...
class Dex {
  ...
  async getAllMatchedOrders() {
    ...
    debug(`getAllMatchedOrders(): matched orders ${matchedOrders.length} in ${Date.now() - start}ms`);
  ...
  async getMatchedOrders(bcAddress, from, to) {
    ...
    debug(`getMatchedOrders(): found matched ${matchedOrders.length} in ${Date.now() - start}ms`);
    return matchedOrders;
  }
  ...
  async getOpenOrders(bcAddress, from, to) {
    ...
    let { utxos: callbackUtxos } = await this._utxoManager.getUtxos(ScriptType.TAKER_CALLBACK, {
      address: bcAddress,
      from,
      to
    });
    ...
    for (const uc of callbackUtxos) {
      await this.addMakerCallbackOrder(openOrders, uc);
    }
    debug(`getOpenOrders(): found open orders ${openOrders.length} in ${Date.now() - start}ms`);
    return openOrders;
  }
  ...
  async addTakerOrder(matchedOrders, utxo) {
    ...
    debug(`addTakerOrder(): found matched taker orders ${matchedOrders.length} in ${Date.now() - start}ms`);
  }
  ...
  async getHistoricalOrders(from, max) {
    ...
    while (block && latestHeight - block.getHeight() < 10000 && count < max) {
      ...
      if (nextBlock) {
        block = await this._persistence.getBlockByHeight(nextBlock);
      } else {
        break;
      }
    }
    debug(`getHistoricalOrders(): found historical orders ${matchedOrders.length} in ${Date.now() - start}ms`);
  ...
  async getUnlockTakerTxParams(txHash, txOutputIndex) {
    ...
    if (takerOutputScript === makerScript || getScriptType(output.getOutputScript()) === 'taker_callback') {
      ...
      } else if (didMakerSettle) {
        scripts = [makerUnlockScript];
      } else if (didTakerSettle) scripts = [takerUnlockScript];
