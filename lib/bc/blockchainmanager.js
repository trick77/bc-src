...
class BlockchainManager {
  ...
  async blockExtendsChain(block, latestBlock) {
    ...
    debug(`evaluating isLatestBlock for ${blockchain}`);
    return this._defaultBlockExtendsChain(block, latestBlock);
  }
   ...
   */
  async _defaultBlockExtendsChain(block, latestBlock) {
    // DO NOT CHANGE THE FOLLOWING -> TYPEOF DOES NOT WORK
    const blockchain = block.getBlockchain ? block.getBlockchain().toLowerCase() : 'bc';
    let latestBlockHash = await this._persistence.get(`${blockchain}.block.latest.hash`);
    let latestBlockHeight = await this._persistence.get(`${blockchain}.block.latest.height`);

    if (!latestBlockHash) {
      latestBlock = latestBlock ? latestBlock : await this._persistence.get(`${blockchain}.block.latest`);
      latestBlockHash = latestBlock.getHash();
    }

    if (!latestBlockHeight) {
      latestBlock = latestBlock ? latestBlock : await this._persistence.get(`${blockchain}.block.latest`);
      latestBlockHeight = parseInt(latestBlock.getHeight(), 10);
    }
