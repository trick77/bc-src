...
class Wallet {
  ...
  removeTxPendingPoolOutpoints(wallet) {
  ...
  async getWalletData(address) {
    ...
    // this.removeTxPendingPoolOutpoints(wallet)
    return wallet;
  }

  async getSpendableOutpointsList(address) {
  ...
  async getTransferHistory(address, from, max = 20) {
    ...
    // let block = await this._persistence.get(`${blockchain}.block.${from}`)
  ...
  async tradeStatus(latestHeight, blockHeight, makerScript, hash, index) {
    ...
    let isMakerWithinSettlement = await this.isTxWithinSettlement(hash, index, true);
    let isTakerWithinSettlement = await this.isTxWithinSettlement(hash, index, false);
