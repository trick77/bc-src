...
const { Transaction, OutPoint } = require('@overline/proto/proto/core_pb');
...
const { scriptTypes } = require('../config/networks');
...
class TxPendingPool {
  ...
  constructor(persistence, utxoManager, relayMode = false) {
  ...
  _recordWaypointTx(waypointAddress) {
    ...
    if (!this._txsByWaypoint[w]) {
      ...
      for (const k of Object.keys(this._txsByWaypoint)) {
  ...
  // loads the block that an outpoint was spent in
  async getBlockByOutPoint(outPoint) {
    //const utxos = scriptTypes
    //const utxoIndex = await this._persistence.get(`opunspent.${}`)
    //const utxoObject = await this._persistence.get(``)
  }

  async isAnyInputSpent(tx, optionalBlock) {
    ...
    for (const input of inputs) {
      if (spentStatus) continue;
      const outpoint = input.getOutPoint();
      let unspent = await this._persistence.isOutPointUnspent(outpoint.getHash(), outpoint.getIndex());
      if (!unspent && optionalBlock) {
        const compareBlock = await this._persistence.getBlockByTxHash(outpoint.getHash());
        if (compareBlock && compareBlock.getHash) {
          debug(`block containing outpoint ${outpoint.getHash()} is ${compareBlock.getHeight()}:${compareBlock.getHash()}`);
          debug(`optional block provided ${optionalBlock.getHeight()}:${optionalBlock.getHash()}`);
          if (optionalBlock.getHash() === compareBlock.getHash() || optionalBlock.getHeight() < compareBlock.getHeight()) {
            spentStatus = false;
          } else {
            spentStatus = true;
          }
        } else if (compareBlock && !compareBlock.getHash) {
          //  this means that there are multiple block options at this height using this tx
          let stayFailed = true;
          for (const b of compareBlock) {
            if (b.getHash() === optionalBlock.getHash()) {
              // means the optional block being provided is one of the blocks at the best height of the tx
              stayFailed = false;
            }
          }
          // means there are multiple blocks at this height
          debug(`multiple blocks ${comparseBlock.length} at this height`);
          spentStatus = stayFailed;
        } else {
  ...
  //adding ephemeral messages to an array of the last 1000 messages
  addNewEphemeralMessage(msg) {
    if (this._messages.length > 1000) this._messages.shift();
    this._messages.push(msg);
  }

  getEphemeralFeedMessages(from, to) {
  ...
  async tryAddingNewTx(tx, checkFee) {
    ...
    if (tx.getInputsList().length === 0) {
      let err = `Tx ${tx.getHash()} has no inputs`;
      debug(err);
      return err;
    }
    ...
    if (Math.floor(BASE_BLOCK_SIZE / 3 * 2) < txSize) {
      let err = `Tx ${tx.getHash()} size ${txSize} is bigger than ${Math.floor(BASE_BLOCK_SIZE / 3 * 2)}`;
      debug(err);
      return err;
    }
    ...
    for (let input of tx.getInputsList()) {
      if (this.isBeingSpent(input.getOutPoint())) {
        let err = `Tx ${tx.getHash()} has an input being spent`;
        debug(err);
        return err;
      }
    }
    ...
    if (checkFee && tx.getInputsList().length === 1) {
      try {
        let isWithinSettlement = await this._utxoManager.isTxWithinSettlement(tx.getInputsList()[0].getOutPoint().getHash(), tx.getInputsList()[0].getOutPoint().getIndex());
        debugFee(`Tx ${tx.getHash()} isWithinSettlement is ${isWithinSettlement}`);
    ...
    if (!isUnlock && checkFee && feePerByte.lt(new BN(new _decimal.Decimal(this.getByteFeeMultiplier()).mul(BOSON_PER_BYTE).round().toString()))) {
      let err = `Tx ${tx.getHash()} fee per byte ${feePerByte.toString()} is less than current requirement - ${new _decimal.Decimal(this.getByteFeeMultiplier()).mul(BOSON_PER_BYTE).round().toString()}`;
      debugFee(err);
      return err;
    }
