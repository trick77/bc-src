...
const BC_TX_PENDING_POOL_MIN = exports.BC_TX_PENDING_POOL_MIN = isNaN(process.env.BC_TX_PENDING_POOL_MIN) ? 9990 : Number(process.env.BC_TX_PENDING_POOL_MIN);
const BC_TX_PENDING_POOL_MAX = exports.BC_TX_PENDING_POOL_MAX = isNaN(process.env.BC_TX_PENDING_POOL_MAX) ? 36600 : Number(process.env.BC_TX_PENDING_POOL_MAX);
...
class TxPendingPool {
  ...
  constructor(persistence) {
    ...
    this._txPoolRandomLimit = getRandomRange(BC_TX_PENDING_POOL_MIN, BC_TX_PENDING_POOL_MAX);
    this._txsByWaypoint = {};
  ...
  getSummary() {
    ...
    obj.dataSize = this._outPointPool._data.size > this._txPoolRandomLimit || this._txMemPool._data.size > this._txPoolRandomLimit;
  ...
  _recordWaypointTx(waypointAddress) {
    ...
    if (waypointAddress) {
      if (!this._txsByWaypoint[w][waypointAddress]) {
        ...
        if (this._txsByWaypoint[w][waypointAddress] > Math.floor(this._txPoolRandomLimit / 80)) {
  ...
  tryAddingNewTx(tx) {
    ...
    if (this._outPointPool._data.size > 10000 || this._txMemPool._data.size > 10000) {
