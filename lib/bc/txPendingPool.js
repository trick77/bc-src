  ...
  async isAnyInputSpent(tx) {
    ...
    for (const input of inputs) {
      ...
      if (!spent) {
        ...
        const spendingBlock = await this._persistence.getBlockByHash(outpoint.getHash());
        if (spendingBlock) {
          debugSpent(`spending block: ${spendingBlock.getHash()}, spending height: ${spendingBlock.getHeight()}`);
        }
        spentStatus = true;
      }
    }
