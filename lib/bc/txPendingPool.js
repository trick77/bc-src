...
class TxPendingPool {
  ...
  tryAddingNewTx(tx) {
    ...
    if (Math.floor(BASE_BLOCK_SIZE / 3 * 2) < txSize) {
      this._logger.info(`Tx ${tx.getHash()} size ${txSize} is bigger than ${Math.floor(BASE_BLOCK_SIZE / 3 * 2)}`);
      return false; // TODO should end with error stating what happened
    }

    for (let input of tx.getInputsList()) {
      if (this.isBeingSpent(input.getOutPoint())) {
        this._logger.info(`Tx ${tx.getHash()} size ${txSize} has an input being spent`);
        return false;
      }
    }
