...
class UtxoManager {
  ...
  async updateUTXOs(block, remove = false, calledFrom = 'utxoManager') {
    ...
    debugUTXO(`timing section A: ${(Date.now() - date) / 1000} request: ${block.getHeight()}-${block.getHash().slice(0, 8)}`);
    ...
    for (let tx of txsList) {
      ...
      for (let input of inputs) {
        ...
        if (!remove) {
          utxosToSave.push(this._persistence.put(`opspent.${hash}.${index}`, tx.getHash()));
        } else {
          utxosToSave.push(this._persistence.del(`opspent.${hash}.${index}`));
        }
      }
    }
    await Promise.all(utxosToSave);

    // save each utxo for each script type
    for (const scriptType of Object.keys(utxos)) {
      debugUTXO(`timing section C-${scriptType}: ${(Date.now() - date) / 1000} request: ${block.getHeight()}-${block.getHash().slice(0, 8)}`);
    ...
    debugUTXO(`timing section C: ${(Date.now() - date) / 1000} request: ${block.getHeight()}-${block.getHash().slice(0, 8)}`);
    ...
    if (remove) {
      ...
      await Promise.all([this._removeBcBlock(block), this._persistence.put('bc.block.last.utxoSaved', prevBlock), this._persistence.del(`bc.block.${block.getHeight()}.utxoSaved`)]);
      debugUTXO(`timing section D: ${(Date.now() - date) / 1000} request: ${block.getHeight()}-${block.getHash().slice(0, 8)}`);
      ...
      await Promise.all([this._persistence.put('bc.block.last.utxoSaved', block), this._onNewBcBlock(block), this._persistence.put(`bc.block.${block.getHeight()}.utxoSaved`, block.getHash())]);
      debugUTXO(`timing section E: ${(Date.now() - date) / 1000} request: ${block.getHeight()}-${block.getHash().slice(0, 8)}`);
    ...
    if (updateNRG && parseInt(updateNRG) === parseInt(block.getHeight()) - 1 && !remove) {
      ...
      debugUTXO(`timing section F: ${(Date.now() - date) / 1000} request: ${block.getHeight()}-${block.getHash().slice(0, 8)}`);
      ...
      debugUTXO(`timing section G: ${(Date.now() - date) / 1000} request: ${block.getHeight()}-${block.getHash().slice(0, 8)}`);
    }

    const utxosRefsToRemove = [this._persistence.del(`updateUTXOs`)];
    for (const scriptType of Object.keys(utxos)) {
      utxosRefsToRemove.push(this._persistence.del(`utxo.${scriptType}.length.${block.getHash()}`));
    }

    await Promise.all(utxosRefsToRemove);
  ...
  async addUTXO(scriptType, utxos, block) {
    ...
    if (!OL_FAST_SYNC) {
      for (const addr of Object.keys(addresses)) {
        indexes.push(this._persistence.put(`${addr}.${scriptType}`, [...new Set(addresses[addr])]));
        indexes.push(this._persistence.put(`address.last.${addr}`, block.getHash()));
      }
    }

    debugUTXO(`addUTXO(): section 4 ${Math.floor(Date.now() - date)}`);

    indexes.push(this._persistence.put(`utxo.${scriptType}.length`, length));
    indexes.push(this._persistence.put(`utxo.${scriptType}.count`, count));
  ...
  async delUTXO(scriptType, utxos, block) {
    ...
    debugUTXO(`delUTXO(): section 1D ${Math.floor(Date.now() - date)}`);
    ...
    for (let i = 0; i < utxos.length; i++) {
      ...
      debugUTXO(`delUTXO(): section 2D ${Math.floor(Date.now() - date)}`);
    ...
    debugUTXO(`delUTXO(): section 3D ${Math.floor(Date.now() - date)}`);
