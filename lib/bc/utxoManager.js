...
class UtxoManager {
  ...
  async updateUTXOs(block, remove = false, calledFrom = 'utxoManager') {
    ...
    for (const scriptType of Object.keys(utxos)) {
      ...
      tasks.push(this.addUTXO(scriptType, remove ? stxos[scriptType] : utxos[scriptType], block));
      tasks.push(this.delUTXO(scriptType, remove ? utxos[scriptType] : stxos[scriptType], block));
    }
    ...
    if (remove) {
      ...
      await Promise.all([this._persistence.put('bc.block.last.utxoSaved', block), this._persistence.put(`bc.block.${block.getHeight()}.utxoSaved`, block.getHash()), this._onNewBcBlock(block)]);
