...
class UtxoManager {
  ...
  async updateUTXOs(block, remove = false, calledFrom = 'utxoManager') {
    ...
    if (!lastSavedBlock && block.getHeight() !== 1) {
      ...
      if (pb && pb.getHash) {
        ...
        await this._persistence.put('bc.block.last.utxoSaved', pb);
        await this._persistence.put(`bc.block.${pb.getHeight()}.utxoSaved`, pb.getHash());
      } else {
  ...
  async areUTXOsSavedForBlock(height, hash) {
    const saved = await this._persistence.get(`bc.block.${height}.utxoSaved`);
    return saved === hash;
  }
