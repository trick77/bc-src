...
const { NRG_MINTED_PERISTENCE_KEY } = require('../persistence/rocksdb');
...
class UtxoManager {
  ...
  async updateUTXOs(block, remove = false, calledFrom = 'utxoManager') {
    ...
    if (!remove) {
      ...
      if (!childBlockPassesRoveredBlock) {
        this._logger.warn(`block ${block.getHeight()} failed childBlockPassesRoveredBlock`);
        return result;
      }
      await this._persistence.saveBlock(block);
      if (!BC_NO_CLEAN && parseInt(block.getHeight(), 10) % 100000 === 0) await this.cleanUpUTXOs();
    } else {
      const txs = block.getTxsList();
      if (txs) {
        this._logger.info(`unindexing ${txs.length} from ${block.getHeight()}:${block.getHash()}`);
        for (const tx of txs) {
    ...
    let lastSavedBlock = await this._persistence.get(`${BC_SUPER_COLLIDER}.block.last.utxoSaved`, { sync: true });
    if (!lastSavedBlock && block.getHeight() !== 1) {
      this._logger.warn(`last block has not been stored`);

      const pb = await this._persistence.getBlockByHash(block.getPreviousHash(), BC_SUPER_COLLIDER);
      if (pb && pb.getHash) {
        this._logger.info(`setting mount for utxo index`);
        await this._persistence.put(`${BC_SUPER_COLLIDER}.block.last.utxoSaved`, pb);
        await this._persistence.put(`${BC_SUPER_COLLIDER}.block.${pb.getHeight()}.utxoSaved`, pb.getHash());
      } else {
        this._logger.warn(`unable to find mount for utxo index`);
        return false;
      }
    }
    let isUpdating = await this._persistence.get(`updateUTXOs`, { sync: true });

    if (isUpdating === `${remove ? 'REMOVING' : 'SAVING'}.${block.getHash()}`) {
      debug(`running update on ${block.getHash()}`);
    } else if (lastSavedBlock) {
      // check if block being evaluated is in order
      if (remove && lastSavedBlock.getHash() !== block.getHash()) {
        debug(`${lastSavedBlock.getHeight()} was the last block saved`);
        return result;
      }
    ...
    if (isUpdating) {
      debug(`isUpdating is ${isUpdating}`);
    }
    if (isUpdating && isUpdating !== `${remove ? 'REMOVING' : 'SAVING'}.${block.getHash()}`) {
      debug(`in the midst of an update of ${isUpdating} while evaluating ${block.getHeight()}:${shortenHash(block.getHash())}`);
      return result;
    } else {
      debug(`setting update evaluation ${block.getHeight()}:${shortenHash(block.getHash())}`);
      await this._persistence.put(`updateUTXOs`, `${remove ? 'REMOVING' : 'SAVING'}.${block.getHash()}`, { sync: true });
    }

    const txs = await this._persistence.getUnspentAndSpentForBlock(block);
    if (txs === false) {
      await this._persistence.del(`updateUTXOs`, { sync: true });
      return result;
    }
    ...
    for (let i = 0; i < Object.keys(utxos).length; i++) {
      ...
      if (scriptType == 'taker_output' && utxos[scriptType].length > 0) {
        if (remove) await this._persistence.delBlockHasTaker(block.getHeight());else await this._persistence.setBlockHasTaker(block.getHeight());
      }
    ...
    if (updateNRG && parseInt(updateNRG) === parseInt(block.getHeight()) - 1 && !remove) {
      ...
      debug(`nrg supply is ${mintedNrgTotal.toString()}`);
  ...
  async removeUTXOsFrom(height) {
    this._logger.info(`attempting to remove utxos from ${height}`);
    let result = { result: false, tryAddingNewTxs: [], markAsMinedTxs: [] };
    let lastSavedBlock = await this._persistence.get('bc.block.last.utxoSaved', { sync: true });
    if (lastSavedBlock) {
      // just remove the last block if the same height as the latest
      if (lastSavedBlock.getHeight() === height) {
        debug(`last save block ${lastSavedBlock.getHash()} at the height ${height}`);
        ...
        debug(`blocks to remove ${blocksToRemove.length}`);

        for (let i = 0; i < blocksToRemove.length; i++) {
          const removeBlock = blocksToRemove[i];
          debug(`processing block to remove ${removeBlock.getHeight()}: ${removeBlock.getHash()}`);
          let updateUTXOsResult = await this.updateUTXOs(removeBlock, true);
          if (!updateUTXOsResult.result) return result;else {
      ...
      debug(`no last saved block available from ${height}`);
    }
    result.result = true;
    return result;
  }
  ...
  async calculateNRGSupply() {
    let mintedNrgTotal = new Decimal(0);
    debugUTXO('calling cleanup');
    // save each utxo for each script type
    ...
    for (let i = 0; i < utxos.length; i++) {
      ...
      for (let j = 0; j < length; j++) {
        let utxo = await this._persistence.get(`utxo.${scriptType}.${j}`);
        if (utxo) {
          mintedNrgTotal = mintedNrgTotal.add(new Decimal(internalToHuman(utxo.getOutput().getValue(), NRG)));
        }
      }
    }
    let last = await this._persistence.get(`bc.block.last.utxoSaved`);
    await this._persistence.put(`nrg_calculated`, last.getHeight());
    await this.setNrgMintedSoFar(mintedNrgTotal.toString());
  }
  ...
  */
  async unsettleEmbTx({ to, from, chain, amount, height, tokenType, hash, childHash, blockHeight }, block) {
    try {
      ...
      if (currentHeight && new BN(currentHeight).gt(new BN(6717275))) {
        embIndexKey = 'alpha';
        if (!height && blockHeight) {
          height = blockHeight;
        }
      ...
      if (isUniqueEMBTx === `${block.getHeight()}.${block.getHash()}`) {
        ...
        if (fromBalance) {
          ...
          await this._persistence.put(fromBalanceKey, `${updatedFromBalance.toNumber()}:${height}:${timestamp}`, { sync: true });
        } else {
          await this._persistence.put(fromBalanceKey, `${amount.toNumber()}:${height}:${timestamp}`, { sync: true });
        }
        ...
        if (toBalance) {
          ...
          if (0 >= updatedToBalance.toNumber()) {
            ...
            await this._persistence.put(toBalanceKey, `${updatedToBalance.toNumber()}:${height}:${timestamp}`, { sync: true });
          }
        }
      } else {
  ...
  */
  async settleEmbTx({ to, from, chain, amount, height, tokenType, hash, childHash, blockHeight }, block) {
    try {
      ...
      if (!isUniqueEMBTx) {
        ...
        // RUN EMB UPDATE HERE
        await this._persistence.put(`${chain}.${tokenType}.${hash}.${childHash}.${height}`, `${block.getHeight()}.${block.getHash()}`, { sync: true });
        ...
        if (!toBalance) {
          ...
          await this._persistence.put(toBalanceKey, `${amount.toNumber()}:${height}:${timestamp}`, { sync: true });
        } else {
          const toBalanceAmount = parseInt(toBalance.split(":")[0], 10);
  ...
  async unsettleTx(markedTxHash, childChainId, childChainHeight, childChainHash, bcHeight, txHash, txOutputIndex, isMaker) {
    try {
      ...
      if (markedKey === tradeExisting && tradeKey === markedExisting) {
        await this._persistence.del(markedKey, { sync: true });
        await this._persistence.del(`${markedKey}.hash`, { sync: true });
        await this._persistence.del(tradeKey, { sync: true });
        await this._persistence.del(markTxSavedKey, { sync: true });
        await this._persistence.del(markTxSavedHashKey, { sync: true });
        await this._persistence.del(`${markedTxHash}.ref`, { sync: true });
      }
  ...
  async settleTx(markedTxHash, childChainId, childChainHeight, childChainHash, bcHeight, txHash, txOutputIndex, isMaker) {
    try {
      ...
      if (bcHeight > 2590000) {
        await this._persistence.put(`${markedKey}.hash`, childChainHash, { sync: true });
        await this._persistence.put(`${markedTxHash}.ref`, markedKey, { sync: true });
      }

      this._logger.info(`setting ${tradeKey} - ${markedKey} . ${childChainHash}`);
      await this._persistence.put(tradeKey, markedKey, { sync: true });
      await this._persistence.put(markedKey, tradeKey, { sync: true });
      await this._persistence.put(markTxSavedKey, true, { sync: true });
      if (bcHeight >= 3950000) {
        await this._persistence.put(markTxSavedHashKey, true, { sync: true });
      }
  ...
  async setNrgMintedSoFar(nrg) {
    await this._persistence.put(NRG_MINTED_PERISTENCE_KEY, nrg);
  }
  ...
  async addUTXO(scriptType, utxos, block) {
    ...
    if (oldLength) {
      ...
      await this._persistence.put(`utxo.${scriptType}.length.${block.getHash()}`, length, { sync: true });
    }
    ...
    for (let i = 0; i < utxos.length; i++) {
      ...
      if (!utxoIndex) {
        await this._persistence.put(`utxo.${scriptType}.${length}`, utxo, { sync: true });
        await this._persistence.put(`opunspent.${tx.getHash()}.${index}`, length, { sync: true });
      } else {
    ...
    if (!OL_FAST_SYNC) {
      ...
      for (const addr of Object.keys(addresses)) {
        ...
        await this._persistence.put(`${addr}.${scriptType}`, [...new Set(addresses[addr])], { sync: true });
        await this._persistence.put(`address.last.${addr}`, block.getHash(), { sync: true });
      }
      //await this._persistence.putBulk(puts)
    }

    await this._persistence.put(`utxo.${scriptType}.length`, length, { sync: true });
    await this._persistence.put(`utxo.${scriptType}.count`, count, { sync: true });
  ...
  async delUTXO(scriptType, utxos, block) {
    ...
    for (const del of dels) {
      await this._persistence.del(del, { sync: true });
    }
  ...
  async viewUTXOs() {
    ...
    for (let i = 0; i < Object.keys(utxos).length; i++) {
      ...
      // collect all null and full indexes
      for (let j = 0; j < length; j++) {
        let utxo = await this._persistence.get(`utxo.${scriptType}.${j}`, { sync: true });
      ...
      for (let j = 0; j < notFound.length; j++) {
        ...
        count--;
        await this._persistence.del(`utxo.${scriptType}.${index}`, { sync: true });
        await this._persistence.del(`opunspent.${tx.getHash()}.${txIndex}`, { sync: true }); // txHash.txIndex -> index
      }
