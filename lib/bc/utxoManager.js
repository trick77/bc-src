...
class UtxoManager {
  ...
  async updateUTXOs(block, remove = false, calledFrom = 'utxoManager') {
    ...
    if (!remove) {
      ...
      if (!BC_NO_CLEAN && parseInt(block.getHeight(), 10) % 50000 === 0) await this.cleanUpUTXOs();
    } else {
    ...
    if (isUpdating === `${remove ? 'REMOVING' : 'SAVING'}.${block.getHash()}`) {
      this._logger.info(`running update on ${block.getHash()}`);
    ...
    if (txs === false) {
      this._logger.info(`no valid utxos found for block ${block.getHeight()}`);
    ...
    // TD DEBUG
    //console.log(`timing section A: ${(Date.now() - date) / 1000} request: ${block.getHeight()}-${block.getHash().slice(0, 8)} called from ${calledFrom}`)
  ...
  async removeUTXOsFrom(height) {
    let result = { result: false, tryAddingNewTxs: [], markAsMinedTxs: [] };
    let lastSavedBlock = await this._persistence.get(`${BC_SUPER_COLLIDER}.block.last.utxoSaved`);
    this._logger.info(`removing utxos from ${height}`);
    if (lastSavedBlock) {
      ...
      if (lastSavedBlock.getHeight() === height) {
        ...
        for (let i = 0; i < blocksToRemove.length; i++) {
          ...
          this._logger.info(`reindexing utxos ${removeBlock.getHeight()}`);
  ...
  async _onNewBcBlock(block) {
    try {
      ...
      if (uncleBlocks) {
        for (const uncle of uncleBlocks) {
          debugMarking(`uncle block is ${uncle.getHash()}:${uncle.getHeight()}`);
          let markedList = uncle.getMarkedTxsList();
          for (const ml of markedList) {
            let mtx = getDetailsFromMtx(ml);
      ...
      for (const mTx of markedBlockTransactions) {
        debugMarking({ mTx });
        ...
        if (trades) {
          ...
          for (const t of trades) {
            let [txHash, txIndex,,, type] = t.split('.');
  ...
  async settleEmbTx({ to, from, chain, amount, height, tokenType, hash, childHash, blockHeight }, block) {
    try {
      ...
      if (!isUniqueEMBTx) {
        this._logger.info(`settleEmbTx(): ${embIndexKey} to: ${to}, from: ${from}, amount: ${amount.toNumber()}`);
        // RUN EMB UPDATE HERE
  ...
  async isTxSettled(txHash, txOutputIndex, isMaker) {
    ...
    try {
      ...
      if (!output) {
        let block = await this._persistence.getBlockByTxHash(txHash);
        if (block && !block.getHash) {
          // this means there were multiple blocks at this height using this tx so return the one found first
          block = block[0];
        }
  ...
  async isTxWithinSettlement(txHash, txOutputIndex, latest, onlyMaker = false) {
    try {
      ...
      let block = await this._persistence.getBlockByTxHash(txHash); // getOutputByHashAndIndex
      if (block && !block.getHash) {
        // this means there were multiple blocks at this height using this tx so return the one found first
        block = block[0];
      }
  ...
  async _getInitialMakerOrderData(outputScript, blockHeight = 0) {
    ...
    if (parentTxHash) {
      let block = await this._persistence.getBlockByTxHash(parentTxHash);
      if (block && !block.getHash) {
        block = block[0];
      }
      if (block) {
  ...
  async addUTXO(scriptType, utxos, block) {
    ...
    if (oldLength) {
      ...
      // support for old index format
      if (oldLength.indexOf && oldLength.indexOf(':') > -1) {
        oldLength = oldLength.split(':')[0];
      }
      length = oldLength;
    } else {
    ...
    for (const u of utxos) {
      let { index, tx, hash, height } = u;

      // support for old index format
      if (index && index.indexOf && index.indexOf(':') > -1) {
        index = index.split(':')[0];
      }
      ...
      debugUTXO(`addUTXO(): section 2 ${Math.floor(Date.now() - date)}ms ${utxo.getBlockHeight()}`);
  ...
  buildUtxo(tx, index, block, hash, height) {
    // support for block height with concatenated blockchain
  ...
  async delUTXO(scriptType, utxos, block) {
    ...
    debugUTXO(`delUTXO(): ${block && block.getHeight ? block.getHeight() : 'NA'} section 1D ${Math.floor((Date.now() - date) / 1000)} utxos ${utxos.length}, utxoToAddresses: ${Object.keys(utxoToAddresses).length}, addresses: ${Object.keys(addresses).length}`);
    ...
    for (const u of utxos) {
      ...
      let utxoIndex = await this._persistence.get(`opunspent.${tx.getHash()}.${index}`);
      // support the old index which had a concatenated blockchain
      if (utxoIndex && utxoIndex.indexOf && utxoIndex.indexOf(':') > -1) {
        utxoIndex = utxoIndex.split(':')[0];
      }
      ...
      if (utxoIndex >= 0) {
        ...
        nextDels.push(`opunspent.${tx.getHash()}.${index}:bc`); // support old index
        count = count - 1;
      } else {
        //debugUTXODetail(`utxo not saved saved for ${tx.getHash()}.${index}`)
        // TD
        this._logger.warn(`utxo not saved saved for ${tx.getHash()}.${index}`);
      }
    }
  ...
  async viewUTXOs() {
    ...
    for (let i = 0; i < Object.keys(utxos).length; i++) {
      ...
      for (let j = 0; j < notFound.length; j++) {
        ...
        await this._persistence.del(`opunspent.${tx.getHash()}.${txIndex}:bc`); // txHash.txIndex -> index // support old format
      }
  ...
  async getUtxos(scriptType, opts = { from: null, to: null, address: false }) {
    ...
    let utxos = [];
    for (const u of keys) {
      const ux = await this._persistence.get(u);
      if (ux !== null) {
        utxos.push(ux);
      }
    }
      ...
      for (const input of inputs) {
        ...
        if (outPoint.getHash() === makerTxHash && outPoint.getIndex() === makerTxOutputIndex) {
          ...
          for (let i = 0; i < keys.length; i++) {
            ...
            let hashes = await this._persistence.getTradesByKey(key);
            if (remove) {
              if (hashes) {
                ...
                if (check > -1) {
                  ...
                  await this._persistence.putTradesByKey(key, hashes);
                }
              }
            } else {
              if (!hashes) {
  ...
  async extractAddressesFromUtxos(scriptType, utxos) {
    ...
    // TD very ineffecient fix it
    for (const a of addresses) {
      const utx = await this.getUtxoIndexesByAddress(scriptType, a, true);
      utxoIndexes.push(utx);
    }
  ...
  async getUtxoIndexesByAddress(scriptType, address, hashed) {
    ...
    if (!hashed) address = '0x' + blake2bl(blake2bl(address) + address);
