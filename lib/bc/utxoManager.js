...
class UtxoManager {
      ...
      if (lastSavedBlock.getHeight() === height) {
        ...
        for (let i = lastSavedBlock.getHeight(); i > height; i--) {
          if (block) {
            ...
            if (!block) {
              const blocks = await this._persistence.getBlocksByHeight(block.getHeight(), BC_SUPER_COLLIDER);
  ...
  */
  ...
  async delUTXO(scriptType, utxos, block) {
    ...
    await this._persistence.delBulk(dels);
    await this._persistence.delBulk(nextDels);
