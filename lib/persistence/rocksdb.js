...
class PersistenceRocksDb {
  ...
  async putReorgBlocks(reorgFromBlock, reorgToBlock, opts = {
    ...
    if (!opts.force && blocksAlreadySet && multiverseChanging) {
      ...
      // LDL
      debugReorg('simplex in place yielding multiverse change');
      return Promise.resolve(false);
    }
  ...
  async updateUTXOs(block, remove = false) {
    ...
    if (remove) {
      ...
      for (const tx of block.getTxsList()) {
        this._logger.info(`updating ${tx.getHash()}`);
  ...
  async putBlock(block, branch = 0, blockchain = 'bc', opts = {
    ...
    if (newBlocks > 228) {
      this._logger.warn(`new block has more than maximum 228 headers (${newBlocks})`);
    }
    ...
    if (block.getTxsList !== undefined) {
      ...
      if (opts.saveHeaders) {
        ...
        for (let h of headers) {
          await this.putBlock(h, 0, h.getBlockchain());
        }
      }
      // DEBUG
    }
  ...
  async getMarkedBalanceData(address, tokenAddress = EMBLEM_CONTRACT_ADDRESS, connectedChain = 'eth', blockchain = 'bc', opts = { asBuffer: true }) {
    ...
    try {
      ...
      const edge = await this.get(`${blockchain}.sync.edge`);
      if (edge && new BN(edge).lte(new BN(2999999)) && !BC_MARKED_DRY_RUN) {
        return new BN(0);
      }
      ...
      const balance = await this.get(`${connectedChain}.${tokenType}.${tokenAddress}.${address}.balance`);

      if (!balance) {
        return new BN(0);
      }

      const balanceAmount = new BN(Buffer.from(balance.split(':')[0], 'hex'));
  ...
  */
  async settleEmbTx({ to, from, chain, amount, height, tokenType, hash, childHash }, block) {
    ...
    if (!isUniqueEMBTx) {
      ...
      const toBalanceKey = `${chain}.${tokenType}.${EMBLEM_CONTRACT_ADDRESS}.${to}.balance`;
      const fromBalanceKey = `${chain}.${tokenType}.${EMBLEM_CONTRACT_ADDRESS}.${from}.balance`;
      ...
      const fromBalance = await this.get(toBalanceKey);
      if (fromBalance) {
        const fromBalanceAmount = Buffer.from(fromBalance.split(":")[0], 'hex');
        const updatedFromBalance = new BN(fromBalanceAmount).minus(new BN(amount));
        if (new BN(0).gte(updatedFromBalance)) {
          await this.del(fromBalanceKey);
        } else {
          await this.put(fromBalanceKey, `${updatedFromBalance.toString('hex')}:${height}:${timestamp}`);
        }
      }

      // second attempt to update the to balance
      const toBalance = await this.get(toBalanceKey);
      if (!toBalance) {
        await this.put(toBalanceKey, `${new BN(amount).toString('hex')}:${height}:${timestamp}`);
      } else {
        const toBalanceAmount = Buffer.from(toBalance.split(":")[0], 'hex');
        const updatedToBalance = new BN(toBalanceAmount).add(new BN(amount)).toString('hex');
        await this.put(toBalanceKey, `${updatedToBalance}:${height}:${timestamp}`);
      }
    }
  }
  ...
  async unsettleEmbTx({ to, from, chain, amount, height, tokenType, hash, childHash }, block) {
    ...
    if (isUniqueEMBTx === `${block.getHeight()}.${block.getHash()}`) {
      ...
      if (fromBalance) {
        ...
        await this.put(fromBalanceKey, `${new BN(amount).toString('hex')}:${height}:${timestamp}`);
      }
      ...
      if (toBalance) {
        const toBalanceAmount = Buffer.from(toBalance.split(":")[0], 'hex');
        const updatedToBalance = new BN(toBalanceAmount).minus(new BN(amount));
        if (new BN(0).gte(updatedToBalance)) {
          await this.del(toBalanceKey);
        } else {
          await this.put(toBalanceKey, `${updatedToBalance.toString('hex')}:${height}:${timestamp}`);
        }
      }
      //RUN EMB UPDATE HERE
    }
