...
const { COINBASE_TX_ESTIMATE_SIZE, getMaxBlockSize, getMaxDistanceWithEmblems, getNrgGrant, getTxsDistanceSum, calcTxFee } = require('../core/txUtils');
...
class PersistenceRocksDb {
  ...
  constructor(location = '_data', chainState, engine) {
    ...
    this._blockByHeightCache = LRUCache({
      max: 1000
    });
    this._blocksByHeightCache = LRUCache({
      max: 1000
    });
    this._blockByHashCache = LRUCache({
      max: 1000
    });
    this._latestBlockCache = LRUCache({ max: 100 });

    this._blockByTxCache = LRUCache({
      max: 1000
    });

    this._transactionByHashCache = LRUCache({
      max: 100000
    });

    this._utxoLengthCache = LRUCache({
      max: 10
    });

    this._blockByUtxoCache = LRUCache({
      max: 2000
    });

    this._currBlockEvalCache = LRUCache({
      max: 1
    });
    ...
    this._completedBlockSegmentsCache = LRUCache({
      max: 250
    });
  ...
  async reorgBlockchain(blockchain = BC_SUPER_COLLIDER, opts = { toBlock: false, fromBlock: false, iterateUp: true, peer: false, force: false, reorgTo: false }) {
    ...
    try {
      ...
      latestBlock = latestBlock ? latestBlock : reorgFromBlock;
      await this.put(`${blockchain}.sync.edge`, parseInt(latestBlock.getHeight(), 10));
      ...
      if (synced === 'pending' && !opts.reorgTo) {
        ...
        await this.put(`${BC_SUPER_COLLIDER}.data.latest`, `${latestBlock.getHeight()}:${tn}`);
      }
  ...
  async processPeerExpiration(opts = {}) {
    ...
    if (currentPeer && new BN(time).gt(new BN(currentPeer.getExpires()))) {
      // peer expired
      this._logger.info(`peer unlocked, ${BC_SUPER_COLLIDER} new assignment created`);
      ...
      const reorgFrom = await this.get(`${BC_SUPER_COLLIDER}.block.reorgfrom`);
      if (this._chainState && reorgFrom) {
        // LDL
        debug(`no current peer assigned despite open multiverse change request <- reloading multiverse on previous state`);
        await this.reorgBlockchain(BC_SUPER_COLLIDER, { chainState: this._chainState });
      } else if (reorgFrom) {
        // LDL
        debug(`no current peer assigned despite open multiverse change request <- reloading multiverse on previous state`);
    ...
    try {
      ...
      if (opts.cached) {
        if (this._transactionByHashCache.has(`${blockchain}.tx.${txHash}`)) {
          if (this._transactionByHashCache.get(`${blockchain}.tx.${txHash}`) != null) {
  ...
  */
  ...
  async putTransaction(tx, blockHash, branch = 0, blockchain = 'bc', opts = { asBuffer: true, force: false }) {
    ...
    try {
      ...
      // if (tx.getOutputsList !== undefined && tx.getInputsList !== undefined&& branch !== undefined && branch === 0) {
      // await this.addTxDetails(tx,blockchain)
      // }
      return true;
    } catch (err) {
      this._logger.error(err);
  ...
  async putChildBlocksIndexFromBlock(block, blockchain = 'bc', opts = { asBuffer: true, storeOnlyOne: false }) {
    ...
    try {
      ...
      for (const header of headers) {
        ...
        let hashes = await this.get(childKey);
        if (!hashes) {
  ...
  async putTransactionBlockIndex(txHashes, blockHash, blockHeight, branch = 0, blockchain = 'bc', opts = { asBuffer: true }) {
    ...
    try {
      let _this = this;
      let bulk = await this.putBulk(txHashes.map(hash => {
  ...
  */
  async getRootedBlockFromBlock(block, chainToReturn = [], opts = { asBuffer: true, returnParents: false }) {
    ...
    this._logger.info(`search for root block by hash ${block.getPreviousHash()} ${blockchain}`);
    const prevBlock = await this.getBlockByHash(block.getPreviousHash(), blockchain);
    if (prevBlock) {
      chainToReturn.push(prevBlock);

      const key = `${blockchain}.child.${prevBlock.getHash()}`;
      this._logger.info(`searching for ${BC_SUPER_COLLIDER} block by child key ${key.slice(0, 21)}`);
  ...
  */
  async putLatestBlock(block, defaultBlockchain = 'bc', opts = { asBuffer: true, previousBlock: false, reloadTxs: false, saveHeaders: false, iterateUp: true }) {
    ...
    try {
      ...
      const potentialHigherBlocks = await this.getBlocksByHeight(givenHeight, blockchain, { asHeader: true });
      let fastBlock = false;
      if (opts.reloadTxs) {
      ...
      let edge = await this.get(`${blockchain}.sync.edge`);
      if (!edge) {
  ...
  async removeUTXOsFrom(height) {
    let blocksToRemove = [];
    do {
      let hash = await this.get(`bc.block.${height}.utxoSaved`);
      if (!hash) break;
      let candidate = await this.getBlockByHash(hash, 'bc');
      if (candidate) {
        blocksToRemove.unshift(candidate);
      } else {
        let lastSaved = await this.get('bc.block.last.utxoSaved');
        this._logger.info(`Retrieving #${height} last block saved for UTXOs - Block Height is ${lastSaved.getHeight()}`);
  ...
  async areUTXOsSavedForBlock(height, hash) {
    let saved = await this.get(`bc.block.${height}.utxoSaved`);
    return saved === hash;
  }
  async updateCoinbase(newBlock, remove = false) {
    ...
    if (newBlock.getHeight() == 1) {
      ...
      for (let output of txOutputs) {
        mintedNrg += parseInt(internalToHuman(output.getValue(), NRG));
      }
    } else {
      ...
      mintedNrg = parseInt(internalToHuman(minerRewardBN.sub(txFeesBN), NRG));
    }

    if (remove) mintedNrg = 0 - mintedNrg;
    this._logger.debug(`${remove ? 'Removed' : 'Added'} hash: ${shortenHash(newBlock.getHash())}, Height: ${newBlock.getHeight()}, PrevHash: ${shortenHash(newBlock.getPreviousHash())}, ${mintedNrg} NRG indexed, Total: ${mintedNrg + mintedNrgTotal} NRG`);
    await this.setNrgMintedSoFar(mintedNrg + mintedNrgTotal);
    return;
  }

  async updateUTXOs(block, remove = false) {
    let date = Date.now();
    let txs = await this.getUnspentAndSpentForBlock(block);
    ...
    if (remove) {
      ...
      for (let scriptType of Object.keys(utxos)) {
  ...
  async getTxFromInput(txInput) {
    let tx2 = await this.getTransactionByHash(txInput.getHash());

    const id = `${BC_SUPER_COLLIDER}.txblock.${txInput.getHash()}`;
    let key = this._blockByTxCache.has(id) ? this._blockByTxCache.get(id) : await this.get(id);
    let [blockchain, _, hash, height] = key.split('.');

    let scriptType = getScriptType(tx2.getOutputsList()[txInput.getIndex()].getOutputScript());
  ...
  async getUnspentAndSpentForBlock(block) {
    try {
      let utxos = { 'nrg_transfer': [], 'maker_output': [], 'taker_output': [], 'taker_callback': [] };
      let stxos = { 'nrg_transfer': [], 'maker_output': [], 'taker_output': [], 'taker_callback': [] };
  ...
  async extractAddressesFromUtxos(scriptType, utxos) {
    ...
    for (let _ref of utxos) {
      ...
      if (scriptType == ScriptType.NRG_TRANSFER) {
        ...
        let [originalScript, blockHeight, makerOutput] = await this.getInitialMakerOrder(script);
    ...
    let utxoIndexes = await Promise.all(addresses.map(a => {
  ...
  buildUtxo(tx, index, block, hash, height) {
    let arr = [tx.getOutputsList()[index].toObject(), tx.getHash(), index, hash ? hash : block.getHash(), height ? height : block.getHeight(), tx.getInputsList().length == 0];
    // console.log(arr)
    let utxo = new Utxo(arr);
  ...
  async addUTXO(scriptType, utxos, block, unspent = true) {
    ...
    if (unspent) {
      let extraction = await this.extractAddressesFromUtxos(scriptType, utxos.map(({ index, tx }) => {
    ...
    for (let _ref2 of utxos) {
      let { index, tx, hash, height } = _ref2;

      const key = (unspent ? `opunspent.` : `opspent.`) + `${tx.getHash()}.${index}`;
      let indexKey = (unspent ? `utxo.` : `stxo.`) + `${scriptType}.${length}`;
      let utxo = this.buildUtxo(tx, index, block, hash, height);
      if (unspent) {
    ...
    // console.log(`took ${Date.now()-date} secs`)

    let lengthKey = (unspent ? `utxo.` : `stxo.`) + `${scriptType}.length`;
    puts.push([lengthKey, length]);

    await this.putBulk(puts);
    // console.log(`Block #${block.getHeight()} took ${(Date.now() - date)/1000}`)
  ...
  async delUTXO(scriptType, utxos, block, unspent = true) {
    // try {
    if (utxos.length === 0) return true;
    let type = unspent ? `utxo.${scriptType}` : `stxo.${scriptType}`;
    let opType = unspent ? `opunspent` : `opspent`;
    let length = await this.getUtxosLength(scriptType, !unspent);

    //get the index of utxos we need to remove
    let deletions = utxos.map(({ index, tx }) => {
      // console.log(`delete - ${opType}.${tx.getHash()}.${index}`)
      return `${opType}.${tx.getHash()}.${index}`;
    });
    let utxoIndexes = await this.getBulk(deletions);
    let utxoToIndex = zip(utxos.map(({ index, tx }) => {
      return `${tx.getHash()}.${index}`;
    }), utxoIndexes);

    //keep track of utxos we don't need to replace vs ones we do
    let onlyRemove = [];
    let indexesToReplace = utxoIndexes.filter((index, i) => {
    ...
    let replaceIndexes = indexesToReplace.map(_ => {
      replace = replace - 1;
    ...
    let replaceUTXOs = await this.getBulk(replaceIndexes.map(r => {
      return `${type}.${r}`;
    }), { utxoCache: true });

    let utxoToIndex2 = zip(replaceUTXOs.map(utxo => {
  ...
  async getUtxoIndexesByAddress(scriptType, address, hashed) {
    ...
    let indexes = await this.get(`${address}.${scriptType}`);
  ...
  async getUtxos(scriptType, opts = { spent: false, from: null, to: null, address: false }) {
    ...
    if (opts.address) {
      ...
      let length = await this.getUtxosLength(scriptType, opts.spent);
      let from = opts.from ? opts.from : 0;
      let to = opts.to ? opts.to : length;
    ...
    let utxos = await this.getBulk(keys, { utxoCache: true });
    return { utxos, scriptType };
  }

  async getUtxosLength(scriptType, spent) {
    let key = spent ? `stxo.${scriptType}.length` : `utxo.${scriptType}.length`;
    const length = await this.get(key);
    debug(`${scriptType} LENGTH of UTXOS ${length ? length : 0}`);
  ...
  async getTxClaimedBy(hash, index, blockchain = 'bc') {
    const key = `${blockchain}.opspent.${hash}.${index}`;
  ...
  */
  async putBlock(block, branch = 0, blockchain = 'bc', opts = { asBuffer: true, saveHeaders: false, force: false, storeOnlyOne: false, updateHeight: true, rovered: false }) {
    ...
    if (cachedBlock) {
      if (cachedBlock.getHash && cachedBlock.getHash() === block.getHash() && !opts.force) {
        return [];
    ...
    try {
      ...
      let existingBlock = await this.get(`${blockchain}.block.${block.getHash()}`);
      ...
      } else if (!this._blockByHeightCache.has(`${blockchain}.block.${parseInt(block.getHeight(), 10)}`)) {
        this._blockByHeightCache.set(`${blockchain}.block.${parseInt(block.getHeight(), 10)}`, block);
      }
      const externalOps = [];
      ...
      if (block.getTxsList !== undefined) {
        ...
        if (opts.saveHeaders) {
          await Promise.all(headers.map(header => this.putBlock(header, 0, header.getBlockchain(), { force: opts.saveHeaders }))); // put child blocks
        }
        //DEBUG
      }
      return externalOps;
   ...
   * @param blockchain string
   */
  async delBlock(hash, branch = 0, blockchain = 'bc', opts = { asBuffer: true }) {
    try {
      let block;
      let key;
      if (hash === undefined) {
      ...
      let txs = block.getTxsList !== undefined ? block.getTxsList() : block.getMarkedTxsList();
  ...
  async getBlockByHeight(height, blockchain = 'bc', opts = { asBuffer: true }) {
    ...
    let block;
    try {
      if (opts.cached && this._blockByHeightCache.has(key)) {
  ...
  async getBlocksByRange(from, to, blockchain = 'bc', opts = { asBuffer: true, asSet: false, cached: false }) {
    ...
    if (!opts.asSet) {
      ...
      let best = null;
      for (let mount of Object.keys(blockSet)) {
        for (let branch of blockSet[mount]) {
  ...
  async getBlockByTxHash(txHash) {
    ...
    try {
      ...
      if (key) {
        let [blockchain, _, hash, height] = key.split('.');
        let block = await this.getBlockByHash(hash, blockchain, { asHeader: true, cached: true });
        return block;
      }
      return null;
    } catch (err) {
  ...
  async getMarkedTxsForMatchedTx(txHash, txOutputIndex) {
    let makerMarkedTx = await this.get(`settle.tx.${txHash}.${txOutputIndex}.maker`);
    let takerMarkedTx = await this.get(`settle.tx.${txHash}.${txOutputIndex}.taker`);
    let markedTxs = [];
    ...
    for (const markedKey of [makerMarkedTx, takerMarkedTx]) {
      if (markedKey) {
        ...
        let bcBlock = await this.getBlockByHeight(bcHeight);
        ...
        for (let childBlock of bcBlock.getBlockchainHeaders()[`get${childChainId}List`]()) {
          if (childBlock.getHeight() === Number(childChainHeight)) {
            for (let markedTx of childBlock.getMarkedTxsList()) {
              if (markedTx.getHash() === markedTxHash) markedTxs.push(markedTx);
            }
          }
        }
      }
  ...
  */
  async settleTx(markedTxHash, childChainId, childChainHeight, bcHeight, txHash, txOutputIndex, isMaker) {
    try {
      let tradeParty = isMaker ? 'maker' : 'taker';

      let markedKey = `${BC_SUPER_COLLIDER}.${bcHeight}.${childChainId}.${childChainHeight}.markedTx.${markedTxHash}`;
      let tradeKey = `settle.tx.${txHash}.${txOutputIndex}.${tradeParty}`;
  ...
  */
  async unsettleTx(markedTxHash, childChainId, childChainHeight, bcHeight) {
    try {
      let markedKey = `${BC_SUPER_COLLIDER}.${bcHeight}.${childChainId}.${childChainHeight}.markedTx.${markedTxHash}`;
  ...
  */
  async isTxSettled(txHash, txOutputIndex, isMaker) {
    ...
    let tradeKey = `settle.tx.${txHash}.${txOutputIndex}.${tradeParty}`;
    try {
      let markedKey = await this.get(tradeKey);

      if (!markedKey) {
        return false;
      }

      let checkTradeKey = await this.get(markedKey);
      ...
      // found the marked key, double check it is actually within the appropriate BC block and its child block
      for (let childBlock of getChildBlocks(bcBlock, childChainId.toLowerCase())) {
        if (childBlock.getHeight() === Number(childChainHeight)) {
          for (let markedTx of childBlock.getMarkedTxsList()) {
   ...
   *
   */
  async isTxWithinSettlement(txHash, txOutputIndex, isMaker, latest) {
    try {
      // should be the taker tx hash
      let tx = await this.getTransactionByHash(txHash);
      let block = await this.getBlockByTxHash(txHash);
      let latestBlock = latest ? latest : await this.get(`${BC_SUPER_COLLIDER}.block.latest`);
      ...
      let [originalScript, originalBlockHeight, originalMakerTxOutput] = await this.getInitialMakerOrder(outputScript, block.getHeight());

      let { settlement, shiftMaker, shiftTaker, receivesToChain, sendsFromChain } = parseMakerLockScript(originalScript);
      ...
      if (originalBlockHeight + settlement > latestBlock.getHeight()) {
        ...
        // chain and shift from which to compare
        let childChain = isMaker ? sendsFromChain.toLowerCase() : receivesToChain.toLowerCase();
        let shift = isMaker ? shiftMaker : shiftTaker;
        shift = shift + 1;

        // block at which tx settlement ends
        let settleBlock = await this.getBlockByHeight(originalBlockHeight + settlement);

        let childHeightToBeat = last(getChildBlocks(settleBlock, childChain)).getHeight() + parseFloat(shift, 10);
        let lastestChildHeight = last(getChildBlocks(latestBlock, childChain)).getHeight();
        console.log({ childHeightToBeat });
        console.log({ lastestChildHeight });
        if (childHeightToBeat <= lastestChildHeight) {
          return false;
        } else {
          return true;
        }
  ...
  async getInitialMakerOrder(outputScript, blockHeight = 0) {
    ...
    while (outputScript.includes('OP_CALLBACK')) {
      [parentTxHash, parentOutputIndex] = outputScript.split(' ');
  ...
  async getUnlockTakerTxParams(txHash, txOutputIndex) {
    ...
    let isMakerWithinSettlement = await this.isTxWithinSettlement(txHash, txOutputIndex, true);
    let isTakerWithinSettlement = await this.isTxWithinSettlement(txHash, txOutputIndex, false);
    ...
    if (!isMakerWithinSettlement && !isTakerWithinSettlement) {
      ...
      let didMakerSettle = await this.isTxSettled(txHash, txOutputIndex, true);
      let didTakerSettle = await this.isTxSettled(txHash, txOutputIndex, false);
      ...
      let { base } = parseMakerLockScript(makerScript);
  ...
  async delBlocks(roverName, startHeight, endHeight) {
    for (let i = startHeight; i <= endHeight; i++) {
      let block = await this.getBlockByHeight(i, roverName);
  ...
  async purgeOldBlocks(roverName, startHeight, endHeight, chainConfirmationsNeeded = ROVER_CONFIRMATIONS) {
    ...
    let nextBlocks = [];
    let confirmationLength = chainConfirmationsNeeded[roverName] || 15;
  ...
  checkIfBlockIsValid(validatedBlock, blocksAtHeight, confirmationLength) {
    ...
    if (blocksAtHeight.length + 10 < confirmationLength) {
      // DEBUG
      this._logger.warn(`blocks at height (${blocksAtHeight.length}) do not equal confirmation length (${confirmationLength})`);
      return false;
    }
    // DEBUG
  ...
  async getMissingBlocks(roverName, startHeight, endHeight, chainConfirmationsNeeded = ROVER_CONFIRMATIONS, backsyncEpoch = 0, opts = { breakIfInvalid: true }) {
    ...
    let missingBlocks = [];
    let cacheKey = `${roverName}:${startHeight}:${endHeight}:${chainConfirmationsNeeded}`;
    ...
    // DEBUG
    let confirmationLength = is(Number, chainConfirmationsNeeded) ? chainConfirmationsNeeded : chainConfirmationsNeeded[roverName];
    // DEBUG
    ...
    let startHeightBlocks = await this.getBlocksByHeight(startHeight, roverName, { asHeader: true });
  ...
  async getInvalidBlockIntervals(roverName, chainConstants = ROVER_SECONDS_PER_BLOCK, chainConfirmationsNeeded = ROVER_CONFIRMATIONS) {
    ...
    let missingBlocks = await this.getMissingBlocks(roverName, lowestHeightOfDecisivePeriod, latest.getHeight(), chainConfirmationsNeeded);
    ...
    if (missingBlocks.length === 0) {
      ...
      for (let i = 1; i < missingBlocks.length; i++) {
        let block = missingBlocks[i];
        if (block === end + 1) {
          end = block;
        } else {
          let beforeBlock = await this.findClosestBlock(start, roverName, true);
          let afterBlock = await this.findClosestBlock(end, roverName, false);
  ...
  async checkIfBcBlockIsValid(block) {
    ...
    let nextBlocks = [];
    let max = 3;
    let latestBlock = this._latestBlockCache.has(`${BC_SUPER_COLLIDER}.block.latest`) ? this._latestBlockCache.get(`${BC_SUPER_COLLIDER}.block.latest`) : await this.get(`${BC_SUPER_COLLIDER}.block.latest`);
