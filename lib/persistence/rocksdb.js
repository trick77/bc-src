...
class PersistenceRocksDb {
  ...
  async delUtxoRoot(utxoManager, scriptType, txData, blockHash) {
    ...
    this._logger.info(`###### TX: ${txData.tx.getHash()} + HOST UNAVAIL: ${!hostTx}, CONTAINED MATCH: ${containingTxHash}`);
    const script = toASM(Buffer.from(txData.tx.getOutputsList()[txData.index].getOutputScript()), 0x01);
    this._logger.info(`      ${script}`);
    // TD
  ...
  async rebuildUtxoGraph(utxoManager, floatingUtxoHashes) {
    ...
    for (const scriptType of scriptTypes) {
      ...
      this._logger.info(`---- TYPE: ${scriptType} TOTAL ADDRESSES FOUND: ${Object.keys(addressesIndex).length}`);
      this._logger.info(`---- TYPE: ${scriptType} TOTAL UTXOS FOUND: ${fullIndexCount}`);
  ...
  async rebuildTxGraph(utxoManager, txHandler, walletHandler, blockHeightToIndexFrom, optionalBlock) {
        ...
        utxos[type].map(async pair => {
          ...
          if (txHash) {
            ...
            if (!spent[utxoHash]) {
              ...
              this._logger.info(`remove utxo tx ${pair.tx.getHash()}`);
              spent[utxoHash]++;
              //await this.del(`opunspent.${pair.tx.getHash()}.${pair.index}`)
              //await utxoManager.delUTXO(type, [pair], block)
      ...
      for (const type of Object.keys(stxos)) {
        while (stxos[type].length > 0) {
          ...
          if (pair.multipleBlocks.length > 0) {
            ...
            if (!checkSpent) {
              this._logger.info(`multiple blocks for ${hash} in block ${block.getHeight()} ${pair.multipleBlocks.length}`);
              this._logger.info(pair.multipleBlocks.join(','));
            }
          }
          checkSpent = true;
    ...
    for (const blockHash of Object.keys(txRemovalByBlock)) {
      const block = await this.getBlockByHash(blockHash, 'bc');
      ...
      this._logger.info(`updating block ${unindexTxs.length} txs from block ${block.getHeight()} `);
