    ...
    while (block) {
      ...
      for (let i = 0; i < txs.length; i++) {
        ...
        if (!ti) {
          ...
          if (hash !== block.getHash()) {
            this._logger.info(`putting ${tx.getHash()}`);
            console.log({ hash, height, blockHash: block.getHash() });
            await this.putTransactionBlockIndex([tx.getHash()], block.getHash(), block.getHeight(), 0, 'bc');
          }
        }
      }
      ...
      if (!block) {
        ...
        if (blocks.length === 1) {
          block = blocks[0];
          // console.log({block})
          // await this.put(`bc.block.${block.getHash()}`,block)
        }
  ...
  */
  async settleTx(markedTxHash, childChainId, childChainHeight, bcHeight, txHash, txOutputIndex, isMaker) {
    try {
      ...
      this._logger.info(`setting ${tradeKey} - ${markTxSavedKey}`);
    ...
    try {
      ...
      if (block.getHeight() >= Number(childChainHeight) + shiftAmount) {
        while (block && block.getHeight() >= Number(childChainHeight)) {
          // debugShift(`comapring ${childChainId} ${block.getHeight()} with ${childChainHeight}`)
