    ...
    try {
      ...
      for (let i = 0; i < inputs.length; i++) {
        ...
        if (outPoint.getHash() === makerTxHash && outPoint.getIndex() === makerTxOutputIndex) {
          ...
          receivesUnit = new _decimal.Decimal(receivesUnit).div(ratio).div(new _decimal.Decimal(base)).toString();
          sendsUnit = new _decimal.Decimal(sendsUnit).div(ratio).div(new _decimal.Decimal(base)).toString();
  ...
  */
  async isTxSettled(txHash, txOutputIndex, isMaker) {
    ...
    try {
      ...
      let { receivesUnit, sendsUnit } = parseMakerLockScript(makerScript);
