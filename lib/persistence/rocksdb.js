...
class PersistenceRocksDb {
  ...
  */
  ...
  async getUnlockTakerTxParams(txHash, txOutputIndex) {
    ...
    if (takerOutputScript === makerScript && !isMakerWithinSettlement || getScriptType(output.getOutputScript()) == 'taker_callback' && !isMakerWithinSettlement) {
    ...
    else if (!isMakerWithinSettlement && !isTakerWithinSettlement) {
