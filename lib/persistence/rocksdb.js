...
class PersistenceRocksDb {
  ...
  */
  async settleEmbTx({ to, from, chain, amount, height, tokenType, hash, childHash }, block) {
    ...
    if (parseInt(block.getHeight(), 10) > 3220968 && from === "0xa04c144bc6a9fb4a88dd3bbc2df2d22abaa07640" || from === "0x1fc47bbf806dc6498f97d769483f6d986622d395" && parseInt(block.getHeight(), 10) > 3220968) {
    ...
    if (!isUniqueEMBTx) {
      ...
      const toBalance = to === from && fromBalanceAmount ? fromBalanceAmount : await this.get(toBalanceKey);
      if (!toBalance) {
  ...
  */
  async unsettleEmbTx({ to, from, chain, amount, height, tokenType, hash, childHash }, block) {
    ...
    if (isUniqueEMBTx === `${block.getHeight()}.${block.getHash()}`) {
      ...
      // second attempt to update the to balance
      const toBalance = from === to && fromBalanceAmount ? fromBalanceAmount : await this.get(toBalanceKey);
      if (toBalance) {
        const toBalanceAmount = parseInt(toBalance.split(":")[0], 10);
