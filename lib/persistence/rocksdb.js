...
class PersistenceRocksDb {
  ...
  async putBlock(block, branch = 0, blockchain = 'bc', opts = {
    ...
    debugPutBlock(`would store ${blockchain} ${txs.length} from block ${block.getHeight()}`);
    ...
    if (existingBlock && !pass && parseInt(block.getHeight(), 10) > 3236762) {
      ...
      debugPutBlock(`putBlock(): ${blockchain} block ${block.getHeight()} already exists as hash`);
      ...
      debugPutBlock(`putBlock(): storing ${blockchain} block ${block.getHeight()} : ${block.getHash()} as new block`);
    }
