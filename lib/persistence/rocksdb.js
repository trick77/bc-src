...
const { COINBASE_TX_ESTIMATE_SIZE, getNrgGrant, getTxsDistanceSum } = require('../core/txUtils');
...
let CHECK_SPENT_HASH = process.env.CHECK_SPENT_HASH;
if (CHECK_SPENT_HASH) CHECK_SPENT_HASH = CHECK_SPENT_HASH.split(',');
...
const BC_LOOKBACK = process.env.BC_LOOKBACK === 'true';
...
class PersistenceRocksDb {
  ...
  async saveTxs(count) {
    let block = await this.get(`${BC_SUPER_COLLIDER}.block.last.utxoSaved`);
    while (block) {
      if (!count && block.getHeight() % 10000 === 0) console.log(`checking ${block.getHeight()}`);
  ...
  putBulk(key, opts = { asBuffer: true }) {
    try {
      ...
      return new Promise((resolve, reject) => {
        this.db.batch(op, { sync: true }, err => {
          if (err) {
            return reject(err);
          }
          return resolve(true);
  ...
  async getBlockBoundariesToFetch(headers) {
    let headerList;
    if (!this._readEventTable['getBlockBoundariesToFetch']) {
      this._readEventTable['getBlockBoundariesToFetch'] = 0;
    }
    this._readEventTable['getBlockBoundariesToFetch']++;
    try {
  ...
  async reorgBlockchain(blockchain = BC_SUPER_COLLIDER, opts = {
    ...
    try {
      ...
      // LDL
      this._logger.info(`organizing edge <- ${reorgFromBlock.getHeight()}:${reorgToBlock.getHeight()} <- setting sync to complete...`);
  ...
  async putTransactionBlockIndex(txHashes, blockHash, blockHeight, branch = 0, blockchain = 'bc', opts = { asBuffer: true }) {
    try {
      ...
      for (const hash of txHashes) {
        const h = await this.get(`${blockchain}.txblock.${hash}`);
        if (!h) {
  ...
  async putBlock(block, branch = 0, blockchain = 'bc', opts = {
    ...
    if (this._blockHashAtHeightCache.has(key) && opts.cached) {
      return;
    } else {
    ...
    if (block.getTxsList !== undefined) {
      ...
      if (opts.saveHeaders) {
        ...
        for (const header of headers) {
          await this.putBlock(header, 0, header.getBlockchain(), { cached: true });
        }
      }
      // DEBUG
    }
  ...
  async getBlocksByRangeUnique(from, to, blockchain = 'bc', opts = {
    ...
    if (!this._readEventTable['getBlocksByRangeUnique']) {
      this._readEventTable['getBlocksByRangeUnique'] = 0;
    }
    this._readEventTable['getBlocksByRangeUnique']++;
    try {
      if (to === -1) {
        const latestBlock = await this.get(`${blockchain}.block.latest`);
        if (latestBlock === null) {
  ...
  async settleTx(markedTxHash, childChainId, childChainHeight, childChainHash, bcHeight, txHash, txOutputIndex, isMaker) {
    try {
      ...
      debugShift(`attempting ${tradeKey} - ${markedKey} . ${childChainHash}`);
  ...
  async isRoveredBlockPastShift(childChainHash, childChainHeight, childChainId, shiftAmount, latestBlock) {
    try {
      ...
      debugShift(`childHashBlock is ${childHashBlock}`);
      if (!childHashBlock) {
  ...
  */
        ...
        if (onlyMaker) {
          ...
          if (takerDetails) {
            if (takerDetails.childBlock) {
              ...
              return true; //this happens when takerDetails just returns true, if the receivesAmount = 0;
            }
          }
          ...
          if (makerDetails) {
            if (makerDetails.childBlock) {
              ...
              return true; //this happens when makerDetails just returns true, if the sendsAmount = 0;
            }
          }
  ...
  async getMultichainReport() {
    ...
    if (!this._readEventTable['getMultichainReport']) {
      this._readEventTable['getMultichainReport'] = 0;
    }
    this._readEventTable['getMultichainReport']++;
  ...
  async pruneFromBlock(block, depth = 12, confirmations = 5, defaultBlockchain = 'bc') {
    ...
    try {
      ...
      if (!lastPrunedData) {
        await this.put(`${blockchain}.pruned`, parseInt(block.getHeight(), 10));
      } else {
  ...
  async getInvalidBlockIntervals(roverName, chainConstants = ROVER_SECONDS_PER_BLOCK, chainConfirmationsNeeded = ROVER_CONFIRMATIONS) {
    ...
    if (!this._readEventTable['getInvalidBlockIntervals']) {
      this._readEventTable['getInvalidBlockIntervals'] = 0;
    }
    this._readEventTable['getInvalidBlockIntervals']++;
