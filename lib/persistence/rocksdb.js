  ...
  async cleanUpUTXOs() {
    ...
    for (let i = 0; i < Object.keys(utxos).length; i++) {
      ...
      for (let j = 0; j < fullIndexes.length; j++) {
        ...
        console.log(`putting opunspent.${txHash}.${txIndex} at ${j}`);
        console.log(`putting utxo.${scriptType}.${j} at ${utxo}`);
      ...
      console.log({ null: nullIndexes.length, full: fullIndexes.length, length, count });
    }
    console.log(`clean up took ${Date.now() - date} ms`);
  }
