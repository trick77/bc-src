...
class PersistenceRocksDb {
  ...
  async delTransactionBlockIndex(txHashes, blockHash, blockHeight, branch = 0, blockchain = 'bc', opts = { asBuffer: true }) {
    try {
      ...
      this._logger.info(`unindex txs -${blockHash}:${blockHeight}`);
  ...
  async saveBlockHeaders(block) {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (!hashSaved) {
        ...
        if (!heightSaved) {
          await this.put(`${child.getBlockchain()}.block.${child.getHeight()}`, child, { sync: DEFAULT_SYNC_LOCAL });
        }
      }
    }
    this._setHeaderMapByBlock(block.getHash(), headersObj);
  ...
  async saveBlock(block) {
    ...
    const saved = await this.get(`${BC_SUPER_COLLIDER}.block.${block.getHash()}`, block, { sync: DEFAULT_SYNC_LOCAL });
    ...
    await this.put(`${BC_SUPER_COLLIDER}.block.${block.getHash()}`, block); // ensure block gets saved at height
    await this.putBlockHashAtHeight(block.getHash(), block.getHeight(), 'bc');
    ...
    await this.del(`${BC_SUPER_COLLIDER}.block.lock.${block.getHeight()}`); // remove any flagged blocks at this height
