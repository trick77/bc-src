  ...
  */
  ...
  async removeUTXOsFrom(height) {
    ...
    do {
      ...
      if (!hash) break;
      const candidate = await this.getBlockByHash(hash, 'bc'); // this must exist given during updateUTXO this is saved
  ...
  async updateUTXOs(block, remove = false) {
    ...
    if (isUpdating) {
      debugUTXO(`in the midst of an update while evaluating ${block.getHeight()}:${shortenHash(block.getHash())}`);
      return false;
    } else {
      debugUTXO(`${remove ? 'REMOVING' : 'SAVING'} ${block.getHeight()}:${shortenHash(block.getHash())}:${shortenHash(block.getPreviousHash())}, LAST is ${lastSavedBlock.getHeight()}:${shortenHash(lastSavedBlock.getHash())}`);
      await this.put(`updateUTXOs`, true);
    }

    await this.updateCoinbase(block, remove);

    if (remove) {
      if (lastSavedBlock.getHash() !== block.getHash()) {
        await this.del(`updateUTXOs`);
        return false;
      }
      // XXX
      // debugUTXO(`REMOVED UTXOS for ${block.getHeight()}:${block.getHash()}`)
      ...
      if (lastSavedBlock.getHash() !== block.getPreviousHash()) {
        await this.del(`updateUTXOs`);
        return false;
      }
      // XXX
      // debugUTXO(`UDPATED UTXOS for ${block.getHeight()}:${block.getHash()}`)
  ...
  */
  async putBlock(block, branch = 0, blockchain = 'bc', opts = { asBuffer: true, fromWaypoint: false, saveHeaders: false, force: false, storeOnlyOne: false, updateHeight: true, rovered: false }) {
    ...
    if (existingBlock && !pass) {
      ...
      // await this.put(`${blockchain}.block.${block.getHash()}`, block)
      if (blockchain === 'bc') await this.put(`${blockchain}.block.${block.getHash()}`, block);
    ...
    if (blockchain === BC_SUPER_COLLIDER && !opts.fromWaypoint) {
