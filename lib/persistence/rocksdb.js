...
class PersistenceRocksDb {
  ...
  */
  async putBlock(block, branch = 0, blockchain = 'bc', opts = { asBuffer: true, fromWaypoint: false, saveHeaders: false, force: false, storeOnlyOne: false, updateHeight: true, rovered: false }) {
    ...
    this._logger.info(`block ${block.getHeight()} being stored`);
    ...
    this._logger.info(`block ${block.getHeight()} saved`);
  ...
  async getBlocksByHeight(height, blockchain = 'bc', opts = { asBuffer: true }) {
    ...
    try {
      ...
      if (blockHashes === null) {
        let block = await this.get(`${blockchain}.block.${height}`);
        if (block) {
          blockList.push(block);
          return blockList;
        } else {
          this._logger.info(`could not get block hashes for height: ${height}, with key ${key}`);
          return false;
        }
      }
