...
class PersistenceRocksDb {
  ...
  put(key, value, opts = {}) {
    ...
    return new Promise((resolve, reject) => {
      ...
      opts.sync = opts.sync ? DEFAULT_SYNC_LOCAL : forceSyncOpt;
  ...
  async saveBlock(block) {
    ...
    await this.put(`${BC_SUPER_COLLIDER}.block.${block.getHeight()}`, block, { sync: DEFAULT_SYNC_LOCAL }); // this overrides the height
    await this.put(`${BC_SUPER_COLLIDER}.block.${block.getHash()}`, block, { sync: DEFAULT_SYNC_LOCAL }); // ensure block gets saved at height
