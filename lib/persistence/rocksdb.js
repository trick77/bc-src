  ...
  async addUTXO(scriptType, utxos, block) {
    ...
    for (let i = 0; i < utxos.length; i++) {
      ...
      if (!utxoIndex) {
        if (!OL_FAST_SYNC) {
          for (let addr of utxoToAddresses[`${tx.getHash()}.${index}`]) {
            ...
            if (arrIndex === -1) {
              ...
              await this.put(`${addr}.${scriptType}`, addresses[addr]);
            }
          }
        }
        ...
        //temp debug
        debugUTXODetail(`opunspent.${tx.getHash()}.${index} is ${utxoIndex}`);
      }
    }
        ...
        if (!OL_FAST_SYNC) {
          for (let addr of utxoToAddresses[`${tx.getHash()}.${index}`]) {
            ...
            if (arrIndex !== -1) {
              ...
              addresses[addr] = arr;
              await this.put(`${addr}.${scriptType}`, arr);
            }
          }
        }
        await this.del(`utxo.${scriptType}.${utxoIndex}`);
        count = count - 1;
        await this.put(`utxo.${scriptType}.count`, count);
        await this.del(`opunspent.${tx.getHash()}.${index}`); // txHash.txIndex -> index
      } else {
