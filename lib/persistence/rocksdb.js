...
const loadBasedPeerExpiration = 30000 + Math.floor(80000 / numCPUs);
    ...
    try {
      ...
      if (this._chainState && latestBlock && latestBlock.getHash) {
        if (new BN(latestBlock.getHeight()).toNumber() !== 1) {
          ...
          await Promise.all([this._chainState.putLatestBlock(blockchain, latestBlock.getHeight(), latestBlock.getHash()), this._chainState._persistence.put(`${blockchain}.latestBlock.latest.hash`, latestBlock.getHash()), this._chainState._persistence.put(`${blockchain}.latestBlock.latest.height`, new BN(latestBlock.getHeight()).toNumber()), this._chainState._persistence.put(`${blockchain}.range.lowest.height`, previousHeight), this._chainState._persistence.put(`${blockchain}.range.lowest.hash`, latestBlock.getPreviousHash()), this._chainState._persistence.put(`${blockchain}.range.highest.height`, new BN(latestBlock.getHeight()).toNumber()), this._chainState._persistence.put(`${blockchain}.range.highest.hash`, latestBlock.getHash())]);
        }
      }
