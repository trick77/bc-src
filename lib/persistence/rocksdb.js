...
class PersistenceRocksDb {
    ...
    try {
      ...
      if (this._chainState && latestBlock && latestBlock.getHash) {
        if (new BN(latestBlock.getHeight()).toNumber() !== 1) {
          ...
          await Promise.all([this._chainState.putLatestBlock(blockchain, latestBlock.getHeight(), latestBlock.getHash()), this.put(`${blockchain}.latestBlock.latest.hash`, latestBlock.getHash()), this.put(`${blockchain}.latestBlock.latest.height`, new BN(latestBlock.getHeight()).toNumber()), this.put(`${blockchain}.range.lowest.height`, new BN(latestBlock.getHeight()).toNumber() - 1), this.put(`${blockchain}.range.lowest.hash`, latestBlock.getPreviousHash()), this.put(`${blockchain}.range.highest.height`, new BN(latestBlock.getHeight()).toNumber()), this.put(`${blockchain}.range.highest.hash`, latestBlock.getHash())]);
        }
      }
  ...
  async updateUTXOs(block, remove = false) {
    ...
    // settle any trades that occured during this block
        ...
        await Promise.all([this.saveBlock(block), this._unsettledTxManager.onNewBcBlock(block), this.put('bc.block.last.utxoSaved', block), this.put(`bc.block.${block.getHeight()}.utxoSaved`, block.getHash())]);
    ...
    if (block.getTxsList !== undefined) {
      ...
      if (opts.saveHeaders) {
        ...
        await Promise.all(headers.map(header => {
          return this.putBlock(header, 0, header.getBlockchain());
        }));
      }
      // DEBUG
    }
  ...
  async getMarkedBalanceData(address, tokenAddress = EMBLEM_CONTRACT_ADDRESS, connectedChain = 'eth', blockchain = 'bc', opts = { asBuffer: true }) {
    ...
    try {
      ...
      const edge = await this.get(`${blockchain}.sync.edge`);
      if (edge && new BN(edge).lte(new BN(3108880)) && !BC_MARKED_DRY_RUN) {
        return new BN(0);
      }
