...
exports.NRG_MINTED_PERISTENCE_KEY = exports.addressToHost = undefined;
...
const NRG_MINTED_PERISTENCE_KEY = exports.NRG_MINTED_PERISTENCE_KEY = `${BC_SUPER_COLLIDER}.nrg.granted`;
...
class PersistenceRocksDb {
  ...
  async saveTxs(count, optionalBlock) {
    ...
    while (block) {
      ...
      this._logger.info(`indexing ${block.getHeight()} : ${block.getHash()}, count: ${count}`);
      //}
  ...
  async getBlocksByRoveredHash(hash, blockchain, opts = {
    ...
    try {
      ...
      const blocks = await Promise.all(hashes.map(headerHash => {
    ...
    try {
      ...
      for (let i = 0; i < headers.length; i++) {
        ...
        if (header.getBlockchain && header.getBlockchain() === 'btc' && parseInt(header.getHeight(), 10) >= 671020 && parseInt(header.getHeight(), 10) <= 699990) {
          continue;
        } else if (header.getBlockchain && header.getBlockchain() === 'btc' && parseInt(header.getHeight(), 10) >= 731950 && parseInt(header.getHeight(), 10) <= 732611) {
          continue;
          ...
          continue;
        } else if (header.getBlockchain && header.getBlockchain() === 'lsk' && parseInt(block.getHeight(), 10) < 5933111 && parseInt(block.getHeight(), 10) > 5889746 && parseInt(header.getHeight(), 10) <= 17354534) {
          continue;
        } else {
  ...
  async putTransactionBlockIndex(txHashes, blockHash, blockHeight, branch = 0, blockchain = 'bc', opts = { asBuffer: true }) {
    try {
      ...
      for (const hash of txHashes) {
        await this.put(`${blockchain}.txblock.${hash}`, `${blockchain}.block.${blockHash}.${blockHeight}`);
      }
      return true;
    } catch (e) {
  ...
  async getBlockByTxHash(txHash) {
    ...
    try {
      const key = this._blocksByHeightCache.has(id) ? this._blocksByHeightCache.get(id) : await this.get(id);
      if (key) {
  ...
  async isRoveredBlockPastShift(childChainHash, childChainHeight, childChainId, shiftAmount, latestBlock) {
    try {
      ...
      if (!childHashBlock) {
        const now = Date.now();
        this._logger.warn(`${childChainId} ${childChainHash} could not be found`);
        return 1;
  ...
  */
  async putLeaseDb(db, key) {
    ...
    let endkey = key === 'built' ? 'sigma' : 'alpha';
