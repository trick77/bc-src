...
class PersistenceRocksDb {
  ...
  async rebuildChainIndexFromHeight(utxoManager, txHandler, blockHeightToIndexFrom, optionalBlock) {
    ...
    try {
      ...
      while (blocksToAdd.length > 0) {
        const pair = blocksToAdd.shift();
        const [height, hash] = pair;
        this._logger.info(`creating ref: ${height} ${hash}`);
        const block = await this.getBlockByHash(hash, 'bc');
  ...
  async getBlockByTxHash(txHash, opts = { blockchain: BC_SUPER_COLLIDER, returnLikelyWinner: true }) {
    ...
    try {
      ...
      if (!potentialBlocks) {
        debug(`getBlockByTxHash(): no blocks found for txhash ${txHash}`);
        return null;
      }
      ...
      for (const key of list) {
        if (key) {
          ...
          } else if (lowestHeight && height === lowestHeight) {
