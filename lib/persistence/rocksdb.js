          ...
          if (roveredHeaderMerkle !== purposedMerkle) {
            if (parseInt(block.getTimestamp(), 10) - 3101 < now) {
              ...
              if (!b || b && b.getHash() === block.getHash()) {
                ...
                if (parseInt(block.getHeight(), 10) > 6842875) {
                  this._logger.warn(`rover confirmed malformed child ${h.getBlockchain()}:${h.getHeight()} in proposed ${BC_SUPER_COLLIDER} ${block.getHeight()}...`);
  ...
  async isRoveredBlockPastShift(childChainHash, childChainHeight, childChainId, shiftAmount, latestBlock) {
    try {
      ...
      if (!childHashBlock) {
        ...
        if (latestBlock.getHeight() < 6500000) {
          return 1;
        } else {
