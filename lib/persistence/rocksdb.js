...
class PersistenceRocksDb {
  ...
  */
  ...
  async updateUTXOs(block, remove = false) {
    ...
    this._logger.info(`updating txs for ${block.getHeight()}:${block.getHash()}`);
  ...
  */
  ...
  async isRoveredBlockPastShift(childChainHash, childChainHeight, childChainId, shiftAmount, latestBlock) {
    try {
      // this._logger.info(`calling isRoveredBlockPastShift`)
      ...
      if (block.getHeight() >= Number(childChainHeight) + shiftAmount) {
        ...
        // this._logger.info(`${childChainId} ${childChainHeight} ${childChainHash} is invalid`)
        return -1; // block was actually invalid
      } else {
        // waiting for shift period to pass
        // this._logger.info(`waiting for shift period to pass for ${childChainId} ${childChainHash} ${childChainHeight}`)
        return 0;
      }
    } catch (err) {
      console.log({ err });
    ...
    try {
      ...
      if (originalBlockHeight + settlement > latestBlock.getHeight()) {
        ...
        if (onlyMaker) {
          ...
          // console.log({takerDetails})
          // console.log({makerDetails})

          //taker order is still within shift
          if (takerDetails) {
            let pastShiftTaker = await this.isRoveredBlockPastShift(takerDetails.childBlock.getHash(), takerDetails.childBlock.getHeight(), takerDetails.childBlock.getBlockchain(), takerDetails.shiftAmount + 1);
            if (pastShiftTaker === 0) return true;
          }

          //maker order is still within shift
          if (makerDetails) {
            let pastShiftMaker = await this.isRoveredBlockPastShift(makerDetails.childBlock.getHash(), makerDetails.childBlock.getHeight(), makerDetails.childBlock.getBlockchain(), makerDetails.shiftAmount + 1);
