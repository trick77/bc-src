...
class PersistenceRocksDb {
  ...
  async getTransactionByHash(txHash, blockchain = 'bc', opts = { asBuffer: true, cached: true }) {
    ...
    try {
      ...
      //       let txs = block.getTxsList().filter((tx)=>{return tx.getHash() === txHash})
      //       if(txs.length === 1) {
  ...
  */
  async putBlock(block, branch = 0, blockchain = 'bc', opts = { asBuffer: true, fromWaypoint: false, saveHeaders: false, force: false, storeOnlyOne: false, updateHeight: true, rovered: false }) {
    ...
    debug(`returning putBlock from ${block.getHeight()}.${block.getHash()}`);
    return externalOps;
    // } catch (err) {
    //   this._logger.info(err)
    //   return []
  ...
  async getBlocksByRange(from, to, blockchain = 'bc', opts = { asBuffer: true, asSet: false, cached: false }) {
    ...
    this._logger.info(`getBlocksByRange(): request ${from}->${to} for ${blockchain}`);
