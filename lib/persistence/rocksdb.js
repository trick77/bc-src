...
class PersistenceRocksDb {
  ...
  */
    ...
    if (remove) {
      ...
      for (let scriptType of Object.keys(utxos)) {
        if (scriptType == 'taker_output' && utxos[scriptType].length > 0) {
          ...
          this._logger.info(`Remove Block #${block.getHeight()} has a Taker Tx`);
        }
      ...
      for (let scriptType of Object.keys(utxos)) {
        if (scriptType == 'taker_output' && utxos[scriptType].length > 0) {
          ...
          this._logger.info(`Block #${block.getHeight()} has a Taker Tx`);
        }
  ...
  async setBlockHasTaker(height) {
    let lastBlock = await this.get(`bc.block.lastTaker`);
    if (lastBlock) {
  ...
  async delBlockHasTaker(height) {
    let newLastBlock = await this.get(`bc.block.taker.${height}`);
    if (newLastBlock) {
  ...
  async getNextTakerBlock(height) {
    let nextHeight = await this.get(`bc.block.taker.${height}`);
    return nextHeight;
  }
  ...
  async getTradeIndices(sendsFromAddress, receivesToAddress, sendsFromChain, sendsUnit) {
    let key = `${sendsFromAddress}.${receivesToAddress}.${sendsFromChain}.${sendsUnit}`;
  ...
  async delTradeIndex(utxo) {
    try {
      ...
      let { makerTxHash, makerTxOutputIndex } = parseTakerLockScript(script);
      ...
      for (let i = 0; i < inputs.length; i++) {
        ...
        if (outPoint.getHash() === makerTxHash && outPoint.getIndex() === makerTxOutputIndex) {
          let inputScript = input.getInputScript();
          ...
          let [makerSendsFrom, makerReceivesTo] = [sendsFromAddress, receivesToAddress];
          let taker = parseTakerUnlockScript(toASM(Buffer.from(inputScript), 0x01));
          let [takerSendsFrom, takerReceivesTo] = [taker.sendsFromAddress, taker.receivesToAddress];
          ...
          receivesUnit = new _decimal.Decimal(receivesUnit).div(ratio).div(new _decimal.Decimal(base)).toNumber();
          sendsUnit = new _decimal.Decimal(sendsUnit).div(ratio).div(new _decimal.Decimal(base)).toNumber();

          let makerKey = `${makerSendsFrom}.${takerReceivesTo}.${sendsFromChain}.${sendsUnit}`;
          let takerKey = `${takerSendsFrom}.${makerReceivesTo}.${receivesToChain}.${receivesUnit}`;
          let keys = [makerKey, takerKey];
          let type = ['maker', 'taker'];

          let val = new BN(makerOutput.getValue()).toString();
          ...
          for (let i = 0; i < keys.length; i++) {
            ...
            let hashes = await this.get(key);
            if (hashes) {
              let check = hashes.indexOf(`${txHash}.${index}.${blockHeight}.${val}.${type[i]}`);
              if (check > -1) {
  ...
  async extractAddressesFromUtxos(scriptType, utxos) {
    ...
    for (let _ref of utxos) {
      ...
      if (this._engine._onlyTrack) {
        addr = addr.filter(a => {
          let blakeAddr = '0x' + blake2bl(blake2bl(this._engine._onlyTrack) + this._engine._onlyTrack);
          return a === blakeAddr;
        });
      }
      addr.forEach(a => {
  ...
  async addUTXO(scriptType, utxos, block, unspent = true) {
    ...
    for (let i = 0; i < utxos.length; i++) {
      let { index, tx, hash, height } = utxos[i];
  ...
  */
  ...
  async delBlock(hash, branch = 0, blockchain = 'bc', opts = { asBuffer: true, pruning: false }) {
    try {
      ...
      let txs = block && block.getTxsList !== undefined ? block.getTxsList() : block.getMarkedTxsList();
  ...
  async getBlockByHeight(height, blockchain = 'bc', opts = { asBuffer: true }) {
    ...
    const uniqueHeight = hashes && hashes.length > 1 ? false : true;
    try {
  ...
  */
  async settleTx(markedTxHash, childChainId, childChainHeight, bcHeight, txHash, txOutputIndex, isMaker) {
    try {
      ...
      let markTxSavedKey = `${childChainId}.${childChainHeight}.${markedTxHash}`;
  ...
  */
  async unsettleTx(markedTxHash, childChainId, childChainHeight, bcHeight, txHash, txOutputIndex, isMaker) {
    try {
      ...
      let markedExisting = await this.get(markedKey);
      let tradeExisting = await this.get(tradeKey);

      if (markedKey === markedExisting && tradeKey === tradeExisting) {
  ...
  */
  ...
  async isTxWithinSettlement(txHash, txOutputIndex, latest, onlyMaker = false) {
    try {
      ...
      if (originalBlockHeight + settlement > latestBlock.getHeight()) {
        ...
        let lastestChildMaker = last(getChildBlocks(latestBlock, sendsFromChain)).getHeight();
        let lastestChildTaker = last(getChildBlocks(latestBlock, receivesToChain)).getHeight();
        let settleChildMaker = last(getChildBlocks(settleBlock, sendsFromChain)).getHeight() + parseFloat(shiftMaker) + 1;
        let settleChildTaker = last(getChildBlocks(settleBlock, receivesToChain)).getHeight() + parseFloat(shiftTaker) + 1;
        if (onlyMaker) {
  ...
  async getUnlockTakerTxParams(txHash, txOutputIndex) {
    ...
    let isWithinSettlement = await this.isTxWithinSettlement(txHash, txOutputIndex);
    ...
    if (takerOutputScript === makerScript && isWithinSettlement || getScriptType(output.getOutputScript()) == 'taker_callback' && isWithinSettlement) {
  ...
  async pruneFromBlock(block, depth = 12, confirmations = 6, defaultBlockchain = 'bc') {
    ...
    let blockchain = block.getBlockchain ? block.getBlockchain() : defaultBlockchain;
    let b = block;
    ...
    while (depth > 0 && b && hashesAtHeight) {
      ...
      if (hashesAtHeight && hashesAtHeight.length > 1 && hashesAtHeight.indexOf(b.getHash()) > -1) {
        for (let hash of hashesAtHeight) {
