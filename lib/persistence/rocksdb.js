...
class PersistenceRocksDb {
  ...
  async saveBlock(block) {
    ...
    this.put(`${BC_SUPER_COLLIDER}.block.${block.getHeight()}`, block), // this overrides the height
    this.del(`${BC_SUPER_COLLIDER}.block.lock.${block.getHeight()}`) // remove any flagged blocks at this height
    ]);
  ...
  async getRoverSyncReport(blockchain, opts = {
    ...
    try {
      ...
      this._logger.info(`${blockchain} required block count ${requiredBlockCount}, lowest block height ${lowestBlockHeight}, given lowest block height: ${opts.givenLowestHeight}, latest (highest) block height ${highestBlockHeight}, given highest block height: ${opts.givenHighestHeight}`);
