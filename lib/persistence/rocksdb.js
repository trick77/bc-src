      ...
      for (let h of headers) {
        ...
        if (roveredHeaderMerkle) {
          ...
          if (roveredHeaderMerkle !== purposedMerkle) {
            ...
            if (parseInt(currentLatest.getHeight(), 10) < parseInt(block.getHeight(), 10) + 2000) {
  ...
  */
  async settleEmbTx({ to, from, chain, amount, height, tokenType, hash, childHash }, block) {
    ...
    if (!isUniqueEMBTx) {
      ...
      if (fromBalance) {
        const fromBalanceAmount = parseInt(fromBalance.split(":")[0], 10);
        const updatedFromBalance = fromBalanceAmount - amount;
        this._logger.info(`settle emb from updated balance: ${updatedFromBalance}`);
        if (0 >= updatedFromBalance) {
          await this.del(fromBalanceKey);
        } else {
          await this.put(fromBalanceKey, `${updatedFromBalance}:${height}:${timestamp}`);
        }
      }
      ...
      if (!toBalance) {
        await this.put(toBalanceKey, `${amount}:${height}:${timestamp}`);
      } else {
        const toBalanceAmount = parseInt(toBalance.split(":")[0], 10);
        const updatedToBalance = toBalanceAmount + amount;
  ...
  */
  async unsettleEmbTx({ to, from, chain, amount, height, tokenType, hash, childHash }, block) {
    ...
    if (isUniqueEMBTx === `${block.getHeight()}.${block.getHash()}`) {
      ...
      const timestamp = Math.floor(Date.now() / 1000);
      const toBalanceKey = `${chain}.${tokenType}.${EMBLEM_CONTRACT_ADDRESS}.${to}.miner`;
      const fromBalanceKey = `${chain}.${tokenType}.${EMBLEM_CONTRACT_ADDRESS}.${from}.miner`;
      ...
      if (fromBalance) {
        ...
        await this.put(fromBalanceKey, `${amount}:${height}:${timestamp}`);
      }
