...
class PersistenceRocksDb {
  ...
  */
  async settleEmbTx({ to, from, chain, amount, height, tokenType, hash, childHash }, block) {
    const isUniqueEMBTx = await this.get(`${chain}.${tokenType}.${hash}.${childHash}.${childHeight}`);
    if (!isUniqueEMBTx) {
      await this.put(`${chain}.${tokenType}.${hash}.${childHash}.${childHeight}`, `${block.getHeight()}.${block.getHash()}`);
  ...
  */
  async unsettleEmbTx({ to, from, chain, amount, height, tokenType, hash, childHash }, block) {
    ...
    if (isUniqueEMBTx === `${block.getHeight()}.${block.getHash()}`) {
      await this.del(`${chain}.${tokenType}.${hash}.${childHash}.${childHeight}`);
