...
class PersistenceRocksDb {
  ...
  async getMarkedBalanceData(address, tokenAddress = EMBLEM_CONTRACT_ADDRESS, connectedChain = 'eth', blockchain = 'bc', opts = { asBuffer: true }) {
    ...
    try {
      ...
      if (currentHeight && new BN(currentHeight).gt(new BN(6711006))) {
        embIndexKey = 'alpha';
      }
  ...
  */
  async putLeaseDb(db) {
    const dbBuilt = await this.get(`lease.db.update`);
    if (dbBuilt || !db) {
      return;
    }

    const timestamp = Math.floor(Date.now() / 1000);
    await this.put(`lease.db.update`, timestamp);
    this._logger.info(`building lease index ${Object.keys(db).length}`);
    for (const addr of Object.keys(db)) {
      const balanceKey = `eth.emb.${EMBLEM_CONTRACT_ADDRESS}.${addr}.alpha`;
