...
class PersistenceRocksDb {
  ...
  */
  ...
  async delTransaction(tx, branch = 0, blockchain = 'bc', opts = { asBuffer: true, force: false }) {
    ...
    try {
      ...
      this._logger.info(`deleting ${txKey}`);
      ...
      if (blockchain === BC_SUPER_COLLIDER && opts.saveHeaders) {
        ...
        if (parseInt(block.getHeight(), 10) % 100 === 0) {
          ...
          if (currentSyncedStatus && currentSyncedStatus === 'complete') {
            // select tree
            await this.pruneFromBlock(block, 200, 93);
          }
        }
      }
      return Promise.resolve(block);
  ...
  */
   ...
   * @param blockchain string
   */
  async getBlocksByHeight(height, blockchain = 'bc', opts = { asBuffer: true, iterateUp: true, asHashes: false }) {
    if (isNaN(height)) return false;
        ...
        if (opts.iterateUp) {
          ...
          if (bls) {
            for (let b of bls) {
              ...
              if (hb) {
                await this.putBlockHashAtHeight(b.getPreviousHash(), parseInt(block.getHeight(), 10) - 1, blockchain);
                blockList.push(hb);
              }
            }
          } else {
  ...
  */
  ...
  async pruneFromBlock(block, depth = 12, confirmations = 6, defaultBlockchain = 'bc') {
    ...
    // LDL
    this._logger.info(`compressed ${compressed} from ${block.getHeight() - confirmations} remaining: {depth}`);
