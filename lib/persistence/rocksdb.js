...
const { humanToBN, internalToBN, internalToHuman, MAX_NRG_VALUE, COIN_FRACS: { NRG, BOSON } } = require('../core/coin');
...
const DEFAULT_SYNC_LOCAL = false;

const debugShift = require('debug')('bcnode:persistence:rovershift');
const LRUCache = require('lru-cache');
const mkdirp = require('mkdirp');
const { concat, contains, flatten, is, isEmpty, last, min, max, sum } = require('ramda');
...
const { validateCoinbase, validateTxs } = require('../bc/validation');
...
if (BC_REBUILD_TXS) {
  REBUILD = require('fs').createWriteStream('reindex.csv', 'utf-8');
  REBUILD.write(`txHash,index,blockHash,value\n`);
}
...
class PersistenceRocksDb {
  ...
  constructor(location = '_data') {
    ...
    this._tradesByKeyCache = new LRUCache({
      allowStale: true,
      max: 1000
    });
  ...
  async delUtxoRoot(utxoManager, scriptType, txData, blockHash) {

    try {

      // data structure of txData: {tx: tx|tx2, index, height, hash, containingTxHash, multipleBlocks}
      const txDataList = [txData];
      const hostTx = await this.getTxClaimedBy(txData.tx.getHash(), txData.index);
      const containingTxHash = txData.containingTxHash;
      const containingMatchesHost = containingTxHash && hostTx && hostTx.getHash() === containingTxHash;

      this._logger.info(`delUtxoRoot ${txData.tx.getHash()} tx.index: ${txData.index}, tx.height: ${txData.height}, hash: ${txData.hash}, containing hash: ${txData.containingTxHash}`);

      const script = toASM(Buffer.from(txData.tx.getOutputsList()[txData.index].getOutputScript()), 0x01);
      // TD very ineffecient call gets the entire index of utxos for every address in the tx
      let { utxoToAddresses, addresses } = await utxoManager.extractAddressesFromUtxos(scriptType, txDataList.map(t => {
        return { txHash: t.tx.getHash(), index: t.index, script: t.tx.getOutputsList()[t.index].getOutputScript() };
      }));

      this._logger.info(`delUtxoRoot() found ${Object.keys(addresses).length} addresses in tx ${txData.tx.getHash()}, contained by: ${containingTxHash}`);

      // this key may or may not exist depending on if the UTXO has been spent making it impossible to evaluate a block at the same height spending the same utxo
      let opu = `opunspent.${txData.tx.getHash()}.${txData.index}`;
      let ops = `opspent.${txData.tx.getHash()}.${txData.index}`;
      let utxoIndex = await this.get(opu);
      if (utxoIndex && utxoIndex.indexOf && utxoIndex.indexOf(':') > -1) {
        utxoIndex = utxoIndex.split(':')[0];
      }

      const indexesToRemove = [];

      if (utxoIndex) {
        indexesToRemove.push(utxoIndex);
      }

      ////////////////////////////////////////////////////////////////////
      await this.del(`opunspent.${txData.tx.getHash()}.${txData.index}`);

      for (const address of Object.keys(addresses)) {
        // no index was found iterate through the addresses available utxos and delete the ones that are spent
        const possibleUtxos = await utxoManager.getUtxoIndexesByAddress(scriptType, address);
        for (const length of addresses[address]) {
          const utxo = await this.get(`utxo.${scriptType}.${length}`);
          if (utxo !== undefined && utxo) {
            //this._logger.info(`delUtxoRoot() found utxo.getTxHash(): ${utxo.getTxHash()} utxos for address ${address} and length ${length}`)
            if (utxo.getTxHash() === txData.tx.getHash()) {
              this._logger.info(`!!!!!!!!! found match delUtxoRoot() found utxo.getTxHash(): ${utxo.getTxHash()} utxos for address ${address} and length ${length}`);
              indexesToRemove.push(length);
            } else if (hostTx && hostTx.getHash() === utxo.getTxHash()) {
              indexesToRemove.push(length);
            } else if (containingTxHash === utxo.getTxHash()) {
              indexesToRemove.push(length);
            }
          } else {
            this._logger.info(`could not find the utxos for address ${address} at length ${length}`);
            indexesToRemove.push(length);
          }
        }

        // utxoToAddresses[`${txHash}.${index}`] = addr
        if (utxoToAddresses[`${txData.tx.getHash()}.${txData.index}`] === address) {
          this._logger.info(`utxo mapped to ${txData.tx.getHash()} for address: ${address}`);
        }

        if (indexesToRemove.length > 0) {

          ////////////////////////////////////////////////////////////////////
          for (const length of indexesToRemove) {
            let arr = addresses[address];
            let arrIndex = arr.indexOf(length);
            if (arrIndex !== -1) {
              await this.del(`utxo.${scriptType}.${length}`);
              arr.splice(arrIndex, 1);
              addresses[address] = arr;
            }
          }
        }
      }
      ...
      if (indexesToRemove.length > 0) {
        this._logger.info(`indexes to remove: ${indexesToRemove.length} for tx: ${txData.tx.getHash()}`);
        // update all of the indexes
        for (let addr of Object.keys(addresses)) {
          addr = addr.toLowerCase();
          ////////////////////////////////////////////////////////////////////
          await this.put(`${addr}.${scriptType}`, [...new Set(addresses[addr])]);
        }
      } else {
        this._logger.info(`no indexes found to remove for tx: ${txData.tx.getHash()}`);
      }

      return true;

      // addresses is an array of addresses and the utxo number pointer which they load to calculate balance
    } catch (err) {
      return false;
    }
  }

  async rebuildUtxoGraph(utxoManager, floatingUtxoHashes) {

    const scriptTypes = ['nrg_transfer', 'maker_output', 'taker_output', 'taker_callback', 'feed_update', 'feed_create'];

    for (const scriptType of scriptTypes) {
      const length = await utxoManager.getUtxosLength(scriptType);
      const count = await utxoManager.getUtxosCount(scriptType);
      ...
      for (let j = 0; j < length; j++) {
        ...
        if (!utxo) {
          ...
          await this.del(`utxo.${scriptType}.${j}`);

          if (floatingUtxoHashes.indexOf(utxo.getTxHash()) < 0) {

            this._logger.info(`range: ${j - fullIndexCount}, count: ${count}, length: ${length}, null indexes: ${nullIndexes.length}, stale index: ${j}, fullIndexCount: ${fullIndexCount}`);

            // test if this utxo is flagged
            fullIndexCount++;

            let { utxoToAddresses, addresses } = await utxoManager.extractAddressesFromUtxos(scriptType, [{ txHash: utxo.getTxHash(), index: utxo.getTxIndex(), script: utxo.getOutput().getOutputScript() }]);

            for (const addr of Object.keys(addresses)) {
              if (!addressesIndex[addr]) {
                addressesIndex[addr] = [fullIndexCount];
              } else {
                addressesIndex[addr].push(fullIndexCount);
              }
            }
            ...
            //this._logger.info(`putting opunspent.${txHash}.${txIndex} at ${fullIndexCount}`)
            await this.put(`opunspent.${txHash}.${txIndex}`, fullIndexCount);
            await this.put(`utxo.${scriptType}.${fullIndexCount}`, utxo);
          } else {
            await this.del(`opunspent.${utxo.getTxHash()}.${utxo.getTxIndex()}`, fullIndexCount);
            await this.del(`utxo.${scriptType}.${fullIndexCount}`, utxo);
          }
        }
      }

      for (const addr of Object.keys(addressesIndex)) {
        this._logger.info(`putting address index ${addr}`);
        await this.put(`${addr}.${scriptType}`, [...new Set(addressesIndex[addr])]);
      }

      await this.put(`utxo.${scriptType}.length`, fullIndexCount);
      await this.put(`utxo.${scriptType}.count`, fullIndexCount);
    }
  }

  checkIfTxHasBalance(tx, hashedAddress) {
    ...
    try {
      ...
      for (const o of outputs) {
        ...
        const val = internalToHuman(o.getValue(), NRG);
        if (new BN(val).gt(new BN(50000))) {
          hasBalance = val;
        }
      }
  ...
  async rebuildTxGraph(utxoManager, txHandler, walletHandler, blockHeightToIndexFrom, optionalBlock) {
    ...
    const txsToRemove = [];
    const txRemovalByBlock = {};
    const reasonTxRemove = {};
    ...
    const marker = await this.get(`${BC_SUPER_COLLIDER}.utxorebuilt`);
    if (marker && marker === 'complete') {
      return;
    }

    const lock = await this.get(`${BC_SUPER_COLLIDER}.indexlock`);
    if (lock) {
      return;
    }

    const chainedHashes = [block.getHash()];

    for (let i = optionalBlock.getHeight(); i >= blockHeightToIndexFrom; i--) {
      block = await this.getBlockByHash(block.getPreviousHash(), 'bc');
      // XD
      this._logger.info(`block height: ${block.getHeight()} ${i}`);
      chainedHashes.unshift(block.getHash());
      if (block) {
        firstBlock = block;
      }
    }

    await this.put(`${BC_SUPER_COLLIDER}.indexlock`, 'true');

    const processedHashes = [];

    //while (chainedHashes.length > 0) {
    //for (const blockHash of chainedHashes) {
    while (chainedHashes.length > 0) {
      const blockHash = chainedHashes.shift();
      processedHashes.push(blockHash);
      processed++;
      block = await this.getBlockByHash(blockHash, 'bc');
      const { utxos, stxos } = await this.getUnspentAndSpentForBlock(block);
      const temptxs = block.getTxsList();
      const blockHeight = block.getHeight();

      for (const type of Object.keys(utxos)) {
        utxos[type].map(async pair => {
          const txHash = pair.tx && pair.tx.getHash ? pair.tx.getHash() : false;
          this._logger.info(`block: ${blockHeight} parsed utxo ${txHash}`);

          if (txHash) {
            const hash = pair.tx.getHash() + ':' + pair.index;
            const utxoHash = pair.tx.getHash() + ':' + pair.index + ':utxo';
            const containingTxHash = pair.tx.getHash();
            let delUtxo = false;

            if (!spent[utxoHash]) {
              spent[utxoHash] = 1;
            } else {
              if (!reasonTxRemove[containingTxHash]) {
                reasonTxRemove[containingTxHash] = 1;
              } else {
                reasonTxRemove[containingTxHash] = 1;
              }
              spent[utxoHash]++;
              if (!txRemovalByBlock[blockHash]) {
                txRemovalByBlock[blockHash] = [containingTxHash];
              } else if (txRemovalByBlock[blockHash].indexOf(containingTxHash) === -1) {
                txRemovalByBlock[blockHash].push(containingTxHash);
              }
              delUtxo = true;
              if (block.getHeight() > 7579000) {
                await this.delUtxoRoot(utxoManager, type, pair, block.getHash());
              }
            }
            /////// TD ***********************************************************************************************************
            const outputsByIndex = {};
            const totalOutput = pair.tx.getOutputsList().reduce((all, output, i) => {
              const val = new BN(output.getValue());
              const num = Number(val / 1000000000000000000);
              outputsByIndex[i] = num;
              return all + num;
            }, 0);
            if (totalOutput > 80000) {

              REBUILD.write(`${pair.tx.getHash()},${pair.index},${block.getHash()},${totalOutput}\n`);

              const unspent = await this.isOutPointUnspent(pair.tx.getHash(), pair.index);
              if (unspent && !delUtxo) {
                if (block.getHeight() > 7579000) {
                  await this.delUtxoRoot(utxoManager, type, pair, block.getHash());
                }
      ...
      for (const type of Object.keys(stxos)) {
        while (stxos[type].length > 0) {
          //stxos[type].map(async (pair) => {
          const pair = stxos[type].shift();
          this._logger.info(`block: ${blockHeight} parsed stxo: containing tx ${pair.containingTxHash}, pair tx ${pair.tx.getHash()}`);
          const containingTxHash = pair.containingTxHash;
          let foundContainingTx = false;
          for (const tx of temptxs) {
            if (tx.getHash() === containingTxHash) {
              foundContainingTx = true;
            }
          }
          if (!foundContainingTx) {
            if (block.getHeight() > 7579000) {
              await this.delUtxoRoot(utxoManager, type, pair, block.getHash());
            }
            continue;
          ...
          if (!pair.tx && pair.containingTxHash || !pair.tx.getHash && pair.containingTxHash && block.getHeight() > 7579000) {
            if (!reasonTxRemove[containingTxHash]) {
              reasonTxRemove[containingTxHash] = ['missing block ref'];
            } else {
              reasonTxRemove[containingTxHash].push('missing block ref');
            }
            if (!txRemovalByBlock[blockHash]) {
              txRemovalByBlock[blockHash] = [containingTxHash];
            } else if (txRemovalByBlock[blockHash].indexOf(containingTxHash) === -1) {
          ...
          const hash = pair.tx.getHash() + ':' + pair.index;
          let checkSpent = false;
          if (pair.multipleBlocks.length > 0) {
            for (const mb of pair.multipleBlocks) {
              if (blockHash === mb) {
                checkSpent = true;
              }
            }
          ...
          checkSpent = true;
          const k = `block:${pair.hash} has spent tx hash:${pair.tx.getHash()} + index:${pair.index} contained in ${pair.containingTxHash}`;
          if (checkSpent && block.getHeight() > 7579000) {
            if (!spent[hash]) {
              spent[hash] = 1;
            } else {
              if (!reasonTxRemove[containingTxHash]) {
      ...
      for (const tx of temptxs) {
        ...
        const totalOutput = tx.getOutputsList().reduce((all, output, i) => {
          const val = new BN(output.getValue());
          const num = Number(val / 1000000000000000000);
          return all + num;
        }, 0);

        this._logger.info(`check block txs: ${blockHeight} target: ${tx.getHash()}, output: ${totalOutput}`);

        const checkTx = this.checkIfTxHasBalance(tx);
        if (checkTx && checkTx !== false || totalOutput > 40000) {
          if (!txRemovalByBlock[blockHash]) {
            txRemovalByBlock[blockHash] = [tx.getHash()];
          } else if (txRemovalByBlock[blockHash].indexOf(tx.getHash()) === -1) {
            txRemovalByBlock[blockHash].push(tx.getHash());
          }
          let i = 0;
          for (const o of tx.getOutputsList()) {
            const txData = {
              tx: tx,
              height: blockHeight,
              hash: blockHash,
              containingTxHash: tx.getHash(),
              index: i,
              multipleBlocks: []
            };

            const transfer = await this.delUtxoRoot(utxoManager, "nrg_transfer", txData, blockHash);
            if (!transfer) {
              const maker = await this.delUtxoRoot(utxoManager, "maker_output", txData, blockHash);
              if (!maker) {
                const takert = await this.delUtxoRoot(utxoManager, "taker_output", txData, blockHash);
                if (!takert) {
                  const takerc = await this.delUtxoRoot(utxoManager, "taker_callback", txData, blockHash);
                }
              }
            }
            i++;
    ...
    // remove the utxos to the right height
    await utxoManager.removeUTXOsFrom(blockHeightToIndexFrom);

    for (const blockHash of Object.keys(txRemovalByBlock)) {
      const block = await this.getBlockByHash(blockHash, 'bc', { cached: false });
      const unindexTxs = txRemovalByBlock[blockHash];
      const oldTxs = block.getTxsList();
      const newTxs = oldTxs.filter(tx => {
        if (unindexTxs.indexOf(tx.getHash()) > -1) {
          return false;
        }
        return true;
      });
      block.setTxsList(newTxs);
      block.setTxCount(newTxs.length);
      await this.saveBlock(block);
    }

    await this.put(`${BC_SUPER_COLLIDER}.block.last.utxoSaved`, firstBlock, { sync: true });
    await utxoManager.updateUTXOs(firstBlock, false);

    while (processedHashes.length > 0) {
      const blockHash = processedHashes.shift();
      const block = await this.getBlockByHash(blockHash, 'bc', { cached: false });
      this._logger.info(`creating ref: ${block.getHeight()} ${block.getHash()} prev: ${block.getPreviousHash()}`);
      let updateUTXOsResult = await utxoManager.updateUTXOs(block, false);
    }

    await this.del(`${BC_SUPER_COLLIDER}.indexlock`);
    await this.put(`${BC_SUPER_COLLIDER}.utxorebuilt`, `complete`);

    return;
  }

  async rebuildChainIndexFromHeight(utxoManager, txHandler, blockHeightToIndexFrom, optionalBlock) {
    let block = optionalBlock ? optionalBlock : await this.get(`${BC_SUPER_COLLIDER}.block.last.utxoSaved`);

    const marker = await this.get(`${BC_SUPER_COLLIDER}.indexrebuilt`);
    const lock = await this.get(`${BC_SUPER_COLLIDER}.indexlock`);
    ...
    try {
      ...
      //let lastIndexRebuiltHeight = marker ? marker.split(':')[0] : 7579000
      let lastIndexRebuiltHeight = blockHeightToIndexFrom;
      let lastIndexRebuiltCycle = marker ? marker.split(':')[2] : 0;
      let lastIndexRebuiltHash = marker ? marker.split(':')[1] : "NA";

      blockHeightToIndexFrom = lastIndexRebuiltHeight;

      let processed = 0;
      let totalBadTxs = 0;
      let totalProcessedBlocks = 0;
      let totalBadOl = new BN(0);
      ...
      blockHeightToIndexFrom = blockHeightToIndexFrom < 2 ? 2 : blockHeightToIndexFrom;

      // from the lowest to the heighest blockheight
      const orderedHashes = [[block.getHeight(), block.getHash()]];

      // list of spent txs
      const spent = [];
      const spentTable = {};

      // first build a correct tree of blocks from a point
      while (block && block.getHeight() >= blockHeightToIndexFrom) {
        const blockHash = block.getHash();
        orderedHashes.unshift([block.getHeight(), blockHash]);
        ...
        if (height % 1000 === 0) {
          const diff = startHeight - height;
          const total = startHeight - blockHeightToIndexFrom;
          const perc = parseFloat(diff / total * 100).toFixed(2);
          this._logger.info(`stage (1/2) building hash table: ${orderedHashes.length}, ${perc}% done`);
        }
      }

      this._logger.info(`successfully created ordered hash table: ${orderedHashes.length}`);
      ...
      //while (orderedHashes.length > 0) {
      for (const pair of orderedHashe) {
        //while (orderedHashes.length > 0) {
        //const pair = orderedHashes.shift()
        blocksToAdd.push(pair);
        const [height, hash] = pair;
        const block = await this.getBlockByHash(hash, 'bc');
        ...
        const badTxs = [];
        const txsToRemoveInThisBlockIndex = [];
        const txsToRemoveInThisBlock = [];
        const badOl = new BN(0);
        const txs = block.getTxsList();
        let txCount = txs.length;
        for (const tx of txs) {
          for (const i of tx.getInputsList()) {
            const io = await this.getTxProtoFromInput(i.getOutPoint());
            if (io && io.getOutputsList) {
              const o = io.getOutputsList()[i.getOutPoint().getIndex()];
              const key = i.getOutPoint().getHash() + ':' + i.getOutPoint().getIndex();
              if (spentTable[key]) {
                if (txsToRemoveInThisBlockIndex.indexOf(tx.getHash()) === -1) {
                  txsToRemoveInThisBlockIndex.push(tx.getHash());
                  txsToRemoveInThisBlock.push(tx);
                }
                badTxs.push([key, spentTable[key]]);
                badOl.iadd(new BN(o.getValue()));
                if (!txsToRemove[block.getHash()]) {
                  txsToRemove[block.getHash()] = [tx.getHash()];
                } else {
                  if (txsToRemove[block.getHash()].indexOf(tx.getHash()) === -1) {
                    txsToRemove[block.getHash()].push(tx.getHash());
                  }
                }
              } else {
                spentTable[key] = block.getHeight() + ':' + block.getHash();
              }
            }
          }
        }
        ...
        if (badTxs.length > 0) {
          totalBadTxs = totalBadTxs + badTxs.length;
          ...
          const progress = totalProcessedBlocks;
          const range = startHeight - blockHeightToIndexFrom;
          const perc = parseFloat(progress / range * 100).toFixed(2);
          this._logger.info(`stage (2/2) writing index, ${perc}% done...`);
        }
      }

      await this.put(`${BC_SUPER_COLLIDER}.block.last.utxoSaved`, firstBlock, { sync: true });

      for (const pair of blocksToAdd) {
        //while (blocksToAdd.length > 0) {
        const [height, hash] = pair;
        this._logger.info(`creating ref: ${height} ${hash} ${blocksToAdd.length}`);
        const block = await this.getBlockByHash(hash, 'bc');
        let updateUTXOsResult = await utxoManager.updateUTXOs(block, false);
      }
      lastIndexRebuiltCycle++;
      //if (lastIndexRebuiltCycle > 2) {
      //  await this.put(`${BC_SUPER_COLLIDER}.indexrebuilt`, `complete`)
      //} else {
      //  await this.put(`${BC_SUPER_COLLIDER}.indexrebuilt`, `${previousBlock.getHeight()}:${previousBlock.getHash()}:${lastIndexRebuiltCycle}`)
      //}
      await this.del(`${BC_SUPER_COLLIDER}.indexlock`);
      return;
    } catch (err) {
  ...
  async validateChain(txHandler, count, optionalBlock) {
    ...
    let highestKnownDiff = new BN(0);
    let highestKnownDiffBlock = false;
    const spent = [];
    while (block) {
      //if (block.getHeight() % 10000 === 0) {
      const txs = block.getTxsList();
      const txFees = txs.map(tx => calcTxFee(tx)).reduce((fee, sum) => sum.add(fee), new BN(0));
      ...
      let coinbaseValueSet = false;
      let coinbaseValue = new BN(0);
      let flagged = false;
      const badTxs = [];
      for (const tx of txs) {
        const valueFromInput = new BN(0);
        const valueFromOutput = new BN(0);
        for (const i of tx.getInputsList()) {
          const io = await this.getTxProtoFromInput(i.getOutPoint());
          if (io && io.getOutputsList) {
            const o = io.getOutputsList()[i.getOutPoint().getIndex()];
            const key = i.getOutPoint().getHash() + ':' + i.getOutPoint().getIndex();
            if (spent.indexOf(key) === -1) {
              flagged = true;
              badTxs.push(key);
            } else {
              spent.push(key);
            }
            valueFromInput.iadd(new BN(o.getValue()));
          } else {
            this._logger.info(`reviewing ${block.getHeight()} : missing tx ${tx.getHash()} -> ${i.getOutPoint().getHash()}:${i.getOutPoint().getIndex()}`);
          }
        }
        for (const o of tx.getOutputsList()) {
          if (!coinbaseValueSet) {
            coinbaseValue = new BN(o.getValue()).sub(txFees);
            coinbaseValueSet = true;
            if (tx.getOutputsList().length !== 1) {
              flagged = true;
            }
          }
          valueFromOutput.iadd(new BN(o.getValue()));
        }
        value.totalOutput.iadd(valueFromOutput);
        value.totalInput.iadd(valueFromInput);
      }

      const valueDiffCb = value.totalOutput.sub(coinbaseValue);
      const inputsEqualOutputs = value.totalInput.eq(valueDiffCb);

      if (!inputsEqualOutputs || flagged) {
        if (valueDiffCb.gt(highestKnownDiff)) {
          highestKnownDiff = valueDiffCb;
          highestKnownDiffBlock = block.getHeight();
        }
        this._logger.info(`${block.getHeight()} : ${block.getHash()} : ${txs.length}, remain: ${count}, found: ${blocksToReview.length}, highest: ${highestKnownDiff.toString()} : ${highestKnownDiffBlock}`);
        this._logger.info(`   totalFromInputs: ${value.totalInput.toString()}`);
        this._logger.info(`   totalFromOutputs: ${value.totalOutput.toString()}`);
        this._logger.info(`   txFees: ${txFees.toString()}`);
        this._logger.info(`   badTxs: ${JSON.stringify(badTxs.join(','))}`);
      }
      //}
    ...
    try {
      ...
      if (!reorgFromBlock || !reorgToBlock) {
        ...
        if (reorgFromBlock) {
          ...
          this._logger.info(`setting highest known edge to ${reorgFromBlock.getHeight()}`);
  ...
  /**
   * Get transaction trades by transaction key (not hash)
   * @param key string
   * @param blockchain string
   */
  async getTradesByKey(key, blockchain = 'bc', opts = {
    asBuffer: true,
    cached: true
  }) {
    // example structure of key ${makerSendsFrom}.${takerReceivesTo}.${sendsFromChain}.${sendsUnit}`
    if (this._tradesByKeyCache.has(key)) {
      const cached = this._tradesByKeyCache.get(key);
      return cached;
    }
    const trades = await this.get(key);
    if (trades && trades.length > 0) {
      this._tradesByKeyCache.set(key, trades);
    } else {
      this._tradesByKeyCache.delete(key);
    }
    return trades;
  }

  /**
   * Put trades by transaction key (not hash)
   * @param key string
   * @param trades
   * @param blockchain string
   */
  async putTradesByKey(key, trades, blockchain = 'bc', opts = {
    asBuffer: true,
    cached: true
  }) {
    //
  ...
  async getTransactionByHash(txHash, blockchain = 'bc', opts = {
    ...
    if (tx) {
      ...
      let key = await this.getBlockByTxHash(txHash);
  ...
  async putTransactionBlockIndex(txHashes, blockHash, blockHeight, branch = 0, blockchain = 'bc', opts = { asBuffer: true }) {
    try {
      ...
      for (const hash of txHashes) {
        const key = `${blockchain}.txblock.${hash}`;
        const blockKey = `${blockchain}.block.${blockHash}.${blockHeight}`;
        let list = await this.get(key);
        if (!list) {
          await this.put(key, blockKey);
        } else {
          list = list.split(':');
          if (list.indexOf(blockKey) < 0) {
            list.push(blockKey);
            await this.put(key, list.join(':'));
          }
        }
      }
      return true;
  ...
  // !! this does not return UTXOs but data for building them
  // private
  async getUnspentAndSpentForBlock(block) {
    ...
    try {
      ...
      for (const tx of block.getTxsList()) {
        // TD DEBUG
        debug(`getUnspentAndSpentForBlock(): tx: ${tx.getHash()} for block ${block.getHeight()}`);
        await this.getTxData(tx, utxos, stxos, block.getHash(), tx.getHash());
      }
  ...
  // private
  async getTxProtoFromInput(txInput) {
    ...
    const txCacheKey = `${txInput.getHash()}:${txInput.getIndex()}`;

    if (this._getTxCache.has(txCacheKey)) {
      return this._getTxCache.get(txCacheKey);
    }

    const tx2 = await this.getTransactionByHash(txInput.getHash());
    if (!tx2) {
      this._logger.info(`${txInput.getHash()} not found`);
      return false;
    }
    return tx2;
  }
  ...
  // private
  // txInput is a input proto from a transaction
  // containingTxHash is an optional hash references the parent hash containing this input
  async getTxFromInput(txInput, containingTxHash) {
    if (!this._readEventTable['getTxFromInput']) {
      this._readEventTable['getTxFromInput'] = 0;
    }
    this._readEventTable['getTxFromInput']++;
    ...
    const tx2 = await this.getTransactionByHash(txInput.getHash());
    if (!tx2) {
      this._logger.info(`attempted to get tx by has ${txInput.getHash()} not found`);
    }
    let key = await this.getBlockByTxHash(txInput.getHash(), { asKey: true, containingTxHash });
    if (!key) {
      ...
      const obj = { scriptType, tx2: false, height: false, hash: false, index: txInput.getIndex(), containingTxHash, multipleBlocks };
      return obj;
    } else if (Array.isArray(key)) {
      this._logger.info(`${txInput.getHash()} is in multiple blocks...`);
      multipleBlocks = key.map(k => {
        const [blockchain, _, hash, height] = k.split('.');
        return hash;
      });
      for (const k of key) {
        const [blockchain, _, hash, height] = k.split('.');
        this._logger.info(`${txInput.getHash()} is in ${blockchain} ${hash} ${height}`);
      }
      key = key[0];
    } else if (!key.split) {
      console.log(key);
    ...
    const scriptType = getScriptType(tx2.getOutputsList()[txInput.getIndex()].getOutputScript());
    const obj = { scriptType, tx2, height, hash, index: txInput.getIndex(), containingTxHash, multipleBlocks };
    this._getTxCache.set(txCacheKey, obj);
    return obj;
  }

  // !! TD This does not actually extract UTXO protobufs but rather data similar to the UTXOs
  // private
  async getTxData(tx, utxos, stxos, blockHash, containingTxHash) {
    ...
    if (tx) {
      tx.getOutputsList().map((output, index) => {
        ...
        utxos[scriptType].push({ tx, index, containingTxHash, blockHash });
      });

      let arr = [];

      for (const i of tx.getInputsList()) {
        // TD add here a filter out of values where the input is false
        //const io = await this.getTxFromInput(i.getOutPoint(), containingTxHash)
        const io = await this.getTxFromInput(i.getOutPoint(), tx.getHash());
        if (io && !io.tx2) {
          const outpoint = i.getOutPoint();
          const h = outpoint.getHash();
          //this._logger.info(`tx mount for block ${blockHash} with tx hash: ${h} containing hash: ${containingTxHash} not found`)
        } else {
          arr.push(io);
        }
      }
      ...
      arr.map(({ scriptType, tx2, hash, height, index, containingTxHash, multipleBlocks }) => {
        if (!stxos[scriptType]) {
          stxos[scriptType] = [];
        }
        // TD DEBUG
        debug(`getTxData(): in spent tx: ${tx2.getHash()} index ${index} for block ${height}`);
        stxos[scriptType].push({ tx: tx2, index, height, hash, containingTxHash, multipleBlocks });
      });
    }
  }
  ...
  async getTradeIndices(sendsFromAddress, receivesToAddress, sendsFromChain, sendsUnit) {
    ...
    //const hashes = await this.get(key)
    // moved hash extraction to cache
    const hashes = await this.getTradesByKey(key);
  ...
  async getTxClaimedBy(hash, index) {
    const key = `opspent.${hash}.${index}`;
    const val = await this.get(key);
    if (val) {
      return this.getTransactionByHash(val);
    } else {
  ...
  async getBlockByTxHash(txHash, opts = { blockchain: BC_SUPER_COLLIDER, returnLikelyWinner: true, asKey: false, containingTxHash: false }) {
    ...
    if (!opts.blockchain) {
      opts.blockchain = BC_SUPER_COLLIDER;
    }
    if (!opts.returnLikelyWinner) {
      opts.returnLikelyWinner = true;
    }

    this._readEventTable['getBlockByTxHash']++;
    // opts.returnLikelyWinner = true will return only one block if there is a clear winner otherwise it will return a list
    try {
      // get the list of blocks which may use this hash, generally there is only one
      const potentialBlocks = await this.get(id);

      // no blocks have been found and so return null
      if (!potentialBlocks) {
        this._logger.warn(`getBlockByTxHash(): no blocks found for txhash ${txHash}`);
        return null;
      }

      let list = potentialBlocks.indexOf(":") > -1 ? potentialBlocks.split(":") : [potentialBlocks];
      const refs = {};

      // use only blocks from the blockchain given
      list = list.filter(async b => {
        const [blockchain, _, hash, height] = b.split('.');
        const utxoSaved = await this.get(`${BC_SUPER_COLLIDER}.block.${height}.utxoSaved`);
        if (!utxoSaved && blockchain === opts.blockchain) {
          return true;
        }
        if (hash === utxoSaved && blockchain === opts.blockchain) {
          return true;
        }
      });

      // no transactions found for given blockchain
      if (list.length === 0) {
        console.log(opts);
        this._logger.warn(`getBlockByTxHash(): no blocks found for tx of given blockchain ${opts.blockchain} using txhash ${txHash}`);
        this._logger.warn(`getBlockByTxHash(): potential blocks -> ${potentialBlocks}`);
        return null;
      }

      // now sort the list from lowest to highest block
      list.sort((a, b) => {
        const [aBlockchain, a_, aHash, aHeight] = a.split('.');
        const [bBlockchain, b_, bHash, bHeight] = b.split('.');
        refs[aHash] = a;
        if (aHeight > bHeight) {
          return 1;
        }
      ...
      // this lowestHeight is the lowest height in which a block had a tx
      let lowestHeight = false;

      //for (const l of list) {
      //  const [aBlockchain, a_, aHash, aHeight] = l.split('.')
      //  this._logger.info(`${aBlockchain}.${aHeight} ${aHash} for tx hash: ${txHash}, containing hash: ${opts.containingTxHash}`)
      //}

      for (const key of list) {
        if (key) {
          const [blockchain, _, hash, height] = key.split('.');
          // lowest height has not been set
          if (!lowestHeight) {
            lowestHeight = height;

            // this means the same tx was mined into multiple blocks at the same height so all blocks must be returned
            if (opts.asKey && !opts.containingTxHash) {
              keys.push(key);
            }

            if (opts.containingTxHash || !opts.asKey) {
              const block = await this.getBlockByHash(hash, blockchain, { asHeader: false, cached: false });
              if (block) {
                if (opts.containingTxHash) {
                  let found = false;
                  for (const tx of block.getTxsList()) {
                    if (found) continue;
                ...
                // reset lowest height as block was not found
                lowestHeight = false;
              }
            }
          } else if (lowestHeight && height <= lowestHeight || lowestHeight && opts.containingTxHash) {
            if (opts.asKey && !opts.containingTxHash) {
              keys.push(key);
            }
            if (opts.containingTxHash || !opts.asKey) {
              const block = await this.getBlockByHash(hash, blockchain, { asHeader: false, cached: true });
              if (block) {
                if (opts.containingTxHash) {
                  let found = false;
                  for (const tx of block.getTxsList()) {
            ...
            // this means that the block containing this tx is above a different block already referencing the tx
          }
        }
      }

      // if this is a lite call looking only for the block reference return the keys
      if (opts.asKey && keys.length > 0) {
        if (keys.length === 1) {
          return keys[0];
        }
        keys.sort((a, b) => {
      ...
      // no blocks were found for hash
      if (blocks.length === 0) {
        return null;
      }

      // if multiple blocks are found at the same height return it if there is one clear older block
      let earlierBlockFound = false;
      blocks.sort((a, b) => {
        if (a.getTimestamp() > b.getTimestamp()) {
          earlierBlockFound = true;
          return 1;
        }
        if (a.getTimestamp() < b.getTimestamp()) {
          earlierBlockFound = true;
      ...
      if (blocks.length === 1 && opts.returnLikelyWinner) {
        return blocks[0];
      } else if (opts.returnLikelyWinner && earlierBlockFound) {
        return blocks[0];
      } else {
        return blocks;
      }
