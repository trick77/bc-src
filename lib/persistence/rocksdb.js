...
class PersistenceRocksDb {
  ...
  */
  ...
  async updateUTXOs(block, remove = false) {
    ...
    if (remove) {
      // XXX
      // this._logger.info(`REMOVED UTXOS for ${block.getHeight()}:${block.getHash()}`)
      ...
      // XXX
      // this._logger.info(`UTXOS updated for ${block.getHeight()}:${block.getHash()}`)
  ...
  */
  ...
  async isRoveredBlockPastShift(childChainHash, childChainHeight, childChainId, shiftAmount, latestBlock) {
    try {
      // this._logger.info(`calling isRoveredBlockPastShift comparing ${childChainHash},${childChainHeight}`)
