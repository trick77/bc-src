...
class PersistenceRocksDb {
  ...
  constructor(location = '_data', chainState, engine, multichainState) {
    ...
    this._blockByHashAtHeightCache = new LRUCache({
      max: 10000
    });
  ...
  */
  ...
  async putBlockHashAtHeight(blockHash, height, blockchain = 'bc', opts = { asBuffer: true, storeOnlyOne: false }) {
    ...
    try {
      let change = false;
      if (!blockHash) {
        throw new Error(`putBlockHashAtHeight(): malformed block hash provided`);
      }
      if (this._blockByHashAtHeightCache.has(blockHash)) {
        return true;
      }
      this._blockByHashAtHeightCache.set(blockHash, height);
  ...
  */
  async putBlock(block, branch = 0, blockchain = 'bc', opts = { asBuffer: true, fromWaypoint: false, saveHeaders: false, force: false, storeOnlyOne: false, updateHeight: true, rovered: false }) {
    ...
    // make sure the block is added to the given height
