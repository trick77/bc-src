...
class PersistenceRocksDb {
  ...
  async saveTxsForBlock(block) {
    ...
    try {
      ...
      for (const tx of txs) {
        ...
        txsToSave.push(this.putTransaction(tx, block.getHash(), 0, 'bc'));
        txsToSave.push(this.putTransactionBlockIndex([tx.getHash()], block.getHash(), block.getHeight(), 0, 'bc'));
      }
  ...
  async saveBlock(block) {
    ...
    await Promise.all([this.put(`${BC_SUPER_COLLIDER}.block.${block.getHash()}`, block), // ensure block gets saved at height
    this.putBlockHashAtHeight(block.getHash(), block.getHeight(), 'bc'), this.put(`${BC_SUPER_COLLIDER}.block.${block.getHeight()}`, block), // this overrides the height
    this.saveTxsForBlock(block), this.saveBlockHeaders(block), this.putChildBlocksIndexFromBlock(block), this.del(`${BC_SUPER_COLLIDER}.block.lock.${block.getHeight()}`) // remove any flagged blocks at this height
    ]);
