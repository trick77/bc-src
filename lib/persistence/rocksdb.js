...
const loadBasedPeerExpiration = max(50000, Math.floor(210000 / numCPUs));
...
const sortBlockList = blockList => {
  return blockList.sort((a, b) => {
    ...
    if (new BN(a.getHeight()).gt(new BN(b.getHeight()))) {
      return -1;
    }

    if (new BN(b.getHeight()).gt(new BN(a.getHeight()))) {
      return 1;
    }
...
class PersistenceRocksDb {
  ...
  */
  ...
  async putTransaction(tx, blockHash, branch = 0, blockchain = 'bc', opts = { asBuffer: true, force: false }) {
    ...
    //debugWriteOperations(`putTransaction(): ${key}`)
    ...
    // XXX
    //this._logger.info(`saving tx ${tx.getHash()}`)
  ...
  async putBlockHashAtHeight(blockHash, height, blockchain = 'bc', opts = { asBuffer: true, storeOnlyOne: false }) {
    ...
    if (this._blockHashAtHeightCache.has(indexKey)) {
      return true;
    }
    ...
    if (prevBlock) {
      ...
      if (!parentBlockHashes) {
        ...
        if (opts.returnParents) {
          // LDL
          this._logger.info(`opts.returnParents = true, returning ${parentBlockHashes.length} parents `);
      ...
      this._logger.info(`cannot find previous block ${block.getPreviousHash()}`);
      this._logger.info(`returning block at height ${block.getHeight()} from target height for chain`);
  ...
  * @param blockchain string
  */
  async putLatestBlock(block, defaultBlockchain = 'bc', opts = { asBuffer: true, previousBlock: false, reloadTxs: false, saveHeaders: false, iterateUp: true, context: false }) {
    ...
    try {
      ...
      //      // select tree
      //      await this.pruneFromBlock(block, 100, 93)
      //    }
      //  }
      //}
  ...
  async removeUTXOsFrom(height) {
    ...
    for (const block of blocksToRemove) {
      let success = await this.updateUTXOs(block, true);
  ...
  async updateUTXOs(block, remove = false) {
    ...
    if (lastSavedBlock) debugUTXO(`${remove ? 'REMOVING' : 'SAVING'} ${block.getHeight()}:${shortenHash(block.getHash())}:${shortenHash(block.getPreviousHash())}, LAST is ${lastSavedBlock.getHeight()}:${shortenHash(lastSavedBlock.getHash())}`);
    ...
    if (remove) {
      ...
      for (const tx of block.getTxsList()) {
        this._logger.info(`adding tx ${tx.getHash()}`);
  ...
  async getUnspentAndSpentForBlock(block) {
    ...
    //debugReadOperations(`getUnspentAndSpentForBlock() ${block.getHeight()}:${block.getHash()}`)
  ...
  async getTxFromInput(txInput) {
    ...
    //debugReadOperations(`getTxFromInput() ${txInput.getHash()}`)
   ...
   * @param blockchain string
   */
  async updateTradeIndex(utxo, remove) {
    try {
      const { tx, index } = utxo;
  ...
  async addUTXO(scriptType, utxos, block) {
    ...
    for (let i = 0; i < utxos.length; i++) {
      ...
      if (scriptType === 'taker_output') await this.updateTradeIndex({ tx, index }, false);
  ...
  async delUTXO(scriptType, utxos, block) {
    ...
    for (let i = 0; i < utxos.length; i++) {
      ...
      //add key for marked transactions for this taker order
      if (scriptType === 'taker_output') await this.updateTradeIndex({ tx, index }, true);
  ...
  * @param blockchain string
  */
  async putBlock(block, branch = 0, blockchain = 'bc', opts = { asBuffer: true, fromWaypoint: false, saveHeaders: false, force: false, storeOnlyOne: false, updateHeight: true, rovered: false }) {
    // try {
    ...
    if (newBlocks > 128) {
      debug(`new block has more than maximum 128 headers (${newBlocks})`);
    ...
    if (childBlockSaved && !opts.rovered && !opts.force) {
      return Promise.resolve(true);
    }
    ...
    // this._logger.info(`returning putBlock from ${block.getHeight()}.${block.getHash()}`)
    return externalOps;
  }
  ...
  /**
   * Returns balances object (expensive operation)
   */
  async getMarkedBalanceTable(block, blockchain = 'bc', opts = { asBuffer: true }) {}
   ...
   * @param hash string
   */
  async getBlockByHash(hash, blockchain = 'bc', opts = { asBuffer: true, asHeader: false, cached: true }) {
    const key = `${blockchain}.block.${hash}`;
    //debugReadOperations(`getBlockByHash() ${key}`)
   ...
   * @param blockchain string
   */
  async getBlocksByHeight(height, blockchain = 'bc', opts = { asBuffer: true, iterateUp: true, asHashes: false, searchUp: false }) {
    if (isNaN(height)) return false;
    ...
    try {
      ...
      if (blockHashes === null && opts.iterateUp || opts.searchUp) {
        let block = false;
   ...
   * @param blockchain string
   */
  async getBlocksByRange(from, to, blockchain = 'bc', opts = { asBuffer: true, asSet: false, cached: false, searchUp: false }) {
    // from: 3, to: 30
    // XXX
    if (to === -1) {
    ...
    if (intervalSize > 3000) {
      this._logger.warn('block range lookup limited to 3000');
      intervalSize = 3000;
      to = min(from + 3000, to);
    }
    debug(`getBlocksByRange(): request ${from}->${to} for ${blockchain}`);
  ...
  */
  ...
  async isRoveredBlockPastShift(childChainHash, childChainHeight, childChainId, shiftAmount, latestBlock) {
    try {
      ...
      let isPastShift = await this.get(`${childChainHeight}.${childChainHeight}.${childChainId}.${shiftAmount}`);
      if (isPastShift) {
        debugShift(`${childChainHash},${childChainHeight} is past shift`);
        return 1;
      }

      if (!latestBlock) latestBlock = await this.get(`bc.block.latest`);
      ...
      if (block.getHeight() >= Number(childChainHeight) + shiftAmount) {
        while (block && block.getHeight() >= Number(childChainHeight)) {
          debugShift(`comapring ${childChainId} ${block.getHeight()} ${block.getHash()} with ${childChainHeight} ${childChainHash}`);

          if (block.getHash() === childChainHash) {
            await this.put(`${childChainHeight}.${childChainHeight}.${childChainId}.${shiftAmount}`, true);
            debugShift(`${childChainHash},${childChainHeight} is past shift`);
          ...
          if (!block) {
            ...
            if (blocks.length === 1) {
              ...
              //block was not saved by height either, check by latestBlock
              let childBlocks = getChildBlocks(latestBlock, childChainId.toLowerCase());
              childBlocks = childBlocks.filter(e => {
                return e.getHash() === block1.getPreviousHash();
              });
              let checkBlock = last(getChildBlocks(latestBlock, childChainId.toLowerCase()));

              while (childBlocks.length === 0 && parseInt(checkBlock.getHeight()) > parseInt(block1.getHeight() - 2)) {
                latestBlock = await this.getBlockByHash(latestBlock.getPreviousHash(), 'bc');
                childBlocks = getChildBlocks(latestBlock, childChainId.toLowerCase());
                childBlocks = childBlocks.filter(e => {
                  return e.getHash() === block1.getPreviousHash();
                });
                checkBlock = last(getChildBlocks(latestBlock, childChainId.toLowerCase()));
              }
              //found the block, put it into persistence
  ...
  */
      ...
      if (!output) {
        ...
        if (block && block.getTxsList) {
          ...
          if (txs.length === 1) {
            ...
            if (!output) {
              throw new Error(`unable to get tx hash from ${txHash} after local storage update`);
            }
          } else {
            throw new Error(`unable to get get tx hash from ${txHash} after parent block search`);
          }
        } else {
          throw new Error(`unable to get tx hash from ${txHash}`);
        }
      }
      const [makerScript, _, __] = await this.getInitialMakerOrder(toASM(Buffer.from(output.getOutputScript()), 0x01), 0);
      let makerOrder = parseMakerLockScript(makerScript);
            ...
            if (childBlock.getHeight() === Number(childChainHeight)) {
              for (const markedTx of childBlock.getMarkedTxsList()) {
                if (markedTx.getHash() === markedTxHash) {
                  ...
                  if (pastShift === -1) return false;else return true;
                }
              }
            }
          }
  ...
  async isTxWithinSettlement(txHash, txOutputIndex, latest, onlyMaker = false) {
    try {
      ...
      if (!latestBlock) latestBlock = await this.get(`${BC_SUPER_COLLIDER}.block.latest`);
  ...
  async getInitialMakerOrder(outputScript, blockHeight = 0) {
    ...
    while (outputScript.includes('OP_CALLBACK')) {
      ...
      if (!_makerTx) {
        const id = `bc.txblock.${parentTxHash}`;
        const key = this._blockByTxCache.has(id) ? this._blockByTxCache.get(id) : await this.get(id);
        if (key) {
          // this._logger.info(`key is ${key}`)
          const [chain, _, hash, height] = key.split('.');
          ...
          if (block && block.getTxsList) {
            let txs = block.getTxsList().filter(tx => {
              return tx.getHash() === parentTxHash;
            });
            if (txs.length === 1) {
              await this.putTransaction(txs[0], block.getHash(), 0, 'bc');
              _makerTx = txs[0];
      ...
      if (!_makerTx) this._logger.info(`cannot find ${parentTxHash}`);
