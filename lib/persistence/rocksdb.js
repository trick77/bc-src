  ...
  async getBlocksByRange(from, to, blockchain = 'bc', opts = { asBuffer: true, asSet: false, cached: false, searchUp: false }) {
    ...
    if (!opts.asSet) {
      ...
      for (let i = 0; i < blocks.length; i++) {
        let found = false;
        for (let j = 0; j < blocks.length; j++) {
          if (blocks[i].getPreviousHash() == blocks[j].getHash()) found = true;
        }
      ...
      for (let i = 0; i < heights.length; i++) {
        ...
        let found = false;
        for (let j = 0; j < blocks.length; j++) {
          if (h === blocks[j].getHeight()) found = true;
        }
        if (!found) {
          let b = await this.getBlockByHeight(h, blockchain, opts);
          if (b) {
            blocks.push(b);
          } else {
            debugHeight(`cannot find ${h}`);
            for (let j = 0; j < blocks.length; j++) {
              if (blocks[j].getHeight() === h + 1) {
                let bl = await this.getBlockByHash(blocks[j].getPreviousHash(), blockchain, opts);
                if (bl) {
                  debugHeight(`found ${h} from prevHash`);
                  blocks.push(bl);
                } else {
                  debugHeight(`cannot find ${h} by prevHash`);
                }
              }
            }
          }
