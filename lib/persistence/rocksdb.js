  ...
  async isRoveredBlockPastShift(childChainHash, childChainHeight, childChainId, shiftAmount, latestBlock) {
    try {
      ...
      if (block.getHeight() >= Number(childChainHeight) + shiftAmount) {
        while (block.getHeight() >= Number(childChainHeight)) {
          if (block.getHash() === childChainHash) {
            this._logger.info(`found block ${childChainId} ${childChainHash}`);
            return 1; // block is all good
          }
          block = await this.getBlockByHeight(block.getPreviousHash(), childChainId);
        }
        this._logger.info(`block height ${childChainId} ${childChainHeight} ${childChainHash} is invalid`);
