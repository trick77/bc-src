...
class PersistenceRocksDb {
  ...
  */
  ...
  async isMarkedTxPastShift(txHash, txOutputIndex, isMaker, shiftAmount, latestBlock) {
    try {
      ...
      let block = await this.get(`bc.block.${bcHeight}`);

      // found the marked key, get the childchainHash
      let childChainHash = null;
      for (const childBlock of getChildBlocks(block, childChainId.toLowerCase())) {
        if (childBlock.getHeight() === Number(childChainHeight)) {
          for (const markedTx of childBlock.getMarkedTxsList()) {
            if (markedTx.getHash() === markedTxHash) {
              childChainHash = childBlock.getHash();
              break;
            }
          }
        }
        if (childChainHash != null) break;
      }

      let heightChildBlock = last(getChildBlocks(block, childChainId.toLowerCase()));
      if (heightChildBlock.getHeight() >= Number(childChainHeight) + shiftAmount) {
        let childBlock = await this.getBlockByHash(heightChildBlock.getPreviousHash(), childChainId);
        while (block.getHeight() >= Number(childChainHeight)) {
          if (block.getHash() === childChainHash) return true;
          childBlock = await this.getBlocKByHeight(childBlock.getPreviousHash(), childChainId);
        }
      }
      return false;
    } catch (err) {
