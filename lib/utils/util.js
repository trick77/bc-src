...
util.isNumber = function isNumber(value) {
  return typeof value === 'number' && isFinite(value) && value >= -Number.MAX_SAFE_INTEGER && value <= Number.MAX_SAFE_INTEGER;
};
...
util.log = function log(args) {
  if (!process.stdout) {
    let msg;
    if (args.length > 0) {
      msg = typeof args[0] !== 'object' ? util.fmt(args) : args[0];
    }
    console.log(msg);
    return;
  }
...
util.revHex = function revHex(data) {
  ...
  for (let i = 0; i < data.length; i += 2) out = data.slice(i, i + 2) + out;
...
util.reverse = function reverse(obj) {
  ...
  for (const key of Object.keys(obj)) reversed[obj[key]] = key;
