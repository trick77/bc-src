  ...
  async isValidTx(tx, opts) {
    ...
    if (beingSpent) {
      if (opts && opts.block && opts.block.getHeight) {
        ...
        const compareBlock = await this._persistence.getBlockByTxHash(tx.getHash());
        if (opts.block && compareBlock && compareBlock.getHash() === opts.block.getHash()) {
          this._logger.info(`comparing ${opts.block.getHeight()}:${opts.block.getHash().slice(0, 5)},${opts.block.getPreviousHash().slice(0, 5)} with ${compareBlock.getHeight()}:${compareBlock.getHash().slice(0, 5)}, prev: ${compareBlock.getPreviousHash().slice(0, 5)}`);
        } else {
          this._txPendingPool.markTxsAsMined([tx]);
          return false;
        }
