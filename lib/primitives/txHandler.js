    ...
    if (beingSpent) {
      if (opts && opts.block && opts.block.getHeight) {
        ...
        if (opts.block && compareBlock && compareBlock.getHash() === opts.block.getHash()) {
          this._logger.info(`comparing block ${opts.block.getHeight()}:${opts.block.getHash().slice(0, 5)},${opts.block.getPreviousHash().slice(0, 5)} with ${compareBlock.getHeight()}:${compareBlock.getHash().slice(0, 5)}, prev: ${compareBlock.getPreviousHash().slice(0, 5)}`);
        } else {
          // TD: since multiple
          this._txPendingPool.markTxsAsMined([tx]);
          return false;
