  ...
  async isValidTx(tx, opts) {
    ...
    for (let input of inputs) {
      ...
      if (referencedTx.getLockTime() > latestBlockHeight) {
        this._logger.info(`TX ${hash} invalid - referenced TX hash ${referencedTx.getHash()} is not mature enough, will be mature at: ${referencedTx.getLockTime()}, latest block height: ${latestBlockHeight}`);
        return false;
      }
