  ...
  async isValidTx(tx, opts) {
    ...
    if (beingSpent) {
      if (opts && opts.block && opts.block.getHeight) {
        const compareBlock = await this._persistence.get(`${BC_SUPER_COLLIDER}.block.${opts.block.getHash()}`);
        if (opts.block && compareBlock && compareBlock.getHash() === opts.block.getHash()) {
          debug(`comparing block ${opts.block.getHeight()}:${opts.block.getHash().slice(0, 5)},${opts.block.getPreviousHash().slice(0, 5)} with ${compareBlock.getHeight()}:${compareBlock.getHash().slice(0, 5)}, prev: ${compareBlock.getPreviousHash().slice(0, 5)}`);
        } else {
          // TD
          ...
          if (compareBlock && compareBlock.getHash) {
            // TD
            this._logger.info(`review block ${compareBlock.getHash()} ${compareBlock.getHeight()}`);
          }
          // TD: since multiple
          this._txPendingPool.markTxsAsMined([tx]);
          return false;
