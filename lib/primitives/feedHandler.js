...
class FeedHandler {
  ...
  async processTx(tx) {
    ...
    // //extract address
    const address = tx.getAddrFrom();
    debug(`message from address: ${address}`);
    // //extract message
    const script = tx.getOuputsList()[0].getOutputScript(); //todo Patrick extract message from the output;
    debug(`message script: ${script}`);
    ...
    debug(`message $OL balance: ${ol}, message $EMB balance: ${emb}`);
