...
const { ERC721_WATCHED_TOKENS } = require('../rover/eth/tokens');
const BC_TIME_CAPSULE_FEED = process.env.BC_TIME_CAPSULE_FEED === 'true';
...
const getRandomSetOfTokens = () => {

  const set = randomMinMax(0, 7);
  const names = ERC721_WATCHED_TOKENS.map(a => {
    return a.assetName;
  });

  shuffle(names);

  const nfts = [].concat(names.slice(0, set + 1));
  const balances = [];

  for (let i = 0; i < set; i++) {
    const id = randomMinMax(0, 10000);
...
const getRandomMessage = messages => {
  ...
  if (randomMessage.message && randomMessage.message.length < 5) {
...
class FeedHandler {
  ...
  constructor(persistence, txPendingPool, utxoManager, wallet, pubsub) {
    ...
    if (BC_TIME_CAPSULE_FEED) {
      const messages = TIME_CAPSULE_DATA.signatures;
      let posts = 0;
      setInterval(() => {
        posts++;
        if (posts > 3 && posts % 3 === 0) {

          const randomMessage = getRandomMessage(messages);

          const embBalance = posts % 10 === 0 ? randomMinMax(0, 100000) : 0;
          const olBalance = posts % 5 === 0 ? randomMinMax(0, 1000000) : 0;

          const emb = new _bc_pb.FeedBalance();
          emb.setName('emb');
          emb.setAmount(embBalance);

          const ol = new _bc_pb.FeedBalance();
          ol.setName('ol');
          ol.setAmount(olBalance);

          const randomNfts = getRandomSetOfTokens();
          const balances = [emb, ol].concat(randomNfts);

          const message = new _bc_pb.FeedMessage();
          message.setAddress(randomMessage.address);
          message.setMessage(randomMessage.message);
          message.setTimestamp(Date.now());
          message.setBalancesList(balances); // the ol, emb, and nfts

          this._pubsub.publish('feed.message', message);
        }
      }, 2000);
    }
  }
      ...
      for (const msg of possibleMessages) {
        ...
        if (parsedMessage) {
          ...
          balances = balances.concat(parsedMessage.nfts.map(n => {
            const f = new _bc_pb.FeedBalance();
            f.setName(n);
            f.setAmount(1); // TODO: this should be the id of the nft
            return f;
          }));
          const message = new _bc_pb.FeedMessage();
