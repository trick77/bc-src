...
const { GetMarkedTxs, GetOutPointRequest } = require('../../../protos/bc_pb'); /**
...
function getMarkedTxsForMatchedOrder(context, call, callback) {
  ...
  context.server.engine.persistence.getMarkedTxsForMatchedTx(hash, index).then(tx => {
    if (tx) {
      let response = new GetMarkedTxs();
      response.setTxsList(tx);
      callback(null, response);
    } else callback(new Error(`Tx ${hash} not found`));
  }).catch(err => {
