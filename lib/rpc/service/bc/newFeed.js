...
function newFeed(context, call, callback) {
  try {
    ...
    wallet.getSpendableOutpointsList(fromAddress).then(async walletData => {
      ...
      const tx = await createFeedTransaction(spendableOutpointsList, fromAddress, request.getPrivateKeyHex(), request.getDataType(), request.getDataLength(), request.getData(), request.getAmount(), request.getTxFee(), true);
      const response = new RpcTransactionResponse();
