...
const { RpcUpdateFeedTransactionRequest, RpcTransactionResponse, RpcTransactionResponseStatus } = require('@overline/proto/proto/bc_pb');
...
function updateFeed(context, call, callback) {
  try {
    ...
    // a field to check before parsing the data, in JS protobuf implementation, enum is just and int
    const dataType = request.getDataType().toString(10);

    // a field to check before parsing the data
    const dataLength = request.getDataLength().toString(10);
