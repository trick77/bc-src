...
const debug = require('debug')('bcnode:rpc:service:bc');
const { blake2bl } = require('../../utils/crypto');
const { getVersion } = require('../../helper/version');

const { Utxos } = require('@overline/proto/proto/core_pb');
const {
...
class BcServiceImpl {
  ...
  getEphemeralFeedMessages(call, callback) {
    getEphemeralFeedMessages(this._getContext(), call, callback);
  }
  getOriginalMakerOrder(call, callback) {
    getOriginalMakerOrder(this._getContext(), call, callback);
  }

  getBlocksByRoveredHash(call, callback) {
    getBlocksByRoveredHash(this._getContext(), call, callback);
  }

  getTakerForMaker(call, callback) {
    getTakerForMaker(this._getContext(), call, callback);
  }
  ...
  getFastSyncStatus(call, callback) {
    getFastSyncStatus(this._getContext(), call, callback);
  }
  ...
  getByteFeeMultiplier(call, callback) {
    getByteFeeMultiplier(this._getContext(), call, callback);
  }
   ...
   * Update Feed Tx
   */
  updateFeed(call, callback) {
    updateFeed(this._getContext(), call, callback);
  }
  ...
  /**
   \* Get  balance of EMB for address
   */
  getEmbBalance(call, callback) {
    getEmbBalance(this._getContext(), call, callback);
  }
  ...
  getUnlockTakerTxParams(call, callback) {
    ...
    this._server.engine._dex.getUnlockTakerTxParams(txHash, txOutputIndex).then(res => {
  ...
  getUtxos(call, callback) {
    ...
    const address = req.getAddress().toLowerCase();
    const from = req.getFrom();
    const to = req.getTo();

    this._server.engine.utxoManager.getUtxos(scriptType, { address, from, to }).then(utxos => {
      console.log(utxos.utxos);
      const grpcRes = new Utxos();
      grpcRes.setUtxoList(utxos.utxos);
  ...
  getOpenOrder(call, callback) {
    const req = call.request;
    debug(`request getOpenOrder`);
    const hash = req.getHash();
    const index = req.getIndex();

    this._server.engine._dex.getOpenOrder(hash, index).then(orders => {
  ...
  getMatchedOrder(call, callback) {
    const req = call.request;
    debug(`request getMatchedOrder`);
    const hash = req.getHash();
    const index = req.getIndex();

    this._server.engine._dex.getMatchedOrder(hash, index).then(orders => {
  ...
  getOpenCallbackOrder(call, callback) {
    const req = call.request;
    debug(`request getOpenCallbackOrder`);
    const hash = req.getHash();
    const index = req.getIndex();

    this._server.engine._dex.getOpenCallbackOrder(hash, index).then(orders => {
      const grpcRes = new GetOpenOrdersResponse();
      grpcRes.setOrdersList(orders);
      callback(null, grpcRes);
    }).catch(err => {
