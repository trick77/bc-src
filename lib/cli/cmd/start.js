...
const NO_ROVERS = process.env.NO_ROVERS === 'true';
...
const cmd = exports.cmd = async program => {
  ...
  if (rovers) {
    ...
    if (!NO_ROVERS) engine.startRovers(roversToStart, true).then(() => logger.info(`rovers successfully deployed to network [${roversToStart}]`));
  }
