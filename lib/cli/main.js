...
// Remove pre OT
//process.env.UV_THREADPOOL_SIZE = availCPUs
...
let BC_DEV_STATE = process.env.BC_DEV_STATE && process.env.BC_DEV_STATE !== 'false' && process.env.BC_DEV_STATE !== '0' ? process.env.BC_DEV_STATE : false;
if (BC_DEV_STATE) {
  if (BC_DEV_STATE === 'true' || BC_DEV_STATE === '1' || BC_DEV_STATE.indexOf('http') > 1) {
    BC_DEV_STATE = "https://bc-ephemeral.s3.amazonaws.com/_easysync_db.zip";
  }
}
  ...
  return new Promise((resolve, reject) => {
    figlet('BC', (e, data) => {
      ...
      rmrf(path, err => {
        ...
        console.log(`  WARNING: BLOCK COLLIDER DEVELOPER MODE (BC_DEV_MODE=true)`);
        console.log(``);
...
const main = exports.main = async (args = process.argv) => {
  ...
  if (BC_DEV_STATE) {
    await getDeveloperMultichain(BC_DEV_STATE, DATA_DIR);
  }
