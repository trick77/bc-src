...
const { validateSequenceDifficulty, validateCoinbase, updateUTXO } = require('../bc/validation');
...
class Engine {
  ...
  async init() {
    ...
    try {
      ...
      await this.persistence.put(`${BC_SUPER_COLLIDER}.sync.initialpeer`, null);
      await this.persistence.put(`${BC_SUPER_COLLIDER}.sync.initialpeer`, null);
      await this.persistence.put(`${blockchain}.block.reorgto`, null);
      await this.persistence.put(`${blockchain}.block.reorgfrom`, null);
      ...
      for (let key of this._knownRovers) {
        if (BC_FORCE_ROVERS && BC_FORCE_ROVERS.length > 0 && BC_FORCE_ROVERS.indexOf(key) < 0) {
          ...
          this._logger.warn(`accepting ${key} rover at local latest edge`);
        }
      }
  ...
  async startNode() {
    ...
    this._emitter.on('utxoUpdate', async data => {
      let callback = false;
      if (!data) {
        callback = true;
      ...
      if (callback || evalBlocks.length === 0) {
        ...
        this._emitter.emit(`utxoUpdate${date}${rand}`, res);
        evalBlocks.splice(indexBlocks.indexOf(`${date}${rand}`), 1);
        indexBlocks.splice(indexBlocks.indexOf(`${date}${rand}`), 1);
        this._emitter.emit(`utxoUpdate`, null);
      } else {
        debugUTXO(`PUSHING ${block.getHeight()}:${block.getHash()} onto stack ${evalBlocks.length}`);
        evalBlocks.unshift({ block, date, rand });
        indexBlocks.unshift(`${date}${rand}`);
      }
      return;
    });
  ...
  async blockFromPeer({ conn, newBlock, options }) {
    ...
    if (newBlock && !cache.get(newBlock.getHash()) && !heardFromPeer) {
      ...
      if (fullBlock) {
        ...
        let currentSyncedStatus = 'comlete';
        ...
        const { valid, stored, needsResync, rangeRequest, schedules, assertSyncComplete, blockSendingAlternative } = await this.multiverse.extendMultiverse(newBlock, 'peer', true, false, address);
    ...
    this.workerPool.allRise({ minerKey: this._minerKey, emblemPerformance: this._emblemPerformance }).then(() => {
      ...
      this._roverEmitter.on('collectBlock', async ({
        ...
        try {
          ...
          if (!roverLatestBlock) {
            await this.persistence.put(`${rover}.block.latest`, block);
