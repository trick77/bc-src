    ...
    this._utxoEventEmitter.on('utxoUpdate', async data => {
      setImmediate(async () => {
        ...
        if (!data) {
          ...
          indexBlocks.shift();
          this._logger.info(`POPPING ${data.block.getHeight()}:${data.block.getHash()} off stack ${evalBlocks.length}`);
        ...
        if (callback || evalBlocks.length === 0) {
          ...
          try {
            ...
            this._logger.info(`err processing ${block.getHeight()}:${block.getHash()}`);
  ...
  async blockFromPeer({ conn, newBlock, options }) {
    ...
    if (newBlock && !cache.get(newBlock.getHash()) && !heardFromPeer) {
      ...
      if (fullBlock) {
        ...
        //
        if (remount) {
          this._logger.warn(`remount active on waypoint ${remount}`);
