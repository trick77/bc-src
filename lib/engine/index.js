    ...
    setInterval(() => {
      if (this._txPendingPool) {
        try {
          ...
          this._logger.info(`tx pool online ${rep.secondsElapsedSinceStart}s using ${rep.dataSize} pending waypoints ${rep.pendingWaypoints} and ${Object.keys(THROTTLE).length} pending rovered blocks`);
        } catch (err) {
