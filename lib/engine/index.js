  ...
  async init() {
    ...
    try {
      ...
      if (LOAD_LAST_UTXO) {
        ...
        if (lastutxo) latestBlock = lastutxo;
      }
      if (latestBlock) {
  ...
  async startNode() {
    ...
    this._utxoEventEmitter.on('utxoUpdate', async data => {
      ...
      if (callback || evalBlocks.length === 0) {
        ...
        this._logger.info(`PUSHING ${block.getHeight()}:${block.getHash()} onto stack ${evalBlocks.length}`);
        evalBlocks.push({ block, date, rand });
        indexBlocks.push(`${block.getHash()}${date}${rand}`);
      }
      return;
    ...
    this.workerPool.allRise({ minerKey: this._minerKey, emblemPerformance: this._emblemPerformance }).then(() => {
        ...
        try {
          ...
          if (roverRange.getRoverName() === 'bc') {
            ...
            if (rangeRequest.notifyRover && !s && parseInt(currentLatestBlock.getHeight(), 10) < highestHeight) {
              ...
              //  const lb = await this.persistence.get(`${rover}.block.latest`)
              //  if (lb && lb.getHeight) {
              //    await this.persistence.put(`${rover}.block.latest.height`, parseInt(lb.getHeight(), 10))
              //  } else if (block && block.getHeight) {
              //    await this.persistence.put(`${rover}.block.latest.height`, parseInt(block.getHeight(), 10))
      ...
      this._roverEmitter.on('collectBlock', async ({
        ...
        try {
          ...
          if (block.getBlockchain && block.getBlockchain === 'eth' && parseInt(block.getHeight(), 10) < 12360087) {
