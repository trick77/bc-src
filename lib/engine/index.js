...
const BC_STOP_TRACKING = process.env.BC_STOP_TRACKING ? process.env.BC_STOP_TRACKING : false;
    ...
    try {
      ...
      if (!latestBlock || BC_BUILD_GENESIS) {
        ...
        try {
          ...
          this._logger.info('     SUPER COLLIDER: ' + BC_SUPER_COLLIDER);
          this._logger.info('     MINELOCK: ' + now);
          this._logger.info('     ENGINE CACHE: ' + config.engine.knownBlocksCache.max);
          this._logger.info('     STALE NETWORK: ' + BC_STALE_NETWORK_TIMEOUT);
        ...
        this._logger.info(`     NRG MULTICHAIN BALANCE: ${sumSoFar}`);
