...
class Engine {
  ...
  constructor(opts) {
    ...
    this._asyncEmitter = new QueueEventEmitter();
    ...
    this._miningEmitter.setMaxListeners(0);
    ...
    try {
      ...
      if (!latestBlock && !latestDataStr || BC_BUILD_GENESIS) {
        ...
        try {
          ...
          this._logger.info(``);
          this._logger.info(``);
          this._logger.info(`     BLOCK COLLIDER NODE ${npm} (${new Date().toString()})`);
  ...
  async startRovers(rovers, forceResync = false) {
    ...
    this.pubsub.subscribe('rover.request', '<engine>', msg => {
      ...
      if (this._rovers._roverSyncStatus[msg.data.rover] === true || msg.data.rover === 'eth') {
