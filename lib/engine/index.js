...
class Engine {
      ...
      if (fullBlock) {
        ...
        if (!stored && !needsResync && !rangeRequest) {
          ...
          if (!valid && !blockSendingAlternative) {
            this._logger.info(`sending better edge for block ${newBlock.getHeight()} from waypoint to q...`);
  ...
  async startRovers(rovers, forceResync = false) {
    ...
    if (!BC_MINER_BOOT) {
      ...
      if (now - 32 < this._startTime && syncComplete === 'complete' || now - 32 < this._startTime || syncComplete !== 'complete' && !p) {
