    ...
    this.workerPool.allRise({ minerKey: this._minerKey, emblemPerformance: this._emblemPerformance }).then(() => {
      ...
      this._roverEmitter.on('collectBlock', async ({
        ...
        try {
          ...
          if (block.getBlockchain && block.getBlockchain() === 'lsk' && parseInt(block.getHeight(), 10) > VALIDATION_GAP_BC_HEIGHT_INTERVAL[0] && parseInt(block.getHeight(), 10) < VALIDATION_GAP_BC_HEIGHT_INTERVAL[1]) {
            ...
            if (parseInt(ch, 10) > 4831565 && parseInt(ch, 10) < 4831965) {
