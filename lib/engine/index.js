...
class Engine {
      ...
      if (fullBlock) {
        ...
        if (!stored && !needsResync && !rangeRequest) {
          ...
          if (!valid && !blockSendingAlternative) {
            ...
            this.node._PEER_QUARANTINE.push(addressToHost(conn.remoteAddress));
          }
  ...
  async startRovers(rovers, forceResync = false) {
    ...
    if (!BC_MINER_BOOT) {
      ...
      if (now - 52 < this._startTime && syncComplete === 'complete' || now - 3000 < this._startTime || syncComplete !== 'complete' && !p) {
