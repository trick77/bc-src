...
class Engine {
  ...
  async evaluateForRoverRequests(block) {
    ...
    try {
      ...
      this._logger.info(`evaluating ${blockHeaders.length} rovered blocks from ${block.getHeight()} : ${block.getHash().slice(0, 6)}...`);
      ...
      for (const h of blockHeaders) {
        ...
        if (!rovered) {
          this._logger.info(`requesting ${blockchain} rover search ${h.getHeight()}`);
      ...
      if (mbs && mbs.length > 0) {
        this._logger.info(`requesting rover range from block ${block.getHeight()}:${block.getHash().slice(0, 6)}... ${mbs.join(', ')}`);
      ...
      if (fullBlock) {
        ...
        if (needsResync && rangeRequest && rangeRequest.highestHeight) {
          ...
          if (rangeRequest.highestHeight - rangeRequest.lowestHeight > 16) {
            this._emitter.emit('requestBlockRange', [min(rangeRequest.highestHeight, rangeRequest.lowestHeight + BC_MAX_DATA_RANGE), rangeRequest.lowestHeight]);
          } else {
  ...
  async startRovers(rovers, forceResync = false) {
    ...
    if (!BC_MINER_BOOT) {
      if (now - 42 < this._startTime || syncComplete !== 'complete') {
        setTimeout(async () => {
    ...
    this.workerPool.allRise({ minerKey: this._minerKey, emblemPerformance: this._emblemPerformance }).then(() => {
      ...
      this._roverEmitter.on('collectBlock', async ({
        ...
        try {
          ...
          if (roverLatestBlock && BC_MINER_BOOT && rover === 'lsk') {
          ...
          if (rover !== BC_SUPER_COLLIDER && !BC_LOW_POWER_MODE) {
            ...
            if (report && report.purgeBlocksTo) {
              ...
              this._logger.info(`new ${rover} rovered block ${block.getHeight()} ${BC_SUPER_COLLIDER} ...`);
            }
  ...
  async processTx(newTx, conn) {
    ...
    try {
      ...
      } else if (latestTimeDiff <= 1000 * 60 * 10) {
        const validTx = await this._txHandler.isValidTx(newTx, { block, latestBlockHeight: block.getHeight() });
        if (validTx) {
          const wasAdded = this._txPendingPool.tryAddingNewTx(newTx, block.getHeight() > 4130000);
          if (wasAdded) {
            this._asyncEmitter.emit('announceTx', { data: newTx, connection: conn });
