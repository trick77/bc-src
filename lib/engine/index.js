...
class Engine {
  ...
  async blockFromPeer({ conn, newBlock, options }) {
    ...
    if (newBlock && !cache.get(newBlock.getHash()) && !heardFromPeer) {
      ...
      if (fullBlock) {
        ...
        const { stored, needsResync, rangeRequest, schedules, assertSyncComplete, blockSendingAlternative } = await this.multiverse.extendMultiverse(newBlock, 'peer', false, options.handleAsNewPeer, address, replaceLatestBlock);
        let currentSyncedStatus = await this.persistence.get(`${BC_SUPER_COLLIDER}.sync.initialsync`);
        //let currentSyncedStatus = 'complete'
  ...
  async processTx(newTx, conn) {
    if (this._knownTxsCache.has(newTx.getHash())) {
      return;
    }
