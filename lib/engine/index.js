...
class Engine {
  ...
  constructor(opts) {
    ...
    this._emitter = new QueueEventEmitter({ concurrency: 2 });
  ...
  async startNode() {
    ...
    if (BC_WISE_LINE) {
      ...
      this._logger.warn(`BC_WISE_LINE roverblocks will be weighted, setting time: ${tms}`);
      await this.persistence.put(`${BC_SUPER_COLLIDER}.data.latest`, `1777783:${Date.now()}`);
      ...
      //await this._persistence.delBlocks(BC_SUPER_COLLIDER, 1771221, 1771236)
