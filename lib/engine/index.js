...
const { validateSequenceDifficulty, validateTxs, updateUTXO, VALIDATION_GAP_BC_HEIGHT_INTERVAL } = require('../bc/validation');
...
const BC_VALIDATE_CHAIN = process.env.BC_VALIDATE_CHAIN ? parseBoolean(process.env.BC_VALIDATE_CHAIN) : false;
const BC_INDEX_CHAIN = process.env.BC_INDEX_CHAIN ? parseBoolean(process.env.BC_INDEX_CHAIN) : false;
const BC_GET_UTXOS = process.env.BC_GET_UTXOS && process.env.BC_GET_UTXOS.length > 20 ? process.env.BC_GET_UTXOS.trim() : false;
...
const getLeaseBalances = async type => {
  return new Promise((resolve, reject) => {
    ...
    const urlc = `https://d2hxcwrpmn9jbk.cloudfront.net/EMB_db_land.json`;
    const urld = `https://d2hxcwrpmn9jbk.cloudfront.net/EMB_db_friend.json`;
    let url = urla;
    if (type === 'a') {
      url = urla;
    }
...
class Engine {
  ...
  constructor(opts) {
    ...
    this._blockQueue = queue((task, callback) => {
      let triggered = false;
      let blockId = false;
      if (task.getBlockchain) {
        blockId = `${task.getBlockchain()}:${task.getHash()}`;
      }
      ...
      const t = setTimeout(() => {
        triggered = true;
        // TD
        this._logger.warn(`block ${blockId} timeout error occured`);
        failed.push(blockId);
        callback(null, false);
      }, 29000);

      if (task && task.getBlockchain) {
        this.blockFromRover(task).then(b => {
          if (!triggered) {
            clearTimeout(t);
            debug(`block ${blockId} successfully processed`);
            callback(null, b);
          } else {
            debug(`block ${blockId} unsuccessfully processed after timeout`);
          }
        });
      } else {
        this.blockFromPeer(task).then(b => {
          if (!triggered) {
            clearTimeout(t);
            callback(null, b);
          }
    ...
    this._blockQueue.error(err => {
      this._logger.info(`block queue error`);
      console.trace(err);
    });
    ...
    setInterval(() => {
      if (this._txPendingPool) {
        try {
          ...
          this._logger.info(`tx pool online ${rep.secondsElapsedSinceStart}s using ${rep.dataSize} waypoints ${rep.pendingWaypoints} and ${Object.keys(THROTTLE).length} pending rovered blocks, block queue: ${queueLength}, gpu updated: ${this._latestGpuMinerUpdateCount} (${this._latestGpuMinerUpdate}) utxo listeners: ${this._utxoManager.utxoListeners}, (${this._utxoEventEmitter._eventsCount})`);
        } catch (err) {
    ...
    setInterval(async () => {
      ...
      let currentHeight = await this.persistence.get(`${BC_SUPER_COLLIDER}.block.latest.height`);
      const datalatestRaw = await this.persistence.get(`${BC_SUPER_COLLIDER}.data.latest`);
      const datalatest = datalatestRaw && datalatestRaw.indexOf(':') > -1 ? datalatestRaw.split(':')[0] : false;
      const ab = await this._persistence.get(`lease.db.built`);
      const cb = await this._persistence.get(`lease.db.update`);
      const land = await this._persistence.get(`lease.db.land`);
      const friend = await this._persistence.get(`lease.db.friend`);

      if (datalatest && currentHeight && currentHeight > datalatest) {
        currentHeight = datalatest;
      }
      // 4740006
      if (!ab && currentHeight && new _externals.BN(currentHeight).gt(new _externals.BN(4730006))) {
      ...
      // 6717275
      if (!cb && currentHeight && new _externals.BN(currentHeight).gt(new _externals.BN(6710606))) {
      ...
      if (!land && currentHeight && new _externals.BN(currentHeight).gt(new _externals.BN(7589066))) {
        const balances = await getLeaseBalances('c');
        this._logger.info(`index window loaded ${Object.keys(balances).length}`);
        await this._persistence.putLeaseDb(balances, 'land');
        return;
      }

      if (!friend && currentHeight && new _externals.BN(currentHeight).gt(new _externals.BN(7660066))) {
        const balances = await getLeaseBalances('d');
        this._logger.info(`index window loaded ${Object.keys(balances).length}`);
        await this._persistence.putLeaseDb(balances, 'friend');
        return;
      }
    }, 28000);
  ...
  async init(dbPath = DATA_DIR) {
    ...
    try {
      ...
      await this.persistence.del(`${BC_SUPER_COLLIDER}.indexlock`);

      if (BC_GET_UTXOS) {
        const latest = await this.persistence.get(`${BC_SUPER_COLLIDER}.block.latest`);
        const target = BC_GET_UTXOS;
        if (latest) {
          this._logger.info(`latest block found ${latest.getHeight()} : ${latest.getHash()}`);
          this._logger.info(`getting UTXOs for ${target}`);
          const utxos = await this._utxoManager.getUtxos('nrg_transfer', { address: target });
          if (utxos) {
            this._logger.info(`got ${utxos.length} UTXOs`);
            //const readableUtxos = utxos.utxos.map((utxo) => {
            //  return utxo.getTxHash() + ':' + utxo.getTxIndex()
            //})
            const readableUtxos = utxos.utxos.map(utxo => {
              return utxo.getTxHash();
            });
            console.log(JSON.stringify(readableUtxos, null, 2));
          } else {
            this._logger.info(`no UTXOs found for ${target}`);
          }
        }
        process.exit();
      }
      ...
      if (BC_INDEX_CHAIN) {
        ...
        if (latest) {
          this._logger.info(`latest block found ${latest.getHeight()} : ${latest.getHash()}`);
          this._logger.info(`starting build chain index...`);
          await this.persistence.del(`${BC_SUPER_COLLIDER}.indexlock`);
          //const blocksToReview = await this.persistence.rebuildChainIndexFromHeight(this._utxoManager, this._txHandler, 7763000, latest)
          const blocksToReview = await this.persistence.rebuildChainIndexFromHeight(this._utxoManager, this._txHandler, 279000, latest, true);
          this._logger.info(`chain index build complete <- shutting down...`);
          process.exit();
        }
      }

      if (BC_VALIDATE_CHAIN) {
        const latest = await this.persistence.get(`${BC_SUPER_COLLIDER}.block.latest`);
        if (latest) {
          this._logger.info(`latest block found ${latest.getHeight()} : ${latest.getHash()}`);
          this._logger.info(`running chain validation...`);
          const blocksToReview = await this.persistence.validateChain(this._txHandler, 1000000, latest);
          console.log(JSON.stringify(blocksToReview, null, 2));
          this._logger.info(`chain validation complete <- shutting down...`);
          process.exit();
        }
      }
      ...
      if (BC_REINDEX_TXS) {
        ...
        if (latest) {
          ...
          await this.persistence.saveTxs(7630000, latest);
      ...
      if (BC_REBUILD_TXS) {
        ...
        if (latest) {
          this._logger.info(`latest block found ${latest.getHeight()} : ${latest.getHash()}`);
          this._logger.info(`running large rebuild command...`);
          await this.persistence.del(`${BC_SUPER_COLLIDER}.utxorebuilt`);
          await this.persistence.rebuildTxGraph(this._utxoManager, this._txHandler, this._wallet, 7579000, latest);
          this._logger.info(`tx graph rebuilt <- shutting down...`);
          process.exit();
        }
      }
      // initialize the EVM
      ...
      await this._persistence.del(`${BC_SUPER_COLLIDER}.notified.rovered`);
      ...
      if (!latestBlock && !latestDataStr || BC_BUILD_GENESIS) {
        ...
        try {
          // reset event loop
          const indexrebuilt = await this.persistence.get(`${BC_SUPER_COLLIDER}.indexrebuilt`);
          ...
          this._logger.info('     IR STATUS: ' + indexrebuilt);
        ...
        const ab = await this.persistence.get(`lease.db.built`);
        const cb = await this.persistence.get(`lease.db.update`);
        const land = await this.persistence.get(`lease.db.land`);
        const friend = await this.persistence.get(`lease.db.friend`);
        const indexrebuilt = await this.persistence.get(`${BC_SUPER_COLLIDER}.indexrebuilt`);
        // reset event loop
        await this.persistence.put(`_`, `${1}`);
        await this.persistence.del(`${BC_SUPER_COLLIDER}.miner.radians`);
        ...
        this._logger.info('     ROVERS REPORTING: ' + Object.keys(this._knownRovers).length);
        this._logger.info('     NETWORK AGING RATE: ' + BC_NETWORK_AGING_RATE);
        this._logger.info(`     SIGMA: ${ab} UPDATE: ${cb}, LAND: ${land}, FRIEND: ${friend}`);
        this._logger.info('     IR STATUS: ' + indexrebuilt);
        this._logger.info(`     BLOCKCHAIN FINGERPRINT: ${config.blockchainFingerprintsHash}`);
        this._logger.info(``);
        this._logger.info(``);
    ...
    try {
      ...
      if (mbs && mbs.length > 0) {
        ...
        if (rmraw) {
          ...
          } else if (mbs.indexOf('eth') > -1 && diff > 25) {
            const rov = 'eth';
            const sr = missingBlocks[rov];
    ...
    if (newBlock && !heardFromPeer) {
      ...
      if (fullBlock) {
        ...
        if (!stored && !needsResync && !rangeRequest) {
          ...
          if (!valid && !blockSendingAlternative && !this._peerRequestCache.has(conn.remoteAddress + newBlock.getHash())) {
            this._logger.info(`sending stronger edge for block ${newBlock.getHeight()} from waypoint...`);
            this.node.addToQuarantine(addressToHost(conn.remoteAddress), "sent invalid block");
            this._emitter.emit(`sendblock`, { data: latestBlock, connection: conn });
            return;
          }
  ...
  async startRovers(rovers, forceResync = false) {
    ...
    const lock = await this.persistence.get(`${BC_SUPER_COLLIDER}.indexlock`);
    ...
    setInterval(async () => {
      ...
      const notified = await this._persistence.get(`${BC_SUPER_COLLIDER}.notified.rovered`);
      const synced = await this.persistence.get(`${BC_SUPER_COLLIDER}.sync.initialsync`);
      if (lbc && lbc.getHash && synced && synced === 'complete' && !notified) {
        // set the notified flag which resets on start up or if a rover restarts
        await this._persistence.put(`${BC_SUPER_COLLIDER}.notified.rovered`, new Date());
  ...
  async blockFromRover(block) {
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        debug(`1: rover ${rover}, block: ${blockHeight}, hash: ${blockHash} ${Date.now() - startProcessTime}`);
        ...
        if (!allRoversHaveSufficientBlocks) {
          debug(`2: rover ${rover}, block: ${blockHeight}, hash: ${blockHash} ${Date.now() - startProcessTime}`);
        ...
        debug(`3: rover ${rover}, block: ${blockHeight}, hash: ${blockHash} ${Date.now() - startProcessTime}`);
        ...
        if (rover !== BC_SUPER_COLLIDER && !BC_LOW_POWER_MODE) {
          ...
          const report = await this.persistence.putBlock(block, 0, rover, {
            rovered: multichainSyncStatus === 'complete' && block.getHeight() - roverLatestBlock.getHeight() < 500
          });
          debug(`5: rover ${rover}, block: ${blockHeight}, hash: ${blockHash} ${Date.now() - startProcessTime}`);
          ...
          if (report && report.purgeBlocksTo) {
            ...
            debug(`6: rover ${rover}, block: ${blockHeight}, hash: ${blockHash} ${Date.now() - startProcessTime}`);
            if (safeRoverBlock) {
              this._logger.info(`building ${rover} rover hash map to block: ${safeRoverBlock.getHeight()}`);
        ...
        if (roverLatestBlock.getHash() !== block.getHash()) {
          ...
          if (this._rovers._roverSyncStatus[rover] === true && allRoversHaveSufficientBlocks || this._rovers._roverSyncStatus[rover] === true && BC_FORCE_ROVERS && BC_MINER_BOOT) {

            debug(`7aa: rover ${rover}, block: ${blockHeight}, hash: ${blockHash} ${Date.now() - startProcessTime}`);
            const areRoversSynced = await this.persistence.isBlockchainSynced(rover, {
              givenLowestHeight: await this._chainState.getRangeLowestHeight(rover),
            ...
            debug(`7: rover ${rover}, block: ${blockHeight}, hash: ${blockHash} ${Date.now() - startProcessTime}`);
            let s = true;
            if (!areRoversSynced) {
              s = false;
            } else if (areRoversSynced.length > 0) {
            ...
            if (s || BC_FORCE_ROVERS && BC_MINER_BOOT) {
              ...
              if (allClaimSync === true && roversClaimToBeSynced === true) {
                ...
                debug(`8: rover ${rover}, block: ${blockHeight}, hash: ${blockHash} ${Date.now() - startProcessTime}`);
          ...
          // the block is not considered the next block, if the rover believes it is synced determine if a request must be opened
        } else if (this._rovers._roverSyncStatus[rover] === true && roverHasSufficientBlocks) {
          ...
          if (evaluateRange.length > 1) {
            try {
              ...
              if (!areRoversSynced) {
                ...
                debug(`adding synced range to evaluate complete range`);
  ...
  async reorgTxPendingPool() {
    let date = Date.now();
    debug('calling reorg txpendingpool');
  ...
  async processTx(newTx, conn, reorg, fromAddress) {
    ...
    // check if the tx is from a machine using the new RUST-EVM
  ...
  async _processMinedBlock(newBlock, solution, innerCalls = 0) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const validTxs = await validateTxs(this._txHandler, newBlock);

        if (!validTxs) {
          this._logger.info(`local mined block using stale transactions ${newBlock.getHeight()} : ${newBlock.getHash()}`);
          return resolve(true);
        }
