...
const { addressToHost } = require('../p2p/node');
...
class Engine {
  ...
  async init() {
    ...
    try {
      ...
      if (CLEAN_UP_UTXOS) await this.persistence.cleanUpUTXOs();
      ...
      for (let key of this._knownRovers) {
        if (BC_FORCE_ROVERS && BC_FORCE_ROVERS.length > 0 && BC_FORCE_ROVERS.indexOf(key) < 0) {
          ...
          this._logger.info(`accepting ${key} rover at local latest edge`);
        }
      }
  ...
  async blockFromPeer({ conn, newBlock, options }) {
    ...
    if (newBlock && !cache.get(newBlock.getHash()) && !heardFromPeer) {
      ...
      if (fullBlock) {
        ...
        const { stored, needsResync, rangeRequest, schedules, assertSyncComplete, remount, blockSendingAlternative } = await this.multiverse.extendMultiverse(newBlock, 'peer', true, options.handleAsNewPeer, address, replaceLatestBlock);
        ...
        if (stored) {
          ...
          this.miningOfficer._cleanUnfinishedBlock();
          if (!needsResync) {
            if (!options.alreadyBroadcasted && !blockSendingAlternative) {
    ...
    this.workerPool.allRise({ minerKey: this._minerKey, emblemPerformance: this._emblemPerformance }).then(() => {
        ...
        try {
          ...
          if (roverRange.getRoverName() === 'bc') {
            ...
            if (rangeRequest.notifyRover && !s && parseInt(currentLatestBlock.getHeight(), 10) < highestHeight) {
              ...
              const currentLatestBlockHeight = await this._chainState.getLatestBlockHeight(rover);
      ...
      this._roverEmitter.on('collectBlock', async ({
        ...
        try {
          ...
          ///const blockExtendsChain = await this.multiverse.blockExtendsChain(block)
          ...
          if (roverLatestBlock.getHash() === block.getHash() || hasMount || roverLatestBlock.getPreviousHash() === block.getPreviousHash()) {
