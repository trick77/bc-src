...
class Engine {
  ...
  constructor(opts) {
    ...
    if (BC_LOG_BLOCK_STATS) {
      ...
      blockStatStream.write(`timestamp,blockTimestamp,height,hash,distance,difficulty,totalDistance,miner,minerReward,txs,nrgMined,newBlocks,reportedBlockTimeVsReceived,mineUnique,logType\n`);
    ...
    this._blockQueue.drain(() => {
      debug(`engine ready to process more blocks`);
    });
    ...
    setInterval(() => {
      if (this._txPendingPool) {
        try {
          ...
          this._logger.info(`tx pool online ${rep.secondsElapsedSinceStart}s using ${rep.dataSize} pending waypoints ${rep.pendingWaypoints} and ${Object.keys(THROTTLE)} pending rovered blocks`);
        } catch (err) {
  ...
  async init() {
    ...
    try {
      ...
      if (latestBlock && highestKnownHeight && parseInt(latestBlock.getHeight(), 10) < parseInt(highestKnownHeight, 10)) {
      ...
      //const lskLatest = await this.persistence.get('lsk.block.latest')
      //const lsk = await this.persistence.getBlockByHash('88bffafbe47c6e71b751bdb6e898bdd3160bad8c3c366af2192bb5f56e352333', 'lsk')
      //if (lsk && !lskLatest) {
      //  this._logger.info(`lsk block start: ${lsk.getHash()}`)
      //  await this.persistence.put(`${BC_SUPER_COLLIDER}.block.latest`, lsk)
      //}
  ...
  blockFromPeerQueue(task) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (queueLength > 0) {
        ...
        if (!higher) {
          this._logger.info(`block ${task.newBlock.getHeight()} low in process queue`);
          //return resolve()
        }
  ...
  async blockFromPeer({ conn, newBlock, options }) {
    ...
    this._lastOlBlock = Math.floor(Date.now() / 1000);
        ...
        if (resyncTestNode || resyncFullNode) {
          if (latestBlock !== null) {
            ...
            if (stored) {
              ...
              // drain all of the remaining tasks in the queue
              //this._blockQueue.process()
  ...
  async startRovers(rovers, forceResync = false) {
    ...
    if (!BC_MINER_BOOT) {
      ...
      if (now - 52 < this._startTime && syncComplete === 'complete' || syncComplete !== 'complete' && !p) {
    ...
    this.pubsub.subscribe('rover.request', '<engine>', async msg => {
      ...
      if (this._lastOlBlock && now - this._lastOlBlock < 200 && multichainSyncStatus === 'complete') {
  ...
  async _processMinedBlock(newBlock, solution, innerCalls = 0) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        this._logger.info(`block returned from extend multiverse - ${newBlock.getHeight()}.${newBlock.getHash()}, resync: ${needsResync}`);
        ...
        if (needsResync && rangeRequest && rangeRequest.highestHeight && !blockSendingAlternative && !p) {
        ...
        } else if (p && newBlock.getMiner() !== this._minerKey) {
          await this._persistence.processPeerExpiration({ chainState: this._chainState });
        }
        ...
        if (stored && !needsResync) {
          ...
          if (newBlock.getMiner() === this._minerKey) {
            //await this._broadcastMinedBlock(newBlock, solution)
            this._logger.info('unable to mine on edge mined locally, yielding block -> ');
          } else {
        ...
        if (rangeRequest && rangeRequest.highestHeight) {
          ...
          this._logger.warn(`locally mined block ${newBlock.getHeight()} extends multiverse with invalid sequence iterating back...`);
          const newLb = await this.persistence.getBlockByHash(alreadyExists.getPreviousHash());

          if (newLb) {
            await this.persistence.delBlock(alreadyExists.getHash(), 0, BC_SUPER_COLLIDER);
            await this.persistence.del(`${BC_SUPER_COLLIDER}.block.latest`);
            await this.persistence.put(`${BC_SUPER_COLLIDER}.sync.edge`, newLb.getHeight());
            await this.persistence.put(`${BC_SUPER_COLLIDER}.block.latest`, newLb);
            await this.reorgTxPendingPool();
            await this.persistence.putLatestBlock(newLb, BC_SUPER_COLLIDER, { iterateUp: false });
            this._logger.info(`purposed new block edge set <- ${newLb.getHeight()} : ${newLb.getHash()}`);
          }
