...
class Engine {
  ...
  constructor(opts) {
    ...
    setInterval(() => {
      ...
      //      }
      //    } else {
      //      const s = ROVER_SECONDS_PER_BLOCK[rover] * 5

      //      if ((this._roverTimeTable[rover].updated + s) < now) {
      //        this._logger.info(`direct connect ${rover}`)
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (rangeRequest && rangeRequest.highestHeight) {
          ...
          //await this.miningOfficer.newRoveredBlock(roverList, false, this._blockCache, true, this._knownFullBlocksCache, this._emblemPerformance) resolve(true)
        }
      } catch (err) {
        console.trace(err);
