...
class Engine {
  ...
  async startNode() {
    ...
    if (BC_MINER_BOOT) {
      ...
      //const lb = await this.persistence.get(`${BC_SUPER_COLLIDER}.block.latest`)
      //const slb = await this.persistence.get(`bc.block.${parseInt(lb.getHeight(), 10) - 1}`)
      //await this._persistence.put(`${BC_SUPER_COLLIDER}.sync.edge`, parseInt(slb.getHeight(), 10))
      //await this.persistence.put(`${BC_SUPER_COLLIDER}.block.latest`, slb)
      //console.log(`hash ${slb.getHash()}`)
      //console.log(`difficulty ${slb.getDifficulty()}`)
      //console.log(`distance ${slb.getDistance()}`)
      //console.log(`height ${slb.getHeight()}`)
      ...
      this._roverEmitter.on('collectBlock', async ({
        ...
        try {
          ...
          if (rover !== BC_SUPER_COLLIDER && !BC_LOW_POWER_MODE) {
            const report = await this.persistence.putBlock(block, 0, rover, { rovered: !BC_MINER_MUTEX });
