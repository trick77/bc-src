...
const { txHash, txInputSignature, ScriptTemplates, calcTxFee, getMaxDistanceWithEmblems, BC_COINBASE_MATURITY, COINBASE_TX_ESTIMATE_SIZE } = require('../core/txUtils');
...
const rebasePool = () => {
  try {
    console.log(`rebase pool event fired on process: ${process.pid}`);
...
class Engine {
  ...
  constructor(opts) {
    if (BC_LOG_BLOCK_STATS) {
      ...
      blockStatStream.write(`timestamp,blockTimestamp,height,hash,distance,difficulty,totalDistance,miner,minerReward,txs,nrgMined\n`);
    ...
    this._peerRequestCache = LRUCache({
      max: 50
    });
  ...
  async blockFromPeer({ conn, newBlock, options }) {
    ...
    if (newBlock && !cache.get(newBlock.getHash()) && !heardFromPeer) {
      ...
      const latestBlock = await this.persistence.get(`${BC_SUPER_COLLIDER}.block.latest`);
      const syncStatus = await this.persistence.get(`${BC_SUPER_COLLIDER}.sync.initialsync`);
      const initialPeer = await this.persistence.get(`${BC_SUPER_COLLIDER}.sync.initialpeer`);

      //if (syncStatus && syncStatus === 'pending' && initialPeer)  {
      //  this._logger.info(`block will be stored yielding pending sync status ${syncStatus}`)
      //  await this.persistence.putBlock(newBlock, 0, BC_SUPER_COLLIDER, { updateHeight: false })
      //  return
      //}
      ...
      this._emitter.on('roverBlockRange', async roverRange => {
        ...
        try {
          ...
          if (roverRange.getRoverName() === 'bc') {
            ...
            this._logger.info(`rover ${roverRange.getRoverName()} close block range claim received ${roverRange.getHighestHeight()} ${roverRange.getRoverName()} - ${roverRange.getLowestHeight()} ${roverRange.getRoverName()}`);
            const rover = roverRange.getRoverName();
            const confirmations = ROVER_CONFIRMATIONS[rover] || 8;
            ...
            let missingBlocks = [];
            this._logger.info(`rover ${roverRange.getRoverName()} about to run close block missingBlocks BEFORE are rovers synced : ${missingBlocks.length}`);
            let s = true;
            if (!areRoversSynced) {
              s = false;
            ...
            // DEBUG
            this._logger.info(`rover ${roverRange.getRoverName()} about to run close block range claim with missingBlocks: ${missingBlocks.length}, confirmations: ${confirmations} s: ${s} printing are rovers synced`);
            const rangeRequest = this._chainState.closeBlockRangeRequest(rover, roverRange, missingBlocks);
            const { highestHeight, lowestHeight, highestHash, lowestHash } = rangeRequest;
      ...
      this._roverEmitter.on('collectBlock', async ({
        ...
        try {
          ...
          if (blockExtendsChain || roverLatestBlock.getHash() === block.getHash() || hasMount || roverLatestBlock.getPreviousHash() === block.getPreviousHash()) {
            ...
            this._logger.info(`rover found ${rover} block ${blockHeight} : ${blockHash}`);
  ...
  async _processMinedBlock(newBlock, solution, innerCalls = 0) {
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        const isValidCoinbase = await validateCoinbase(newBlock, this.persistence, this._txHandler, 'engine.processmined');

        if (!isValidCoinbase && innerCalls < 5) {
          process.nextTick(async () => {
            const valid = await this._processMinedBlock(newBlock, solution, 6);
            resolve(valid);
          });
        } else if (!isValidCoinbase) {
          this._logger.info(`mined solution stale txs from coinbase`);
          return Promise.resolve(false);
        }

        this.multiverse.extendMultiverse(newBlock, 'local', true).then(async ({ stored, needsResync, rangeRequest, schedules, assertSyncComplete, blockSendingAlternative }) => {
