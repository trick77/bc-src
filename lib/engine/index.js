  ...
  async blockFromPeer({ conn, newBlock, options }) {
    ...
    if (newBlock && !cache.get(newBlock.getHash()) && !heardFromPeer) {
      ...
      if (fullBlock) {
        ...
        if (!stored && !needsResync && !rangeRequest && !blockSendingAlternative && synced === 'complete') {
  ...
  async startRovers(rovers, forceResync = false) {
    ...
    this.workerPool.allRise({ minerKey: this._minerKey, emblemPerformance: this._emblemPerformance }).then(() => {
      ...
      this._p2pEmitter.on('announceMinedBlock', async ({ unfinishedBlock, solution }) => {
        this._logger.info(`block announced ${unfinishedBlock.getHeight()}.${unfinishedBlock.getHash()}`);
