    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (stored && !needsResync && !assertSyncComplete) {
          ...
          await this._broadcastMinedBlock(newBlock, solution);
        } else if (stored && !needsResync && assertSyncComplete) {
          this._logger.info(`local miner is waiting for new segment -> ${newBlock.getHeight()}`);
        } else {
          if (newBlock.getMiner() === this._minerKey) {
            ...
            if (evalBlock && evalBlock.getMiner() !== this._minerKey) {
              ...
              const lastUpdate = this._lastOlBlock ? Math.floor(Date.now / 1000) - this._lastOlBlock : 'NA';
              this._logger.info(`unable mount edge ${newBlock.getHeight()} <- possible network issues: last update ${lastUpdate}`);
            }
          } else {
        ...
        if (rangeRequest && rangeRequest.highestHeight && needsResync) {
          ...
          this._emitter.emit('requestBlockRange', [rangeRequest.highestHeight - 5, rangeRequest.lowestHeight - 10], 2);
