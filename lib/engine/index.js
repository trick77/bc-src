...
class Engine {
  ...
  constructor(opts) {
    ...
    setInterval(async () => {
      ...
      if (!cb && currentHeight && new BN(currentHeight).gt(new BN(6710606)) && new BN(currentHeight).lt(new BN(6711006))) {
  ...
  blockFromPeerQueue(task) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (queueLength > 0) {
        ...
        if (!higher) {
          this._logger.info(`block ${task.newBlock.getHeight()} not high enough to process`);
          return resolve();
        }
      ...
      this._roverEmitter.on('collectBlock', async ({
        ...
        try {
          ...
          if (!roverLatestBlock) {
            ...
            await this.persistence.put(`${block.getBlockchain()}.block.latest.height`, new BN(block.getHeight()).toNumber());
  ...
  async processFeedTx(tx) {
    ...
    this.pubsub.publish('feed.message', {
      type: 'feed.message',
      data: tx
    });
  }
  ...
  async processTx(newTx, conn, reorg) {
    ...
    if (isFeedTx) {
      await this._feedHandler.processTx(newTx);
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        if (needsResync && rangeRequest && rangeRequest.highestHeight && !blockSendingAlternative && !p) {
          ...
          this._emitter.emit('requestBlockRange', [rangeRequest.highestHeight - 5, rangeRequest.lowestHeight - 10]);
          if (!stored) {
            rebaseWorkers();
          }
        } else if (p) {
