...
const { max, merge, uniq, values, last } = require('ramda');
...
const { validateSequenceDifficulty, validateCoinbase } = require('../bc/validation');
...
const BC_PRUNE_MULTICHAIN = process.env.BC_PRUNE_MULTICHAIN === 'true'; // used during development to descrease size of Super Colliders
...
class Engine {
  ...
  constructor(opts) {
    ...
    this._roverEmitter = new QueueEventEmitter();
  ...
  async init() {
    ...
    try {
      ...
      if (this._chainState) {
        this._chainState.delSimplexCache();
      }

      // let blocks = await this.persistence.getBlocksByHeight(1771220);
      // console.log(`blocks length is now ${blocks.length}`)
      //
      // for(let i = 0; i < blocks.length; i++) {
      //   let block = blocks[i];
      //   if(block.getHash() != 'ed0e95c8175035d15088ae2679f09f82d7b50477ad13d71d417563a5472cd150'){
      //     // console.log(`deleting ${block.getHash()}.${block.getHeight()}`)
      //     await this.persistence.delBlock(block)
      //   }
      // }
      // blocks = await this.persistence.getBlocksByHeight(1771220);
      // console.log(`blocks length is now ${blocks.length}`)
      // if(blocks.length === 0){
      //   let block = await this.persistence.getBlockByHash('ed0e95c8175035d15088ae2679f09f82d7b50477ad13d71d417563a5472cd150');
      //   await this.persistence.putBlockHashAtHeight(block.getHash(),block.getHeight());
      //   // console.log({block})
      //
      //   blocks = await this.persistence.getBlocksByHeight(1771220);
      //   // console.log(blocks)
      ...
      // await this.persistence.removeUTXOsFrom(1765236)
      // let h1 = latestBlock;
      // let blocksToAdd = [h1];
      //
      // while (h1.getHeight() > 1765236) {
      //   let prevHash = h1.getPreviousHash();
      //   h1 = await this.persistence.getBlockByHash(prevHash, 'bc')
      //   blocksToAdd.unshift(h1)
      // }
      //
      // for(let i = 0; i < blocksToAdd.length;i++){
      //   console.log(`updating utxos for ${blocksToAdd[i].getHeight()}.${blocksToAdd[i].getHash()}`)
      //   await this.persistence.updateUTXOs(blocksToAdd[i],false)
      // }
      // await this.persistence.saveTxs()
      // await this.persistence.calculateNRGSupply()
      // if(latestBlock){
      //   let height = 1872220;
      //   let nextBlocks = await this.persistence.getBlocksByHeight(height,'bc');
      //   while(height > latestBlock.getHeight()){
      //     console.log("on " + height)
      //     if(nextBlocks){
      //       for(let i = 0; i < nextBlocks.length; i++){
      //         console.log("deleting " + height)
      //         await this.persistence.delBlock(nextBlocks[i].getHash(),'bc')
      //         await this.persistence.del(`bc.block.${nextBlocks[i].getHeight()}`)
      //         await this.persistence.del(`bc.height.${nextBlocks[i].getHeight()}`)
      //       }
      //     }
      //     height = height - 1;
      //     nextBlocks = await this.persistence.getBlocksByHeight(height,'bc');
      //   }
      // }
      ...
      if (!latestBlock && !latestDataStr || BC_BUILD_GENESIS) {
        ...
        try {
          ...
          this._logger.info(``);
          this._logger.info(``);
          this._logger.info(`     BC PROTOCOL ${npm} (${new Date().toString()})`);
        ...
        this._logger.info(`     NRG MINED: ${sumSoFar}`);
        this._logger.info(`     SUPER COLLIDER CTX: ${BC_SUPER_COLLIDER}`);
        this._logger.info('     DATA: ' + latestDataStr);
        this._logger.info(`     MINELOCK: ${now}`);
        this._logger.info(`     ENGINE CACHE: ${config.engine.knownBlocksCache.max}`);
  ...
  async startNode() {
    ...
    if (BC_MINER_BOOT || BC_MINER_POOL) {

      this._logger.warn(`BCMB & POOL is enabled and heights are being fixed`);
      const dm = await this.persistence.getBlockByHash('7d1bad87a200d4899a3a394bce5b1e8b896c3181d30588b63fef5e7b31115bcb');
      if (dm) {
        await this._persistence.put(`${BC_SUPER_COLLIDER}.sync.edge`, 1893447);
        await this.persistence.put(`bc.block.latest`, dm);
      }

      await this._persistence.del(`${BC_SUPER_COLLIDER}.block.reorgto`);
      await this._persistence.del(`${BC_SUPER_COLLIDER}.block.reorgfrom`);
      await this._persistence.put(`${BC_SUPER_COLLIDER}.sync.initialsync`, 'complete');

      //  const a = await this.persistence.getBlockByHeight(1771226)
      //  const b = await this.persistence.getBlockByHeight(1771225)
      //  const c = await this.persistence.getBlockByHeight(1771224)
      //  const d = await this.persistence.getBlockByHeight(1771223)
      //  const e = await this.persistence.getBlockByHeight(1771222)
      //  const f = await this.persistence.getBlockByHeight(1771221)
      //  let newLatest = await this.persistence.getBlockByHeight(1771220)

      //  if (a && b && c && d && e && f) {
      //     this._logger.warn(`BCMB: removing a`)
      //     await this.persistence.delBlock(a.getHash())
      //     this._logger.warn(`BCMB: removing b`)
      //     await this.persistence.delBlock(b.getHash())
      //     this._logger.warn(`BCMB: removing c`)
      //     await this.persistence.delBlock(c.getHash())
      //     this._logger.warn(`BCMB: removing d`)
      //     await this.persistence.delBlock(d.getHash())
      //     this._logger.warn(`BCMB: removing e`)
      //     await this.persistence.delBlock(e.getHash())
      //     this._logger.warn(`BCMB: removing f`)
      //     await this.persistence.delBlock(f.getHash())
      //     this._logger.warn(`BCMB: setting new latest ${newLatest.getHash()}`)

      //     await this.persistence.putLatestBlock(newLatest, 'bc', { iterateUp: false })
      //     await this.persistence.put(`bc.block.latest`, newLatest)
      this._logger.info(`set init sync to complete`);
      //  }
    }
  ...
  async blockFromPeer({ conn, newBlock, options }) {
    ...
    if (newBlock && !cache.get(newBlock.getHash()) && !heardFromPeer) {
      ...
      if (fullBlock) {
        ...
        if (BC_MINER_BOOT && parseInt(newBlock.getHeight(), 10) > parseInt(latestBlock.getHeight(), 10) + 1) {
        ...
        // prevents the local node from relaying latest blocks
  ...
  async startRovers(rovers, forceResync = false) {
    ...
    this.workerPool.allRise({ minerKey: this._minerKey, emblemPerformance: this._emblemPerformance }).then(() => {
      ...
      this._emitter.on('announceMinedBlock', async ({ unfinishedBlock, solution }) => {
        ...
        this._logger.info(`block announced ${unfinishedBlock.getHeight()}}`);
        if (BC_MINER_MUTEX) {
      ...
      this._emitter.on('requestRoverRange', async data => {
        ...
        if (BC_MINER_MUTEX) {
          //const ts = Math.floor(now * 0.001)
          //const rmraw = await this._persistence.get(`${BC_SUPER_COLLIDER}.rover.mutex`)
          //if (rmraw) {
          //  const rm = parseInt(rmraw, 10)
          //  const diff = ts - rm
          //  this._logger.info(`evaluating last rover request ${diff} seconds ago`)
          //  if (diff >= 120) {
          //    this._logger.info(`last rover request approved`)
          //    await this._persistence.put(`${BC_SUPER_COLLIDER}.rover.mutex`, ts)
          //  } else {
          //    this._logger.info(`last rover request delayed for ${120 - diff} seconds...`)
          //  }
          //} else {
          //  await this._persistence.put(`${BC_SUPER_COLLIDER}.rover.mutex`, ts)
          //}
      ...
      this._roverEmitter.on('collectBlock', async ({
        ...
        try {
          ...
          if (BC_MINER_MUTEX) {
            ...
            if (bcmm !== 'open') {
              this._logger.info(`miner is mining, storing ${block.getBlockchain()} block ${block.getHeight()} directly`);
              await this.persistence.putBlock(block, 0, block.getBlockchain());
              return;
            }
          }
          ...
          let roverHasSufficientBlocks = this._miningOfficer._collectedBlocks[rover] && new BN(this._miningOfficer._collectedBlocks[rover]).gte(new BN(requiredBlockCount)) && this._miningOfficer._collectedBlocks[rover] !== 0;
          ...
          if (!allRoversHaveSufficientBlocks) {
            this._miningOfficer.printState(this._emblemPerformance);
          }

          if (!allRoversHaveSufficientBlocks) {
            allRoversHaveSufficientBlocks = this._miningOfficer.getState(this._emblemPerformance);
  ...
  async processTx(newTx, conn) {
    ...
    try {
      ...
      const synced = await this.persistence.get(`${BC_SUPER_COLLIDER}.sync.initialsync`);
      const block = await this.persistence.get(`${BC_SUPER_COLLIDER}.block.latest`);
      let latestBlockTime = parseInt(block.getTimestamp()) * 1000;
      let latestTime = Date.now();
      let latestTimeDiff = latestTime - latestBlockTime;
      // console.log({latestTime,latestBlockTime,latestTimeDiff,synced})
      if (synced && (synced !== 'complete' || latestTimeDiff > 1000 * 60 * 10)) {
        this._asyncEmitter.emit('announceTx', {
          data: newTx,
          connection: conn
        });
        return { status: RpcTransactionResponseStatus.SUCCESS, txHash: newTx.getHash() };
      } else if (synced && (synced === 'complete' || latestTimeDiff <= 1000 * 60 * 10)) {
        const block = await this.persistence.get(`${BC_SUPER_COLLIDER}.block.latest`);
        const validTx = await this._txHandler.isValidTx(newTx, { block });
        if (validTx) {
