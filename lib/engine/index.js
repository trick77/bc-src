...
const BC_MINER_BOOT = process.env.BC_MINER_BOOT === 'true';
...
class Engine {
  ...
  constructor(opts) {
    ...
    this._roverEmitter = this._emitter;
    this._asyncEmitter = this._emitter;
    this._p2pEmitter = this._emitter;
    this._rovers = new RoverManager(this._persistence, this._p2pEmitter, this._roverEmitter);
    ...
    //this._p2pEmitter.setMaxListeners(0)
  ...
  async init() {
    ...
    this._miningOfficer = new MiningOfficer(this._pubsub, this._persistence, this._workerPool, this._txPendingPool, this._chainState, this._asyncEmitter, miningOfficerOpts);
  ...
  async startNode() {
    ...
    const now = Math.floor(Date.now() * 0.001);
    if (BC_MINER_MUTEX) {
      this._logger.warn(`BC_MINER_MUTEX is enabled rovered blocks will remain unique`);
      await this._persistence.put(`${BC_SUPER_COLLIDER}.miner.mutex`, 'open');
      //setInterval(async () => {
      //   this._logger.warn(`BC_MINER_MUTEX is enabled rovered blocks will remain unique <- setting to open`)
      //   await this._persistence.put(`${BC_SUPER_COLLIDER}.miner.mutex`, 'open')
      //}, 192000)
    }
      ...
      if (fullBlock) {
        ...
        if (!stored && !needsResync && !rangeRequest && !blockSendingAlternative) {
          ...
          if (!this._peerRequestCache.has(conn.remoteAddress + newBlock.getHash())) {
            ...
            this._p2pEmitter.emit(`sendblock`, { data: latestBlock, connection: conn });
          } else {
        ...
        if (needsResync && rangeRequest && rangeRequest.highestHeight) {
          ...
          this._p2pEmitter.emit('requestBlockRange', [rangeRequest.highestHeight, rangeRequest.lowestHeight, conn]);
        ...
        if (needsResync && rangeRequest && rangeRequest.highestHeight) {
          this._p2pEmitter.emit('requestBlockRange', [rangeRequest.highestHeight, rangeRequest.lowestHeight]);
        }
  ...
  async startRovers(rovers, forceResync = false) {
    ...
    this.workerPool.allRise({ minerKey: this._minerKey, emblemPerformance: this._emblemPerformance }).then(() => {
      ...
      this._asyncEmitter.on('announceMinedBlock', async ({ unfinishedBlock, solution }) => {
        if (BC_MINER_MUTEX) {
          return;
        }
