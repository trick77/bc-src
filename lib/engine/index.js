...
class Engine {
    ...
    setInterval(() => {
      if (this._txPendingPool) {
        try {
          ...
          this._logger.info(`tx pool online ${rep.secondsElapsedSinceStart}s using ${repo.dataSize} pending waypoints ${rep.pendingWaypoints} and ${Object.keys(THROTTLE)} pending rovered blocks`);
        } catch (err) {
    ...
    try {
      ...
      if (!latestBlock && !latestDataStr || BC_BUILD_GENESIS) {
        ...
        try {
          ...
          this._logger.info(``);
          this._logger.info(``);
          await updateUTXO(newGenesisBlock, this.persistence, this._txHandler, this._txPendingPool, 'engine');
        } catch (e) {
          console.trace(e);
  ...
  async startNode() {
    ...
    //       try {
    //         setImmediate(async () => {
    //           let res = await updateUTXO(block, this.persistence, this._txHandler, this._txPendingPool, 'engine')
  ...
  blockFromPeerQueue(task) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (higher) {
        ...
        this._blockQueue.push(task, () => {
          return resolve();
        });
      } else {
        // put the block at the start of the queue
        this._blockQueue.unshift(task, () => {
          return resolve();
        });
      }
    });
    ...
    if (newBlock && !cache.get(newBlock.getHash()) && !heardFromPeer) {
      ...
      if (fullBlock) {
        ...
        if (stored) {
          ...
          if (!options.alreadyBroadcasted && !blockSendingAlternative && latestBlock.getHeight() !== newBlock.getHeight() && valid && !needsResync) {
        ...
        if (needsResync && rangeRequest && rangeRequest.highestHeight) {
          ...
          this._logger.info(`request high: ${rangeRequest.highestHeight} low: ${rangeRequest.lowestHeight} <- ${newBlock.getHeight()}:${newBlock.getHash()}`);
          if (rangeRequest.highestHeight - rangeRequest.lowestHeight > 8) {
            this._emitter.emit('requestBlockRange', [min(rangeRequest.highestHeight, rangeRequest.lowestHeight + BC_MAX_DATA_RANGE), rangeRequest.lowestHeight], 3);
          } else {
      ...
      if (newBlock && !cache.get(newBlock.getHash()) && !heardFromPeer) {
        ...
        if (fullBlock) {
          ...
          if (stored) {
            ...
            if (!options.alreadyBroadcasted && parseInt(latestBlock.getHeight(), 10) - parseInt(newBlock.getHeight(), 10) < 20) {
              this.node.broadcastNewBlock(newBlock);
