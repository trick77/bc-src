  ...
  async startNode() {
    ...
    this._emitter.on('utxoUpdate', async data => {
      ...
      if (callback || evalBlocks.length === 0) {
        ...
        debugUTXO(`PUSHING ${block.getHeight()}:${block.getHash()} onto stack`);
      ...
      this._roverEmitter.on('collectBlock', async ({
        ...
        try {
          ...
          if (rover !== BC_SUPER_COLLIDER && !BC_LOW_POWER_MODE) {
            ...
            //this._logger.info(`${rover} newest edge: ${block.getHeight()}, latest edge: ${roverLatestBlock.getHeight()}, hash: ${roverLatestBlock.getHash().slice(0, 12)}...`)
            debug(`${rover} newest edge: ${block.getHeight()}, latest edge: ${roverLatestBlock.getHeight()}, hash: ${roverLatestBlock.getHash().slice(0, 12)}...`);
