...
const debugInvalidMarkedTxs = require('debug')('bcnode:txutils:invalidmarkedtx');
// const { randomBytes } = require('crypto')
const { intToBuffer, bufferToInt } = require('../utils/buffer');
const secp256k1 = require('secp256k1');
const BN = require('bn.js');
const { validChains } = require('../config/networks');
    ...
    try {
      ...
      if (isNRGTrade) {
        ...
        if (!validChains.includes(sendsFromChain)) {
          ...
          // disabled to support counter party assets
          //return false
        }
      }
...
// return 64B long signature with recovery number as the last byte
const signData = exports.signData = (data, privateKey) => {
  data = toBuffer(data);
  const dataHash = blake2bl(data);
  const sig = secp256k1.ecdsaSign(new Uint8Array(Buffer.from(dataHash, 'hex')), new Uint8Array(privateKey));

  if (sig.signature.length !== 64) {
    throw Error(`Signature should always be 64B long, l: ${sig.signature.length}`);
  }

  const signatureWithRecovery = Buffer.concat([Buffer.from(sig.signature), intToBuffer(sig.recid)]);
 ...
 *
 * We don't add slice(1) to the returned public key contains 65B - first byte is a parity by
 *
 ...
 * @returns {Buffer} 65B public key
 */
const pubKeyRecover = exports.pubKeyRecover = (signedData, rawSignature) => {
  signedData = toBuffer(signedData, 'hex');
  const dataHash = Buffer.from(blake2bl(signedData), 'hex');

  const pubKey = secp256k1.ecdsaRecover(new Uint8Array(rawSignature.slice(0, 64)), bufferToInt(rawSignature.slice(64)), new Uint8Array(dataHash));
  return Buffer.from(secp256k1.publicKeyConvert(pubKey, false)); // return uncompressed pubKey
};
...
const getMarkedTransactionsMerkle = exports.getMarkedTransactionsMerkle = block => {
  ...
  const markedTxs = getAllMarkedTxs(block);
  const txs = markedTxs.sort((a, b) => {
    if (a.index !== "" && b.index !== "") {
      if (a.index > b.index) {
        return 1;
  ...
  const hashes = txs.map(tx => {
    return tx.chain + tx.tokenType + tx.from + tx.to + tx.hash + tx.index + tx.height + tx.amount;
  });
