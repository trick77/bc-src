...
const getNrgGrant = exports.getNrgGrant = (potentialEmblemNrg, distanceWithEmblems, txsConsumedDistance, blockHeight) => {
  ...
  debugEmb(`getOlGrant(): inverted: ${inverted}, potential Emblem NRG: ${potentialEmblemNrgBN.toNumber()}, distance with Emblems: ${distanceWithEmblemsBN.toNumber()}, txs consumed distance: ${txsConsumedDistance.toNumber()} `);

  if (blockHeight && new BN(blockHeight).lte(new BN(3390000))) {
    inverted = false;
  }
  ...
  let finalNrgGrant = 0;
  if (blockHeight && new BN(blockHeight).gte(new BN(3390000))) {
