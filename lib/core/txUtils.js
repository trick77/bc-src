...
const { Transaction, TransactionOutput, OutPoint, TransactionInput } = require('../protos/core_pb');
...
//export const MAX_TXDIST_BLOCK = 1092770084109140
//export const HALF_MAX_TXDIST_BLOCK = 546385042054570
const BASE_TX_DISTANCE_BLOCK = exports.BASE_TX_DISTANCE_BLOCK = 21809378917576;
// export const BASE_BLOCK_SIZE = 99600 // (120 * 5) * 166
// export const BASE_BLOCK_SIZE = 67200
...
class ScriptTemplates {
  ...
  static validateOutput(output) {
    ...
    switch (getScriptType(Buffer.from(output.getOutputScript()))) {
      case ScriptType.MAKER_OUTPUT:
        return ScriptTemplates.validateMakerOutput(output);
  ...
  static validateNRGLockScript(script) {
    ...
    let [opBlake, bcAddress, opEqV, opChecksigV] = parts;
  ...
  static validateMakerOutputScript(script) {
    ...
    try {
      ...
      const validChains = ['btc', 'eth', 'lsk', 'wav', 'neo', 'dai', 'emb', 'usdt', 'xaut'];
  ...
  static validateMakerOutput(output) {
    ...
    if (isNRGTrade) {
      ...
      const receivesUnitAsMinimumBN = Currency.toMinimumUnitAsBN(receivesToChain, receivesUnit, receivesToChain);
      ...
      const sendsUnitAsMinimumBN = Currency.toMinimumUnitAsBN(sendsFromChain, sendsUnit, sendsFromChain);
...
const calcTxFee = exports.calcTxFee = tx => {
...
const getTxOwnerHashed = exports.getTxOwnerHashed = tx => {
  ...
  for (let input of tx.getInputsList()) {
    let inputScript = toASM(Buffer.from(input.getInputScript()), 0x01);
...
/* Calculates the additional Distance available to the block based on the Emblem Balance */
const getMaxDistanceWithEmblems = exports.getMaxDistanceWithEmblems = async (address, persistence) => {
...
const getNrgGrant = exports.getNrgGrant = (potentialEmblemNrg, distanceWithEmblems, txsConsumedDistance, blockHeight) => {
  ...
  // console.log(`txUtls.getNrgGrant(): potential Emblem NRG: ${potentialEmblemNrgBN.toNumber()}, distance with Emblems: ${distanceWithEmblemsBN.toNumber()}, txs consumed distance: ${txsConsumedDistance.toNumber()} `)
  ...
  if (distanceWithEmblemsBN.lt(txsConsumedDistanceBN)) {
    ...
    //console.log(`txs consumed above distance with Emblems <- using inverse weight`)
  }
  ...
  const minimumEmblemGrant = Math.round(potentialEmblemNrg / 3); // 33% with 66% empty block penalty
  let finalNrgGrant = 0;
  //console.log(`txUtls.getNrgGrant():  minimum Emblem grant ${minimumEmblemGrant}`)
  if (distanceWithEmblemsBN.gt(new BN(0)) && txsConsumedDistanceBN.gt(new BN(0))) {
    const divisor = distanceWithEmblemsBN.div(txsConsumedDistanceBN);
    finalNrgGrant = potentialEmblemNrgBN.div(divisor).toNumber();
    //console.log(`txUtls.getNrgGrant():  divisor ${divisor.toNumber()}, final nrg grant ${finalNrgGrant}`)
  }
