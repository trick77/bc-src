...
async function createUnifiedBlock(isStandalone, block, roverRpc, transform, oldBlock = false) {
  ...
  try {
    ...
    if (!unifiedBlock.toObject) {
      getLogger(__filename).debug(`block is malformed, unified block cannot be created`);
    ...
    // getLogger(__filename).debug(`unified block built from ${obj.blockchain}: ${hash}, height: ${obj.height}, markedTxsListLength: ${obj.markedTxsList.length}`)
    return unifiedBlock;
  } catch (e) {
    getLogger(__filename).warn(`Error while transforming block ${e.stack}`);
  ...
  return new Promise(function (resolve, reject) {
    roverRpc.isBeforeSettleHeight(req, function (err, response) {
      if (err) {
        ...
        getLogger(__filename).error('isBeforeSettleHeight-response-err:', err.toString(), JSON.stringify(req.toObject()));
        resolve(false);
      } else {
