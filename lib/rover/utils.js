...
const ROVER_RESYNC_PERIOD = exports.ROVER_RESYNC_PERIOD = Math.floor(1 * 60 * 60);
...
function getBacksyncEpoch(rover) {
  ...
  const initialWeight = Number(ROVER_SECONDS_PER_BLOCK[rover.toLowerCase()]) * 2 - shift;
  return Math.floor(Date.now() * 0.001) - ROVER_RESYNC_PERIOD - initialWeight;
}
