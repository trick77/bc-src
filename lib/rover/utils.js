...
const { getLogger, _pathToLogPrefix } = require('../logger');

let BC_BACKSYNC_PERIOD = process.env.BC__BACKSYNC_PERIOD ? Number.parseFloat(process.env.BC__BACKSYNC_PERIOD) : 0.20;
if (Number.isNaN(BC_BACKSYNC_PERIOD)) {
  BC_BACKSYNC_PERIOD = 0.20;
}
/**
...
const ROVER_SECONDS_PER_BLOCK = exports.ROVER_SECONDS_PER_BLOCK = {
  ...
  'lsk': 10.0,
  'neo': 15.0,
  'wav': 40.0 // measured on blocks 1352650 - 1352150
...
const ROVER_RESYNC_PERIOD = exports.ROVER_RESYNC_PERIOD = Math.floor(BC_BACKSYNC_PERIOD * 60 * 60);
...
class StandaloneDummyStream extends Writable {
  ...
  constructor(loggerName) {
    super();
    let name = `${_pathToLogPrefix(loggerName)}.dummyBlockStream`;
    this._logger = getLogger(name, false);
  }
