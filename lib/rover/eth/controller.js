    ...
    try {
      ...
      if (txStatusPass.indexOf(mtxId) > -1) {
        ...
        try {
          ...
          logger.info(`${mtxId} failed because of ${err}`);
        }
      }
...
async function _createUnifiedBlock(roverRpc, block, isStandalone, oldBlock = false) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      while (block && block.transactions && block.transactions.length > 0 && obj.blockNumber > 12390628) {
        //for (const tx of block.transactions) {
        const tx = block.transactions.shift();
        try {
          const serializedTx = tx.toJSON(true);
          // debugTx(`eth tx - ${serializedTx}`);
          const txId = ethUtils.bufferToHex(tx.hash(true));
          let addrFrom = ethUtils.bufferToHex(tx.from);
          debugTx(`from - ${addrFrom}, ${tx.from}`);
          ...
          for (let _ref of ERC20_WATCHED_TOKENS) {
            ...
            if ((decodedInput.method === 'transfer' || decodedInput.method === 'multiTransfer') && ethUtils.bufferToHex(tx.to).toLowerCase() === contractAddress) {
          ...
          if (isEmbTx) {
            ...
            const embRev = await getTxStatus(mTx.getHash(), obj.blockNumber, t === 'eth');
            if (!embRev) {
              emblemTransactions.push(mTx);
            }
            logger.info(`${tokenType} <- ${mTx.getHash()} returned: ${embRev} amount: ${amount}`);
          }
      ...
      if (!oldBlock && obj.blockNumber > 12390428) {
        for (let mt of markedTransactions) {
          try {
            ...
            logger.info(`${token} <- ${mtxId} returned: ${reverted}`);
            if (!reverted) {
      ...
      logger.info(`${totalBlockTxs} / ${finalMarkedTransactions.length} / ${emblemTransactions.length} transactions <- ${obj.blockNumber} - ${obj.blockHash}`);
