    ...
    try {
      ...
      while (block && block.transactions && block.transactions.length > 0) {
        ...
        try {
          ...
          console.trace(e);
          logging.getLogger('rover.eth.controller.createUnifiedBlock', false)(`unable to parse TX's ${ethUtils.bufferToHex(tx.hash(true))}`);
          debug(e);
        }
      }
      ...
      if (!oldBlock) {
        for (let mt of markedTransactions) {
          try {
            ...
            logging.getLogger('rover.eth.controller.createUnifiedBlock', false)(`critical error establishing id ${mt.getHash()}`);
            logging.getLogger('rover.eth.controller.createUnifiedBlock', false)(e);
            process.exit();
          }
        }
      } else {
      ...
      if (markedTransactions.length !== finalMarkedTransactions.length) {
        logging.getLogger('rover.eth.controller.createUnifiedBlock', false)(`marked txs: ${finalMarkedTransactions.length}, failed marked txs: ${failedMarkedIds.length}`);
        logging.getLogger('rover.eth.controller.createUnifiedBlock', false)(JSON.stringify(failedMarkedIds.join('\n')));
      }
