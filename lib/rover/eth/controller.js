      ...
      while (block && block.transactions && block.transactions.length > 0) {
        ...
        try {
          ...
          if (isEmbTx) {
            ...
            console.log(`${tokenType} <- ${mtx.getHash()} returned: ${embRev}`);
          }
      ...
      if (!oldBlock) {
        for (let mt of markedTransactions) {
          try {
            ...
            const reverted = await getTxStatus(mtxId, obj.blockNumber, token === 'eth');

            // if not reverted add the marked transaction to final marketd transactions
            console.log(`${token} <- ${mtxId} returned: ${reverted}`);
            if (!reverted) {
