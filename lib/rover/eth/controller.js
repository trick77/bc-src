    ...
    try {
      ...
      for (const tx of block.transactions) {
        ...
        try {
            ...
            for (let _ref of _tokens.ERC721_WATCHED_TOKENS) {
              ...
              if (ethUtils.bufferToHex(tx.to).toLowerCase() === contractAddress) {
                ...
                if (amount && amount._isBigNumber) {
                  value = new BN(BigInt(amount._hex), 16).toBuffer();
                } else {
          ...
          debugTx(`\namount: ${amount}`);
          debugTx(`tx value: ${tx.value}`);
          debugTx(`---------------- AMOUNT`);
          debugTx(amount);
          debugTx(txId);

          //debugTx(`Token check matched, addrTo: ${addrTo},addrFrom: ${addrFrom}, amount: ${amount.toString(10)}, tokenType: ${tokenType}, isEmbTx: ${isEmbTx}, txId: ${txId}`)
          let value;
          if (amount === tx.value) {
