...
async function _createUnifiedBlock(roverRpc, block, isStandalone, oldBlock = false) {
  ...
  while (block && block.transactions && block.transactions.length > 0) {
    ...
    try {
      ...
      //if (oldBlock) {
      //  console.log('OLD BLOCK:  ' + obj.blockNumber + ' HASH: ' + obj.blockHash)
      //} else {
      //  console.log('NEW BLOCK:  ' + obj.blockNumber + ' HASH: ' + obj.blockHash)
      //}
      ...
      //if (tx.getUpfrontCost) {
      //  const feeCost = tx.getUpfrontCost()
      //  const dataCost = tx.getDataFee()
      //  const baseFee = tx.getBaseFee()
      //  console.log('Total Amount of wei needed: ' + feeCost.toString())
      //  console.log('Total Amount of wei paid for data: ' + dataCost.toString())
      //  console.log('Total Amount of wei base fee: ' + baseFee.toString())
      //}
      ...
      if (!checkTx) {
        logger.debug(`cannot process ${txId}`);
        continue;
      }
      ...
      if (!oldBlock) {
        if (txStatusPass.indexOf(txId) > -1) {
          reverted = false;
        } else if (txStatusFail.indexOf(txId) > -1) {
          reverted = true;
        } else {
          txStatus = await getRevertReason(txId);
          reverted = '' !== txStatus && ' ' !== txStatus && txStatus && txStatus.slice(0, 3).indexOf('Dai') < 0;
          if (!reverted) {
            txStatusPass.push(txId);
          } else {
            txStatusFail.push(txId);
          }
        }
      } else {
        reverted = false;
      ...
      if (!reverted) {
        ...
        logger.debug(`${txId} <- tx status: ${txStatus}`);
      }
      ...
      logger.info(`unable to parse TX's ${ethUtils.bufferToHex(tx.hash(true))} input data, ${e.stack}`);
    }
  }

  // console.log(settlementChecks.length + '/' + totalBlockTxs + ' txs to check' + `for ${obj.blockNumber} - ${obj.blockHash}`)
  ...
  try {
    ...
    debug(`adding ${markedTransactions.length} marked eth transactions for ${obj.blockNumber} - ${obj.blockHash}`);
