...
async function _createUnifiedBlock(roverRpc, block, isStandalone) {
  ...
  logger.debug(`controller map block #${parseInt(d.header.number, 16)} txs: ${d.transactions.length}`);
  const obj = {
...
class Controller {
  ...
  start(config) {
    ...
    network.on('roverBlockRange', blockRange => {
      this._logger.debug(`reporting block range: ${JSON.stringify(blockRange, null, 2)}`);
    ...
    rpcStream.on('data', message => {
      ...
      switch (message.getType()) {// Also could be message.getPayloadCase()
        ...
        case RoverMessageType.ROVER_BLOCK_RANGE:
          ...
          this._logger.info(`open block range request arrived engine sync status <- ${data.getSynced()}`);
