 ...
 * 
 */
const ERC721_WATCHED_TOKENS_LIST = _tokens.ERC721_WATCHED_TOKENS.map(t => {
  return t.contractAddress.toLowerCase();
});

const ERC20_WATCHED_TOKENS_LIST = _tokens.ERC20_WATCHED_TOKENS.map(t => {
  return t.contractAddress.toLowerCase();
});
...
async function _createUnifiedBlock(roverRpc, block, isStandalone, oldBlock = false) {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // support for tethered mining
      if (msg.setDifficulty) {
        msg.setDifficulty(obj.difficulty);
      }
        ...
        try {
            ...
            for (let _ref of _tokens.ERC721_WATCHED_TOKENS) {
              ...
              if (ethUtils.bufferToHex(tx.to).toLowerCase() === contractAddress) {
                ...
                if (amount && amount._isBigNumber) {
                  value = new BN(BigInt(amount._hex)).toBuffer();
                } else {
                  value = new BN(amount).toBuffer();
                }
                const mTx = new MarkedTransaction();
                mTx.setId(ROVER_NAME);
                ...
                debug(`ERC721 Token check matched FROM, addrTo: ${addrTo},addrFrom: ${addrFrom}, tokenID: ${amount.toString(10)}, tokenType: ${assetName}, isEmbTx: ${isEmb}, txId: ${txId}`);
              }
            }
          }
          ...
          for (let _ref2 of _tokens.ERC20_WATCHED_TOKENS) {
            ...
            if (decodedInput.method === 'transfer' && ethUtils.bufferToHex(tx.to).toLowerCase() === contractAddress) {
              ...
              debug(`ERC20 Token check matched, addrTo: ${to},addrFrom: ${addrFrom}, amount: ${transferAmount.toString(10)}, tokenType: ${assetName}, isEmbTx: ${isEmb}`);
              addrTo = to;
            ...
            if (decodedInput.method === 'transferFrom' && ethUtils.bufferToHex(tx.to).toLowerCase() === contractAddress) {
              ...
              addrFrom = from;
              debugTx(`DEBUGTX Token check matched FROM, addrTo: ${to},addrFrom: ${addrFrom}, amount: ${transferAmount.toString(10)}, tokenType: ${assetName}, isEmbTx: ${isEmb}, txId: ${txId}`);
              addrTo = to;
              amount = transferAmount || 0;
              tokenType = assetName;
              isEmbTx = isEmb;
          ...
          debugTx(`DEBUGTX \namount: ${amount}`);
          debugTx(`DEBUGTX tx value: ${tx.value}`);
          debugTx(`DEBUGTX ---------------- AMOUNT`);
          debugTx('DEBUGTX' + amount);
          debugTx('DEBUGTX' + txId);

          debug(`DEBUGTX Token check matched, addrTo: ${addrTo},addrFrom: ${addrFrom}, amount: ${amount.toString(10)}, tokenType: ${tokenType}, isEmbTx: ${isEmbTx}, txId: ${txId}`);
          let value;
          if (amount === tx.value) {
            value = new BN(amount, 16).toBuffer();
          } else if (amount && amount._isBigNumber) {
            value = new BN(BigInt(amount._hex)).toBuffer();
          } else {
            value = new BN(amount, 16).toBuffer();
          }
          //const value = amount === tx.value ? (new BN(amount, 16)).toBuffer() : (new BN(bnToHex(amount), 16)).toBuffer()
          //const value = amount === tx.value ? (new BN(amount, 16)).toBuffer() : new BN(amount.toString(16), 16).toBuffer()
      ...
      logger.info(`${totalBlockTxs} txs / ${finalMarkedTransactions.length} / ${emblemTransactions.length} / queued mtx: ${queuedMarkedTransactions.length} / marked nft: ${markedNfts.length} / x: ${cantParseTxs} <- ${obj.blockNumber} - ${obj.blockHash}`);
