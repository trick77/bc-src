...
class Network extends EventEmitter {
  ...
  requestBlockRange(blockRange) {
    ...
    highest = min(lowest + ETH_MAX_FETCH_BLOCKS + 20, highest);
    if (this._blocksToFetch.length < 1 && msgBroker.bodies.length < 1) {
      this._logger.info(`block range queued for active sync ${lowest} -> ${highest} (${highest - lowest})`);
      this._blocksToFetch.push([highest, lowest]);
    ...
    if (blockRange) {
      const r = [min(highest, lowest + ETH_MAX_FETCH_HEADERS + 20), lowest];
  ...
  async handleMessageBlockHeaders(payload, peer) {
    ...
    if (DAO_FORK_SUPPORT && !msgBroker.validPeer[peerAddr]) {
      ...
      //const headers = receivedHeaders.splice(0, ETH_MAX_FETCH_HEADERS + 20)
