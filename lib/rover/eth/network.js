...
class Network extends EventEmitter {
      ...
      const _cycleGetBlock = () => {
        ...
        if (cyclePeers.length >= WAIT_FOR_PEERS) {
          ...
          if (peer) {
            ...
            //msgBroker.bodies.push(header)
            eth.sendMessage(devp2p.ETH.MESSAGE_CODES.GET_BLOCK_BODIES, [2, [header.hash()]]);
          } else {
            debug(`GET_BLOCK_BODIES broadcast for ${headerHash}`);
            this.broadcastMessage(devp2p.ETH.MESSAGE_CODES.GET_BLOCK_BODIES, [2, [header.hash()]], 3);
          }
        }
      };
      const intervalRetry = setInterval(() => {
  ...
  getBlocks(hashes) {
    this.broadcastMessage(devp2p.ETH.MESSAGE_CODES.GET_BLOCK_BODIES, [3, hashes]);
  }
  ...
  async handleMessageBlockHeaders(payload, peer) {
    ...
    try {
      ...
      if (DAO_FORK_SUPPORT && !msgBroker.validPeer[peerAddr]) {
        ...
        // TD
        //const header = BlockHeader.fromValuesArray(payload[1], {common})
  ...
  async handleMessageNewBlockHashes(payload, peer) {
    ...
    for (const item of payload) {
      ...
      if (msgBroker.directRequests.indexOf(blockHash) < 0 && !this.storage.has(blockHash)) {
        ...
        eth.sendMessage(devp2p.ETH.MESSAGE_CODES.GET_BLOCK_HEADERS, [2, [blockHashBuf, 1, 0, 0]]);
      } else {
  ...
  run(port) {
    ...
    const rlpx = this._rlpx = new devp2p.RLPx(this._key, {
      dpt: this._dpt,
      maxPeers: 54,
      capabilities: [devp2p.ETH.eth66, devp2p.ETH.eth65],
      //timeout: 9900,
      remoteClientIdFilter: REMOTE_CLIENTID_FILTER,
      common
    });
