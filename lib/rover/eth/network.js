...
const commonBerlin = new _common2.default({
  chain: 'mainnet',
  eips: [1559, 2718, 2930, 2929, 2565, 2315, 2537],
  hardfork: 'berlin',
  supportedHardforks: ['berlin', 'istanbul', 'homestead', 'byzantium', 'petersburg', 'constantinople', 'spuriousDragon', 'dao', 'chainstart', 'london', 'arrowGlacier', 'muirGlacier']
});
  ...
  async handleMessageBlockHeaders(payload, peer) {
    ...
    try {
      ...
      if (DAO_FORK_SUPPORT && !msgBroker.validPeer[peerAddr]) {
        ...
        const directRequest = [];
        const receivedHeaders = payload.map(header => BlockHeader.fromValuesArray(header, { common: commonBerlin }));
        // DEBUG
