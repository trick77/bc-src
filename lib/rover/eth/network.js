...
const ETH_INTERVAL_GET_BLOCKS_MS = 900;
...
class Network extends EventEmitter {
  ...
  constructor(config) {
    ...
    this._syncCheckTimeout = setInterval(() => {
      ...
      if (Math.floor(Date.now() / 1000) % 2 === 0) {
        this._logger.info(`graph ${dptPeers.length}-dpt ${peers.length}-full ${allPeers.length - peers.length}-lite, bh: ${!this._bestSeenBlock === true ? false : new BN(this._bestSeenBlock.header.number).toNumber()}, seek: ${this._seekingBlockSegment}, unfetch: ${this._blocksToFetch.length} | bodies: ${msgBroker.bodies.length} ${now - msgBroker.lastUpdate}`);
  ...
  requestBlockRange(blockRange) {
    ...
    if (this._seekingBlockSegment) {
      this._logger.info(`received block range request ${blockRange[1]} -> ${blockRange[0]} (${highest}) while seeking.`);
      return;
    }
    this._logger.info(`received block range request ${blockRange[1]} -> ${blockRange[0]} (${highest})`);
  ...
  async handleMessageBlockHeaders(payload, peer) {
    ...
    if (DAO_FORK_SUPPORT && !msgBroker.validPeer[peerAddr]) {
      ...
      const headers = receivedHeaders.splice(0, ETH_MAX_FETCH_HEADERS + 10);
      // DEBUG
