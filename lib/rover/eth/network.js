...
exports.ETH_MAX_FETCH_BLOCKS = undefined;
...
//const common = new Common({chain: 'mainnet', eips: [2718, 2930, 2929, 2565, 2315, 2537]})

async function isValidBlockDefault(block) {
  return block.validateUnclesHash() && block.transactions.every(isValidTx) && block.validateTransactionsTrie();
}

const common = new _common2.default({ chain: _common.Chain.Mainnet, hardfork: _common.Hardfork.Berlin });

//const common = new Common({
//  chain: 'mainnet',
//  hardfork: 'berlin',
//  supportedHardforks: ['berlin', 'istanbul', 'homestead', 'byzantium', 'petersburg', 'constantinople', 'spuriousDragon', 'dao', 'chainstart', 'london', 'arrowGlacier','muirGlacier','grayGlacier']
//})

const commonBerlin = new _common2.default({
  chain: 'mainnet',
  eips: [1559, 2718, 2930, 2929, 2565, 2315, 2537, 2930],
  hardfork: 'berlin',
  supportedHardforks: ['berlin', 'istanbul', 'homestead', 'byzantium', 'petersburg', 'constantinople', 'spuriousDragon', 'dao', 'chainstart', 'london', 'arrowGlacier', 'muirGlacier', 'grayGlacier']
});
...
const getHeader = async payload => {
  ...
  try {
    ...
    header = BlockHeader.fromValuesArray(payload[1][0], { common });
    hash = header.hash();
    return header;
  } catch (e) {
    ...
    try {
      ...
      header = BlockHeader.fromValuesArray(payload[1], { common });
      hash = header.hash();
      return header;
    } catch (e) {
      ...
      try {
        ...
        header = BlockHeader.fromValuesArray(payload[0], { common });
        hash = header.hash();
        return header;
      } catch (e) {
        return false;
...
// https://github.com/ethereumjs/ethereumjs-monorepo/tree/master/packages/common#readme
// FIX JS CLIENT
// https://github.com/ethereumjs/ethereumjs-monorepo/blob/f2be1b3c6dee219c10c79a448fa428f9bfe3f7e2/packages/common/src/chains/mainnet.json#L76
const hf = common._chainParams.hardforks;
hf.forEach(f => {
  if (f.name === 'berlin') {
    // https://github.com/ethereum/go-ethereum/blob/e536bb52ff87cfffe9b413d02dfcf80e7f265e5a/core/forkid/forkid_test.go#L64
    f.forkHash = '0x0eb440f6';
  }
  //  if (f.name === 'grayGlacier') {
  //    // https://github.com/ethereum/go-ethereum/blob/e536bb52ff87cfffe9b413d02dfcf80e7f265e5a/core/forkid/forkid_test.go#L64
  //    f.forkHash = '0xf0afd0e3'
  //  }
});
//common._chainParams.hardforks = hf
commonBerlin._chainParams.hardforks = hf;
...
const CANDIDATES = [];
const VALIDATED = {};
let VALIDATED_PERFORMANCE_COUNT = 0;
...
const getNewEthereumBlockFromData = payload => {
  try {
    ...
    returnBlock = Block.fromValuesArray(payload, { common: commonBerlin });
    return returnBlock;
  } catch (err) {
    debug(err);
...
const {
  ROVER_RESYNC_PERIOD,
  ROVER_SECONDS_PER_BLOCK,
  getPrivateKey,
  semaphoreSwitch,
  getBacksyncEpoch,
  getIntervalDifficulty,
  shuffle
} = require('../utils');
...
const ETH_MAX_FETCH_BLOCKS = exports.ETH_MAX_FETCH_BLOCKS = config.rovers.maxFetchBlocks || 32;
const ETH_INTERVAL_GET_HEADERS_MS = 2008;
const ETH_INTERVAL_GET_BLOCKS_MS = 3800;
...
const BERLIN_CHECK_BLOCK_TITLE = 'Berlin Fork'; // Only for debugging/console output
const BERLIN_CHECK_BLOCK_NR = 12244000;
const BERLIN_CHECK_BLOCK = '1638380ab737e0e916bd1c7f23bd2bab2a532e44b90047f045f262ee21c42b21';
const BERLIN_CHECK_BLOCK_HEADER = rlp.decode(Buffer.from('', 'hex'));
...
const validateBlockFromData = async (payload, lookupBlock = false) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      // index for already checked
      const dataHash = crypto.blake2bl(data.join());
      const dataHashIndex = Object.keys(VALIDATED);

      if (VALIDATED[dataHash]) {
        VALIDATED_PERFORMANCE_COUNT++;
        debug(`block data hash already available ${dataHash}, saved ${VALIDATED_PERFORMANCE_COUNT}`);
        return resolve(VALIDATED[dataHash]);
      }

      debug(`block data hash: ${dataHash}`);

      if (dataHashIndex.length > 1000) {
        delete VALIDATED[dataHashIndex[0]];
        delete VALIDATED[dataHashIndex[1]];
        delete VALIDATED[dataHashIndex[2]];
      }
...
class Network extends EventEmitter {
  ...
  constructor(config) {
    ...
    this._reportLatestBlockHash = BERLIN_CHECK_BLOCK;
    this._reportLatestBlockHeight = false;
    ...
    this._lowestBlockHeight = CLIMB_BLOCK_NR;
    ...
    this._syncCheckTimeout = setInterval(async () => {
      ...
      if (dptPeers.length < 3 || this._blocksIndexed < 2) {
        this._dptFailed++;
        debug('dpt failure: ' + this._dptFailed);
        if (this._dptFailed > 25) {
          debug('unable to establish DPT table...');
          this._dptFailed--;
          process.exit();
        } else {
      ...
      if (peers.length < 2) {
        this._peerFailed++;
        debug('peer failure: ' + this._peerFailed);
        if (this._peerFailed > 20) {
          debug('unable to establish peer connections table...');
          this._peerFailed--;
          process.exit();
        } else if (!this._peerFailed) {
          this._peerFailed = 1;
        }
      }

      if (peers.length >= 4 && this._bestSeenBlock && this._peerFailed > 0 && this._blocksIndexed > 10) {
        this._peerFailed--;
      }
      ...
      if (msgBroker.bodies.length === 1 && this._seekingBlockSegment) {
        debug(`potentially weak network ${this._peerFailed}`);
        this._peerFailed++;
      }
    }, 8900);
  ...
  async updateLatestBlock(block) {
    ...
    this.storage.flush();
    this._lowestBlockHeight = parseInt(block.getHeight(), 10) - 100;
    this._seekingBlockSegment = false;
  ...
  requestBlockRange(blockRange) {
    ...
    highest = min(lowest + ETH_MAX_FETCH_BLOCKS + 100, highest) + 1;
  ...
  sync() {
    ...
    if (peers.length >= WAIT_FOR_PEERS && !this._seekingBlockSegment && msgBroker.bodies < 3 && this._blocksToFetch) {
  ...
  onNewBlock(block, peer, isBlockFromInitialSync = false) {
    ...
    if (this._bestSeenBlock) {
      if (blockNumber > bestBlockNumber + 500) {
        ...
        if (!msgBroker.newBlockPeers[getPeerAddr(peer)]) {
    ...
    if (bestBlockNumber < blockNumber) {
      const p = getPeerAddr(peer);
      if (!msgBroker.newBlockPeers[p]) {
        msgBroker.newBlockPeers[p] = {
          updated: Math.floor(Date.now() / 1000)
        };
      } else {
        msgBroker.newBlockPeers[p].updated = Math.floor(Date.now() / 1000);
      }
    ...
    if (Number(blockNumber) < Number(this._lowestBlockHeight) - 200) {
      peer && peer.disconnect && peer.disconnect();
    ...
    if (difficultyValid && (blockNumber - bestBlockNumber === 1 || bestBlockNumber === 0)) {
      ...
      if (blockNumber + 100000 < bestBlockNumber) {
        debug(` block ${blockNumber} too far to attach`);
        return;
      } else {
  ...
  scheduleInitialSync(knownBlock) {
    ...
    if (this._blocksToFetch.length > 0) {
      this._logger.info(`schedule initial sync run when ${this._blocksToFetch.length} segments remain`);
      return;
    }
      ...
      const _cycleGetHeaders = peer => {
        ...
        if (peers.length >= WAIT_FOR_PEERS || peer) {
          ...
          if (isNaN(from)) {
            ...
            //message = [1, [from, maxHeaders, 0, 0]]
            //message = [1, [from, maxHeaders, 0, 0]]
            message = [Buffer.from([1]), [devp2p.int2buffer(from), Buffer.from([maxHeaders]), Buffer.from([]), Buffer.from([])]];
          } else {
            // DEBUG
            debug('using complex message');
            // ETH LONDON
            //message = [1, [from > 0 ? from : from + 1, maxHeaders, 0, 0]]
            // ETH BERLIN
            //message = [1, [from + 1, maxHeaders, 0, 0]]
            // ETH MUIER
            message = [Buffer.from([1]), [devp2p.int2buffer(from + 1), Buffer.from([maxHeaders]), Buffer.from([]), Buffer.from([])]];
            // DEBUG
          ...
          if (peer) {
            ...
            debug(message);
            this.broadcastMessage(devp2p.ETH.MESSAGE_CODES.GET_BLOCK_HEADERS, message, 2);
          }
        }
      };
      ...
      const _cycleGetBlock = () => {
        ...
        if (cyclePeers.length >= WAIT_FOR_PEERS) {
          ...
          if (peer) {
            ...
            //msgBroker.bodies.push(header)
            eth.sendMessage(devp2p.ETH.MESSAGE_CODES.GET_BLOCK_BODIES, [Buffer.from([3]), [header.hash()]]);
          } else {
            debug(`GET_BLOCK_BODIES broadcast for ${headerHash}`);
            this.broadcastMessage(devp2p.ETH.MESSAGE_CODES.GET_BLOCK_BODIES, [Buffer.from([3]), [header.hash()]], 3);
          }
        }
      };
      const intervalRetry = setInterval(() => {
  ...
  getBlocks(hashes) {
    this.broadcastMessage(devp2p.ETH.MESSAGE_CODES.GET_BLOCK_BODIES, [Buffer.from([3]), hashes]);
  }
  ...
  async getBlockchain(from, to, peer = false) {
    ...
    if (this._seekingBlockSegment) {
      return Promise.resolve('cannot concurrently call sync blockchain');
    }
    ...
    if (!headers || headers.length < 1) {
      this._logger.warn(`headers not received for range ${from}`);
      //msgBroker.bodies.shift()
      this._fetchCache.set(from, true);
      this._seekingBlockSegment = false;
  ...
  handleMessage(rlpx, code, payload, peer) {
    ...
    switch (code) {
      ...
      case devp2p.ETH.MESSAGE_CODES.GET_BLOCK_BODIES:
        ...
        this.handleMessageGetBlockBodies(payload, peer);
        break;
      ...
      case devp2p.ETH.MESSAGE_CODES.GET_NODE_DATA:
        debug(`Gmessage code GET_NODE_DATA: ${peerAddr}`);
        this.handleMessageGetNodeData(payload, peer);
        break;

      case devp2p.ETH.MESSAGE_CODES.GET_RECEIPTS:
        debug(`----------- message code GET_RECEIPTS: ${peerAddr}`);
        this.handleMessageGetReceipts(payload, peer);
        break;
      ...
      case devp2p.ETH.MESSAGE_CODES.NEW_BLOCK:
        debug(`----------- message code NEW_BLOCK: ${peerAddr}`);
  ...
  async handleMessageBlockBodies(payload, peer, recheck = false) {
    ...
    try {
      ...
      debug(`removed ${removed} bodies for a remaining ${msgBroker.bodies.length}`);
      let txs = false;
      let uncleHeaders = false;
      if (payload.length > 0) {
        if (payload[1][0] && payload[1][0]) {
          txs = payload[1][0][0];
          uncleHeaders = payload[1][0][1];
        }
      }

      if (!txs) {
        debug(`txs not found ${peerAddr}`);
        debug(payload[0]);
        debug(payload[1]);
        peer && peer.disconnect && peer.disconnect(devp2p.DISCONNECT_REASONS.USELESS_PEER);
        return;
      ...
      if (!uncleHeaders) {
        debug(`uncles not found ${peerAddr}`);
        return;
      }
      ...
      while (msgBroker.bodies.length > 0) {
        ...
        if (!header === false) {
          ...
          debug(uncleHeaders);
          //const raw = [header.raw(), payload[1][0], payload[1][1]]
          const raw = [header.raw(), txs, uncleHeaders];
          const data = [raw];
          debug(`items in all columns: ${raw.length}`);
          const validBlock = getNewEthereumBlockFromData(raw);
          //const validBlock = Block.fromValuesArray(raw, { common })
          debug(`txs in block ${blockNumber}: ${validBlock.transactions.length}`);
          const blockHashHex = validBlock.header.hash().toString('hex');
          const syncBlock = blockNumber + 990 < fiberHeight;

          const vb = await isValidBlockDefault(validBlock, syncBlock);
  ...
  async handleMessageBlockHeaders(payload, peer) {
    ...
    try {
      ...
      if (DAO_FORK_SUPPORT && !msgBroker.validPeer[peerAddr]) {
        ...
        // fix broken api
        if (!header.hash && header.cache && header.cache.hash) {
          header.hash = header.cache.hash;
        }
        const expectedHash = DAO_FORK_SUPPORT ? BERLIN_CHECK_BLOCK : ETC_1920000;
        // DEBUG
        debug(`${peerAddr} <- hash from peer ${header.hash().toString('hex')} expected: ${expectedHash}`);
        const timeout = this._forkDrops[peerAddr];
        ...
        debug(`fork is confirmed process block headers `);
        const receivedHeaders = payload[1].map(header => BlockHeader.fromValuesArray(header, { common: commonBerlin }));
        // DEBUG
        ...
        for (const header of headers) {
          ...
          if (this._fetchCache.has(blockNumber) && msgBroker.directRequests.indexOf(blockHash) > -1) {
            ...
            this._fetchCache.delete(blockNumber);
          }
        ...
        while (headers.length > 0) {
          ...
          const headerHash = header.hash().toString('hex');
          const blockNumber = new BN(header.number).toNumber();
          if (!lowestNumber && payload[1].length > 1) {
            debug(`setting lowest number to ${blockNumber}`);
            lowestNumber = blockNumber;
          } else if (lowestNumber > blockNumber && payload[1].length > 1) {
            debug(`setting lowest number to ${blockNumber}`);
            lowestNumber = blockNumber;
          }
          let blockTimeThreshold = getBacksyncEpoch('eth');
  ...
  handleMessageGetBlockBodies(payload, peer) {
    // ETH.MESSAGE_CODES.GET_BLOCK_BODIES
    const peerAddr = getPeerAddr(peer);

    if (msgBroker.getBlockBodies[peerAddr]) {
      msgBroker.getBlockBodies[peerAddr] = 0;
    }

    msgBroker.getBlockBodies[peerAddr]++;

    if (msgBroker.getBlockBodies[peerAddr] > 100) {
      debug(`peer exceeded max block bodies requests`);
      msgBroker.disconnected[getPeerAddr(peer)] = 1;
      peer && peer.disconnect && peer.disconnect(devp2p.DISCONNECT_REASONS.USELESS_PEER);
      return;
    }
  ...
  handleMessageGetBlockHeaders(payload, peer) {
    ...
    if (DAO_FORK_SUPPORT && devp2p.buffer2int(payload[0]) === BERLIN_CHECK_BLOCK_NR) {
      headers.push(BERLIN_CHECK_BLOCK_HEADER);
    ...
    if (msgBroker.requests[peerAddr] < 900) {
  ...
  handleMessageGetNodeData(payload, peer) {
    const eth = peer.getProtocols()[0];
    eth.sendMessage(devp2p.ETH.MESSAGE_CODES.NODE_DATA, [payload[0], []]);
  }

  handleMessageGetReceipts(payload, peer) {
    const eth = peer.getProtocols()[0];
  ...
  async handleMessageNewBlock(payload, peer, forceBlock) {
    ...
    let c = payload[0];
    try {
      b = Block.fromValuesArray(c, { common: commonBerlin });
      //const vb = await validateBlockFromData(payload[0], false)
      const vb = await isValidBlockDefault(b);
      if (vb) {
        this.onNewBlock(b, peer, true);
      }
      return;
    ...
    if (this._ipdTestBlocks.length < ETH_IPD_TEST_BLOCKS && !forceBlock) {
      ...
      try {
        //b = Block.fromValuesArray(c, {common: commonBerlin})
        b = Block.fromValuesArray(c, { common: common });
      } catch (e) {
        debug('payload = %O', payload);
        this._logger.error(`Error while decoding eth block, e = ${e.message}`);
        return;
      ...
      const state = {
        ...
        peer: peer,
        sent: 1
        //const vb = await validateBlockFromData(payload[1], false)
      };const bl = Block.fromValuesArray(payload[1], { common: commonBerlin });
      const vb = await isValidBlockDefault(bl, false);
    ...
    if (this._ipdTestBlocks.length >= ETH_IPD_TEST_BLOCKS && !this._ipdTestComplete && !forceBlock) {
      // run the IPD test
      //const block = new EthereumBlock(payload[1])
      const block = Block.fromValuesArray(payload[1], { common: commonBerlin });
      //const vb = await validateBlockFromData(payload[1], false)
      const vb = await isValidBlockDefault(block, false);
      if (!vb) {
        let h = '';
        let bh = 'NA';
      ...
      if (!block.header.hash && block.header.cache && block.header.cache.hash) {
        block.header.hash = block.header.cache.hash;
      }
      ...
      // this part is made irrelvant once the chain has started as the Eth segment is weighed against the difficulty of
      // Overline blocks to select the strongest blocks
      //
      ...
      setTimeout(async () => {
        const block = Block.fromValuesArray(payload[0], { common: common });
        //const validBlock = await validateBlockFromData(payload, false)
        const validBlock = await isValidBlockDefault(block, false);
  ...
  async handleMessageNewBlockHashes(payload, peer) {
    ...
    for (const item of payload) {
      ...
      if (msgBroker.directRequests.indexOf(blockHash) < 0 && !this.storage.has(blockHash)) {
        ...
        eth.sendMessage(devp2p.ETH.MESSAGE_CODES.GET_BLOCK_HEADERS, [Buffer.from([2]), [blockHashBuf, 1, 0, 0]]);
      } else {
        debug(` ${blockHash} for request is in storage  ${peerAddr}`);
      }
    }
  }
  ...
  handleMessageStatus(peer) {
    ...
    //setTimeout(() => {
    //  const eth = peer.getProtocols()[0]
    //  eth.sendMessage(devp2p.ETH.MESSAGE_CODES.GET_BLOCK_HEADERS, [2, [BERLIN_CHECK_BLOCK_NR, 1, 0, 0]])
    //}, 100)
    //eth.sendStatus({
    //  networkId: CHAIN_ID,
  ...
  async handlePeerAdded(rlpx, peer) {
    ...
    try {
      ...
      eth.once('status', () => {
        ...
        // eth.sendMessage(devp2p.ETH.MESSAGE_CODES.GET_BLOCK_HEADERS, [2, [BERLIN_CHECK_BLOCK_NR, 1, 0, 0]])
        eth.sendMessage(devp2p.ETH.MESSAGE_CODES.GET_BLOCK_HEADERS, [Buffer.from([1]), [devp2p.int2buffer(BERLIN_CHECK_BLOCK_NR), Buffer.from([1]), Buffer.from([]), Buffer.from([])]]);
