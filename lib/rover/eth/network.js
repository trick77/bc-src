...
const ETH_INTERVAL_GET_HEADERS_MS = 1000;
const ETH_INTERVAL_GET_BLOCKS_MS = 8100;
const ETH_IPD_TEST_BLOCKS = 4;
const ETH_MAX_FETCH_BLOCKS = config.rovers.maxFetchBlocks || 5;
const ETH_MAX_FETCH_HEADERS = config.rovers.maxFetchHeaders || 25;
...
class Network extends EventEmitter {
  ...
  requestBlockRange(blockRange) {
    ...
    if (this._seekingBlockSegment) {
      this._logger.info(`received block range request ${blockRange[1]} -> ${blockRange[0]} which seeking segment...`);
      return;
    }
    ...
    return new Promise(resolve => {
      const _cycleGetHeaders = () => {
        ...
        if (peers.length >= WAIT_FOR_PEERS) {
          ...
          this.broadcastMessage(ETH.MESSAGE_CODES.GET_BLOCK_HEADERS, message, 3);
        }
      };
      ...
      const _cycleGetBlock = () => {
        ...
        if (cyclePeers.length >= WAIT_FOR_PEERS) {
          ...
          if (!found) {
            ...
            this.broadcastMessage(ETH.MESSAGE_CODES.GET_BLOCK_BODIES, [header.hash()], 3);
          }
        }
      };
    ...
    try {
      ...
      while (headers.length > 0) {
        ...
        if (new BN(blockTimestamp).lt(new BN(blockTimeThreshold)) && !this._fetchCache.has(blockNumber)) {
          this._logger.warn(`block ${blockNumber} time ${blockTimestamp} is below the backsync threshold ${blockTimeThreshold}`);
          this._seekingBlockSegment = false;
        } else if (stored && this.storage.has(header.hash().toString('hex')) && !this._fetchCache.has(blockNumber) && !pass) {
          // DEBUG
  ...
  async handleMessageBlockBodies(payload, peer, recheck = false) {
    ...
    try {
      ...
      if (this._blocksToFetch.length < 1 && found.length > 0) {
        setTimeout(() => {
          this.sync();
        }, 15000);
      }
  ...
  async handleMessageNewBlock(payload, peer, forceBlock) {
    ...
    if (this._ipdTestBlocks.length >= ETH_IPD_TEST_BLOCKS && !this._ipdTestComplete && !forceBlock) {
      ...
      try {
        ...
        const validBlock = await validateBlock(newBlock);
  ...
  run(port) {
    ...
    BOOTNODES.slice(0, 125).map(bootnode => {
      // $FlowFixMe
