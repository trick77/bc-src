...
const common = new _common2.default({ chain: 'mainnet', eips: [2718, 2930, 2929, 2565], hardfork: 'berlin' });
...
const CLIMB_BLOCK_NR = 12274302;
const CLIMB_BLOCK = '2a09f397b468621ff04c01428feb0e99e757068e568965ca038adcd4c8757567';
...
const validateBlockFromData = async (payload, lookupBlock = false) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const validB = Block.fromValuesArray(data, { common });
      ...
      const checkUncles = await validateUncles(validB, validA);
      if (!checkUncles) {
      ...
      const checkTransactions = await validateTransactions(validA, validB);
      ...
      const checkTrie = await validateTrie(validB, validA, lookupBlock);
      if (!checkTrie) {
...
class Network extends EventEmitter {
  ...
  constructor(config) {
    ...
    this._syncCheckTimeout = setInterval(() => {
      ...
      if (now - msgBroker.lastUpdate > 1130) {
  ...
  async handleMessageBlockBodies(payload, peer, recheck = false) {
    ...
    try {
      ...
      let pass = false;
      const found = [];
      let fiberHeight = this._ipdTestBlocks.length > 0 ? new BN(this._ipdTestBlocks[0].payload.header.number).toNumber() : CLIMB_BLOCK_NR - 10;
      let fiberBlock = true;
      ...
      if (header.hash().toString('hex') === expectedHash) {
        ...
        if (!this._seekingBlockSegment) {
          if (this._bestSeenBlock && this._blocksIndexed < 3) {
            ...
            debug(`-------------CANDIDATE---------------`);
            debug(`${peerAddr} requesting ${CLIMB_BLOCK_NR}`);
            debug(`-------------------------------------`);
            await this.getBlockchain(CLIMB_BLOCK_NR, CLIMB_BLOCK_NR + 20, peer);
          }
        } else {
          debug(`-------------CANDIDATE---------------`);
          debug(`${peerAddr}`);
  ...
  handleMessageStatus(peer) {
    const okHash = Buffer.from(CLIMB_BLOCK, 'hex');
  ...
  run(port) {
    ...
    BOOTNODES.slice(0, 15).map(bootnode => {
      // $FlowFixMe
