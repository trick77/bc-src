...
const getNewEthereumBlockFromData = payload => {
  try {
    ...
    returnBlock = Block.fromValuesArray(payload);
    return returnBlock;
  } catch (err) {
    debug(err);
...
const validateBlockFromData = async (payload, lookupBlock = false) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const validB = Block.fromValuesArray(data);
...
class Network extends EventEmitter {
    ...
    if (DAO_FORK_SUPPORT && !msgBroker.validPeer[peerAddr]) {
      ...
      if (header.hash().toString('hex') === expectedHash) {
        ...
        if (this._ipdTestBlocks && this._ipdTestBlocks.length < 1) {
          await this.getBlockchain(CLIMB_BLOCK_NR, CLIMB_BLOCK_NR + 5);
        }
  ...
  run(port) {
    ...
    const common = new _common2.default({ chain: 'mainnet' });
