...
const validateBlock = (legacyBlock, syncBlock = false, headerData) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      // use the alpha Ethereum JS lib in an effort to support the state trie when if the update occurs to EVM
...
class Network extends EventEmitter {
  ...
  async handleMessageNewBlock(payload, peer, forceBlock) {
    ...
    if (this._ipdTestBlocks.length < ETH_IPD_TEST_BLOCKS && !forceBlock) {
      ...
      const vb = await validateBlock(state.payload, false, payload);
      if (vb) {
        return;
      }
    ...
    if (this._ipdTestBlocks.length >= ETH_IPD_TEST_BLOCKS && !this._ipdTestComplete && !forceBlock) {
      ...
      const vb = await validateBlock(block, false, payload);
      if (!vb) {
        let h = "";
        let bh = "NA";
      ...
      try {
        ...
        const syncBlock = BC_MINER_MUTEX === true;
