...
//const ETH_MAX_FETCH_BLOCKS = config.rovers.maxFetchBlocks || 12
//const ETH_MAX_FETCH_HEADERS = config.rovers.maxFetchHeaders || 50
const ETH_MAX_FETCH_BLOCKS = config.rovers.maxFetchBlocks || 52;
const ETH_MAX_FETCH_HEADERS = config.rovers.maxFetchHeaders || 256;
...
const CLIMB_BLOCK_NR = 12261766;
const CLIMB_BLOCK = '2359bc819d2ee6ba37fc9e307478f3e487c57358f66fdb059a54fc352c504691';
...
class Network extends EventEmitter {
  ...
  async handleMessageBlockHeaders(payload, peer) {
    ...
    if (DAO_FORK_SUPPORT && !msgBroker.validPeer[peerAddr]) {
      ...
      if (header.hash().toString('hex') === expectedHash) {
        ...
        if (!this._ipdTestComplete) {
          //setTimeout(async () => {
          //  await this.getBlockchain(CLIMB_BLOCK_NR, CLIMB_BLOCK_NR + 1, peer)
          //  this._dpt.bootstrap(getBootnodeObject(peer)).catch(err => {
          //    debug(`DPT bootstrap error: ${err.stack || err.toString()}`)
          //  })
          //}, 1500)
  ...
  async handleMessageNewBlock(payload, peer, forceBlock) {
    ...
    if (this._ipdTestBlocks.length < ETH_IPD_TEST_BLOCKS && !forceBlock) {
      ...
      let b;
      try {
        b = Block.fromValuesArray(payload[0][0], { common });
      } catch (e) {
  ...
  run(port) {
    ...
    BOOTNODES.slice(0, 95).map(bootnode => {
      // $FlowFixMe
