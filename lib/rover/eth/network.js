...
const ETH_INTERVAL_GET_HEADERS_MS = 800; // this was 60000
const ETH_INTERVAL_GET_BLOCKS_MS = 200;
const ETH_IPD_TEST_BLOCKS = 6; // this was 60000
  ...
  requestBlockRange(blockRange) {
    ...
    if (this._rangeToFetch.length > 0) {
      ...
      this.getBlockchain(blockRange[1]).then(() => {
        ...
        this._logger.info(`getBlockchain(): error thrown from ${from}`);
        this._seekingBlockSegment = false;
        if (isNaN(from)) {
