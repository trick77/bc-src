...
const common = new _common2.default({ chain: 'mainnet', eips: [2718, 2930, 2929, 2565, 2315, 2537], hardfork: 'berlin' });
...
const ETH_INTERVAL_GET_HEADERS_MS = 1008;
const ETH_INTERVAL_GET_BLOCKS_MS = 1100;
...
const REMOTE_CLIENTID_FILTER = ['Parity-Ethereum', 'go1.5', 'go1.6', 'go1.7', 'Geth/v1.7', 'quorum', 'pirl', 'ubiq', 'gmc', 'gwhale', 'prichain'];

// BER FORK
//const CHECK_BLOCK_NR = 12244000
//const CHECK_BLOCK = '1638380ab737e0e916bd1c7f23bd2bab2a532e44b90047f045f262ee21c42b21'
// BYZ FORK
const CLIMB_BLOCK_NR = 12256114;
const CLIMB_BLOCK = '32439496ff155ca2b40b1e55eaa0910f108072c9fbb8b5134a604588b874a05f';
...
class Network extends EventEmitter {
  ...
  constructor(config) {
    ...
    this._climbBlockHash = CLIMB_BLOCK;
    this._climbBlockHeight = CLIMB_BLOCK_NR;
    ...
    this._syncCheckTimeout = setInterval(() => {
      ...
      if (Math.floor(Date.now() / 1000) % 2 === 0) {
        this._logger.info(`graph ${dptPeers.length}-dpt ${tablePeerCount}-links ${allPeers.length - peers.length}-lite, m-${now - msgBroker.lastUpdate} | bh: ${!this._bestSeenBlock === true ? false : new BN(this._bestSeenBlock.header.number).toNumber()}, seek: ${this._seekingBlockSegment}, unfetch: ${this._blocksToFetch.length}, bodies: ${msgBroker.bodies.length}`);
  ...
  updateLatestBlock(block) {
    ...
    this._climbBlockHeight = parseInt(block.getHeight(), 10);
    this._climbBlockHash = block.getHash();
  }
  ...
  broadcastMessage(message, messageBody, peersToUse = 1) {
    ...
    //  if (a) {
    //    peers.push(a)
    //    msgBroker.validPeer[getPeerAddr(a)] = 1
    //  }
    //  if (b) {
    //    peers.push(b)
    //    msgBroker.validPeer[getPeerAddr(b)] = 1
    //  }
    //  if (c) {
    //    peers.push(c)
    //    msgBroker.validPeer[getPeerAddr(c)] = 1
    //  }
    //}
    let speers = [];
  ...
  getRandomPeer() {
    ...
    const backupPeers = shuffle(this._dpt.getPeers());
    return backupPeers.pop();
  }
  ...
  getHeaders(from, to, peer = false) {
    ...
    return new Promise(resolve => {
      ...
      const intervalRetry = setInterval(() => {
        ...
        //}
        if (peers.length === 0) {
          debug(`clearing block header request for ${from} with no peers`);
          this._seekingBlockSegment = false;
      ...
      }, getRandomRange(ETH_INTERVAL_GET_HEADERS_MS, ETH_INTERVAL_GET_HEADERS_MS + 6000));
  ...
  getBlock(header, returnHeaderOnError = false) {
    ...
    return new Promise(resolve => {
      ...
      const _cycleGetBlock = () => {
        ...
        if (cyclePeers.length >= WAIT_FOR_PEERS) {
          ...
          if (!found) {
            ...
            this.broadcastMessage(devp2p.ETH.MESSAGE_CODES.GET_BLOCK_BODIES, [header.hash()], 1);
          }
        }
      };
      const intervalRetry = setInterval(() => {
      ...
      }, getRandomRange(ETH_INTERVAL_GET_BLOCKS_MS, ETH_INTERVAL_GET_BLOCKS_MS + 1800));
  ...
  async getBlockchain(from, to, peer = false) {
    ...
    // clearTimeout(timeout)
    // DEBUG
    const iterateHeaders = async headers => {
      if (!headers || headers.length < 1) {
        return Promise.resolve(true);
      }
      try {
  ...
  async handleMessageBlockBodies(payload, peer, recheck = false) {
    ...
    try {
      ...
      let pass = false;
      const found = [];
      let fiberHeight = this._ipdTestBlocks.length > 0 ? new BN(this._ipdTestBlocks[0].payload.header.number).toNumber() : this._climbBlockHeight - 10;
      let fiberBlock = true;
      ...
      if (header.hash().toString('hex') === expectedHash) {
        ...
        if (!this._seekingBlockSegment) {
          if (this._bestSeenBlock && this._blocksIndexed < 3) {
            ...
            debug(`-------------CANDIDATE---------------`);
            debug(`${peerAddr} requesting ${this._climbBlockHeight}`);
            debug(`-------------------------------------`);
            await this.getBlockchain(this._climbBlockHeight, this._climbBlockHeight + 20, peer);
          }
        } else {
          debug(`-------------CANDIDATE---------------`);
          debug(`${peerAddr}`);
  ...
  handleMessageGetBlockBodies(peer) {
    ...
    if (msgBroker.headers.length === 0 && msgBroker.msgTypes[devp2p.ETH.MESSAGE_CODES.GET_BLOCK_BODIES] > 60000) {
  ...
  handleMessageStatus(peer) {
    const okHash = Buffer.from(this._climbBlockHash, 'hex');
  ...
  handlePeerRemoved(rlpx, peer, reason, disconnectWe) {
    ...
    if (devp2p.DISCONNECT_REASONS[reason] === devp2p.DISCONNECT_REASONS.TOO_MANY_PEERS) {} else {
      delete msgBroker.validPeer[peerAddr];
      delete msgBroker.litePeer[peerAddr];
    }
  }
  ...
  run(port) {
    ...
    setInterval(async () => {
      ...
      if (!this._seekingBlockSegment && this._bestSeenBlock && this._blocksIndexed > 0) {
        const bestBlockNumber = parseInt(pathOr(Buffer.from('00', 'hex'), ['header', 'number'], this._bestSeenBlock).toString('hex'), 16);
        debug(`requesting next segment from ${bestBlockNumber}`);
        await this.getBlockchain(bestBlockNumber, bestBlockNumber + 30);
      }
    }, 6000);

    BOOTNODES.slice(0, 25).map(bootnode => {
      // $FlowFixMe
    ...
    //    this._dpt.bootstrap(c[0]).catch(err => {
    //      // debug(`DPT bootstrap error: ${err.stack || err.toString()}`)
    //    })
    //}, 1100)
  }
