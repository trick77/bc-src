...
class Network extends EventEmitter {
  ...
  sync() {
    ...
    if (msgBroker.archive.length > 200) {
      msgBroker.archive = msgBroker.archive.slice(50, 100);
    }
  ...
  getBlock(header, returnHeaderOnError = false) {
    ...
    if (this.storage.has(headerHash) === true && !this._blocksArchive[headerHash] && !this._fetchCache.has(blockNumber)) {
      ...
      if (sb && sb.header && sb.transactions && sb.transactions.length > 0) {
        this.onNewBlock(sb, currentPeers[0], pass);
  ...
  async handleMessageBlockBodies(payload, peer) {
    ...
    // DEBUG
    // console.log(`blocks pending: ${msgBroker.bodies.length} payload: ${payload.length}`)
    const unused = [];
    ...
    while (msgBroker.bodies.length > 0) {
      ...
      if (!header === false) {
        try {
          ...
          // console.log('block hash: ' + blockHashHex + ' ' + blockNumber + ' pass: ' + pass)
          ...
          //  continue
          //}
          blocksToValidate.push(block);
        } catch (err) {
          // catch malformed blocks
          debug(err);
          this._logger.warn(`block considered malformed <- dismiss ETH peer`);
          peer && peer.disconnect && peer.disconnect();
        }
      } else {
        // DEBUG
    ...
    for (let block of blocksToValidate) {
      ...
      if (validBlock) {
        ...
        this._logger.warn(`ignoring block part of fetch cache ${blockNumber}, override block transmission...`);
        unused.push(block.header);
      } else {
    ...
    const uniqueHeaders = [];
    const blocksToValidateFromArchive = [];
    ...
    if (!found) {
      while (msgBroker.archive.length > 0) {
        const header = msgBroker.archive.shift();
        if (!header === false) {
          try {
            let pass = false;
            const blockNumber = new BN(header.number).toNumber();
            ...
            //  continue
            //}
            blocksToValidateFromArchive.push(block);
          } catch (err) {
            // catch malformed blocks
            debug(err);
            this._logger.warn(`block considered malformed <- dismiss ETH peer`);
      ...
      for (let block of blocksToValidateFromArchive) {
        const validBlock = await isValidBlock(block);
        const blockHashHex = block.header.hash().toString('hex');
        const blockNumber = new BN(block.header.number).toNumber();
        let pass = false;
