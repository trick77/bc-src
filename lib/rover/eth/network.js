...
const validateBlockFromData = async (payload, lookupBlock = false) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const validA = new EthereumBlock(data);
      const validB = EthereumBlockContainer.Block.fromValuesArray(data);
      ...
      const checkUncles = await validateUncles(validA, validB);
      if (!checkUncles) {
      ...
      const checkTrie = !lookupBlock ? await validateTrie(validA, validB, lookupBlock) : true;
      if (!checkTrie) {
...
class Network extends EventEmitter {
  ...
  constructor(config) {
    ...
    const restartTime = getRandomRange(748800000, 1497600000);
  ...
  requestBlockRange(blockRange) {
    ...
    if (this._blocksToFetch.length < 1 && msgBroker.bodies.length < 1) {
      this._logger.info(`range request queued for active sync ${lowest} -> ${highest} (${highest - lowest}) pending ${msgBroker.bodies.length} - 1`);
      msgBroker.bodies.unshift();
      this._blocksToFetch.push([highest, lowest]);
    ...
    try {
      ...
      while (msgBroker.bodies.length > 0) {
        ...
        if (!header === false) {
          ...
          const data = [raw];
          const validBlock = new EthereumBlock(raw);
          ...
          const syncBlock = blockNumber + 90 < fiberHeight;
      ...
      if (unusedFullBlocks.length > 0) {
        for (let b of unusedFullBlocks) {
          ...
          if (blockHeight + 120 < fiberHeight && blockHeight !== foundBlockHeight) {
            msgBroker.lastUpdate = Math.floor(new Date() / 1000);
