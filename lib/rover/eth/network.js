  ...
  constructor(config) {
    ...
    this._syncCheckTimeout = setInterval(async () => {
      ...
      if (this._bestSeenBlock) {
        ...
        if (!this._seekingBlockSegment && this._reportLatestBlockHeight && this._blocksIndexed > 0 && this._reportLatestBlockHeight !== blockNumber) {
          debug(`requesting next segment from ${this._reportLatestBlockHeight}`);
          await this.getBlockchain(this._reportLatestBlockHeight, this._reportLatestBlockHeight + 30);
          return;
        } else {
    ...
    try {
      ...
      if (DAO_FORK_SUPPORT && !msgBroker.validPeer[peerAddr]) {
        ...
        const cycleHeaders = async () => {
          ...
          this._seekingBlockSegment = false;
          if (headers.length > 0 && returnedBlock) {
            cycleHeaders();
          } else {
