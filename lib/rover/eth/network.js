...
class Network extends EventEmitter {
  ...
  broadcastMessage(message, messageBody, peersToUse = 1) {
    ...
    if (peers.length >= WAIT_FOR_PEERS) {
      ...
      // DEBUG
      let i = 0;
      for (const peer of this._rlpx.getPeers()) {
        //i++
        //if (i <= peersToUse) {
      ...
      const _cycleGetBlock = () => {
        ...
        if (cyclePeers.length >= WAIT_FOR_PEERS) {
          ...
          if (peer) {
            ...
            eth.sendMessage(devp2p.ETH.MESSAGE_CODES.GET_BLOCK_BODIES, [header.hash().toString('hex')]);
          } else {
            debug(`GET_BLOCK_BODIES broadcast for ${headerHash}`);
            this.broadcastMessage(devp2p.ETH.MESSAGE_CODES.GET_BLOCK_BODIES, [header.hash().toString('hex')], 1);
          }
        }
      };
      const intervalRetry = setInterval(() => {
    ...
    try {
      ...
      if (DAO_FORK_SUPPORT && !msgBroker.validPeer[peerAddr]) {
        ...
        if (header.hash().toString('hex') === expectedHash) {
          ...
          if (!this._seekingBlockSegment || this._blocksIndexed === 0) {
            if (this._bestSeenBlock && this._blocksIndexed < 3) {
              ...
              debug(`-------------------------------------`);
              //debugawait this.getBlockchain(this._climbBlockHeight, this._climbBlockHeight + 20, peer)
            }
          } else {
            debug(`-------------CANDIDATE---------------`);
            debug(`${peerAddr}`);
        ...
        if (directRequest.length > 0) {
          //while (headers.length > 0 && returnedBlock) {
          for (const header of headers) {
        ...
        if (lowestNumber && returnedBlock && pendingRequest.length > 0) {
