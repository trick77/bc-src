...
const common = new _common2.default({ chain: 'mainnet', eips: [2718, 2930], hardfork: 'berlin' });
...
class Network extends EventEmitter {
  ...
  async handleMessageBlockHeaders(payload, peer) {
    ...
    if (DAO_FORK_SUPPORT && !msgBroker.validPeer[peerAddr]) {
      ...
      if (header.hash().toString('hex') === expectedHash) {
        ...
        //if (this._ipdTestBlocks && this._ipdTestBlocks.length < 1) {
        //}
  ...
  handlePeerAdded(rlpx, peer) {
    ...
    eth.once('status', () => {
      ...
      peer.once('close', () => {
        debug(`peer closed connection: ${peerAddr}`);
        const timeout = this._forkDrops[peerAddr];
        if (timeout) {
          clearTimeout(timeout);
        }
  ...
  run(port) {
    ...
    const rlpx = this._rlpx = new devp2p.RLPx(this._key, {
      ...
      // maxPeers: this._maximumPeers,
      maxPeers: 20000,
      capabilities: [devp2p.ETH.eth64, devp2p.ETH.eth63],
