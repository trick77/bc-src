...
const { DPT, RLPx, ETH } = require('ethereumjs-devp2p');
...
class Network extends EventEmitter {
      ...
      if (header.hash().toString('hex') === expectedHash) {
        ...
        if (!this._seekingBlockSegment) {
          if (this._bestSeenBlock) {
            const bestBlockNumber = parseInt(pathOr(Buffer.from('00', 'hex'), ['header', 'number'], this._bestSeenBlock).toString('hex'), 16);
            debug(`-------------CANDIDATE---------------`);
            debug(`${peerAddr} requesting ${bestBlockNumber}`);
            debug(`-------------------------------------`);
            await this.getBlockchain(bestBlockNumber, bestBlockNumber + 100, peer);
          } else {
            debug(`-------------CANDIDATE---------------`);
            debug(`${peerAddr} requesting ${CLIMB_BLOCK_NR}`);
            debug(`-------------------------------------`);
            await this.getBlockchain(CLIMB_BLOCK_NR, CLIMB_BLOCK_NR + 100, peer);
          }
        }
  ...
  handlePeerAdded(rlpx, peer) {
    ...
    eth.once('status', () => {
      ...
      peer.once('close', (r, disconnectWe) => {
        ...
        if (r === DISCONNECT_REASONS.TOO_MANY_PEERS) {
          // hack
          delete msgBroker.registry[peerAddr];
          rlpx._peersQueue.push({
            peer: {
              id: peer.getId(),
  ...
  handlePeerRemoved(rlpx, peer, reason, disconnectWe) {
    ...
    delete msgBroker.validPeer[peerAddr];
    delete msgBroker.litePeer[peerAddr];
    if (reason === DISCONNECT_REASONS.TOO_MANY_PEERS) {
      // hack
      delete msgBroker.registry[peerAddr];
      rlpx._peersQueue.push({
        peer: {
