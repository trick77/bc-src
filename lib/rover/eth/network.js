...
const commonBerlin = new _common2.default({
  ...
  hardfork: 'berlin',
  supportedHardforks: ['berlin', 'istanbul', 'homestead', 'byzantium', 'petersburg', 'constantinople', 'spuriousDragon', 'dao', 'chainstart', 'london']
});
...
class Network extends EventEmitter {
  ...
  async getBlockchain(from, to, peer = false) {
    ...
    if (!headers || headers.length < 1) {
      this._logger.info(`headers not received for range ${from}`);
      msgBroker.bodies.shift();
      this._seekingBlockSegment = false;
