...
const isValidBlock = (block, strict = true) => {
  try {
    ...
    if (!block.transactions.every(isValidTx)) {
      debug(`isValidBlock(): block ${new BN(block.header.number).toNumber()} with ${block.transactions.length} txs, invalid txs!`);
      // DEBUG
      return Promise.resolve(false);
    }
...
const isValidBlockTrie = (blockWithTrie, strict = true) => {
  try {
    debug(`isValidBlockTrie(): block ${new BN(blockWithTrie.header.number).toNumber()}`);
    if (blockWithTrie.validateTransactionsTrie()) {
      return Promise.resolve(blockWithTrie);
    } else {
      debug(`isValidBlockTrie(): block ${new BN(blockWithTrie.header.number).toNumber()}, invalid trie!`);
      return Promise.resolve(false);
    }
  } catch (err) {
...
class Network extends EventEmitter {
  ...
  constructor(config) {
    ...
    this._syncCheckTimeout = setInterval(() => {
      ...
      if (now - msgBroker.lastUpdate > 1500) {
  ...
  requestBlockRange(blockRange) {
    ...
    if (this._seekingBlockSegment) {
      ...
      for (let i = blockRange[1] - 1; i < blockRange[0] + 1; i++) {
  ...
  sync() {
    ...
    if (msgBroker.bodies.length > 100 && this._blocksToFetch.length < 1) {
      msgBroker.bodies = msgBroker.bodies.slice(msgBroker.bodies.length - 100, msgBroker.bodies.length);
    }

    if (peers.length >= WAIT_FOR_PEERS && !this._seekingBlockSegment && msgBroker.bodies < 30 && this._blocksToFetch) {
    ...
    try {
      ...
      while (headers.length > 0) {
        ...
        if (new BN(blockTimestamp).lt(new BN(blockTimeThreshold)) && !this._fetchCache.has(blockNumber)) {
          ...
          if (stored) {
            this.emit('newBlock', {
              block: this.storage.get(header.hash().toString('hex')),
              isBlockFromInitialSync: false
            });
          } else {
  ...
  async handleMessageBlockBodies(payload, peer, recheck = false) {
    ...
    msgBroker.bodies = msgBroker.bodies.concat(msgBroker.archive);
    msgBroker.bodies = msgBroker.bodies.reduce((all, header) => {
      const h = header.hash().toString('hex');
      if (uniqh.indexOf(h) < 0) {
        uniqh.push(h);
        all.push(header);
      }
      return all;
      ...
      if (!header === false) {
        try {
          ...
          if (valid) {
            ...
            if (blockTrie) {
              debug(` ${blockNumber} trie generated`);
              const v = await isValidBlockTrie(blockTrie);
              let pass = false;
              ...
              if (v) {
                // DEBUG
                found.push(blockHashHex);
                // Get the block with the txTrie out of the LRUCache
                this.onNewBlock(blockTrie, peer, pass);
              } else {
                unused.push(header);
              }
            } else {
    ...
    const uniq = [];
    msgBroker.archive = msgBroker.archive.concat(unused);
    msgBroker.archive = msgBroker.archive.reduce((all, header) => {
      const h = header.hash().toString('hex');
      if (uniq.indexOf(h) < 0 && found.indexOf(h) < 0) {
        uniq.push(h);
        all.push(header);
      }
      return all;
    ...
    if (found.length === 0) {
      if (msgBroker.bodies.length > 30) {
        msgBroker.bodies.shift();
      }
      msgBroker.payloads.push(payload);
    } else {
      if (msgBroker.payloads.length > 0 && msgBroker.archives.length > 0) {
        const p = msgBroker.payloads.shift();
        return this.handleMessageBlockBodies(p, peer, true);
      }
    }

    if (this._blocksToFetch.length < 1) {
      setTimeout(() => {
        this.sync();
      }, 15000);
    }
        ...
        while (headers.length > 0) {
          ...
          if (!this._fetchCache.has(blockNumber) && new BN(this._lowestBlockHeight).gt(new BN(blockNumber)) && !this.storage.has(headerHash) && new BN(blockTimestamp).gt(new BN(blockTimeThreshold))) {
            ...
            if (this.storage.has(headerHash) && pass && !this._blocksArchive[headerHash]) {
              const sb = this.storage.get(headerHash);
              if (sb && sb.transactions && sb.transactions.length > 0) {
                this.onNewBlock(sb, peer, pass);
              }
            }
          }
        }
    ...
    if (this._ipdTestBlocks.length >= ETH_IPD_TEST_BLOCKS && !this._ipdTestComplete && !forceBlock) {
      ...
      try {
        ...
        if (valid) {
          ...
          const blockTrie = await generateTxTrie(newBlock);
          // XXX
          debug(`valid block received ${blockNumber}`);
          if (blockTrie) {
            const trie = await isValidBlockTrie(blockTrie);
            if (trie) {
              console.log(`valid block from eth peer ${blockNumber}`);
              this.onNewBlock(newBlock, peer, pass);
            } else if (this._fetchCache.has(blockNumber) && this._blocksToFetch.length < 0) {
              this._logger.warn(`passing header of fetch cache ${blockNumber} <- override block transmission`);
              peer && peer.disconnect && peer.disconnect();
            }
          } else {
            debug(`block from eth peer ${blockNumber} failed trie`);
            return;
          }
        } else {
    ...
    this.on('compressBlock', block => {
      try {
        if (block && block.header && block.header.hash) {
          ...
          if (archive) {
            logger.info(`block ${num} compressed ${hash}`);
