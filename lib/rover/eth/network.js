...
const ETH_MAX_FETCH_BLOCKS = config.rovers.maxFetchBlocks || 10;
...
const isValidBlock = (block, oldBlock = false) => {
  try {
...
const validateBlock = (legacyBlock, syncBlock = false, headerData) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      //console.log(Object.keys(legacyBlock))
      //console.log(22222222)
      //console.log(Object.keys(legacyBlock.header))
      //if (headerData) {
      //  console.log(`items in header: ${headerData.length}`)
      //  console.log(`items in first: ${headerData[0].length}`)
      //  console.log(`keys in first: ${Object.keys(headerData[0])}`)

      //}
      const block = new EthereumBlockContainer.Block(legacyBlock.header, legacyBlock.transactions, legacyBlock.uncleHeaders);
      const height = new BN(block.header.number).toNumber();
      const validBlockFramework = await isValidBlock(block, syncBlock);
...
class Network extends EventEmitter {
    ...
    try {
      ...
      while (msgBroker.bodies.length > 0) {
        ...
        if (!header === false) {
          ...
          const blockHashHex = block.header.hash().toString('hex');
          const syncBlock = blockNumber + 50 < fiberHeight || BC_MINER_MUTEX;
      ...
      if (unusedFullBlocks.length > 0) {
        for (let b of unusedFullBlocks) {
          ...
          if (blockHeight + 80 < fiberHeight && blockHeight !== foundBlockHeight) {
            ...
            //const darkFiberBlock = new EthereumBlockContainer.Block({ header: b.header.raw, transactions: [], uncleHeaders: [] })
  ...
  async handleMessageNewBlock(payload, peer, forceBlock) {
    ...
    if (this._ipdTestBlocks.length < ETH_IPD_TEST_BLOCKS && !forceBlock) {
      ...
      await validateBlock(state.payload, false, payload);
    ...
    if (this._ipdTestBlocks.length >= ETH_IPD_TEST_BLOCKS && !this._ipdTestComplete && !forceBlock) {
      ...
      // XXX
      await validateBlock(block, false, payload);
      ...
      try {
        ...
        const syncBlock = BC_MINER_MUTEX;
        const validBlock = await validateBlock(newBlock, syncBlock);
