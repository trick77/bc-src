...
const getNewEthereumBlockFromData = payload => {
  const hbsr = new EthereumBlock(payload[0]);
  try {
    let returnBlock = false;
    const uncleHeaders = hbsr.uncleHeaders.map(h => {
      return new EthereumBlock.Header(h);
    });
    returnBlock = new EthereumBlockContainer.Block(hbsr.header, hbsr.transactions, uncleHeaders);
    return returnBlock;
  } catch (err) {
    debug(err);
    return hbsr;
...
const ETH_MAX_FETCH_HEADERS = config.rovers.maxFetchHeaders || 50;
...
const isValidBlock = (legacyBlock, block, oldBlock = false, directSend = false) => {
  try {
    ...
    if (oldBlock) {
      if (directSend) {
        debug(`isValidBlock(): block ${new BN(block.header.number).toNumber()} with ${block.transactions.length} txs is considered an old block`);
      }
...
const validateBlock = (legacyBlock, syncBlock = false, headerData, directSend) => {
  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const uncleHeaders = [];
      legacyBlock.uncleHeaders.forEach(h => {
...
class Network extends EventEmitter {
  ...
  requestBlockRange(blockRange) {
    ...
    highest = min(lowest + ETH_MAX_FETCH_BLOCKS + 10, highest);
  ...
  async handleMessageBlockBodies(payload, peer, recheck = false) {
    ...
    try {
      ...
      let pass = false;
      const found = [];
      let fiberHeight = new BN(this._ipdTestBlocks[0].payload.header.number).toNumber();
      let fiberBlock = true;
      ...
      while (msgBroker.bodies.length > 0) {
        ...
        if (!header === false) {
          ...
          const blockHashHex = block.header.hash().toString('hex');
          const syncBlock = blockNumber + 90 < fiberHeight || BC_MINER_MUTEX;
          ...
          if (validBlock) {
            ...
            unused.push(header);
            if (blockNumber + 50 < fiberHeight && Object.keys(this._fiberBlocks).indexOf(blockNumber) < 0) {
              unusedFullBlocks.push(block);
            }
          }
        } else {
          unused.push(header);
      ...
      if (unusedFullBlocks.length > 0) {
        for (let b of unusedFullBlocks) {
          ...
          if (blockHeight + 80 < fiberHeight && blockHeight !== foundBlockHeight) {
            msgBroker.lastUpdate = Math.floor(new Date() / 1000);
            const darkFiberBlock = new EthereumBlock([b.header.raw, null, null]);
            delete this._fiberBlocks[blockHeight];
            this.onNewBlock(darkFiberBlock, peer, true);
  ...
  async handleMessageNewBlock(payload, peer, forceBlock) {
    ...
    if (this._ipdTestBlocks.length < ETH_IPD_TEST_BLOCKS && !forceBlock) {
      ...
      const hb = getNewEthereumBlockFromData(payload);
      const b = new EthereumBlock(payload[0]);
      const state = {
        payload: b,
        peer: peer,
        sent: 1
      };
      const vb = await validateBlock(state.payload, false);
      if (!vb) {
    ...
    if (this._ipdTestBlocks.length >= ETH_IPD_TEST_BLOCKS && !this._ipdTestComplete && !forceBlock) {
      // run the IPD test
      const block = getNewEthereumBlockFromData(payload);
      const vb = await validateBlock(block, false);
      if (!vb) {
        let h = "";
        let bh = "NA";
        ...
        this._logger.info(`unable to process block ${bh}:${h.slice(0, 12)}...disconnecting ETH peer`);
        peer && peer.disconnect && peer.disconnect(RLPx.DISCONNECT_REASONS.USELESS_PEER);
        return;
      }
      ...
      try {
        ...
        let validationBlock = newBlock;
        if (!newBlock) {
          validationBlock = getNewEthereumBlockFromData(payload);
        ...
        this._logger.info(`discovered block ${blockNumber}:${headerHash}`);
        ...
        const validBlock = await validateBlock(validationBlock, false, null, true);
        ...
        if (validBlock) {
          ...
          this._logger.info(`block data structure could not be constructed ${bh}:${h.slice(0, 12)}...disconnecting ETH peer`);
          peer && peer.disconnect && peer.disconnect(RLPx.DISCONNECT_REASONS.USELESS_PEER);
          return;
        }
      } catch (err) {
