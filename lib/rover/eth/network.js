...
class Network extends EventEmitter {
  ...
  constructor(config) {
    ...
    const blockCacheConf = {
      max: 250, // full 3 hours of ETH blocks
      maxAge: 3 * 60 * 60 * 1000,
      noDisposeOnSet: true,
      dispose: (key, val) => {
        const k = String(key);
    ...
    this._syncCheckTimeout = setInterval(() => {
      ...
      if (now - msgBroker.lastUpdate > 11130) {
  ...
  getBlock(header, returnHeaderOnError = false) {
    ...
    return new Promise(resolve => {
      ...
      }, getRandomRange(ETH_INTERVAL_GET_BLOCKS_MS, ETH_INTERVAL_GET_BLOCKS_MS + 2800));
  ...
  run(port) {
    ...
    this._dpt = new DPT(this._key, {
      refreshInterval: 99000,
      timeout: 9900,
      maxPeers: 300,
      shouldGetDnsPeers: true,
      dnsRefreshQuantity: 32,
      endpoint: {
        address: '0.0.0.0',
        udpPort: null,
        tcpPort: null
    ...
    setInterval(async () => {
      ...
      if (!this._seekingBlockSegment && this._bestSeenBlock && this._blocksIndexed > 0) {
        ...
        await this.getBlockchain(bestBlockNumber, bestBlockNumber + 10);
      }
    }, 6000);
