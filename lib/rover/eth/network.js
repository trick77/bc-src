...
const getNewEthereumBlockFromData = payload => {
  try {
    ...
    returnBlock = EthereumBlockContainer.Block.fromValuesArray(payload[0]);
    return returnBlock;
  } catch (err) {
    debug(err);
...
const ETH_MAX_FETCH_BLOCKS = config.rovers.maxFetchBlocks || 30;
const ETH_MAX_FETCH_HEADERS = config.rovers.maxFetchHeaders || 30;
  ...
  async handleMessageNewBlock(payload, peer, forceBlock) {
    ...
    if (this._ipdTestBlocks.length >= ETH_IPD_TEST_BLOCKS && !this._ipdTestComplete && !forceBlock) {
      ...
      if (!vb) {
        ...
        this._logger.info(`unable to process block ${bh}...disconnecting ETH peer`);
        peer && peer.disconnect && peer.disconnect(RLPx.DISCONNECT_REASONS.USELESS_PEER);
        return;
      }
      ...
      try {
        ...
        `discovered block ${blockNumber}:${headerHash}`;
