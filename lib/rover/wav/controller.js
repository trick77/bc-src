...
class Controller {
  ...
  async fetchBlock(previousLatest, currentLatest) {
    ...
    if (to - from > WAV_MAX_FETCH_BLOCKS) {
      this._logger.warn(`decreasing requested range of ${to - from} blocks to waves max ${WAV_MAX_FETCH_BLOCKS + 10}`);
      to = from + WAV_MAX_FETCH_BLOCKS + 10;
    }
