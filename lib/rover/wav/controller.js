...
const logger = getLogger('wav.controller.util', false);
const getRandomPeer = exports.getRandomPeer = function () {
  return new Promise((resolve, reject) => {
    for (let p of peerFailed) {
      if (Array.isArray(peerList) && peerList.includes(p)) {
        peerList = false;
      }
    }
    ...
    if (peerList) {
      ...
      request({
        ...
        if (err) {
          ...
          if (Array.isArray(peerList) && !peerList.includes(WAVES_NODE_ADDRESS)) {
        ...
        try {
          ...
          logger.warn('Could not parse data, body = %o', body);
          return resolve(WAVES_NODE_ADDRESS);
        }
      });
    }
...
const getBlock = function (height) {
  return new Promise(function (resolve, reject) {
    ...
    wavEvents.once(id, (block, context) => {
      ...
      if (!block) {
        logger.debug(`unable to connect to waves peer host`);
        process.exit();
      } else {
        resolve(block);
      }
...
async function _createUnifiedBlock(roverRpc, block, isStandalone) {
  ...
  try {
    ...
    logger.warn('Error occured while checking settlement or queued marked txs, e = %o', e);
  }
}
