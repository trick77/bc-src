      ...
      getRandomPeer().then(peerAddr => {
        request({
          ...
          try {
            ...
            peerList = false;
            debug(`wav centralized peer connection failed: ${peerAddr}`);
            debug(e);
            peerFailed.unshift(); // cycle through peers who may have lost the connection
            peerFailed.push(peerAddr);
            wavEvents.emit(id, false);
          }
        });
      }).catch(err => {});
...
class Controller {
  ...
  init() {
    ...
    this._checkFibersIntervalID = setInterval(checkFibers, randomInt(2000, 3900));
  }
  ...
  async fetchBlock(previousLatest, currentLatest) {
    ...
    if (to - from > WAV_MAX_FETCH_BLOCKS) {
      this._logger.warn(`decreasing requested range of ${to - from} blocks to waves max ${WAV_MAX_FETCH_BLOCKS + 2}`);
