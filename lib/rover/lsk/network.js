...
class Network extends Emittery {
  ...
  constructor(client, { networkName = 'mainnet', seedPeers = [] } = {}) {
    ...
    this._latestBlockCache = new LRUCache({
      max: 500
    });
  ...
  _configureP2P() {
    ...
    debug(`configuring P2P`);
    this._p2p.on(EVENT_REQUEST_RECEIVED, request => {
      //this._logger.debug('event = %s, %O', EVENT_REQUEST_RECEIVED, request)
      debug('event = %s, %O', EVENT_REQUEST_RECEIVED, request);
    });
    // Listen to message events
    this._p2p.on(EVENT_MESSAGE_RECEIVED, async message => {
      message.event === 'postNodeInfo' ? debug('event = %s', EVENT_MESSAGE_RECEIVED) : debug('event = %s, %O', EVENT_MESSAGE_RECEIVED, message);
      ...
      if (message.event === 'postBlock') {
        try {
          ...
          debug('postblock event = %O, block buffer size = %d', event, event.block.length);
