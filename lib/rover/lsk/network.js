...
const networks = {
  ...
  //mainnet: '23ce0366ef0a14a91e5fd4b1591fc880ffbef9d988ff8bebf8f3666b0c09597d', // TODO take from lisk config
  //mainnet: '01e47ba4e3e57981642150f4b45f64c2160c10bac9434339888210a4fa5df097', // TODO take from lisk config
  //mainnet: 'ed14889723f24ecc54871d058d98ce91ff2f973192075c0155ba2b7b70ad2511', // TODO take from lisk config
...
async function createNetwork(config) {
  ...
  let ips = [{
    'ipAddress': '51.15.232.205',
    'port': 8001
  }, {
    'ipAddress': '51.15.60.27',
    'port': 8001
  }, {
    'ipAddress': '84.201.187.188',
    'port': 8001
  }, {
    'ipAddress': '91.244.248.89',
    'port': 8001
  }, {
    'ipAddress': '185.185.127.43',
    'port': 8001
  }, {
    'ipAddress': '157.245.150.22',
    'port': 8001
  }, {
    'ipAddress': '51.79.120.195',
    'port': 8001
  }, {
    'ipAddress': '168.119.180.26',
    'port': 8001
  }, {
    'ipAddress': '139.59.134.79',
    'port': 8001
  }];
  ...
  //const client = await apiClient.createWSClient('wss://mainnet-api.lisktools.eu/ws')
  //const client = await apiClient.createWSClient('wss://service.lisk.com/rpc-v2')
  //const io = require('socket.io-client');
  //const connection = io.connect('wss://service.lisk.com/blockchain', { transports: ['websocket'] });
  //connection.on('update.block', (block) => { (...) });
  //connection.on('update.transactions', (block) => { (...) });

  debug(`client launched`);

  seedPeers = ips;

  moduleRootLogger.info('client launched');
  //const n = new Network(client, config)
...
class Network extends Emittery {
  ...
  _configureP2P() {
    ...
    this._p2p.on(EVENT_REQUEST_RECEIVED, request => {
      ...
      this._logger.info('event = %s, %O', EVENT_REQUEST_RECEIVED, request);
    });
    // Listen to message events
    this._p2p.on(EVENT_MESSAGE_RECEIVED, async message => {
      message.event === 'postNodeInfo' ? this._logger.info('event = %s', EVENT_MESSAGE_RECEIVED) : this._logger.info('event = %s, %O', EVENT_MESSAGE_RECEIVED, message);
      //? this._logger.debug('event = %s', EVENT_MESSAGE_RECEIVED)
      //: this._logger.debug('event = %s, %O', EVENT_MESSAGE_RECEIVED, message)

      if (message.event === 'postBlock') {
        try {
          const event = codec.decode(postBlockEventSchema, message.data);
          this._logger.info('postblock event = %O, block buffer size = %d', event, event.block.length);
      ...
      if (message.event === 'postTransactionsAnnouncement') {
        debug('postTransactionsAnnouncement');
      } else {
        debug(message.event);
      }
    });
    ...
    this._p2p.on(EVENT_FAILED_TO_ADD_INBOUND_PEER, inboundFailedError => {
      debug('event = %s, %O', EVENT_FAILED_TO_ADD_INBOUND_PEER, inboundFailedError);
    });
    this._p2p.on(EVENT_NETWORK_READY, () => {
      debug('event = %s', EVENT_NETWORK_READY);
    });
    this._p2p.on(EVENT_DISCOVERED_PEER, peerInfo => {
      debug('event = %s, peer = %O', EVENT_DISCOVERED_PEER, peerInfo);
    });
  ...
  async start() {
    ...
    debug('P2P node is running successfully');
  }

  async stop() {
    await this._p2p.stop();
    debug('P2P node stopped');
  }
  ...
  async getBlocksSinceHash(blockId) {
    ...
    debug('got raw requested blocks = %O', decodedResponse);
    const decodedBlocks = [];
    for (const rawBlock of decodedResponse.blocks) {
      debug('getBlocksSinceHash() block buffer size = %d', rawBlock.length);
      decodedBlocks.push(decodeBlock(false, this._logger, rawBlock));
    }

    debug('got %d requested blocks', decodedBlocks.length);
