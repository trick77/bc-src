...
class Controller {
  ...
  init() {
    ...
    this._intervalDescriptor = setInterval(() => {
      if (skip.length > 0) {
        ...
        this._logger.debug(`flushing request pool <-  ${skip.length} remaining est. ${estTime} ${t}`);
        skip.pop();
      } else {
        this._cycleFn().then(() => {
          debug('tick');
  ...
  async _httpApi(method, opts, data) {
    ...
    const urls = ['http://192.99.228.99:8000/api/', 'http://104.237.9.142:8000/api/', 'http://104.238.167.150:8000/api/', 'http://116.203.63.37:8000/api/', 'http://116.203.73.30:8000/api/', 'http://157.90.168.16:8000/api/', 'http://159.69.154.132:8000/api/', 'http://160.16.139.105:8000/api/', 'http://192.154.224.145:7000/api/', 'http://192.99.228.99:8000/api/', 'http://2.56.213.101:8000/api/', 'http://23.88.45.135:8000/api/', 'http://5.9.99.62:8000/api/', 'http://51.158.182.3:4000/api/', 'http://78.46.147.196:8000/api/', 'http://80.240.30.26:8000/api/'];
