...
async function _createUnifiedBlock(roverRpc, block, isStandalone) {
  ...
  for (let tx of block.transactions) {
    ...
    // debug({tx})
    const addrFrom = tx.senderId;
    const addrTo = tx.recipientId;
    const value = new BN(tx.amount).toBuffer();
...
class Controller {
  ...
  init() {
    ...
    rpcStream.on('data', message => {
      ...
      switch (message.getType()) {// Also could be message.getPayloadCase()
        ...
        case RoverMessageType.ROVER_BLOCK_RANGE:
          // DEBUG
          this._logger.debug(`open block range request arrived`);
    ...
    this._cycleFn = getOpts => {
      ...
      const opts = merge({ offset: 0, limit: 1 }, getOpts);
      return this._request('blocks', opts).then(async lastBlocks => {
  ...
  async _httpApi(method, opts, data) {
    ...
    const url = "https://node.lisk.io/api/" + method + qs;
  ...
  startResync(resyncMsg) {
    this._request('blocks').then(lastBlocks => {
      ...
      if (resyncMsg && !isEmpty(resyncMsg.getIntervalsList())) {
        ...
        const from = to - (Math.floor(ROVER_RESYNC_PERIOD / ROVER_SECONDS_PER_BLOCK['lsk']) - 2);
        const boundaries = rangeStep(from, LSK_MAX_FETCH_BLOCKS, to);
...
Total: ${taskDescriptors.length}, success: ${successCount}`);
  ...
  requestBlockRange(blockRange) {
    ...
    if (this._rangeToFetch.length > 0) {
      ...
      if (prevHigh < highest || prevLow > lowest) {
        // DEBUG
        this._logger.debug(`updated block range prevHigh: ${prevHigh} -> highest: ${r[0]} prevLow: ${prevLow} -> lowest ${lowest}`);
    ...
    if (this._rangeToFetch.length > 0) {
      ...
      toBlock.setHeight(range[0]);
      fromBlock.setHeight(range[1]);
      //this.startResync()
  ...
  fetchBlock(currentLast, previousLast) {
    ...
    if (to - from > LSK_MAX_FETCH_BLOCKS) {
      this._logger.warn(`would fetch ${to - from} blocks but LSK can't handle that load, fetching only ${LSK_MAX_FETCH_BLOCKS}`);
      to = from + LSK_MAX_FETCH_BLOCKS;
    }
    const whichBlocks = range(from, to);
    if (to - from > 0) {
      // DEEBUG
      this._logger.debug(`fetching missing blocks ${whichBlocks}`);
      const opts = { offset: max(0, to - from), limit: max(1, to - from) };
      this._cycleFn(opts).then(() => {
        // DEBUG
        this._logger.debug(`Fetched missing blocks ${whichBlocks}`);
        this._seekingBlockSegment = false;
        //this._rpc.rover.reportSyncStatus(new RoverSyncStatus(['lsk', true]), (_, res) => {
