...
const { concat, flatten, isEmpty, merge, max } = require('ramda');
...
const LSK_MAX_FETCH_BLOCKS = 103;
...
async function _createUnifiedBlock(roverRpc, block, isStandalone) {
  ...
  for (let tx of transactions) {
    ...
    if (isEmbTx) {
      ...
      logger.debug('addrFrom = %s addrTo = %s value = %d bridgedChain = %s txId = %s blockHeight = %d tokenType = %s', addrFrom, addrTo, value, bridgedChain, txId, blockHeight, tokenType);
    }
  }
  ...
  try {
    ...
    logger.debug('error in isBeforeSettleHeight or getQueuedMarkedTxs, e = %O', e);
  }
}
...
class Controller {
  ...
  constructor(config) {
    this._fixedIntervalFetched = false;
    this._config = config;
    ...
    this._logger.debug('network config = %O', networkConfig);
  ...
  async init() {
    this._logger.debug('controller init start');
        ...
        case RoverMessageType.REQUESTRESYNC:
          {
            ...
            this._logger.debug('Got resync message %O', payload.toObject());
        ...
        case RoverMessageType.ROVER_BLOCK_RANGE:
          {
            ...
            this._logger.debug('open block range request arrived, %O', data.toObject());
    ...
    this._network.on('newLatestBlock', async latestBlock => {
      this._logger.debug('got new latest block from network = %O', latestBlock);
      if (this._fixedIntervalFetched === false) {
        setTimeout(() => {
          this._logger.info('Fetching abandoned LSK interval');
          this._fixedIntervalFetched = true;
          const lowest = new IntervalLimit(17327279, 'a4586c0ed196e235efe706608778eb50a382c8116209a84ddeb7a3fd227de3f8');
          const highest = new IntervalLimit(17327282, '2fc48cdb0b0b937245b58b3ce5a5e69dac671dcfb53ef4a864aee4cfd7a2c31a');
      ...
      this._logger.debug('LSK Going to call this._rpc.rover.collectBlock() with block %o', latestBlock.header.height);
      if (!this._config.isStandalone) {
    ...
    this._network.on('blocksSinceHash', async ({ blockId, blocks }) => {
      this._logger.info('fetched %d blocks since hash %s, first block = %o', blocks.length, blockId.toString('hex'), blocks[0]);
      const highestBlock = blocks[0]; // blocks come from network in highest height -> lowest height order
      ...
      for (const block of blocks) {
        ...
        this._logger.warn('LSK Going to call this._rpc.rover.collectBlock() with from blockSinceHash, block.height = %d', block.header.height);
        if (!this._config.isStandalone) {
          writeBlock(this._blockStream, unifiedBlock);
        } else {
          this._logger.info('Rovered unified LSK block = %O', unifiedBlock.toObject());
      ...
      if (!this._fetchIntervalStore.isSeekingBlocks()) {
    ...
    this._logger.debug('controller initialized');
  }
  ...
  fetchBlock(highest, lowest) {
    ...
    this._logger.info(`fetchBlock() from = %o to = %o`, lowest, highest);
  ...
  close() {
    this._network.stop().then(() => {
      this._logger.debug('network module stopped successfully');
    }).catch(e => {
      this._logger.warn('error occured while stopping network module, e = %O', e);
    });
