...
const LSK_MAX_FETCH_BLOCKS = exports.LSK_MAX_FETCH_BLOCKS = 10;
...
class Controller {
  ...
  constructor(config) {
    ...
    if (this._config.isStandalone) {
      ...
      this._blockStream = this._rpc.rover.collectBlock((err, status) => {
        ...
        this._logger.info(`RPC stream closed`);
        console.trace(err);
      });
    }
  }
  ...
  async init() {
    ...
    this._network.on('blocksSinceHash', async ({ blockId, blocks, isRequestByHeight }) => {
      try {
        this._logger.info('event: blocksSinceHash fetched %d blocks since hash %s, first block = %o', blocks.length, blockId, blocks[0]);
  ...
  startResync(resyncMsg) {
    ...
    if (!this._highestBlockFromNetwork || this._highestBlockFromNetwork.getHeight) {
      setTimeout(() => {
        this.startResync(resyncMsg);
      }, 10 * 3000);
      return;
  ...
  requestBlockRange(blockRange) {
    ...
    let highestHeight = blockRange.getHighestHeight();
    ...
    highestHeight = min(LSK_MAX_FETCH_BLOCKS + lowestHeight, highestHeight);
    ...
    if (this._fetchIntervalStore.hasRange() && this._fetchIntervalStore.isSeekingBlocks()) {
      ...
      //this.fetchBlock(Number(highest), Number(lowest))
      //this.fetchBlock(highest, lowest)
      // TD
      //this._logger.info(`range not detected ${lowest} -> ${highest}`)
