...
const rovers = exports.rovers = {
  ...
  constructor(persistence, emitter, roverEmitter) {
    ...
    this._roverRangeRequests = Object.keys(rovers).reduce((all, k) => {
      all[k] = 0;
      return all;
    }, {});
    this._timeouts = {};
  ...
  async startRover(roverName, restarted = false, forceResync = false) {
    ...
    const eas = process.execArgv.filter(opt => opt !== '--inspect');
    eas.push('--max-old-space-size=3096');
    ...
    this._logger.info(`rover successfully deployed '${roverName}'`);
    ...
    rover.on('exit', async (code, signal) => {
      ...
      if (code !== ROVER_DF_VOID_EXIT_CODE && !BC_PREVENT_INITIAL_SYNC) {
        setTimeout(async () => {
          const shouldRestart = roverName !== 'wav' && roverName !== 'neo' && roverName !== 'lsk' && roverName !== 'btc';
          await this.startRover(roverName, shouldRestart);
  ...
  async messageRover(roverName, message, payload) {
    ...
    switch (message) {
      ...
      case 'latest_block':
        if (!payload) {
          throw new Error(`latest_block payload not provided <- ${payload} received`);
        }

        this._logger.info(`sending rover ${payload.getLatestBlock().getRoverName()} latest block`);
        roverRpc.write(payload);
        break;
