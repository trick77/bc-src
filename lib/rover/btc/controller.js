...
async function _createUnifiedBlock(roverRpc, block, isStandalone) {
  ...
  for (let tx of block.transactions) {
    ...
    try {
      ...
      logger.error(utilFormat('error while parsing XCP transaction %O', err));
    }
  ...
  try {
    ...
    logger.info(`USED MEM ${Math.round(process.memoryUsage().rss / 1024 / 1024 * 100) / 100}, BTC block processed height = ${msg.getHeight()}`);
    ...
    pool.on('peerblock', async (peer, _ref) => {
      try {
        ...
        if (network.bestHeight !== undefined) {
          ...
          this._logger.info(`USED MEM ${Math.round(process.memoryUsage().rss / 1024 / 1024 * 100) / 100}, before _onNewBlock, BTC block processed height = ${blockNumber}`);
          ...
          if (isNew || this._firstVerifiedHeaderHash === block.header.hash) {
            ...
            this._logger.info(`USED MEM ${Math.round(process.memoryUsage().rss / 1024 / 1024 * 100) / 100}, isNew == true, BTC block processed height = ${blockNumber}`);
