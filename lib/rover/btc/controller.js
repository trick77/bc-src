...
const BTC_NULL_HASH = "0000000000000000000610a27e5b49659c13ff2f7d4a2a1a343476111f0b53f2";
...
const BTC_SYNCHRONIZATION_STOP = exports.BTC_SYNCHRONIZATION_STOP = '000000000000000000091cffe11e1f959ef6181265cffe37c6adae8bcca54827'; // 670850
    ...
    rpcStream.on('data', message => {
      ...
      switch (message.getType()) {// Also could be message.getPayloadCase()
        ...
        case RoverMessageType.ROVER_BLOCK_RANGE:
          {
            if (this._initialBlocksToFetchCount && this._initialBlocksToFetchCount > 5) {
              this._logger.warn(`block range request made during active sync... count ${this._initialBlocksToFetchCount}`);
              return;
            }
            if (this._initialSync && this._initialSync > 0) {
              this._logger.warn(`block range request made during active sync... count ${this._initalSync}`);
              return;
            }
