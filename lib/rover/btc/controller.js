...
const { concat, contains, difference, isEmpty, last, none, pathOr, splitEvery, takeLast, without, xprod } = require('ramda');
...
const BLOCK_VERSIONS = [0x20000000, 0x20002000, 0x20640000, 0x2000e000, 0x203de000, 0x20400000, 0x20800000, 0x20ab4000, 0x20c00000, 0x210a6000, 0x2180c000, 0x2aee6000, 0x2ee5a000, 0x2fffc000, 0x2fffe000, 0x20eb6000, 0x2fff4000, 0x2fff8000, 0x320a0000, 0x33a8c000, 0x348a6000, 0x3ad36000, 0x3bae6000, 0x3fff0000, 0x3fffc000, 0x3fffe000, 0x68e34000, 0x7fffe000, 0x7fff0000, 0x7fffc000];
const BTC_MAX_FETCH_BLOCKS = Math.floor(ROVER_RESYNC_PERIOD / ROVER_SECONDS_PER_BLOCK['btc']) + 20;
...
const BTC_SYNCHRONIZATION_STOP = exports.BTC_SYNCHRONIZATION_STOP = '00000000000000000005c79ef2a2479ee388053211190d732aad2703367aba85'; // 618813
...
const decodeBlockHeightFromCoinbase = exports.decodeBlockHeightFromCoinbase = function (tx) {
  ...
  // if (script[0] !== bitcoin.Transaction.SIGHASH_SINGLE) {
  //  throw new Error(`TX ${tx.getId()} is not coinbase - script doesn't start with 0x03`)
  // }
...
async function _createUnifiedBlock(roverRpc, block, isStandalone) {
  ...
  for (let tx of block.transactions) {
    try {
      ...
      if (message !== undefined) {
        ...
        if (sourcePublicKey) {
          ...
          if (txDestination) {
            ...
            if (!quantity) {
              logger.debug(`unparsable XCP transaction ${tx.id}`);
              continue;
            }
      ...
      // const inputs = t.ins
      // const outputs = t.outs
      const inputs = t.ins.filter(input => bitcoin.script.classifyInput(input.script) === bitcoin.script.types.P2PKH);
      const outputs = t.outs.filter(output => bitcoin.script.classifyOutput(output.script) === bitcoin.script.types.P2PKH);

      let inputAddresses;
      try {
        // inputAddresses = inputs.map(input => bitcoin.ECPair.fromPublicKeyBuffer(bitcoin.script.pubKeyHash.input.decode(input.script).pubKey, bitcoin.networks.bitcoin).getAddress())
        inputAddresses = inputs.map(input => bitcoin.ECPair.fromPublicKeyBuffer(bitcoin.script.pubKeyHash.input.decode(input.script).pubKey, bitcoin.networks.bitcoin).getAddress());
        // const inputAddresses = t.ins.reduce((all, item, i) => {
        //  let pubKey = false
        //  const chunks = bitcoin.script.decompile(ins[0].script)
        //  if (bitcoin.script.isScriptHashInput(chunks)) {
        //    let hash = bitcoin.crypto.hash160(chunks[chunks.length - 1])
        //    pubKey = bitcoin.address.toBase58Check(hash, bitcoin.networks.bitcoin.scriptHash)
        //  } else {
        //    pubKey = bitcoin.ECPair.fromPublicKeyBuffer(chunks[1], bitcoin.networks.bitcoin).getAddress()
        //  }
        //  if (pubKey) all.push(pubKey)
        //  return all
        // }, [])
      } catch (e) {
        logger.error(e);
        logger.info(`ignoring unsupported tx input: ${tx.id}`);
        continue;
      }
      ...
      if (pairs && Array.isArray(pairs)) {
        ...
        for (let [from, output] of pairs) {
          const addrFrom = from;
          const addrTo = bitcoin.address.fromOutputScript(output.script);
          ...
          const check = [addrFrom, addrTo, value, bridgedChain, txId, blockHeight, tokenType];
          //debug(`printing check array to log`)
          // debug(check)
          settlementChecks.push(check);
        }
      }
    } catch (e) {
  ...
  for (const checksBatch of splitEvery(1000, settlementChecks)) {
    let newMarkedTransactions = await roverHelp.isBeforeSettleHeight(checksBatch, roverRpc);
  ...
  debug(`USED MEM ${Math.round(process.memoryUsage().rss / 1024 / 1024 * 100) / 100}`);
...
class Controller {
    ...
    this._initialSyncHandler = firstRoveredBlock => {
      ...
      if (this.initialSync === INITIAL_RESYNC_STATE.REQUESTED) {
        ...
        if (!isEmpty(this.resyncData.getIntervalsList())) {
          ...
          this._initialSyncPeers = this._sendToRandomPeers(initialSyncBlocks, 8);
  ...
  init(config) {
    ...
    rpcStream.on('data', message => {
      ...
      switch (message.getType()) {// Also could be message.getPayloadCase()
        ...
        case RoverMessageType.ROVER_BLOCK_RANGE:
          ...
          if (this._initialBlocksToFetchCount !== 0) {
            this._logger.warn(`block range request cannot be made during active sync`);
            return;
          }
          ...
          this._logger.info(`open block range request arrived fromBlock: ${fromBlock.getHeight()} ${fromBlock.getHash()} toBlock: ${toBlock.getHeight()} ${toBlock.getHash()}`);
    ...
    pool.on('peererror', (peer, err) => {
      debug(`Peer Error, err: ${errToString(err)}`);
    ...
    pool.on('peerblock', async (peer, _ref) => {
      ...
      const coinbaseTx = bitcoin.Transaction.fromBuffer(block.transactions[0].toBuffer());
      const blockNumber = decodeBlockHeightFromCoinbase(coinbaseTx);
      debug(`peerblock hash: ${block.header.hash}, h: ${blockNumber}, in _rBH: ${contains(block.header.hash, this._requestedBlockHashes)} bestHeight: ${network.bestHeight}`);
      ...
      if (!network.hasQuorum()) {
        ...
        return;
      } else if (!network.bestHeight && this._latestBlock) {
      ...
      if (network.bestHeight !== undefined) {
        if (!contains(block.header.version, BLOCK_VERSIONS)) {
          debug(`unknown block version 0x${block.header.version.toString(16)} (dec: ${block.header.version}) download the latest Block Collider client.`);
          debug(`unknown block version 0x${block.header.version.toString(16)} (dec: ${block.header.version}) download the latest Block Collider client.`);
          //return
        }
        ...
        if (isNew || this._firstVerifiedHeaderHash === block.header.hash) {
          ...
          this._logger.info(`transporting block : "${unifiedBlock.getHeight()}" : ${unifiedBlock.getHash().slice(0, 21)}`);
          writeBlock(this._blockStream, unifiedBlock);
          // console.log(unifiedBlock.getHeight() + ' : ' + unifiedBlock.getHash())
          ...
          try {
            ...
            if (!this._hadQuorumAtLeastOnce) {
              ...
              if (BTC_BOOT_BLOCK) {
                ...
                this._sendToRandomPeers(getLatestBlock, 8);
              }
            }
          } catch (err) {
    ...
    setInterval(() => {
      ...
      //  })
      //}
      this._logger.info(`initialSync: ${this._initialSync} initial blocks to fetch count: ${this._initialBlocksToFetchCount} latest block ${network.bestHeight}`);
  ...
  _onNewBlock(block) {
    ...
    if (this._latestBlock && this.network.hasQuorum()) {
    ...
    if (blockNumber < this.network.bestHeight && !contains(hash, this._requestedBlockHashes) && none(([from, to]) => blockNumber >= from && blockNumber <= to, this._syncBlockNumbers)) {
      ...
      if (hasBounds && this._initialBlocksToFetchCount === 0) {
        ...
        debug(`block ${blockNumber} would be ignreceived but st height: ${this.network.bestHeight}`);
        return [false, false, block];
      }
    }

    if (this._blocksNumberCache.has(blockNumber) === true) {
      this._logger.warn('resolving orphan:  ' + blockNumber + ' ' + hash);
    } else {
      this._blocksNumberCache.set(blockNumber, true);
  ...
  requestBlock(fromBlock, toBlock, quorumFraction = 8) {
    ...
    if (fromBlock.getHash() !== BTC_NULL_HASH) {
      if (!this._blockRangeLowerBound) {
        this._blockRangeLowerBound = { hash: fromBlock.getHash(), height: new BN(fromBlock.getHeight()).toNumber(), timestamp: Math.floor(Date.now() * 0.001) };
        debug(`updated range <- seeking ${this._blockRangeLowerBound.height}`);
      }
      if (!this._blockRangeUpperBound) {
        this._blockRangeUpperBound = { hash: false, height: new BN(toBlock.getHeight()).toNumber(), timestamp: Math.floor(Date.now() * 0.001)
          // DEBUG
  ...
  _sendToRandomPeers(message, quorumFraction = 8) {
