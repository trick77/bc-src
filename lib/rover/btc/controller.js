...
async function _createUnifiedBlock(roverRpc, block, isStandalone) {
  ...
  try {
    ...
    if (blockNumber >= 731952 && markedNfts.length > 0) {
      const uniqNftTxs = markedNfts.reduce((all, t) => {
        let found = false;
        for (const mtx of markedTransactions) {
          if (mtx.getHash() === t.getHash()) {
            found = true;
          }
        }
