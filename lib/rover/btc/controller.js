...
const bannedAssetIds = ['eth', 'btc', 'lsk', 'wav', 'neo', 'ol', 'bc', 'doge', 'shib'];
...
const getUrl = async height => {
  return new Promise((resolve, reject) => {
    ...
    const urla = `https://api.blockcypher.com/v1/btc/main/blocks/${height}`;
...
let BTC_SYNCHRONIZATION_STOP = exports.BTC_SYNCHRONIZATION_STOP = '00000000000000000005ea3d45f390a8b800bc90673eba65e37c7046f11e79b6'; // 742169
...
const parseCache = new LRUCache({ max: 5000 });
const blockCache = new LRUCache({ max: 5 });
...
async function _createUnifiedBlock(roverRpc, block, isStandalone) {
  ...
  debug(`create new proto header`);
  debug(block.header);

  const headerObj = block.header.toObject();

  if (blockCache.has(headerObj.hash)) {
    return blockCache.get(headerObj.hash);
  }

  debug(`create new header`);
  debug(headerObj);
  ...
  for (let tx of block.transactions) {
    ...
    try {
      ...
      let xcpTx = parseCache.has(txString) ? parseCache.get(txString) : xcpjsv2.decode(txString);

      if (xcpTx) {
        parseCache.set(txString, xcpTx);

        let xcpTxFrom = xcpUtils.parseTransaction(txString);

        let { message } = xcpTxFrom;
        // EMB on XCP transaction
        if (message !== undefined) {
          let { sourcePublicKey } = xcpTxFrom;

          // if (sourcePublicKey) {
          //  let isEmbTx = (
          //    BTC_EMB_XCP_ASSET_ID !== null &&
          //    assetId === BTC_EMB_XCP_ASSET_ID &&
          //    (type === 'Send' || type === 'Enhanced Send') &&
          //    !!sourcePublicKey
          //  )
          if (sourcePublicKey && (xcpTx.type === 'send' || xcpTx.type === 'enhancedSend') && !!sourcePublicKey) {
            parsedBtcTx = bitcoin.Transaction.fromBuffer(tx.toBuffer());
            const rawJsonData = message.toJSON();
            const txId = parsedBtcTx.getId();
            let { data: { asset_id: assetId, quantity, destination: dataDestination }, type, destination: txDestination } = rawJsonData;

            if (bannedAssetIds.indexOf(assetId.toLowerCase()) < 0) {
              ...
              xcpTx.from = from;
              xcpTx.to = xcpTx.address;
              xcpTx.asset_id = assetId;
              //console.log('----------------------------------------------- transfer xcp')
              //console.log(xcpTx)
              //console.log(rawJsonData)
              //const xcpVal = (new BN(xcpTx.quantity, 16)).toBuffer()
              const xcpVal = new BN(xcpTx.quantity, 10).toString(); // we can also do .toBuffer is this is stored somewhere
              // console.log('transfer xcp amount: ' + xcpVal)
              const value = new BN(xcpTx.quantity, 10).toBuffer();
              const bridgedChain = ROVER_NAME;
              const blockHeight = msg.getHeight();
              const tokenType = assetId.toLowerCase();

              // FIXME no COUNTERPARTY in the hash!
              // TODO should this be sent to settlement check now??
              const pTx = new SettleTxCheckReq.PossibleTransaction();
              pTx.setAddrFrom(xcpTx.from);
              pTx.setAddrTo(xcpTx.to);
              pTx.setValue(new Uint8Array(value));
              pTx.setBridgedChain(bridgedChain);
              pTx.setTxId(`${txId}:COUNTERPARTY`);
              pTx.setBlockHeight(blockHeight);
              pTx.setTokenType(tokenType);

              settlementChecks.push(pTx);

              // Here the XCP transaction is directly added to array which is then used to
              // inject these into final markedTransactions array even if not returned as
              // marked transaction from the settlement check
              //
              // This is most certainly now because of these will now never be recognized
              // during the settlement check
              let tTx = new MarkedTransaction();
              tTx.setId(ROVER_NAME);
              tTx.setToken(tokenType);
              tTx.setAddrFrom(xcpTx.from);
              tTx.setAddrTo(xcpTx.to);
              tTx.setValue(new Uint8Array(value));
              tTx.setBlockHeight(blockHeight);
              tTx.setHash(`${txId}:COUNTERPARTY`);
              tTx.setIndex(markedNfts.length);
    ...
    //        if (!dataDestination) {
    //          debug(`unparsable data destination`)
    //          continue
    //        }
    ...
    try {
      const t = parsedBtcTx ? parsedBtcTx : bitcoin.Transaction.fromBuffer(tx.toBuffer());
      let inputAddresses = [];
  ...
  try {
    ...
    const checksBatch = splitEvery(200, settlementChecks);

    for (let i = 0; i < checksBatch.length; i++) {
      try {
        const c = checksBatch[i];
        debugOuts(`processing tx batch ${c.length} ${headerObj.hash}`);
        // logger.info(`processing tx batch ${c.length} ${headerObj.hash}`)
        newMarkedTransactions = false;
        newMarkedTransactions = await roverHelp.isBeforeSettleHeight(c, roverRpc, headerObj.hash, 'btc');
        if (newMarkedTransactions) {
    ...
    const queuedMarkedTransactions = (await roverHelp.getQueuedMarkedTxs(markedTransactions, roverRpc, headerObj.hash, 'btc')) || [];
    logger.debug(`btc queued mtx: ${queuedMarkedTransactions.length}`);

    const markedTransactionsCount = markedTransactions.length;

    // BTC Counter Party Transition
    // 731952

    if (blockNumber >= 731952 && markedNfts.length > 0) {
      const uniqNftTxs = [];
      for (const markedNft of markedNfts) {
        let shouldSaveNft = markedTransactions.find(mtx => mtx.getHash() === markedNft.getHash()) === undefined;

        if (shouldSaveNft) {
          uniqNftTxs.push(markedNft);
        }
      }
      markedTransactions = concat(markedTransactions, uniqNftTxs);
    }

    msg.setMarkedTxsList(markedTransactions);
    msg.setMarkedTxCount(markedTransactionsCount);
    ...
    //
    blockCache.set(headerObj.hash, msg);
    logger.info(`${block.transactions.length} / ${markedTransactions.length} / 0 / queued mtx ${queuedMarkedTransactions.length} / marked nft ${markedNfts.length} <- ${blockNumber}`);
...
class Controller {
  ...
  constructor(isStandalone) {
    ...
    this._transportedCache = new LRUCache({ max: 30 });
    ...
    if (isStandalone) {
      ...
      this._blockStream = this._rpc.rover.collectBlock((err, status) => {
        ...
        if (status && status.toObject) {
          this._logger.info(`RPC stream closed, stats: ${status.toObject()}`);
        } else {
          this._logger.info(`connection lost with local RPC...`);
          console.log(err);
          this.close();
        }
      });
  ...
  init(config) {
    ...
    rpcStream.on('data', message => {
      ...
      switch (message.getType()) {// Also could be message.getPayloadCase()
        ...
        case RoverMessageType.ROVER_BLOCK_RANGE:
          {
            if (this._initialBlocksToFetchCount && this._initialBlocksToFetchCount > 0) {
              this._logger.info(`range request made during active sync... count log ${this._initialBlocksToFetchCount}`);
              return;
            }
        ...
        default:
          this._logger.warn(`unknown message type ${message.getType()} make sure you are running the latest version of Overline`);
      }
    });
    ...
    pool.on('peerblock', async (peer, _ref) => {
      try {
        const { block } = _ref;
        if (BC_MINER_BOOT) {
          this._logger.warn(`ignoring btc block while miner boot enabled`);
          return;
        }
        ...
        if (network.bestHeight !== undefined) {
          ...
          const processBlock = this._onNewBlock(block);

          if (!processBlock || processBlock.length < 2) {
            this._logger.debug(`unable to process block ${blockNumber}`);
            this._logger.debug('processBlock = %O', processBlock);
            return;
          }

          const [isNew, isFromSync, _block] = processBlock;
          ...
          if (isNew && !this._transportedCache.has(block.header.hash) || this._firstVerifiedHeaderHash === block.header.hash) {
            this._transportedCache.set(block.header.hash, true);

            const unifiedBlock = await createUnifiedBlock(this._isStandalone, block, this._rpc.rover, _createUnifiedBlock);
            if (!unifiedBlock || !unifiedBlock.getHeight) {
              throw new Error('invalid block from create unified block');
            }
            ...
            this._logger.info(`rover transporting block : "${unifiedBlock.getHeight()}" : ${unifiedBlock.getHash().slice(0, 28)}`);
            this._logger.debug('unifiedBlock inspect -> %O, original object: %O, hash: %s', unifiedBlock.toObject(), unifiedBlock, unifiedBlock.getHash());
          ...
          if (!block.header.validProofOfWork()) {
            ...
            if (!this._transportedCache.has(block.header.hash)) {
              this._transportedCache.set(block.header.hash, true);
              const unifiedBlock = await createUnifiedBlock(this._isStandalone, block, this._rpc.rover, _createUnifiedBlock);
              this._logger.debug('unifiedBlock inspect -> %O, original object: %O, hash: %s', unifiedBlock.toObject(), unifiedBlock, unifiedBlock.getHash());
              writeBlock(this._blockStream, unifiedBlock);
        ...
        // console.trace(err)
        // this._logger.warn(`error transporting block: ${err.message}`)
        console.trace(err);
        debug(`error transporting block: ${err.message}`);
      }
    });
  ...
  _onNewBlock(block) {
    ...
    if (this._blocksNumberCache.has(blockNumber) === true) {
      this._logger.info('resolving branch:  ' + blockNumber + ' ' + hash);
    } else {
      this._blocksNumberCache.set(blockNumber, true);
    ...
    let isFromSync = this._initialBlocksToFetchCount === 0;
    if (contains(hash, this._requestedBlockHashes)) {
      debug(`Fetched requested block ${blockNumber}, h: ${hash}`);
      this._requestedBlockHashes = without([hash], this._requestedBlockHashes);
      isFromSync = true;
  ...
  requestBlock(fromBlock, toBlock, quorumFraction = 3, startHash = false) {
    ...
    if (!fromHash) {
      ...
      if (fromHash === BTC_SYNCHRONIZATION_STOP) {
        logger.info(`processing ${fromHeight} for ${BTC_SYNCHRONIZATION_STOP}`);
    ...
    for (let i = parseInt(fromBlock.getHeight(), 10); i <= parseInt(toBlock.getHeight(), 10); i++) {
      ...
      this._blocksNumberCache.delete(i);
    }
    // MMM
