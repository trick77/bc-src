...
class Controller {
  ...
  init() {
    ...
    rpcStream.on('data', message => {
      ...
      switch (message.getType()) {// Also could be message.getPayloadCase()
        ...
        case RoverMessageType.ROVER_BLOCK_RANGE:
          ...
          debug(`open block range request arrived highest ${data.getHighestHeight()} lowest ${data.getLowestHeight()}`);
    ...
    const rateOfReq = BC_MINER_BOOT ? 60000 : 9000;

    setInterval(() => {
      const stats = this._neoMesh.getStats();
      if (stats) {
        const { all, correct, bestHeight } = this._neoMesh.getStats();
        debug(`mesh count pool: ${correct}/${all}, bh: ${bestHeight}`);
  ...
  requestBlockRange(blockRange) {
    ...
    debug(`requestBlockRange(): range to fetch count is: ${this._rangeToFetch.length}`);

    //if (this._rangeToFetch.length > 0 && !this._seekingSegment) {
    if (this._rangeToFetch.length > 0) {
      debug(`requestBlockRange(): starting resync from this._rangeToFetch[0]`);
      this.startResync();
    }
  }
