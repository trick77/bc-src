...
const { RoverMessageType, RoverIdent, RoverSyncStatus, RoverMessage } = require('../../protos/rover_pb');
...
const BC_NEO_MAX_FAILURES = process.env.BC_NEO_MAX_FAILURES || 10;
const NEO_NETWORK_PROFILE = 'mainnet'; // do not use NEO testnet // BC_NETWORK !== 'main' ? 'testnet' : 'mainnet'
const NEO_MAX_FETCH_BLOCKS = 5;
...
async function _createUnifiedBlock(roverRpc, block, isStandalone) {
    ...
    if (tx.type === 'ContractTransaction') {
      ...
      for (let out of tx.vout) {
        if (out.asset === '0xc56f33fc6ecfcd0c225c4ab356fee59390af8560be0e930faebe74a6daff7c9b') {
          settlementChecks.push([addrFrom, out.address, new _bn2.default(out.value).toBuffer(), bridgedChain, tx.txid, block.index, tokenType]);
        }
      }
  ...
  try {
    let markedTransactions = (await isBeforeSettleHeight(settlementChecks, roverRpc, block.hash)) || [];
  ...
  startResync(resyncMsg) {
    ...
    if (!this._timeoutResync) {
      this._timeoutResync = setTimeout(() => {
        ...
        }, { factor: 1.1, randomize: true }).then(height => {
          ...
          batchProcess(batchLength, whichBlocks.map(height => {
            ...
            return () => pRetry(function () {
              ...
              onFailedAttempt: error => {
                debug(`Block ${height} attempt ${error.attemptNumber} failed. There are ${error.retriesLeft} retries left.`);
              },
              factor: 1.1,
              randomize: true
            }).then(async block => {
              ...
              // console.log(`whichBlocks.length ${whichBlocks.length} successCount ${successCount}`)
