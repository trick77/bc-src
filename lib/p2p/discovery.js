...
//let seeds = BC_BUILD_GENESIS || BC_MINER_POOL ? seederBootstrap : getRandomSubarray(seederBootstrap, 9)
let seeds = seederBootstrap;

function Discovery(nodeId) {
  // bootstrap from two randomly selected nodes
  if (process.env.MIN_HEALTH_NET === 'true') {
    //seeds = BC_BUILD_GENESIS || BC_MINER_POOL ? seederBootstrap : getRandomSubarray(seederBootstrap, 9)
  ...
  this.options = {
    ...
    dht: {
      concurrency: 64,
      timeBucketOutdated: 300000 + random(200000),
      bootstrap: dhtBootstrap,
      interval: 250000 + random(200000),
      maxConnections: maxConnections
    }
  };
  this.streamOptions = {
