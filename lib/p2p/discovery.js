...
let seeds = BC_BUILD_GENESIS || BC_MINER_POOL ? seederBootstrap : getRandomSubarray(seederBootstrap, 5);

function Discovery(nodeId) {
  // bootstrap from two randomly selected nodes
  if (process.env.MIN_HEALTH_NET === 'true') {
    seeds = BC_BUILD_GENESIS || BC_MINER_POOL ? seederBootstrap : getRandomSubarray(seederBootstrap, 5);
  }
