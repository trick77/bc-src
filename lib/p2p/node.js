...
const BC_EXTENDED_QUARANTINE_FORGIVENESS = BC_BLOODLINE ? 885000 : getRandomWithinRange(290166, 944900);
      ...
      if (newBlock !== undefined && newBlock !== null && !invalidData && prevBlock && prevBlock.getHeight && parseInt(newBlock.getHeight(), 10) > 1) {
        ...
        if (prevBlock && prevBlock.getHeight() !== newBlock.getHeight()) {
          ...
          if (!isValidSeq.valid && blockQueue[0] && blockQueue[0].getHeight() !== newBlock.getHeight()) {
            ...
            // TD
            this._logger.info(`isolated validation ${newBlock.getHeight()} : ${newBlock.getHash().slice(0, 8)} ${newBlock.getPreviousHash()} <- ${isolatedValidation} connected: ${newBlock.getBlockchainHeadersCount()}`);
