...
const loadBasedPeerExpiration = 53000 + Math.floor(30000 / numCPUs);
/* OVERLINE
...
const MAX_DATA_RANGE = exports.MAX_DATA_RANGE = Number(process.env.MAX_DATA_RANGE) || 15;
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    do {
      ...
      if (newBlock !== undefined && newBlock !== null && !invalidData && prevBlock && prevBlock.getHeight && parseInt(newBlock.getHeight(), 10) > 1) {
        ...
        flag = blockQueue.length === 0 && parseInt(newBlock.getHeight(), 10) % 5 === 0 || this._utxoCycles > utxoCycleLimit && blockQueue.length === 0 || highestKnownHeight - 12 < parseInt(newBlock.getHeight(), 10) && blockQueue.length === 0;
        if (!flag) {
          this._utxoCycles++;
        }
        const isValidCoinbase = blockQueue.length === 0 && syncedBeforeData !== 'pending' || flag ? await validateCoinbase(newBlock, this._engine.persistence, this._engine._txHandler, 'p2p.node') : true;
        if (isValidCoinbase) {
          if (!lowestBoundary || lowestBoundary <= parseInt(newBlock.getHeight(), 10)) {
            // the lowest boundary cannot be the genesis block
            if (parseInt(newBlock.getHeight(), 10) > 1) {
          ...
          if (opts.innerCall) {
            ...
            if (blockQueue.length === 0) {
              ...
              if (missingBlockHeight) {
                this._logger.warn(`sequence confirmation failed at ${missingBlockHeight} for block ${newBlock.getHeight()} : ${newBlock.getHash().slice(0, 21)}, prev: ${newBlock.getPreviousHash().slice(0, 21)} `);
    ...
    if (blocksLength < MAX_DATA_RANGE && validBlocks > 0 && parseInt(evalBlock.getHeight(), 10) + MAX_DATA_RANGE < highestKnownHeight && finalValidBlocks.length > 0 && parseInt(evalBlock.getHeight(), 10) < highestKnownHeight) {
      ...
      if (this._PEER_QUARANTINE.length <= 12 && synced === 'pending' && validBlock) {
        ...
        await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
      } else if (this._PEER_QUARANTINE.length <= 12 && synced === 'complete' && shouldStepBack) {
        // push null counter
      ...
      if (this._PEER_QUARANTINE.length >= 12 && this._PEER_BLACKLIST.length < 16) {
        ...
        return;
      } else if (this._PEER_QUARANTINE.length <= 12 && synced === 'pending') {
        delete this._PEER_RECORD[addressToHost(address)];
