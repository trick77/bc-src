...
const TABLE_REQUEST_TIMEOUT = 4 * 1000; // 3 seconds used for the first request of qsend
const GIVEN_REQUEST_TIMEOUT = 7 * 1000; // 5 seconds used for the second attempt on failure OR timeout of the table request
...
const BC_MINIMUM_HEADER_TEST = isNaN(process.env.BC_MINIMUM_HEADER_TEST) ? 1 : Number.parseInt(process.env.BC_MINIMUM_HEADER_TEST, 10);
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (!currentPeer) {
      ...
      if (reorgBlock && !opts.innerCall && !BC_MINER_POOL) {
        ...
        // TD
        this._logger.info(`Unset DATA assigned to initial waypoint blocks contained lowest open req: ${waypointRequestLow}`);
        currentPeer = new InitialPeer();
        currentPeer.setAddress(addressToHost(address));
        currentPeer.setExpires(Number(new Date()) + BC_PEER_HEADER_SYNC_EXPIRE);
        await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.initialpeer`, currentPeer);
