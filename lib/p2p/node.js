    ...
    if (blocks && blocks.length > 0) {
      ...
      if (startGivenBlockHeight < waypointRequestLow - BC_MAX_DATA_RANGE * 10) {
        ...
        if (waypointRequestHigh && waypointRequestLow) {
          ...
          this._engine._emitter.emit('requestBlockRange', [waypointRequestHigh, waypointRequestLow - 2], 5);
        }
        return;
      }
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (currentPeer && currentPeer.getAddress() === addressToHost(address)) {
        debug(`received GET_DATA request from waypoint ${address} during sync <- yielding...`);
        return;
      }
      ...
      if (parseInt(latestBlock.getHeight(), 10) + 200 < high && synced === 'pending') {
      ...
      // TD
      debug(`waypoint connection ${address} low ${low} -> ${high}`);
