...
const BC_MAX_DATA_RANGE = exports.BC_MAX_DATA_RANGE = Number(process.env.BC_MAX_DATA_RANGE) || 12;
...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (blocksLength < BC_MAX_DATA_RANGE && validBlocks > 0 && parseInt(evalBlock.getHeight(), 10) + BC_MAX_DATA_RANGE < highestKnownHeight && finalValidBlocks.length > 0 && parseInt(evalBlock.getHeight(), 10) < highestKnownHeight) {
      ...
      if (this._PEER_QUARANTINE.length >= 6 && this._PEER_BLACKLIST.length > 4) {
        ...
        if (BC_ORTHOGONAL_WAYPOINTS) {
          ...
          const movingEdge = dlh - Math.floor(Math.random() * Math.floor(128));
          ...
          this._engine._emitter.emit('requestBlockRange', [movingEdge + BC_MAX_DATA_RANGE + 8, movingEdge]);
          return;
        } else {
    ...
    if (unseenBlocks === 0 && blocksLength >= BC_MAX_DATA_RANGE) {
      ...
      this._PEER_RECORD[addressToHost(address)].badBlocks++;
      debug(`waypoint bad block weight ${addressToHost(address)} ${this._PEER_RECORD[addressToHost(address)].badBlocks}`);
    }
          ...
          if (range[2] && pass) {
            ...
            for (const remoteConnection of connections) {
              ...
              if (hosts.indexOf(host) < 0) {
                debug(`skipping waypoint ${host} with no record`);
                continue;
              }
