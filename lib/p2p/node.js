      ...
      if (this._knownHashes.indexOf(newBlock.getHash()) > -1) {
        ...
        if (syncProgress > 0) {
          ...
          if (rawSyncProgress > 99.999 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
            ...
            if (blockQueue.length === 0) {
              this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} (${max(0, Math.abs(this._highestRequestedRangeHeight - parseInt(newBlock.getHeight(), 10)))}) ${this._childBlocksSynced} <- ${newBlock.getHash().slice(0, 8)} ${newBlock.getPreviousHash().slice(0, 8)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
            }
          }
        }
            ...
            if (syncProgress > 0) {
              ...
              if (rawSyncProgress > 99.9999 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
                ...
                if (rawSyncProgress > 99.95) {
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} (${max(0, Math.abs(this._highestRequestedRangeHeight - parseInt(newBlock.getHeight(), 10)))}) ${newBlock.getHash().slice(0, 8)}:${newBlock.getPreviousHash().slice(0, 8)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
                } else if (newBlock.getHeight() % 2 === 0) {
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} (${max(0, Math.abs(this._highestRequestedRangeHeight - parseInt(newBlock.getHeight(), 10)))}) ${this._childBlocksSynced} ${syncProgress}% <- ${newBlock.getHash().slice(0, 8)}:${newBlock.getPreviousHash().slice(0, 8)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
                } else {
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} (${max(0, Math.abs(this._highestRequestedRangeHeight - parseInt(newBlock.getHeight(), 10)))}) ${this._childBlocksSynced} ${syncProgress}% <- ${newBlock.getPreviousHash().slice(0, 8)}:${newBlock.getHash().slice(0, 8)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
                }
              }
            }
      ...
      if (highest !== lowest) {
        ...
        if (this._discovery.connections) {
          ...
          if (range[2] && pass) {
            ...
            await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.requesthold`, `${Math.floor(Date.now() / 1000)}`);
