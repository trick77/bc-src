...
const getMissingBlock = async (host, hash, persistence) => {
  ...
  return new Promise(async resolve => {
    try {
      fetch(`http://${host}:3000/rpc`, { method: "post", headers: headers, body: body }).then(async response => {
...
class PeerNode {
  ...
  constructor(engine) {
    ...
    if (quarantineStatStream) {
      quarantineStatStream.write(`timestamp,address,reason\n`);
    }
  ...
  addToQuarantine(addr, reason, extended) {
    ...
    if (quarantineStatStream) {
      quarantineStatStream.write([Math.floor(new Date() / 1000), host, reason].join(',') + '\n');
    }
  }
      ...
      if (remount) {
        ...
        if (unmount) {
          if (new BN(unmount).gt(new BN(checkCoinbaseBlock.getHeight()))) {
            ...
            //this.addToQuarantine(address, 'unable to provide mount', 1)
            ...
            ////const data = [unmount - 1, unmount + 6]
            //const data ="40[*]80"
            //const payload = encodeTypeAndData(MESSAGES.GET_BLOCKS, data)
            ...
            if (block) {
              ...
              this.addToQuarantine(address, `block not provided ${uheight}:${uhash}`, true);
              await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.blocks`);
              await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
              await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`);
              this._engine._emitter.emit('requestBlockRange', [unmount + BC_MAX_DATA_RANGE, unmount - 4], 3);
            }
            return;
          }
        } else {
          ...
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`);
          this._engine._emitter.emit('requestBlockRange', [checkCoinbaseBlock.getHeight() + BC_MAX_DATA_RANGE + 500, checkCoinbaseBlock.getHeight() + 500, conn]);
          return;
        }
      }
    }
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (highestKnownHeight && parseInt(highestKnownHeight, 10) - 60 > parseInt(block.getHeight(), 10)) {
          ...
          this.addToQuarantine(address, `block is more than 30 blocks lower than highest new height`, 1);
      ...
      if (parseInt(latestBlock.getHeight(), 10) - 1000 < high && parseInt(latestBlock.getHeight(), 10) - 50 > highestKnownHeight && synced === 'complete') {
