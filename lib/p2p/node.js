...
class PeerNode {
  ...
  constructor(engine) {
    ...
    this._dataObjectCache = new LRUCache({
      max: 30,
      ttl: getRandomWithinRange(1500, 32900)
    });
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (blocks && blocks.length > 0) {
      ...
      if (startGivenBlockHeight < waypointRequestLow - BC_MAX_DATA_RANGE * 2) {
        ...
        if (waypointRequestHigh && waypointRequestLow) {
          ...
          this._engine._emitter.emit('requestBlockRange', [waypointRequestHigh, waypointRequestLow - 2], 1);
        }
        return;
      }
    ...
    if (validcb && checkCoinbaseBlock) {
      ...
      await this._engine.persistence.putLatestBlock(checkCoinbaseBlock, BC_SUPER_COLLIDER, {
        iterateUp: false
      });
    }
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (highestKnownHeight > parseInt(latestBlock.getHeight(), 10) + 100 && synced && synced === 'pending') {
        ...
        if (peer) {
          ...
          this._logger.info(`received request from waypoint which cannot be completely fulfilled...${address}`);
          return;
        } else {
