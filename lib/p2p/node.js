  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (blocks && blocks.length > 0) {
      ...
      if (startGivenBlockHeight < waypointRequestLow - BC_MAX_DATA_RANGE * 2) {
        ...
        this.addToQuarantine(address, `waypoint responded with block range lower than requested: ${waypointRequestLow} given by waypoint: ${startGivenBlockHeight}`, true);
    ...
    if (!validcb) {
      ...
      if (remount) {
        ...
        if (unmount && unmount < checkCoinbaseBlock.getHeight()) {
          ...
          let scanBackHeight = checkCoinbaseBlock.getHeight() + 240 * direction;
