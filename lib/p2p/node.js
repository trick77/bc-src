...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    const validcb = checkCoinbaseBlock.getHeight() + 100 > highestKnownHeight ? await validateCoinbase(checkCoinbaseBlock, this._engine.persistence, this._engine._txHandler, 'p2p.node') : true;
    ...
    if (blocksLength < BC_MAX_DATA_RANGE && validBlocks > 0 && parseInt(evalBlock.getHeight(), 10) + BC_MAX_DATA_RANGE < highestKnownHeight && finalValidBlocks.length > 0 && parseInt(evalBlock.getHeight(), 10) < highestKnownHeight) {
      ...
      if (this._PEER_QUARANTINE.length >= 16 && this._PEER_BLACKLIST.length > 4) {
        ...
        if (BC_ORTHOGONAL_WAYPOINTS) {
          ...
          this._engine._emitter.emit('requestBlockRange', [movingEdge + BC_MAX_DATA_RANGE - 2, movingEdge - 2]);
        } else {
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      if (this._requestRegistry[peerRequestKey] > 600) {
        ...
        if (sent !== undefined && sent.success) {
          // LDL
          debug(`GET_DATA sent to ${sent.address}, message: ${sent.message}`);
          const validcblazy = parseInt(checkCoinbaseBlock.getHeight(), 10) + 1000 < highestKnownHeight && !validcb ? await validateCoinbase(checkCoinbaseBlock, this._engine.persistence, this._engine._txHandler, 'p2p.node') : true;
          if (!validcblazy) {
            let toHeight = data[1];
            ...
            this._PEER_QUARANTINE.unshift(addressToHost(address));
            if (this._discovery.connected && this._discovery.connected - this._PEER_QUARANTINE.length < 5) {
              this._PEER_QUARANTINE.pop();
            }
            this._engine._emitter.emit('requestBlockRange', [toHeight - 50, fromHeight - 50]);
            return;
          }
        } else {
          // LDL
          ...
          this._PEER_QUARANTINE.unshift(addressToHost(address));
          this._engine._emitter.emit('requestBlockRange', [data[1] - 50, data[0] - 50]);
          return;
        }
      }
  ...
  async start(nodeId) {
    ...
    this._engine._emitter.on('requestBlockRange', async range => {
      ...
      if (difference < 16 && !givenConnection) {
        limit = 1;
      }
