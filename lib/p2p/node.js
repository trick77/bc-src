...
class PeerNode {
    ...
    if (!validcb) {
      ...
      if (remount) {
        ...
        if (unmount && unmount < checkCoinbaseBlock.getHeight()) {
          ...
          if (new BN(unmount).gt(new BN(checkCoinbaseBlock.getHeight()))) {
            ...
            if (unmountRaw) {
              ...
              if (!block) {
                ...
                this._engine._emitter.emit('requestBlockRange', [Number(uheight) + BC_MAX_DATA_RANGE - 40, Number(uheight) - 400], 2);
                return;
              }
          ...
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
          this._engine._emitter.emit('requestBlockRange', [scanBackHeight + BC_MAX_DATA_RANGE, scanBackHeight], 3);
          return;
        }
      }
    }
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (high > parseInt(latestBlock.getHeight(), 10) + 300 && synced && synced === 'pending') {
