...
let BC_MAX_WAYPOINT_LATENCY = isNaN(process.env.BC_MAX_WAYPOINT_LATENCY) ? 15260 : process.env.BC_MAX_WAYPOINT_LATENCY;
...
const PEER_DATA_SYNC_EXPIRE = 26602; // Peer must return a block / data / network request before time elapsed (milliseconds)
...
const loadBasedPeerExpiration = logline ? 50000 : 90000 + Math.floor(80000 / numCPUs);
...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (!currentPeer) {
      ...
      if (blocks && blocks[0] && reorgBlockRaw && new BN(reorgBlockRaw.getHeight()).lte(new BN(blocks[0].getHeight())) && syncedBeforeData && syncedBeforeData === 'complete') {
        ...
        if (syncedBeforeData && syncedBeforeData === 'pending' && new Date() > parseInt(currentPeer.getExpires(), 1) || syncedBeforeData && syncedBeforeData === 'pending' && !currentPeer) {
          currentPeer = new InitialPeer();
          currentPeer.setAddress(addressToHost(address));
          currentPeer.setExpires(Number(new Date()) + BC_PEER_HEADER_SYNC_EXPIRE);
          await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.initialpeer`, currentPeer);
    ...
    do {
      ...
      debug(`processing ${BC_SUPER_COLLIDER} block ${newBlock.getHeight()} : ${newBlock.getHash()}`);
      ...
      if (this._knownHashes.indexOf(newBlock.getHash()) > -1) {
        ...
        if (syncProgress > 0) {
          ...
          if (syncProgress > 99.9999 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
            syncThrottleMS = 150;
            syncLookBack = 1;
            ...
            if (blockQueue.length === 0) {
              this._logger.info(`#${newBlock.getHeight()}:${highestKnownHeight} ${syncProgress}% <- ${newBlock.getHash().slice(0, 20)} ${newBlock.getPreviousHash().slice(0, 20)} <- Emblem Miner Boost: +${this._engine._emblemPerformance}%`);
            }
          }
        }
        continue;
      ...
      if (newBlock !== undefined && newBlock !== null && !invalidData && prevBlock && prevBlock.getHeight && parseInt(newBlock.getHeight(), 10) > 1) {
        ...
        //flag = blockQueue.length === 0 && highestKnownHeight - 1000 < newBlock.getHeight() ||
        //  blockQueue.length === 0 && (highestKnownHeight - 1000 > newBlock.getHeight() && newBlock.getHeight() % randomWindow === 0) ||
        //  blockQueue.length === 0 && this._utxoCycles > randomBound
        ...
        if (isValidCoinbase) {
          ...
          if (prevBlock && prevBlock.getHeight() !== newBlock.getHeight()) {
            ...
            if (mountBlockReq) {
                  ...
                  for (let mb of mountBlocks) {
                    ...
                    if (bl) {
                      ...
                      if (isValidSeq.valid) {
                        ...
                        this._logger.info(`mount block located ${bl.getHash().slice(0, 21)} with invalid sequence`);
                      }
                    }
                  }
                  if (!foundRootBlockToMountBranch) {
              ...
              if (isValidSeq.valid) {
                ...
                this._logger.info(`${newBlock.getHeight()} : ${newBlock.getHash().slice(0, 4)} required isolated review`);
              }
            }
            ...
            if (!isValidSeq.valid && blockQueue[0] && blockQueue[0].getHeight() !== newBlock.getHeight()) {
              ...
              //await this._engine.persistence.putBlock(newBlock, 0, BC_SUPER_COLLIDER, { asHeader: false, fromWaypoint: true, saveHeaders: false })
            ...
            if (syncProgress > 0) {
              ...
              if (syncProgress > 99.98 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
                ...
                if (syncProgress > 99.99) {
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} <- ${newBlock.getHash().slice(0, 20)}:${newBlock.getPreviousHash().slice(0, 20)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}%`);
                } else if (newBlock.getHeight() % 2 === 0) {
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} ${syncProgress}% <- ${newBlock.getHash().slice(0, 20)}:${newBlock.getPreviousHash().slice(0, 20)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}%`);
                } else {
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} ${syncProgress}% <- ${newBlock.getPreviousHash().slice(0, 20)}:${newBlock.getHash().slice(0, 20)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}%`);
                }
              }
            }
          ...
          if (opts.innerCall) {
            ...
            //  await this.processDataMessage(conn, innerCallBlocks, {innerCall: true})
            //} else if (blockQueue.length > 0) {
            //  debug(`block search for new branch from options ${newBlock.getHeight()} : ${newBlock.getHash()}`)
            //  // there is more branchs to evaluate or a block is valid at this height

            //} else {
            //  debug(`block search for new branch ${newBlock.getHeight()} : ${newBlock.getHash()}`)
            this._logger.info(`processing ${blockQueue.length} paths forward to edge ${newBlock.getHeight()}`);
            if (unmount) {
              await this._engine.persistence.delUtxoUnmount();
    ...
    } while (blockQueue.length > 0 && !blocksProvidedOutsideBoundary && !segmentKnown);
    ...
    //let findRoot = false
    //let h = parseInt(checkCoinbaseBlock.getHeight(), 10)
    //while (!findRoot) {
    //  h--
    //  let options = await this._engine.persistence.getBlocksByHeight(h, BC_SUPER_COLLIDER)
    //  if (!options || options.length < 1) {
    //    options = await this._engine.persistence.getBlockByHeight(h, BC_SUPER_COLLIDER)
    //    options = [options]
    //  }
    //  if (options.length === 1) {
    //    findRoot = options[0]
    //    this._logger.info(`found root of ${findRoot.getHeight()} from ${checkCoinbaseBlock.getHeight()}`)
    //  }
    //}
    ...
    if (!validcb) {
      ...
      if (unmount) {
        ...
        if (remount) {
          ...
          if (invalidBlockHeights.indexOf(parseInt(checkCoinbaseBlock.getHeight(), 10)) < 0) {
            invalidBlockHeights.push(parseInt(checkCoinbaseBlock.getHeight(), 10));
          }
        }
      }
    }

    debug(`valid: ${validBlocks}, from memory: ${blocksLoadedFromMemory}, height: ${currentHeight}`);
      ...
      if (this._PEER_RECORD[addressToHost(address)]) {
        if (finalValidBlocks.length > 0) {
          ...
          if (lbkh === lastSeenHash) {
            ...
            if (finalValidBlocks.length < 4 || blocksLoadedFromMemory !== validBlocks) {
              ...
              debug(`waypoint demoted: ${this._PEER_RECORD[addressToHost(address)].lastSeen}`);
              this._engine._emitter.emit('requestBlockRange', [currentHeight + 8, sFrom]);
              return;
            }
          } else {
    ...
    if (blocksLength < BC_MAX_DATA_RANGE && validBlocks > 0 && parseInt(evalBlock.getHeight(), 10) + BC_MAX_DATA_RANGE < highestKnownHeight && finalValidBlocks.length > 0 && parseInt(evalBlock.getHeight(), 10) < highestKnownHeight) {
      ...
      if (lowestBoundary && invalidBlockHeights[0] < lowestBoundary) {
        this._logger.info(`lower segment detected ${lowestBoundary} -> searching waypoint...`);
        this._engine._emitter.emit('requestBlockRange', [invalidBlockHeights[0] + BC_MAX_DATA_RANGE, invalidBlockHeights[0] - 2, conn]);
        return;
      }
      ...
      if (BC_ORTHOGONAL_WAYPOINTS) {
        ...
        //await this._engine.persistence.putLatestBlock(finalValidBlock, BC_SUPER_COLLIDER, {iterateUp: true})
      }
      ...
      if (lowestBoundary && invalidBlockHeights[0] < lowestBoundary) {
        this._logger.info(`lower segment detected ${lowestBoundary}`);
        this._engine._emitter.emit('requestBlockRange', [invalidBlockHeights[0] + BC_MAX_DATA_RANGE, invalidBlockHeights[0] - 2, conn]);
        return;
      }
      ...
      if (this._PEER_QUARANTINE.length >= 16 && this._PEER_BLACKLIST.length > 4) {
        ...
        if (BC_ORTHOGONAL_WAYPOINTS) {
          ...
          if (finalInvalidBlocks.length > 0) {
            movingEdge = finalInvalidBlocks - Math.floor(Math.random() * Math.floor(4));
          }
          ...
          this._engine._emitter.emit('requestBlockRange', [movingEdge + BC_MAX_DATA_RANGE, movingEdge]);
          return;
        } else {
    ...
    if (unseenBlocks === 0 && blocksLength >= BC_MAX_DATA_RANGE) {
      debug(`blocks are above MAX_DATA_RANGE`);
    }
    ...
    if (validBlocks >= minimumValidBlocks && new BN(currentHeight).gte(new BN(latestBlock.getHeight() - 1)) && !invalidData) {
      ...
      await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.data.latest`, `${currentHeight - 100}:${now}`);
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      if (currentPeer) {
        ...
        weightedTime = max(0, 2000 + meanTime - processTime);
        currentPeer.setExpires(Number(new Date()) + 2800);
        await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.initialpeer`, currentPeer);
      } else {
      ...
      if (this._requestRegistry[peerRequestKey] > 600) {
        ...
        if (blocksLoadedFromMemory === validBlocks && validBlocks > 10 && invalidBlockHeights.length < 1) {
          this._logger.info(`warping ahead from ${nextHeight} -> ${nextHeight + 50}`);
          nextHeight = nextHeight + 50;
        }
        // LDL
      ...
      debug(`[] -> [] completed traverse to edge of global state -> ${currentHeight} <- monitoring ${this._PEER_QUARANTINE.length} sessions`);
          ...
          if (range[2] && pass) {
            ...
            for (const remoteConnection of connections) {
              ...
              if (sendToPool && p < limit && this._PEER_QUARANTINE.indexOf(host) < 0 && this._PEER_BLACKLIST.indexOf(host) < 0) {
                ...
                debug(`yielding broadcast range request to address ${address}`);
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      await this._engine.persistence.putBlockHashAtHeight(block.getHash(), parseInt(block.getHeight(), 10), BC_SUPER_COLLIDER);
      ...
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (currentPeer && this._PEER_QUARANTINE.indexOf(currentPeer.getAddress()) > -1) {
          this._logger.info(`unlocking waypoint from sync process`);
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
          currentPeer = false;
        }
        ...
        if (!currentPeer && this._processedBlocks > 2 || nodeIsSynced && synced === 'complete' && this._processedBlocks > 2) {
            ...
            if (new BN(time).gt(new BN(currentPeer.getExpires()))) {
              ...
              if (highestKnownHeight) {
                ...
                if (highestKnownHeight < parseInt(block.getHeight(), 10)) {
                  if (parseInt(currentPeer.getExpires(), 10) - 3000 > Number(Date.now())) {
                    await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.edge`, parseInt(block.getHeight(), 10));
                    await this._engine.persistence.putLatestBlock(block, BC_SUPER_COLLIDER, { chainState: this._engine.chainState });
                  }
                }
              } else {
                //await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.edge`, parseInt(block.getHeight(), 10))
        ...
        if (!currentPeer && this._processedBlocks > 2 || synced && synced === 'complete' && this._processedBlocks > 2) {
          this._SEEN_BLOCKS_MEMORY[parseInt(block.getHeight(), 10)] = 1;
          debug(`sending block to engine -> ${block.getHeight()}`);
          const fastSend = getRandomWithinRange(2, 4);

          // EMIT BLOCK FROM PEER

          await this._engine.blockFromPeer({ conn: conn, newBlock: block, options: options, alreadyBroadcasted: fastSend === 3 });
          if (fastSend === 3) {
            debug(`fast send activated: ${block.getHeight()}`);
      ...
      if (high > parseInt(latestBlock.getHeight(), 10) + 3) {
        debug(`received GET_DATA request from waypoint ${address} which cannot be fulfilled`);
      }
      ...
      for (let block of validBlocks) {
        debug(`sending ${block.getHeight()}:${block.getHash()}`);
      }
