...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    do {
      ...
      if (newBlock && newBlock.getHeight) {
        if (evaluatedBlocks.length > 1) {
          ...
          if (!foundMount && evaluatedBlocks[evaluatedBlocks.length - 2].getHeight() === newBlock.getHeight()) {
            this._logger.info(`would mount coinbase set to previous block ${evaluatedBlocks[evaluatedBlocks.length - 2].getHash()}`);
    ...
    if (!validcb && branchCoinbaseBlock) {
          ...
          if (new BN(unmount).gt(new BN(checkCoinbaseBlock.getHeight()))) {
            ...
            if (unmountRaw) {
              ...
              if (!block) {
                ...
                this._engine._emitter.emit('requestBlockRange', [Number(uheight) + BC_MAX_DATA_RANGE - 140, Number(uheight) - 200], 2);
                return;
              }
