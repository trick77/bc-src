            ...
            if (syncProgress > 0) {
              ...
              if (rawSyncProgress > 99.9999 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
                ...
                if (rawSyncProgress > 99.99) {
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} (${max(0, Math.abs(this._highestRequestedRangeHeight - parseInt(newBlock.getHeight(), 10)))}) ${childBlocks} ${newBlock.getHash().slice(0, 8)}:${newBlock.getPreviousHash().slice(0, 8)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
    ...
    if (!validcb) {
      ...
      if (remount) {
        ...
        if (remount >= lowestBoundaryBlocks) {
          ...
          const bh = await this._engine.persistence.getBlockByHeight(remount - 30);
