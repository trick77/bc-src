...
class PeerNode {
      ...
      if (this._knownHashes.indexOf(newBlock.getHash()) > -1 && blockQueue.length !== 0) {
        ...
        if (syncProgress > 0) {
          ...
          if (syncProgress > 99.97 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
            ...
            this._logger.info(`COLLIDER ${newBlock.getHeight()} ${this._childBlocksSynced}:${highestKnownHeight} [] ${newBlock.getHash().slice(0, 8)}:${newBlock.getPreviousHash().slice(0, 8)} NRG Multichain ${syncProgress}% <- Miner EMB Boost: +${this._engine._emblemPerformance}%`);
          }
        }
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (parseInt(block.getHeight(), 10) < 153772) {
