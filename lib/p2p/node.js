      ...
      if (this._knownHashes.indexOf(newBlock.getHash()) > -1 && blockQueue.length !== 0) {
        ...
        if (syncProgress > 0) {
          ...
          if (syncProgress > 99.98 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
            ...
            if (blockQueue.length === 0) {
              this._logger.info(`NRG MULTICHAIN #${newBlock.getHeight()} : ${this._childBlocksSynced} ${highestKnownHeight} [] ${newBlock.getHash().slice(0, 5)} ${newBlock.getPreviousHash().slice(0, 5)} ${syncProgress}% <- EMBLEM PERF. +${this._engine._emblemPerformance}% NRG`);
            }
          }
        }
        continue;
            ...
            if (syncProgress > 0) {
              ...
              if (syncProgress > 99.97 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
                ...
                if (syncProgress > 99.99) {
                  this._logger.info(`NRG MULTICHAIN #${newBlock.getHeight()} : ${this._childBlocksSynced} ${highestKnownHeight} [] ${newBlock.getHash().slice(0, 5)}:${newBlock.getPreviousHash().slice(0, 5)} <- EMBLEM PERF. +${this._engine._emblemPerformance}% NRG`);
                } else {
                  this._logger.info(`NRG MULTICHAIN #${newBlock.getHeight()} : ${this._childBlocksSynced} ${highestKnownHeight} [] ${newBlock.getHash().slice(0, 5)}:${newBlock.getPreviousHash().slice(0, 5)} ${syncProgress}% <- EMBLEM PERF. +${this._engine._emblemPerformance}% NRG`);
                }
              }
