...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    let waypointRequest = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.req.range`, { sync: true });
    ...
    if (blocks && blocks.length > 0) {
      ...
      if (startGivenBlockHeight < waypointRequestLow - BC_MAX_DATA_RANGE * 6) {
    ...
    do {
      ...
      if (newBlock !== undefined && newBlock !== null && !invalidData && prevBlock && prevBlock.getHeight && parseInt(newBlock.getHeight(), 10) > 1) {
        ...
        // TD: await this._engine.persistence.putChildBlocksIndexFromBlock(prevBlock)
        // TD: await this._engine.persistence.saveBlockHeaders(prevBlock)


        // TD: await this._engine.persistence.putChildBlocksIndexFromBlock(newBlock)
        // TD: await this._engine.persistence.saveBlockHeaders(newBlock)
        ...
        if (unmount && unmount < checkCoinbaseBlock.getHeight()) {
          ...
          if (new BN(unmount).gt(new BN(checkCoinbaseBlock.getHeight()))) {
            ...
            if (unmountRaw) {
              ...
              this._engine._emitter.emit('requestBlockRange', [unmount + BC_MAX_DATA_RANGE - 40, unmount - 40], 2);
              return;
            }
          }
          ...
          const prevFlaggedBlock = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.block.flag.${checkCoinbaseBlock.getHeight()}`);
          if (prevFlaggedBlock) {
          ...
          await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.block.flag.${checkCoinbaseBlock.getHeight()}`, scanBackHeight, { sync: true });
          this._engine._emitter.emit('requestBlockRange', [scanBackHeight + BC_MAX_DATA_RANGE, scanBackHeight], 2);
          return;
        }
      }
    }
    ...
    if (blocksLength < BC_MAX_DATA_RANGE && validBlocks > 0 && parseInt(evalBlock.getHeight(), 10) + BC_MAX_DATA_RANGE < highestKnownHeight && finalValidBlocks.length > 9 && finalValidBlocks.length !== 1 && parseInt(evalBlock.getHeight(), 10) < highestKnownHeight) {
      ...
      if (this._PEER_QUARANTINE.length >= 16 && this._PEER_BLACKLIST.length > 4) {
        ...
        if (BC_ORTHOGONAL_WAYPOINTS) {
          ...
          await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.data.latest`, `${movingEdge}:${tn}`, { sync: true });
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`, { sync: true });
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`, { sync: true });
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      const t = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.data.latest`, { sync: true });
      ...
      if (this._requestRegistry[peerRequestKey] > 600) {
        ...
        await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.req.range`, `${nextHeight}:${currentHeight + 1}:${Math.floor(now / 1000)}:${addressToHost(address)}`, { sync: true });
        const payload = encodeTypeAndData(MESSAGES.GET_DATA, data);
        const sent = await this.qsend(conn, payload);
        if (sent !== undefined && sent.success) {
          // LDL
  ...
  async start(nodeId) {
    ...
    this._engine._emitter.on('requestBlockRange', async (range, openLink) => {
      ...
      if (givenConnection) {
        hasAddress = addr;
        currentPeer = await this._engine.persistence.get('bc.sync.initialpeer', { sync: true });
        if (currentPeer !== null && parseInt(currentPeer.getExpires(), 10) >= Number(new Date()) && currentPeer.getAddress() !== addr) {
          ...
          currentPeer.setExpires(Number(new Date()) + BC_PEER_HEADER_SYNC_EXPIRE);
          await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.initialpeer`, currentPeer, { sync: true });
        } else {
        ...
        currentPeer = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.sync.initialpeer`, { sync: true });
        if (currentPeer !== null && parseInt(currentPeer.getExpires(), 10) >= Number(new Date()) && currentPeer.getAddress() !== addr) {
        ...
        if (this._discovery.connections) {
          ...
          if (range[2] && pass) {
            ...
            if (sendToAnyPeer && p < 1 && this._PEER_EXTENDED_QUARANTINE.indexOf(address) < 0 && this._PEER_BLACKLIST.indexOf(address) < 0 && this._PEER_BLACKLIST.indexOf(host) < 0 && this._PEER_QUARANTINE.indexOf(addressToHost(address)) < 0 && sendToPool) {
              ...
              p += 1;
              await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.req.range`, `${highest}:${lowest}:${now}:${host}`, { sync: true });
          ...
          if (this._discovery.connections.length > 0) {
            ...
            for (const remoteConnection of connections) {
              ...
              if (sendToPool && p < limit && this._PEER_QUARANTINE.indexOf(host) < 0 && this._PEER_BLACKLIST.indexOf(host) < 0 && this._PEER_BLACKLIST.indexOf(address) < 0) {
                p += 1;
                this._logger.info(`direct request to address from ${lowest} to ${highest}`);
                await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.req.range`, `${highest}:${lowest}:${now}:${host}`, { sync: true });
    ...
    this._discovery._seeder.on('peer', peer => {
      ...
      this._logger.info(`waypoint discovered ${h}, raw ${peer}`);
      const obj = {
        host: h[0],
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (synced !== 'complete' && initialpeer) {
        ...
        if (waypoint && waypoint.getAddress) {
          this.addToQuarantine(waypoint.getAddress(), `no reponse for range request`, true);
        } else {
