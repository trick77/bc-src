...
const BC_QUARANTINE_FORGIVENESS = BC_BLOODLINE ? 35000 : getRandomWithinRange(70166, 124900);
...
class PeerNode {
  ...
  addToQuarantine(addr, reason) {
    ...
    this._logger.info(`added ${addr} to quarantine <- "${reason}"`);
  }
              ...
              for (let req of mountBlockReq) {
                ...
                if (mountBlocks) {
                  ...
                  if (!foundRootBlockToMountBranch) {
                    ...
                    await this._engine.persistence.delBlock(newBlock.getHash(), 0, BC_SUPER_COLLIDER);
                  } else {
      ...
      if (remount) {
        ...
        if (unmount) {
          ...
          if (new BN(unmount).gt(new BN(checkCoinbaseBlock.getHeight()))) {
            ...
            await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
            this._engine._emitter.emit('requestBlockRange', [unmount + BC_MAX_DATA_RANGE, unmount - 8], 4);
            return;
          }
        } else {
          forceBreak = true;
          this._logger.info(`unmount not set on current block: ${checkCoinbaseBlock.getHeight()}`);
          this.addToQuarantine(address, 'unable to find range of blocks on which to mount');
      ...
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (latestBlock && latestBlock.getHeight() > 1 && block.getPreviousHash() === block.getHash()) {
          ...
          if (latestBlockChildSum > newBlockChildSum) {
            ...
            this.addToQuarantine(conn.remoteAddress, `weight of child sum for latest block below local latest`);
        ...
        if (latestBlock) {
          ...
          if (latestBlockChildSum > newBlockChildSum && block.getHeight() < latestBlock.getHeight()) {
            ...
            this.addToQuarantine(address, `weight of latest child block and main chain height is low`);
            const waypointKey = conn.remoteAddress + latestBlock.getHash();
            if (!this._engine._peerRequestCache.has(waypointKey)) {
              this._engine._peerRequestCache.set(waypointKey, true);
              this._engine._emitter.emit(`sendblock`, { data: block, connection: conn });
        ...
        if (!currentPeer && synced && synced === 'complete' && this._processedBlocks > 2) {
          ...
          await this._engine.persistence.putBlock(block, 0, BC_SUPER_COLLIDER, { asHeader: false, fromWaypoint: true, saveHeaders: false });
