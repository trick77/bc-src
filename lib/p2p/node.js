...
const { InitialPeer, BcBlocks, Record, Config, PutConfig, Service, PutService } = require('@overline/proto/proto/p2p_pb');
...
const BC_QUARANTINE_FORGIVENESS = BC_BLOODLINE ? 85000 : getRandomWithinRange(90166, 144900);
const BC_EXTENDED_QUARANTINE_FORGIVENESS = BC_BLOODLINE ? 8850000 : getRandomWithinRange(9000166, 14490000);
...
const loadBasedPeerExpiration = logline ? 80000 : 120000 + Math.floor(80000 / numCPUs);
...
class PeerNode {
  ...
  constructor(engine) {
    ...
    this._dataObjectCache = new LRUCache({
      max: 30,
      maxAge: getRandomWithinRange(2500, 5900)
    });
    ...
    setInterval(() => {
      ...
      if (sq) {
        ...
        debug(`${this._PEER_EXTENDED_QUARANTINE.length} waypoints in quarantine`);
      }
  ...
  addToQuarantine(addr, reason, extended) {
    ...
    this._PEER_QUARANTINE.splice(this._PEER_QUARANTINE.indexOf(addr), 1);
    this._PEER_QUARANTINE.push(addr);

    if (extended) {
      this._PEER_EXTENDED_QUARANTINE.splice(this._PEER_QUARANTINE.indexOf(addr), 1);
      this._PEER_EXTENDED_QUARANTINE.push(addr);
    }
    debug(`added ${addr} to quarantine <- "${reason}"`);
  }
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (blocks && blocks.length > 0) {
      ...
      if (startGivenBlockHeight > waypointRequestLow + BC_MAX_DATA_RANGE * 20) {
        ...
        this.addToQuarantine(address, 'waypoint responded with block range higher than requested');
      ...
      if (startGivenBlockHeight < waypointRequestLow - BC_MAX_DATA_RANGE * 2) {
        ...
        this.addToQuarantine(address, 'waypoint responded with block range lower than requested');
        await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
    ...
    do {
      ...
      if (newBlock !== undefined && newBlock !== null && !invalidData && prevBlock && prevBlock.getHeight && parseInt(newBlock.getHeight(), 10) > 1) {
        ...
        //const isValidCoinbase = flag ? await validateCoinbase(newBlock, this._engine.persistence, this._engine, this._engine._txHandler, this._engine._txPendingPool, this._engine._utxoManager, 'p2p.node') : true
        //const isValidCoinbase = await validateCoinbase(newBlock, this._engine.persistence, this._engine, this._engine._txHandler, this._engine._txPendingPool, this._engine._utxoManager, 'p2p.node')
        ...
        if (isValidCoinbase) {
          ...
          if (opts.innerCall) {
            ...
            if (blockQueue.length === 0) {
              ...
              const stepBack = Math.floor(Math.random() * w) + Math.floor(Math.random() * Math.floor(6));
              let missingBlockHeight = true;
              const unmount = await this._engine.persistence.getUtxoUnmount();
              if (!unmount) {
                missingBlockHeight = await getCoinbaseMissingBlockHeight(newBlock, this._engine.persistence, this._engine._txHandler, this._engine._txPendingPool, this._engine._utxoManager, 'p2p.node.missing.height');
              } else if (unmount) {
                missingBlockHeight = await getCoinbaseMissingBlockHeight(newBlock, this._engine.persistence, this._engine._txHandler, this._engine._txPendingPool, this._engine._utxoManager, 'p2p.node.missing.height');
              ...
              if (BC_ORTHOGONAL_WAYPOINTS) {
                ...
                this.addToQuarantine(address, 'waypoint sent missing range of blocks');
    ...
    //let validcb = (checkCoinbaseBlock.getHeight() + getRandomWithinRange(50,500)) > highestKnownHeight ? await validateCoinbase(checkCoinbaseBlock, this._engine.persistence, this._engine, this._engine._txHandler, this._engine._txPendingPool, this._engine._utxoManager, 'p2p.node') : true
    ...
    if (!validcb) {
      ...
      if (remount) {
        ...
        if (remount >= lowestBoundaryBlocks) {
          ...
          this.addToQuarantine(addressToHost(address), `critical reindex required`);

          const stepBackHeight = remount - getRandomWithinRange(180, 280);
          ...
          if (bh) {
            ...
            // await this._engine.persistence.saveTxs(180000, evalBlock)
          ...
          if (evalBlock) {
            // await this._engine.persistence.saveTxs(180000, evalBlock)
              ...
              // await this._engine.persistence.saveTxs(180000)
            }
          this._engine._emitter.emit('requestBlockRange', [stepBackHeight + BC_MAX_DATA_RANGE, stepBackHeight - 1], 2);
          return;
        }
        ...
        if (unmount) {
          ...
          if (new BN(unmount).gt(new BN(checkCoinbaseBlock.getHeight()))) {
            ...
            this._logger.info(`searching for mount at: ${unmount} without penalty`);
            if (invalidBlockHeights.indexOf(unmount) < 0) {
              invalidBlockHeights.push(unmount);
            }
            await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
            await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`);
            this._engine._emitter.emit('requestBlockRange', [unmount + BC_MAX_DATA_RANGE, unmount - 4], 4);
            return;
          }
        } else {
          forceBreak = true;
          this._logger.info(`unmount not set on current block: ${checkCoinbaseBlock.getHeight()}`);
          //this.addToQuarantine(address, 'unable to find range of blocks on which to mount', true)
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`);
          this._engine._emitter.emit('requestBlockRange', [checkCoinbaseBlock.getHeight() + BC_MAX_DATA_RANGE, checkCoinbaseBlock.getHeight() - 8, conn]);
          return;
        }
      }
    }
    ...
    if (blocksLength < BC_MAX_DATA_RANGE && validBlocks > 0 && parseInt(evalBlock.getHeight(), 10) + BC_MAX_DATA_RANGE < highestKnownHeight && finalValidBlocks.length > 0 && finalValidBlocks.length !== 1 && parseInt(evalBlock.getHeight(), 10) < highestKnownHeight) {
      ...
      if (logline) {
        ...
        await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
        this.addToQuarantine(address, 'logline enabled strict block validation enforced');
        await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`);
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      if (this._requestRegistry[peerRequestKey] > 600) {
        ...
        if (sent !== undefined && sent.success) {
          // LDL
          debug(`GET_DATA sent to ${sent.address}, message: ${sent.message}`);
          // const validcblazy = getRandomWithinRange(1, 10) < 3 && checkCoinbaseBlock.getHeight() + 1000 < highestKnownHeight && validcb === true ? await validateCoinbase(checkCoinbaseBlock, this._engine.persistence, this._engine, this._engine._txHandler, this._engine._txPendingPool, this._engine._utxoManager, 'p2p.node') : true
          ...
          if (!validcblazy) {
            ...
            if (unmount) {
              await this._engine.persistence.delUtxoUnmount();
              this.addToQuarantine(address, 'unmount found on local node from waypoint');

              if (new BN(unmount).gt(new BN(checkCoinbaseBlock.getHeight()))) {
                this._logger.info(`active sync in progress from mount ${unmount}`);
              } else {
                this._logger.info(`cleared block queue <- searching for mount at: ${unmount}`);
                fromHeight = unmount;
              ...
              if (remount) {
                ...
                this.addToQuarantine(address, 'waypoint issued temporary unmountable sequence');
                if (this._discovery.connected && this._discovery.connected - this._PEER_QUARANTINE.length < 2) {
                  this._PEER_QUARANTINE.pop();
                }
                this._engine._emitter.emit('requestBlockRange', [toHeight - 10, fromHeight - 12], 3);
  ...
  async start(nodeId) {
    ...
    this._engine._emitter.on('requestBlockRange', async (range, openLink) => {
      ...
      if (this._engine._blockQueue.length() > 0 && openLink !== 1) {
        this._logger.info(`block queue has not drained`);
        this._engine._blockQueue.process();
        //return
      }
          ...
          if (range[2] && pass) {
            ...
            if (sendToAnyPeer && p < 1 && this._PEER_BLACKLIST.indexOf(address) < 0 && this._PEER_BLACKLIST.indexOf(host) < 0 && this._PEER_QUARANTINE.indexOf(addressToHost(address)) < 0 && sendToPool) {
              ...
              if (sent && sent.success) {
                ...
                this.addToQuarantine(address, 'connection failed range request');
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (this._PEER_EXTENDED_QUARANTINE.indexOf(addressToHost(address)) > -1 && addressToHost(address) !== BC_BIND_PEER) {
        // LDL
        debug(`waypoint ${address} in quarantine and attempted to send BLOCK`);
        debug(`[EXTENDED] <- OL BLOCK ${block.getHeight()} ${block.getHash()} ${block.getDifficulty()} <- ${address} <- ${block.getMiner()}`);
        return;
      }
      ...
      const exp = await this._engine.persistence.processPeerExpiration();
      ...
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (latestBlock) {
          ...
          if (latestBlockChildSum > newBlockChildSum && block.getHeight() < latestBlock.getHeight()) {
            this._logger.info(`weight too low <- ${latestBlock.getHeight()} <> ${block.getHeight()}`);
            this.addToQuarantine(address, `weight of latest child block and main chain height is low`, true);
            const waypointKey = conn.remoteAddress + latestBlock.getHash();
            if (!this._engine._peerRequestCache.has(waypointKey)) {
              this._engine._peerRequestCache.set(waypointKey, true);
              this._engine._emitter.emit(`sendblock`, { data: block, connection: conn });
        ...
        if (highestKnownHeight && parseInt(highestKnownHeight, 10) - 60 > parseInt(block.getHeight(), 10)) {
      ...
      if (parseInt(latestBlock.getHeight(), 10) - 16 < high && synced !== 'complete') {
