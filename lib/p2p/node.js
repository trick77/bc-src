            ...
            if (syncProgress > 0) {
              ...
              if (rawSyncProgress > 99.9999 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
                ...
                if (rawSyncProgress > 99.99) {
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} ${this._childBlocksSynced}:${childBlocks} ${newBlock.getHash().slice(0, 8)}:${newBlock.getPreviousHash().slice(0, 8)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
                } else if (newBlock.getHeight() % 2 === 0) {
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} ${this._childBlocksSynced}:${childBlocks} ${syncProgress}% <- ${newBlock.getHash().slice(0, 8)}:${newBlock.getPreviousHash().slice(0, 8)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
                } else {
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} ${this._childBlocksSynced}:${childBlocks} ${syncProgress}% <- ${newBlock.getPreviousHash().slice(0, 8)}:${newBlock.getHash().slice(0, 8)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
                }
              }
            }
