...
const BC_QUARANTINE_FORGIVENESS = BC_BLOODLINE ? 85000 : getRandomWithinRange(30166, 94900);
...
const loadBasedPeerExpiration = logline ? 20000 : 30000 + Math.floor(80000 / numCPUs);
    ...
    if (!validcb) {
      ...
      if (remount) {
        ...
        if (remount >= lowestBoundaryBlocks) {
          ...
          const stepBackHeight = remount - getRandomWithinRange(10080, 20080);
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (latestBlock && latestBlock.getHeight() > 1 && block.getPreviousHash() === block.getHash()) {
          ...
          if (latestBlockChildSum - 3 > newBlockChildSum) {
        ...
        if (latestBlock) {
          ...
          if (latestBlockChildSum - 10 > newBlockChildSum && synced === 'complete' || latestBlock.getHeight() - 30 > block.getHeight()) {
        ...
        if (highestKnownHeight && parseInt(highestKnownHeight, 10) - 30 > parseInt(block.getHeight(), 10)) {
      ...
      if (parseInt(latestBlock.getHeight(), 10) + 100 < high && synced === 'pending') {
