...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (blocks && blocks.length > 0) {
      // LDL
      this._logger.info(`MESSAGES.DATA received [] <- ${blocks.length} blocks`);
    ...
    if (newQueue.length === 0) {
      this._logger.info(`block queue seen, minimumValidBlocks = ${minimumValidBlocks}`);
      ...
      this._logger.info(`newQueue = ${newQueue.length}, minimumValidBlocks = ${minimumValidBlocks}`);
      minimumValidBlocks = max(0, min(newQueue.length - 4, BC_MAX_DATA_RANGE) - 3);
      blockQueue = newQueue;
    }
      ...
      if (this._knownHashes.indexOf(newBlock.getHash()) > -1) {
        ...
        if (syncProgress > 0) {
          ...
          if (syncProgress > 99.99 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
            ...
            if (blockQueue.length === 0) {
              this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} <- ${newBlock.getHash().slice(0, 20)} ${newBlock.getPreviousHash().slice(0, 20)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
            }
          }
        }
            ...
            if (syncProgress > 0) {
              ...
              if (syncProgress > 99.99 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
                ...
                if (syncProgress > 99.99) {
                  ...
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} ${syncProgress}% <- ${newBlock.getHash().slice(0, 20)}:${newBlock.getPreviousHash().slice(0, 20)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
                } else {
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} ${syncProgress}% <- ${newBlock.getPreviousHash().slice(0, 20)}:${newBlock.getHash().slice(0, 20)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
                }
              }
            }
    ...
    if (!validcb) {
      ...
      if (unmount) {
        ...
        if (new BN(unmount).gt(new BN(checkCoinbaseBlock.getHeight()))) {
          this._logger.info(`active sync in progress`);
        } else {
  ...
  async start(nodeId) {
      ...
      if (highest !== lowest) {
        ...
        if (this._discovery.connections) {
          ...
          if (range[2] && pass) {
            ...
            this._logger.info(`sending range request from ${lowest} to ${highest}`);
    ...
    this._discovery._seeder.on('peer', peer => {
      ...
      if (this._discovery.connected > BC_MAX_CONNECTIONS + 30) {
