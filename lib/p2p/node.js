...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    const lowestHeightRawBoundary = reorgFromBlock && reorgFromBlock.getHeight ? parseInt(reorgFromBlock.getHeight(), 10) - 19000 : latestHeightRaw - 19000;
    ...
    if (!validcb) {
      ...
      if (remount) {
        ...
        if (unmount) {
          ...
          let scanBackHeight = checkCoinbaseBlock.getHeight() - 20400;
          ...
          if (prevFlaggedBlock) {
            ...
            scanBackHeight = prevFlaggedBlock - 20400;
          }
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`);
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (latestBlock.getTimestamp() + 60 > currentTimeSeconds && synced === 'complete' && highestKnownHeight && parseInt(highestKnownHeight, 10) - 30 > parseInt(block.getHeight(), 10)) {
