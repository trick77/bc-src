  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (blocks && blocks.length > 0) {
      ...
      if (startGivenBlockHeight < waypointRequestLow - BC_MAX_DATA_RANGE * 2) {
        ...
        this.addToQuarantine(address, `waypoint responded with block range lower than requested: ${waypointRequestLow} given: ${startGivenBlockHeight}`, true);
          ...
          if (new BN(unmount).gt(new BN(checkCoinbaseBlock.getHeight()))) {
            ...
            if (unmountRaw) {
              ...
              if (!block) {
                ...
                this._engine._emitter.emit('requestBlockRange', [Number(uheight) + BC_MAX_DATA_RANGE, Number(uheight) - 4, conn]);
                return;
              }
