...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
      ...
      if (this._knownHashes.indexOf(newBlock.getHash()) > -1) {
        ...
        if (syncProgress > 0) {
          ...
          if (syncProgress > 99.99 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
            ...
            if (blockQueue.length === 0) {
              this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} <- ${this._childBlocksSynced} <- ${newBlock.getHash().slice(0, 20)} ${newBlock.getPreviousHash().slice(0, 20)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
            }
          }
        }
            ...
            if (syncProgress > 0) {
              ...
              if (syncProgress > 99.99 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
                ...
                if (syncProgress > 99.99) {
                  ...
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} <- ${this._childBlocksSynced} ${syncProgress}% <- ${newBlock.getHash().slice(0, 20)}:${newBlock.getPreviousHash().slice(0, 20)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
                } else {
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} <- ${this._childBlocksSynced} ${syncProgress}% <- ${newBlock.getPreviousHash().slice(0, 20)}:${newBlock.getHash().slice(0, 20)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
                }
              }
            }
    ...
    const validcb = checkCoinbaseBlock.getHeight() + 1000 > highestKnownHeight ? await validateCoinbase(checkCoinbaseBlock, this._engine.persistence, this._engine._txHandler, 'p2p.node') : true;
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      if (this._requestRegistry[peerRequestKey] > 600) {
        ...
        if (sent !== undefined && sent.success) {
          // LDL
          debug(`GET_DATA sent to ${sent.address}, message: ${sent.message}`);
          const validcblazy = parseInt(checkCoinbaseBlock.getHeight(), 10) + 1000 < highestKnownHeight ? await validateCoinbase(checkCoinbaseBlock, this._engine.persistence, this._engine._txHandler, 'p2p.node') : true;
          if (!validcblazy) {
