    ...
    if (!validcb) {
      ...
      if (remount) {
        ...
        if (remount >= lowestBoundaryBlocks) {
          ...
          const stepBackHeight = remount - getRandomWithinRange(580, 980);
          ...
          if (bh) {
            ...
            await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
            await this._engine.persistence.reorgBlockchain(BC_SUPER_COLLIDER, { fromBlock: bh });
            this._engine._emitter.emit('requestBlockRange', [stepBackHeight + BC_MAX_DATA_RANGE, stepBackHeight - 1], 3);
            return;
          }
        ...
        if (unmount) {
          ...
          if (new BN(unmount).gt(new BN(checkCoinbaseBlock.getHeight()))) {
            ...
            this._engine._emitter.emit('requestBlockRange', [unmount + BC_MAX_DATA_RANGE, unmount - 1], 3);
            return;
          }
        } else {
