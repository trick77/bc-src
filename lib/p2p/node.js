...
class PeerNode {
  ...
  constructor(engine) {
    ...
    this._logger.info(`node waypoint launching with expiration ${BC_PEER_HEADER_SYNC_EXPIRE}`);
  ...
  async start(nodeId) {
    ...
    this._engine._p2pEmitter.on('requestBlockRange', async range => {
      ...
      if (highest !== lowest) {
        ...
        if (this._discovery.connections) {
          ...
          if (range[2]) {
            ...
            if (sendToAnyPeer && p < 1 && this._PEER_BLACKLIST.indexOf(address) < 0 && this._PEER_QUARANTINE.indexOf(addressToHost(address)) < 0) {
              // LDL
              ...
              if (sent && sent.success) {
                ...
                setTimeout(() => {
                  this._engine._p2pEmitter.emit('requestBlockRange', [range[0], range[1]]);
                }, 500);
              }
    ...
    this._discovery.join(this._discovery.hash, this._discovery.port, data => {
      ...
      this._engine._p2pEmitter.on('sendblock', msg => {
