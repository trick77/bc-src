              ...
              if (BC_ORTHOGONAL_WAYPOINTS) {
                ...
                if (missingBlockHeight && missingBlockHeight !== true) {
                  ...
                  if (!assertValidTxs) {
                    ...
                    validBlocks++;
                    this._logger.info(`segment delayed for block ${newBlock.getHeight()} : ${newBlock.getHash().slice(0, 21)}, prev: ${newBlock.getPreviousHash().slice(0, 21)} `);
                  }
                }
              }
            } else {
    ...
    this._engine._asyncEmitter.on('requestBlockRange', async range => {
      ...
      if (givenConnection) {
        ...
        if (currentPeer !== null && parseInt(currentPeer.getExpires(), 10) >= Number(new Date()) && currentPeer.getAddress() !== addr) {
          this._logger.info(`range request pending waypoint ${currentPeer.getAddress()} unlock: ${parseInt(currentPeer.getExpires(), 10)}`);
          return;
        }
      }
      ...
      if (highest !== lowest) {
        ...
        if (this._discovery.connections) {
          ...
          if (range[2]) {
            ...
            if (sendToAnyPeer && p < 9 && this._PEER_BLACKLIST.indexOf(address) < 0 && this._PEER_QUARANTINE.indexOf(addressToHost(address)) < 0) {
              // LDL
