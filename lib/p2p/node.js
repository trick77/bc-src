...
const _MAX_FRAME_SIZE = 20 * 1024 * 1024; // 20MB
const FRAMING_OPTS = {
  lengthSize: 4,
...
const BC_QUARANTINE_FORGIVENESS = BC_BLOODLINE ? 55000 : getRandomWithinRange(50166, 92600);
...
const loadBasedPeerExpiration = logline ? 140000 : 150000 + Math.floor(80000 / numCPUs);
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
        ...
        if (syncProgress > 0) {
          ...
          if (rawSyncProgress > 99.999 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
            ...
            if (blockQueue.length === 0) {
              ...
              this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} (${max(0, Math.abs(this._highestRequestedRangeHeight - parseInt(newBlock.getHeight(), 10)))}) ${this._childBlocksSynced}:${childBlocks} <- ${newBlock.getHash().slice(0, 8)} ${newBlock.getPreviousHash().slice(0, 8)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
            }
          }
        }
            ...
            if (syncProgress > 0) {
              ...
              if (rawSyncProgress > 99.9999 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
                ...
                if (rawSyncProgress > 99.99) {
                  ...
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} (${max(0, Math.abs(this._highestRequestedRangeHeight - parseInt(newBlock.getHeight(), 10)))}) ${this._childBlocksSynced}:${childBlocks} ${syncProgress}% <- ${newBlock.getHash().slice(0, 8)}:${newBlock.getPreviousHash().slice(0, 8)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
                } else {
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} (${max(0, Math.abs(this._highestRequestedRangeHeight - parseInt(newBlock.getHeight(), 10)))}) ${this._childBlocksSynced}:${childBlocks} ${syncProgress}% <- ${newBlock.getPreviousHash().slice(0, 8)}:${newBlock.getHash().slice(0, 8)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
                }
              }
            }
    ...
    if (!validcb) {
      ...
      if (remount) {
        ...
        if (remount >= lowestBoundaryBlocks) {
          ...
          const stepBackHeight = remount - getRandomWithinRange(30, 80);
        ...
        if (unmount) {
          ...
          forceBreak = true;
          this._logger.info(`unmount not set on current block: ${checkCoinbaseBlock.getHeight()}`);
          if (invalidBlockHeights.indexOf(checkCoinbaseBlock.getHeight() - 10) < 0) {
            invalidBlockHeights.push(checkCoinbaseBlock.getHeight() - 10);
          }
        }
      }
    }
    ...
    if (blocksLength < BC_MAX_DATA_RANGE && validBlocks > 0 && parseInt(evalBlock.getHeight(), 10) + BC_MAX_DATA_RANGE < highestKnownHeight && finalValidBlocks.length > 0 && parseInt(evalBlock.getHeight(), 10) < highestKnownHeight) {
      ...
      if (BC_ORTHOGONAL_WAYPOINTS) {
        ...
        await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
        this._engine._emitter.emit('requestBlockRange', [sFrom + BC_MAX_DATA_RANGE, sFrom - 2], 2);
        return;
      ...
      if (this._PEER_QUARANTINE.length >= 16 && this._PEER_BLACKLIST.length > 4) {
        ...
        if (BC_ORTHOGONAL_WAYPOINTS) {
          ...
          if (finalInvalidBlocks.length > 0) {
            movingEdge = finalInvalidBlocks[0] - Math.floor(Math.random() * Math.floor(moveBackRange + 100));
          }
          ...
          this._engine._emitter.emit('requestBlockRange', [movingEdge + BC_MAX_DATA_RANGE - 2, movingEdge - 2], numberOfWaypoints);
        } else {
          ...
          //  }
          //  this._engine._emitter.emit('requestBlockRange', [parseInt(finalValidBlock.getHeight(), 10) - 8, parseInt(finalValidBlock.getHeight(), 10) + BC_MAX_DATA_RANGE], numberOfWaypoints)
          //} else {
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      if (this._requestRegistry[peerRequestKey] > 600) {
        ...
        if (sent !== undefined && sent.success) {
          ...
          this._PEER_QUARANTINE.unshift(addressToHost(address));
          this._engine._emitter.emit('requestBlockRange', [data[1] - 10, data[0] - 10], 1);
          return;
        }
      }
          ...
          if (range[2] && pass) {
            ...
            for (const remoteConnection of connections) {
              ...
              if (sendToPool && p < limit && this._PEER_QUARANTINE.indexOf(host) < 0 && this._PEER_BLACKLIST.indexOf(host) < 0) {
                ...
                await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.req.range`, `${highest}:${lowest}:${now}:${host}`);
                const t = p === 1 ? 0 : getRandomWithinRange(100, 3000);
                await this.qsend(remoteConnection, payload);
        ...
        for (const conn of shuffle(this._discovery.connections)) {
          (async () => {
            try {
              ...
              await this.qsend(conn, payload);
              //this.qsend(conn, payload).then(() => {
              debug(`announced block to ${addressToHost(addr)}`);
              //}).catch((e) => {
              //  debug(e)
              //})
            } catch (e) {
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (latestBlock && parseInt(latestBlock.getHeight(), 10) >= parseInt(block.getHeight(), 10)) {
          this._logger.info(`weight too low <- ${latestBlock.getHeight()} <> ${block.getHeight()}`);
          this._PEER_QUARANTINE.push(addressToHost(address));
          return;
        }

        if (highestKnownHeight && parseInt(highestKnownHeight, 10) >= parseInt(block.getHeight(), 10)) {
          this._logger.info(`weight is low <- ${highestKnownHeight} <> ${block.getHeight()}`);
          //return
        }
