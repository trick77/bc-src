...
class PeerNode {
  ...
  getWriteSocket(addr) {
    ...
    const possibleConnection = this._discovery.connections.reduce((found, conn) => {
      if (!found) {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    blockQueue.forEach(async block => {
    ...
    if (!validcb) {
      ...
      if (highestBlockRemount) {
        await this._engine.persistence.delUtxoRemount();
        if (highestBlockRemount >= lowestBoundaryBlocks) {
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      if (Object.keys(this._engine._lastCollectedBlock).length > 0 && this._engine._lastOlBlockTime) {
        const cachedBlocks = Object.keys(this._engine._lastCollectedBlock);
        const r = cachedBlocks[Math.floor(Math.random() * cachedBlocks.length)];
        this._engine._roverEmitter.emit('collectBlock', { block: this._engine._lastCollectedBlock[r] });
      }
      ...
      ///const roverList = []
      ///for (let key of this._engine._knownRovers) { roverList.push(key) }
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (synced !== 'complete' && initialPeer && latestBlock && block.getHeight() - 200 > latestBlock.getHeight()) {
        ...
        if (initialPeer && initialPeer.getAddress() !== addressToHost(address)) {
          ...
          const waypoint = await this._engine.persistence.processPeerExpiration();
          return;
      ...
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (highestKnownHeight && highestKnownHeight + 1 === block.getHeight() || highestKnownHeight && highestKnownHeight + 1 === block.getHeight()) {
          if (latestBlock.getHeight() + 12 > block.getHeight()) {
