    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      if (this._requestRegistry[peerRequestKey] > 600) {
        ...
        if (sent !== undefined && sent.success) {
          // LDL
          debug(`GET_DATA sent to ${sent.address}, message: ${sent.message}`);
          const validcblazy = getRandomWithinRange(1, 10) < 5 && checkCoinbaseBlock.getHeight() + 1000 < highestKnownHeight && validcb ? await validateCoinbase(checkCoinbaseBlock, this._engine.persistence, this._engine._txHandler, 'p2p.node') : true;
          if (!validcblazy) {
            let toHeight = data[1];
            ...
            this._logger.warn(`waypoint issued invalid sequence at ${checkCoinbaseBlock.getHeight()}`);
            this._PEER_QUARANTINE.unshift(addressToHost(address));
