...
let BC_MAX_WAYPOINT_LATENCY = isNaN(process.env.BC_MAX_WAYPOINT_LATENCY) ? 660 : process.env.BC_MAX_WAYPOINT_LATENCY;
...
const loadBasedPeerExpiration = 45000 + Math.floor(30000 / numCPUs);
/* OVERLINE
...
const MAX_DATA_RANGE = exports.MAX_DATA_RANGE = Number(process.env.MAX_DATA_RANGE) || 8;
...
const PEER_DATA_SYNC_EXPIRE = 62661; // Peer must return a block / data / network request before time elapsed (milliseconds)
...
class PeerNode {
  ...
  constructor(engine) {
    ...
    if (waypointStatStream) {
      waypointStatStream.write(`processTime,elapsed,currentHeight,nextHeight,meanElapsed,weightedElapsed,address\n`);
    }

    this._logger.info(`node p2p interface starting with expiration ${BC_PEER_HEADER_SYNC_EXPIRE}`);

    setInterval(() => {
      this._PEER_QUARANTINE.shift();
      //}, 3000)
    }, 103000);
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (this._PEER_QUARANTINE.indexOf(addressToHost(address)) > -1 && addressToHost(address) !== BC_BIND_PEER) {
      this._logger.debug(`waypoint ${address} in quarantine and attempted to send DATA consider adding to blacklist`);
      return;
    }
    if (this._PEER_BLACKLIST.indexOf(addressToHost(address)) > -1) {
      this._logger.debug(`waypoint ${address} in BLACKLIST and attempted to send DATA`);
      return;
    }
    ...
    if (!currentPeer || !currentPeer.setExpires) {
      ...
      if (reorgBlock) {
        ...
        this._logger.info(`yielding DATA from unset new waypoint with request in place`);
        //return
        currentPeer = new InitialPeer();
        currentPeer.setAddress(addressToHost(address));
        currentPeer.setExpires(Number(new Date()) + BC_PEER_HEADER_SYNC_EXPIRE);
    ...
    if (dataLatestStr === null) {
      const now = Date.now();
      await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.data.latest`, `${lowestBlock[0]}:${now}`);
      latestHeightRaw = 2;
    } else {
    ...
    if (this._requestRegistry[peerKey] > 1500) {
      this._PEER_QUARANTINE.push(address);
    ...
    do {
      ...
      temporaryBlockStore[newBlock.getHeight()] = true;
      // LDL
      ...
      if (this._knownHashes.indexOf(newBlock.getHash()) > -1 && blockQueue.length !== 0) {
        ...
        if (syncProgress > 0) {
          ...
          if (syncProgress > 99.97 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
            ...
            this._logger.info(`COLLIDER ${newBlock.getHeight()} ${this._childBlocksSynced}:${highestKnownHeight}-${this._multiverseCompressions} [] ${newBlock.getHash().slice(0, 8)}:${newBlock.getPreviousHash().slice(0, 8)} NRG Multichain ${syncProgress}% <- Miner EMB Boost: +${this._engine._emblemPerformance}%`);
          }
        }
      ...
      let prevBlock = temporaryBlockStore[newBlock.getPreviousHash()] ? temporaryBlockStore[newBlock.getPreviousHash()] : await this._engine.persistence.getBlockByHash(newBlock.getPreviousHash(), BC_SUPER_COLLIDER, { asHeader: false, cached: true });
      if (!prevBlock) {
        const prevBlockOpts = await this._engine.persistence.getBlocksByHeight(parseInt(newBlock.getHeight(), 10) - 1, BC_SUPER_COLLIDER, { asHeader: false, cached: true });
        if (prevBlockOpts) {
      ...
      if (prevBlock) {
        temporaryBlockStore[prevBlock.getHash()] = prevBlock;
      }
      ...
      if (!prevBlock && newBlockBoundary < highestKnownHeight) {
        ...
        const nb = await this._engine.persistence.getBlocksByHeight(parseInt(newBlock.getHeight(), 10) - 1, BC_SUPER_COLLIDER, { asHeader: false, cached: true });
        await this._engine.persistence.putBlock(newBlock, 0, BC_SUPER_COLLIDER, { asHeader: false, updateHeight: true, saveHeaders: true });
        if (nb) {
          for (let b of nb) {
        ...
        if (isValidCoinbase) {
          ...
          if (prevBlock && prevBlock.getHeight() !== newBlock.getHeight()) {
            ...
            if (mountBlockReq) {
              ...
              for (let req of mountBlockReq) {
                ...
                if (mountBlocks) {
                  ...
                  for (let mb of mountBlocks) {
                    ...
                    const bl = await this._engine.persistence.getBlockByHash(mb, BC_SUPER_COLLIDER, { asHeader: false, cached: true });
                    if (bl) {
              ...
              await this._engine.persistence.putBlock(prevBlock, 0, BC_SUPER_COLLIDER, { asHeader: false, force: true, saveHeaders: true });
            ...
            if (prevBlock && new BN(newBlock.getTotalDistance()).lt(new BN(prevBlock.getTotalDistance()))) {
              if (blockQueue[0] && parseInt(blockQueue[0].getHeight(), 10) !== parseInt(newBlock.getHeight(), 10)) {
                const nb = await this._engine.persistence.getBlocksByHeight(parseInt(newBlock.getHeight(), 10), BC_SUPER_COLLIDER, { asHeader: false, cached: true });
                if (nb) {
                  for (let b of nb) {
                    if (b.getHash() !== newBlock.getHash() && reviewBlockHashes.indexOf(b.getHash()) < 0) {
                      reviewBlockHashes.push(b.getHash());
              ...
              await this._engine.persistence.putBlock(newBlock, 0, BC_SUPER_COLLIDER, { asHeader: false, saveHeaders: true });
              continue;
            }
            ...
            if (new BN(purposedBlockChildHeightSum).lt(new BN(latestBlockChildHeightSum))) {
              ...
              //this._logger.info(`removing correct block ${newBlock.getHeight()}`)
              //if (invalidBlockHeights.length > 0) {
              //  invalidBlockHeights = invalidBlockHeights.filter((b) => {
              //    if (b !== parseInt(newBlock.getHeight(), 10) &&
              //      parseInt(newBlock.getHeight(), 10) !== 1) {
              //      return b
              //    }
              //  })
              //}
          ...
          if (opts.innerCall) {
            ...
            if (blockQueue.length === 0) {
              ...
              sequenceFailed = true;
              this._logger.warn(`sequence confirmation failed block for ${newBlock.getHeight()} : ${newBlock.getHash().slice(0, 21)}, prev: ${newBlock.getPreviousHash().slice(0, 21)}`);
              if (missingBlockHeight) {
                const t = Date.now() + 10000;
                const w = 2 * MAX_DATA_RANGE;
                const stepBack = Math.floor(Math.random() * w) + 4;
            ...
            // LDL
            //this._logger.info(`multiverse yielding evaluation of ${blockQueue.length} in set of ${innerCallBlocks.length} blocks`)
            const nb = await this._engine.persistence.getBlocksByHeight(parseInt(newBlock.getHeight(), 10), BC_SUPER_COLLIDER, { asHeader: false, cached: true });
            if (nb) {
              for (let b of nb) {
                if (b.getHash() !== newBlock.getHash() && reviewBlockHashes.indexOf(b.getHash()) < 0 && blockQueue.length === 0) {
    ...
    if (scheduledForNextTick) {
      this._logger.info(`segment has been scheduled for evaluation on next cycle`);
      return;
    }
    ...
    if (lowestBoundary) {
      if (lowestBoundaryBlocks[0] && this._knownHashes.indexOf(lowestBoundaryBlocks[0].getPreviousHash()) > -1) {
        ...
        const blocksToAttach = await this._engine.persistence.getBlocksByHeight(lowestBoundary - 1, BC_SUPER_COLLIDER, { asHeader: true, cached: true });
        if (blocksToAttach) {
    ...
    if (blocksLength < MAX_DATA_RANGE && validBlocks > 0 && parseInt(evalBlock.getHeight(), 10) + MAX_DATA_RANGE < highestKnownHeight && finalValidBlocks.length > 0 && parseInt(evalBlock.getHeight(), 10) < highestKnownHeight) {
      ...
      return;
    } else if (!mountBlockFound || segmentUnlinked || sequenceFailed) {
      this._PEER_QUARANTINE.push(addressToHost(address));
      const synced = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.sync.initialsync`);
      this._logger.warn(`waypoint unable to provide segment: ${segmentUnlinked}, mount: ${mountBlockFound}, all other blocks have been stored pending future evaluation...synced status: ${synced}`);
      const reorgBlock = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.block.reorgfrom`);
      await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
      if (this._PEER_QUARANTINE.length <= 16 && synced === 'pending' && finalValidBlocks.length > 0 && finalValidBlocks[0].getHash) {
        this._PEER_QUARANTINE.push(addressToHost(address));
        const finalValidBlock = finalValidBlocks[0];
        await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
        await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.block.reorgfrom`, finalValidBlock);

        await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.data.latest`, `${parseInt(finalValidBlock.getHeight(), 10) - 24}:${tn}`);
        await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.edge`, parseInt(finalValidBlock.getHeight(), 10) - 12);
        await this._engine.persistence.putLatestBlock(finalValidBlock, BC_SUPER_COLLIDER, { iterateUp: false });
      } else if (this._PEER_QUARANTINE.length <= 20 && synced === 'complete' && finalValidBlocks.length > 0) {

        // push null counter
        this._PEER_QUARANTINE.push(0);
        const finalValidBlock = finalValidBlocks[0];
        const fromStepBack = parseInt(finalValidBlock.getHeight(), 10) - 612;
        const toStepBack = parseInt(finalValidBlock.getHeight(), 10) - 606;
        this._logger.info(`searching waypoint directly for segment ${fromStepBack} - ${toStepBack}`);

        await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.data.latest`, `${fromStepBack}:${tn}`);
        await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.edge`, toStepBack);

        const data = [fromStepBack, toStepBack];
      ...
      if (this._PEER_QUARANTINE.length >= 16 && this._PEER_BLACKLIST.length < 16) {
        ...
        if (this._PEER_BLACKLIST.length > 36) {
          this._logger.warn(`this node is unable to clear local data, please manually remove data and restart the node...`);
          process.exit();
        }
        ...
        return;
      } else if (this._PEER_QUARANTINE.length <= 16 && synced === 'pending') {
        this._PEER_QUARANTINE.push(addressToHost(address));
        ...
        //if (invalidBlockHeights.length > 0 && new BN(finalValidBlock.getHeight()).gt(new BN(invalidBlockHeights[0]))) {
        //  this._logger.info(`setting search edge and data.latest to ${invalidBlockHeights[0] - 1}`)
        //  await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.data.latest`, `${invalidBlockHeights[0] - 1}:${tn}`)
        //  await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.edge`, invalidBlockHeights[0] - 1)
        //} else if (dls && new BN(dlh).lt(new BN(finalValidBlock.getHeight()))) {
        if (!finalValidBlock || dls && new BN(dlh).lt(new BN(finalValidBlock.getHeight()))) {
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      if (waypointStatStream) {
        waypointStatStream.write(`${processTime},${elapsed},${currentHeight},${nextHeight},${meanTime},${weightedTime},${addressToHost(address)}\n`);
      ...
      if (finalValidBlocks.length > 0 && meanTime > 0 && BC_MAX_WAYPOINT_LATENCY < weightedTime && this._PEER_QUARANTINE.length < 6) {
        ...
        await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.block.reorgto`);
        await this._engine.persistence.putLatestBlock(finalValidBlock, BC_SUPER_COLLIDER, { iterateUp: false });
        debug(`limited connection to waypoint (${meanTime} ms), searching for faster connections...`);
      ...
      //  const comp = await this._engine.persistence.pruneFromBlock(finalValidBlocks[0], 6, 20)
      //  if (comp && !isNaN(comp)) {
      ...
      await this._engine.persistence.putLatestBlock(finalValidBlocks[finalValidBlocks.length - 1], BC_SUPER_COLLIDER, { chainState: this._engine.chainState });
      await this._engine.persistence.reorgBlockchain(BC_SUPER_COLLIDER, {
      ...
      //const reorgBlock = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.block.reorgfrom`)
      //if (reorgBlock && parseInt(finalValidBlocks[0].getHeight(), 10) > parseInt(reorgBlock.getHeight(), 10)) {
      //  const comp = await this._engine.persistence.pruneFromBlock(finalValidBlocks[0], 2000, 20)
      //  if (comp && !isNaN(comp)) {
      //    this._multiverseCompressions = this._multiverseCompressions + comp
      //  }
      //}
        ...
        if (highest !== lowest) {
          ...
          if (this._discovery.connections) {
            ...
            if (range[2]) {
              ...
              if (sendToAnyPeer && p < 1 && this._PEER_BLACKLIST.indexOf(address) < 0) {
                // LDL
              ...
              for (const remoteConnection of this._discovery.connections) {
                ...
                if (hasCurrentPeer && this._PEER_QUARANTINE.indexOf(addressToHost(address)) < 0 && this._PEER_BLACKLIST.indexOf(addressToHost(address)) < 0) {
                  p += 1;
                  ...
                  continue;
                } else if (!hasCurrentPeer && p < BC_MAX_CONNECTIONS && this._PEER_QUARANTINE.indexOf(addressToHost(address)) < 0 && this._PEER_BLACKLIST.indexOf(addressToHost(address)) < 0) {
                  t = t + 350;
                  p += 1;
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (parseInt(block.getHeight(), 10) < 157591) {
        //this._logger.info(`rejected low block ${block.getHeight()}`)
        this._PEER_BLACKLIST.push(addressToHost(address));
        return;
      }
      ...
      if (!this._SEEN_BLOCKS_MEMORY[block.getHash()] && parseInt(block.getHeight(), 10) !== 1) {
        ...
        this._logger.info(`[] <- NRG BLOCK ${block.getHeight()} : ${block.getHash()} ${block.getDifficulty()}`);
      ...
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (synced && synced === 'complete') {
          if (this._SEEN_BLOCKS_MEMORY[parseInt(block.getHeight(), 10) + 1] || this._SEEN_BLOCKS_MEMORY[parseInt(block.getHeight(), 10)]) {
