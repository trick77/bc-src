...
let BC_MAX_WAYPOINT_LATENCY = isNaN(process.env.BC_MAX_WAYPOINT_LATENCY) ? 460 : process.env.BC_MAX_WAYPOINT_LATENCY;
const BC_MINIMUM_HEADER_TEST = isNaN(process.env.BC_MINIMUM_HEADER_TEST) ? 18 : process.env.BC_MINIMUM_HEADER_TEST;
...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
      ...
      if (this._knownHashes.indexOf(newBlock.getHash()) > -1 && blockQueue.length !== 0) {
        ...
        if (syncProgress > 0) {
          ...
          if (syncProgress > 99.97 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
            ...
            this._logger.info(`COLLISION #${newBlock.getHeight()} : ${this._childBlocksSynced} ${highestKnownHeight} [] ${newBlock.getHash().slice(0, 5)} ${newBlock.getPreviousHash().slice(0, 5)} ${syncProgress}% <- MINER EMB BOOST: +${this._engine._emblemPerformance}% NRG`);
          }
        }
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      if (evalBlock) {
        // only keep 200 blocks in the temporary block store
        for (let h of Object.keys(this._temporaryBlockStore)) {
          if (parseInt(this._temporaryBlockStore[h].getHeight(), 10) + 200 < parseInt(evalBlock.getHeight(), 10)) {
            delete this._temporaryBlockStore[parseInt(this._temporaryBlockStore[h].getHeight(), 10)];
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (this._blockHeaderTest && this._blockHeaderTest.length < BC_MINIMUM_HEADER_TEST && block && block.getHeight) {
        this._logger.info(`____ NRG BLOCK ${block.getHeight()} ___  ${block.getHash()} ${block.getDifficulty()}`);
        this._blockHeaderTest.push(block);
        return;
      } else if (this._blockHeaderTest && this._blockHeaderTest.length >= BC_MINIMUM_HEADER_TEST) {
      ...
      if (this._blockHeaderTest) {
        const highestTestBlock = this._blockHeaderTest[0];
        if (parseInt(block.getHeight(), 10) < parseInt(highestTestBlock.getHeight(), 10)) {
          this._PEER_BLACKLIST.push(addressToHost(address));
          return;
        }
        this._logger.info(`threshold passed ${highestTestBlock.getHeight()}`);
