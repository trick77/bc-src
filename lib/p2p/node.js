...
const PEER_DATA_SYNC_EXPIRE = 35661; // Peer must return a block / data / network request before time elapsed (milliseconds)
const BC_QUARANTINE_FORGIVENESS = BC_BLOODLINE ? 35000 : getRandomWithinRange(63000, 128000);
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (blocks && blocks.length > 0) {
      ...
      if (startGivenBlockHeight < waypointRequestLow - BC_MAX_DATA_RANGE * 3) {
        this._logger.warn(`start given height ${startGivenBlockHeight} BELOW maximum range ${BC_MAX_DATA_RANGE * 10}`);
        this._PEER_QUARANTINE.push(addressToHost(address));
        await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
        if (logline) {
          logline.write(addressToHost(address) + '\n');
