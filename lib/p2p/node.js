...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (validcb && checkCoinbaseBlock) {
      if (latestBlock && checkCoinbaseBlock.getHeight() > latestBlock.getHeight()) {
        this._logger.info(`${checkCoinbaseBlock.getHeight()} -> has valid mount from: ${latestBlock.getHeight()} ${checkCoinbaseBlock.getHeight()}`);
        //await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.block.reorgfrom`, checkCoinbaseBlock)
      } else if (!latestBlock) {
        this._logger.info(`${checkCoinbaseBlock.getHeight()} -> has valid base segment known: ${segmentKnown}`);
        //await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.block.reorgfrom`, checkCoinbaseBlock)
      }
    }
        ...
        if (highestBlockUnmount && highestBlockUnmount < checkCoinbaseBlock.getHeight()) {
          ...
          if (new BN(highestBlockUnmount).gt(new BN(checkCoinbaseBlock.getHeight()))) {
            ...
            if (unmountRaw) {
              ...
              await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
              await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`);
              await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.data.latest`, `${unmount - 40}:${Math.floor(Date.now() / 1000)}`);
              this._engine._emitter.emit('requestBlockRange', [unmount + BC_MAX_DATA_RANGE - 40, unmount - 40], 3);
              return;
            }
          }
        } else if (!unmount || unmount > checkCoinbaseBlock.getHeight()) {
          await this._engine.persistence.delUtxoUnmount();
          forceBreak = true;
          ...
          if (pendingArrivalOfLocalHash) {
            ...
            this._logger.info(`unmount unset: ${!unmount} <- current block: ${checkCoinbaseBlock.getHeight()} searching higher...`);
          }
          ...
          if (prevFlaggedBlock) {
            ...
            //this.addToQuarantine(addressToHost(address), 'previous flag block at height requested boundary')
            //await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`)
          }
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`);
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      debug(`block from host ${address} while length is ${queueLength}`);
      ...
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (latestBlock.getHeight() - 20 > block.getHeight()) {
