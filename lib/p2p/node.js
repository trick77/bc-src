...
let WAYPOINT_TIMES = [];
let BW_WAYPOINTS = [];
const BC_MINIMUM_HEADER_TEST = isNaN(process.env.BC_MINIMUM_HEADER_TEST) ? 12 : Number.parseInt(process.env.BC_MINIMUM_HEADER_TEST, 10);
      ...
      if (this._knownHashes.indexOf(newBlock.getHash()) > -1 && blockQueue.length !== 0) {
        ...
        if (syncProgress > 0) {
          ...
          if (syncProgress > 99.98 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
            ...
            if (blockQueue.length === 0) {
              this._logger.info(`NRG MULTICHAIN #${newBlock.getHeight()} : ${this._childBlocksSynced} ${highestKnownHeight} [] ${newBlock.getHash().slice(0, 5)} ${newBlock.getPreviousHash().slice(0, 5)} ${syncProgress}% <- Emblem (EMB) adding +${this._engine._emblemPerformance}% NRG`);
            }
          }
        }
        continue;
        ...
        if (isValidCoinbase) {
          ...
          if (prevBlock && prevBlock.getHeight() !== newBlock.getHeight()) {
            ...
            if (!isValidSeq.valid && blockQueue[0] && blockQueue[0].getHeight() !== newBlock.getHeight()) {
              ...
              //wtc.write(`19,${Number(Date.now())}\n`)
              await this._engine.persistence.putBlock(newBlock, 0, BC_SUPER_COLLIDER, {
                fromWaypoint: true,
                asHeader: false
              });
              continue;
            } else if (!isValidSeq.valid && parseInt(newBlock.getHeight(), 10) === parseInt(prevBlock.getHeight(), 10) + 1) {
            ...
            if (syncProgress > 0) {
              ...
              if (syncProgress > 99.97 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
                ...
                if (syncProgress > 99.99) {
                  this._logger.info(`NRG MULTICHAIN #${newBlock.getHeight()} : ${this._childBlocksSynced} ${highestKnownHeight} [] ${newBlock.getHash().slice(0, 5)}:${newBlock.getPreviousHash().slice(0, 5)} <- Emblem (EMB) adding +${this._engine._emblemPerformance}% NRG`);
                } else {
                  this._logger.info(`NRG MULTICHAIN #${newBlock.getHeight()} : ${this._childBlocksSynced} ${highestKnownHeight} [] ${newBlock.getHash().slice(0, 5)}:${newBlock.getPreviousHash().slice(0, 5)} ${syncProgress}% <- Emblem (EMB) adding +${this._engine._emblemPerformance}% NRG`);
                }
              }
        ...
        if (this._discovery.connections) {
          ...
          if (range[2]) {
            ...
            if (sendToAnyPeer && p < 1 && this._PEER_BLACKLIST.indexOf(address) < 0 && this._PEER_QUARANTINE.indexOf(addressToHost(address)) < 0 && sendToPool) {
              // LDL
              this._logger.info(`sending range request to address ${address} from ${lowest} to ${highest}`);
              p += 1;
            ...
            for (const remoteConnection of this._discovery.connections) {
              ...
              } else if (!hasCurrentPeer && sendToPool) {
                t = t + 350;
                p += 1;
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (!BC_MINIMUM_HEADER_TEST || BC_MINIMUM_HEADER_TEST === 0) {
        ...
        this._logger.info(`_XX_ NRG BLOCK ${block.getHeight()} ___ BC_MINIMUM_HEADER_TEST === 1, yielding block from waypoint ${block.getHeight()} ${block.getDifficulty()}`);
        return;
      }
      ...
      if (this._latestBlock) {
        ...
        } else if (block.getPreviousHash() === this._latestBlock.getHash() && this._PEER_RECORD[host].goodBlocks >= 3) {
      ...
      const address = conn.remoteAddress + ':' + conn.remotePort;
      this._logger.info(`received GET_DATA request from waypoint ${address}`);
      const latestBlock = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.block.latest`);
      const parts = bufferSplit(str, Buffer.from(MSG_SEPARATOR[type]));
      ...
      this._logger.info(`M.GET_DATA: getting blocks data from range from: ${from} -> ${to}, request window: ${requestWindow}`);
      let onlyBlocks = [];
      let validBlocks = [];
