...
let BC_MAX_WAYPOINT_LATENCY = isNaN(process.env.BC_MAX_WAYPOINT_LATENCY) ? 9260 : process.env.BC_MAX_WAYPOINT_LATENCY;
...
const PEER_DATA_SYNC_EXPIRE = 29602; // Peer must return a block / data / network request before time elapsed (milliseconds)
const BC_QUARANTINE_FORGIVENESS = BC_BLOODLINE ? 15000 : getRandomWithinRange(10000, 16000);
...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (blockQueue.length !== blocks.length) {
      this._logger.warn(`waypoint delivered blocks out of segment received: ${blocks.length}, accepted: ${blockQueue.length}`);
    }
    ...
    this._logger.info(`valid: ${validBlocks}, from memory: ${blocksLoadedFromMemory}, height: ${currentHeight}`);
      ...
      if (this._PEER_RECORD[addressToHost(address)]) {
        if (finalValidBlocks.length > 0) {
          ...
          if (lbkh === lastSeenHash) {
            ...
            if (finalValidBlocks.length < 12) {
              this._logger.info(`waypoint age reset (${this._PEER_RECORD[addressToHost(address)].lastSeen})`);
              this._syncComplete = true;
            } else {
              this._logger.info(`waypoint demoted: ${this._PEER_RECORD[addressToHost(address)].lastSeen}`);
              this._PEER_QUARANTINE.push(addressToHost(address));
              return;
            }
          } else {
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      if (this._engine._blockStats) {
        for (const finalBlock of finalValidBlocks) {
          ...
          await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.block.${finalBlock.getHash()}`, finalBlock);
      ...
      this._logger.info(`[] -> [] completed traverse to edge of global state -> ${currentHeight} -> opening ${this._PEER_QUARANTINE.length} connections`);
      ...
      if (highest !== lowest) {
        ...
        if (this._discovery.connections) {
          ...
          if (range[2] && pass) {
            ...
            this._logger.info(`creating direct request -> agnostic: ${sendToAnyPeer} p: ${p}`);
            if (sendToAnyPeer && p < limit && this._PEER_BLACKLIST.indexOf(address) < 0 && this._PEER_QUARANTINE.indexOf(addressToHost(address)) < 0 && sendToPool) {
              // LDL
            ...
            for (const remoteConnection of connections) {
              ...
              if (sendToPool && p < limit && this._PEER_QUARANTINE.indexOf(host) < 0 && this._PEER_BLACKLIST.indexOf(host) < 0) {
                p += 1;
                this._logger.info(`current peer handling broadcast range request to address from ${lowest} to ${highest}`);
                await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.req.range`, `${highest}:${lowest}:${now}:${host}`);
                await this.qsend(remoteConnection, payload);
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (BC_MINER_POOL && BC_MINER_POOL === host) {
        ...
        this._logger.info(`____ OVERLINE BLOCK ${block.getHeight()} ___  ${block.getHash()} ${block.getDifficulty()} min: ${BC_MINIMUM_HEADER_TEST}`);
      ...
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (currentPeer && this._PEER_QUARANTINE.indexOf(currentPeer.getAddress()) > -1) {
          ...
          await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
          currentPeer = false;
        }
        ...
        if (!currentPeer && this._processedBlocks > 2 || nodeIsSynced && synced === 'complete' && this._processedBlocks > 2) {
          ...
          if (currentPeer && addressToHost(currentPeer.getAddress()) !== addressToHost(address)) {
            ...
            if (new BN(time).gt(new BN(currentPeer.getExpires()))) {
              ...
              this._logger.info(`block waiting for queue to clear ${block.getHeight()}`);
        ...
        if (!currentPeer && this._processedBlocks > 2 || synced && synced === 'complete' && this._processedBlocks > 2) {
          ...
          const fastSend = getRandomWithinRange(1, 4);
