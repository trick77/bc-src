...
class PeerNode {
      ...
      if (newBlock !== undefined && newBlock !== null && !invalidData && prevBlock && prevBlock.getHeight && parseInt(newBlock.getHeight(), 10) > 1) {
        ...
        if (isValidCoinbase) {
          ...
          if (prevBlock && prevBlock.getHeight() !== newBlock.getHeight()) {
            ...
            if (mountBlockReq) {
              debug(`required block mount of ${mountBlockReq[0].getBlockchain()} ${mountBlockReq[0].getHeight()} <- ${mountBlockReq.length} paths`);
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (!this._SEEN_BLOCKS_MEMORY[parseInt(block.getHeight(), 10)] && !this._SEEN_BLOCKS_MEMORY[block.getHash()] && parseInt(block.getHeight(), 10) !== 1) {
        ...
        this._SEEN_BLOCKS_MEMORY[parseInt(block.getHeight(), 10)] = Math.floor(Date.now() / 1000);
      ...
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (!currentPeer && synced && synced === 'complete' && this._processedBlocks > 2) {
          ...
          if (currentPeer && currentPeer.getAddress() === addressToHost(address) && Number(new Date()) < parseInt(currentPeer.getExpires(), 10)) {
            ...
            if (l && !p && parseInt(block.getHeight(), 10) > parseInt(l.getHeight(), 10)) {
