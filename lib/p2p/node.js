...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (blocks && blocks.length > 0) {
      ...
      debug(`MESSAGES.DATA variance ${variance} from waypoint with malformed blocks`);
    } else {
      // LDL
    ...
    do {
      ...
      if (!prevBlock) {
        ...
        } else if (prevBlock && prevBlock.getHash() !== newBlock.getPreviousHash()) {
