...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (this._PEER_QUARANTINE.indexOf(addressToHost(address)) > -1) {
    ...
    do {
      ...
      if (this._knownHashes.indexOf(newBlock.getHash()) > -1 && blockQueue.length !== 0) {
        ...
        if (syncProgress > 0) {
          ...
          this._logger.info(`COLLIDER ${newBlock.getHeight()} ${this._childBlocksSynced} [] ${newBlock.getHash().slice(0, 8)}:${newBlock.getPreviousHash().slice(0, 8)} NRG Multichain ${syncProgress}% <- Miner EMB Boost: +${this._engine._emblemPerformance}%`);
        }
      ...
      if (!prevBlock && newBlockBoundary < highestKnownHeight) {
        ...
        await this._engine.persistence.putBlock(newBlock, 0, BC_SUPER_COLLIDER, { asHeader: false, updateHeight: true });
        if (nb) {
          for (let b of nb) {
          ...
          if (opts.innerCall) {
            ...
            if (blockQueue.length === 0) {
              ...
              if (missingBlockHeight) {
                ...
                const stepBack = Math.floor(Math.random() * w) + 2;
                const step = max(3, missingBlockHeight - stepBack);
                this._logger.info(`setting data location to ${missingBlockHeight} - ${stepBack}`);
                if (invalidBlockHeights.indexOf(parseInt(newBlock.getHeight(), 10)) < 0) {
                  if (validBlockHeights.indexOf(parseInt(newBlock.getHeight(), 10)) < 0) {
                    invalidBlockHeights.push(parseInt(newBlock.getHeight(), 10));
                  }
    ...
    if (blocksLength < MAX_DATA_RANGE && validBlocks > 0 && parseInt(evalBlock.getHeight(), 10) + 1 !== highestKnownHeight && finalValidBlocks.length > 0 && parseInt(evalBlock.getHeight(), 10) < highestKnownHeight) {
      this._PEER_QUARANTINE.push(addressToHost(address));
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      this._logger.info(`[] -> [] NRG multichain reached global state, beginning block collisions`);
      if (BC_BIND_PEER) {
        ...
        if (highest !== lowest) {
          ...
          if (this._discovery.connections) {
            ...
            if (range[2]) {
              ...
              if (sendToAnyPeer && this._PEER_QUARANTINE.indexOf(addressToHost(address)) < 0 && p < 1 && this._PEER_BLACKLIST.indexOf(address) < 0) {
                // LDL
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (parseInt(block.getHeight(), 10) < 101000) {
