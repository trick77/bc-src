...
const PEER_DATA_SYNC_EXPIRE = 18661; // Peer must return a block / data / network request before time elapsed (milliseconds)
const BC_QUARANTINE_FORGIVENESS = BC_BLOODLINE ? 15000 : getRandomWithinRange(13000, 38000);
...
const loadBasedPeerExpiration = logline ? 50000 : 110000 + Math.floor(80000 / numCPUs);
...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (blocks && blocks.length > 0) {
        ...
        if (next) {
          if (b && b.getHeight && next.getHeight) {
            ...
            if (thisHeight !== nextHeight) {
              variance = variance + Math.abs(thisHeight - nextHeight);
            }
          } else {
      ...
      debug(`MESSAGES.DATA variance ${variance} from waypoint with no malformed blocks`);
    } else {
      // LDL
    ...
    if (blockQueue.length > 0) {
      ...
      if (this._knownBlockSegments.has(segmentKey)) {
        this._logger.info(`segment is already processed: ${segmentKey}`);
    ...
    if (!validcb) {
      ...
      if (unmount) {
        ...
        if (invalidBlockHeights.indexOf(parseInt(checkCoinbaseBlock.getHeight(), 10) - BC_MAX_DATA_RANGE * 2) < 0) {
          invalidBlockHeights.push(parseInt(checkCoinbaseBlock.getHeight(), 10) - BC_MAX_DATA_RANGE * 2);
        }
      }
    }
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      await this._engine.persistence.processPeerExpiration({ chainState: this._engine.chainState });
      ...
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (!currentPeer && this._processedBlocks > 3 || synced && synced === 'complete' && this._processedBlocks > 3) {
          ...
          const fastSend = getRandomWithinRange(1, 6);
