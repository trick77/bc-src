...
const loadBasedPeerExpiration = logline ? 20000 : 90000 + Math.floor(80000 / numCPUs);
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (blocks && blocks.length > 0) {
      ...
      if (startGivenBlockHeight < waypointRequestLow - BC_MAX_DATA_RANGE * 8) {
        this._logger.warn(`start given height ${startGivenBlockHeight} BELOW maximum range ${BC_MAX_DATA_RANGE * 8}`);
        this._PEER_QUARANTINE.push(addressToHost(address));
        await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
        if (logline) {
          logline.write(addressToHost(address) + '\n');
    ...
    if (unseenBlocks === 0 && blocksLength >= BC_MAX_DATA_RANGE) {
      this._logger.info(`blocks precented are above MAX_DATA_RANGE`);
    }
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (!currentPeer && this._processedBlocks > 3 || synced && synced === 'complete' && this._processedBlocks > 3) {
          ...
          const fastSend = getRandomWithinRange(1, 5);
          ...
          if (currentPeer && currentPeer.getAddress() === addressToHost(address) && Number(new Date()) < parseInt(currentPeer.getExpires(), 10)) {
            ...
            await this._engine.persistence.putBlock(block);
            return;
