...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    do {
      ...
      debug(`processing OL block ${newBlock.getHeight()} : ${newBlock.getHash()}`);
    ...
    while (!findRoot && h > parseInt(checkCoinbaseBlock.getHeight(), 10) - 1000) {
      h--;
      ...
      if (options.length === 1) {
        findRoot = options[0];
        this._logger.info(`root found ${findRoot.getHeight()} from ${checkCoinbaseBlock.getHeight()}`);
      }
    }

    checkCoinbaseBlock = findRoot ? findRoot : checkCoinbaseBlock;
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      if (currentPeer) {
        ...
        currentPeer.setExpires(Number(new Date()) + 11800);
        await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.initialpeer`, currentPeer);
      } else {
        ...
        process.nextTick(async () => {
          ...
          const tasks = this._discovery.connections.map(conn => {
            try {
              ...
              this._logger.info(`announcing block to ${addressToHost(addr)}`);
