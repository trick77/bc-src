    ...
    if (!validcb) {
      ...
      if (remount) {
        ...
        if (unmount) {
          ...
          forceBreak = true;
          //this.addToQuarantine(address, 'block sent missing unmount')
          this._logger.info(`unmount not set on current block: ${checkCoinbaseBlock.getHeight()} searching higher...`);
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`);
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
          this._engine._emitter.emit('requestBlockRange', [checkCoinbaseBlock.getHeight() + BC_MAX_DATA_RANGE + 500, checkCoinbaseBlock.getHeight() + 550, conn]);
          return;
        }
      }
    }
