    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      if (this._requestRegistry[peerRequestKey] > 600) {
        ...
        if (sent !== undefined && sent.success) {
          // LDL
          debug(`GET_DATA sent to ${sent.address}, message: ${sent.message}`);
          const validcblazy = parseInt(checkCoinbaseBlock.getHeight(), 10) + 1000 < highestKnownHeight && validcb && parseInt(checkCoinbaseBlock.getHeight(), 10) % 5 === 0 ? await validateCoinbase(checkCoinbaseBlock, this._engine.persistence, this._engine._txHandler, 'p2p.node') : true;
          if (!validcblazy) {
            let toHeight = data[1];
