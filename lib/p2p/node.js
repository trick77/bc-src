...
class PeerNode {
  ...
  qsend(conn, msg) {
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        debug(`qsend(): wrote ${wireData.length}b to ${conn.remoteAddress}:${conn.remotePort} with forced connection`);
        return resolve({
          address,
          success: true,
          message: 'success'
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    // TD:
    this._logger.info(`${waypointRequest}`);
    ...
    if (reqHeight && reqHeight < lowestHeightRawBoundary) {
      this._logger.info(`scan for block range detected ${lowestHeightRawBoundary} -> ${reqHeight - 200} cleared cache`);
    ...
    do {
        ...
        if (evaluatedBlocks.length > 1) {
          ...
          if (!foundMount && branchBlock && branchBlock.getHeight() === newBlock.getHeight()) {
            if (this._temporaryBlockStore[branchBlock.getPreviousHash()]) {
              ...
              checkCoinbaseBlock = branchBlock;
            }
          }
        } else {
          checkCoinbaseBlock = newBlock;
      ...
      // TD: await this._engine.persistence.saveBlockHeaders(newBlock)
        ...
        if (nb) {
          for (let b of nb) {
            ...
            if (b.getHash() !== newBlock.getHash() && reviewBlockHashes.indexOf(b.getHash()) < 0) {
              // TD: await this._engine.persistence.saveBlock(b)
              // TD: await this._engine.persistence.putBlock(b, 0, BC_SUPER_COLLIDER, {
              // TD:   cached: false,
              // TD:   saveHeaders: true
              // TD: })
        ...
        if (prevBlock && prevBlock.getHeight() !== newBlock.getHeight()) {
          ...
          if (!isValidSeq.valid && blockQueue[0] && blockQueue[0].getHeight() !== newBlock.getHeight()) {
            ...
            if (!optionsAt || optionsAt && optionsAt.length < 1) {
              ...
              validBlocks++;
              // TD
              this._logger.info(`alternative path already includes block: ${newBlock.getHeight()} yielding...${optionsAt.join(',')}`);
              // TD: await this._engine.persistence.saveBlock(newBlock)
    ...
    // TD
    //this._logger.info(`force break: ${forceBreak}`)
    //this._logger.info(`broken segment: ${brokenSegment}`)
    //this._logger.info(`blocks provided outside boundary: ${blocksProvidedOutsideBoundary}`)
    //this._logger.info(`segment known: ${segmentKnown}`)
    ...
    // TD hard branch check evaluates the other potential paths if there are other blocks at this height
    if (!validcb) {
    ...
    if (!validcb) {
      ...
      if (remount) {
        ...
        if (unmount && unmount < checkCoinbaseBlock.getHeight()) {
          this._logger.warn(`${unmount} block not in position...`);
          ...
          if (new BN(unmount).gt(new BN(checkCoinbaseBlock.getHeight()))) {
            ...
            if (unmountRaw) {
              ...
              if (!block) {
                ...
                this._engine._emitter.emit('requestBlockRange', [Number(uheight) + BC_MAX_DATA_RANGE - 100, Number(uheight) - 100, conn]);
                return;
              }
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      // LDL
      //TD: debug(`opened GET_DATA request for ${address} from local: ${currentHeight} next: ${nextHeight}, e: ${highestKnownHeight}`)
      this._logger.info(`opened GET_DATA request for ${address} from local: ${currentHeight} next: ${nextHeight}, e: ${highestKnownHeight}`);
  ...
  async start(nodeId) {
    ...
    this._engine._emitter.on('requestBlockRange', async (range, openLink) => {
      ...
      if (this._engine._blockQueue && this._engine._blockQueue.length() > 0 && openLink !== 1) {
        // TD
        this._logger.info(`${this._engine._blockQueue.length()} blocks in queue`);
        //this._engine._blockQueue.process()
      }
          ...
          if (this._discovery.connections.length > 0) {
            ...
            for (const remoteConnection of connections) {
              ...
              if (sendToPool && p < limit && this._PEER_QUARANTINE.indexOf(host) < 0 && this._PEER_BLACKLIST.indexOf(host) < 0 && this._PEER_BLACKLIST.indexOf(address) < 0) {
                p += 1;
                // TD
                this._logger.info(`direct request to address ${host} from ${lowest} to ${highest}`);
                await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.req.range`, `${highest}:${lowest}:${now}:${host}`);
    ...
    this._discovery._seeder.on('peer', peer => {
      ...
      if (this._discovery.connected > BC_MAX_CONNECTIONS + 300) {
        ...
        this._logger.info('waypoint about bound <- ' + this._discovery.connected + ' connections');
        //return
      }
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (highestKnownHeight > parseInt(latestBlock.getHeight(), 10) + 100 || from - 100 > parseInt(latestBlock.getHeight(), 10)) {
        // TD: debug(`received request from waypoint which cannot be completely fulfilled...${address}`)
        this._logger.info(`received request from waypoint which cannot be completely fulfilled...${address} from: ${from}, to: ${to}, highest known: ${highestKnownHeight}, latest edge: ${latestBlock.getHeight()}`);
        //return
      }
      ...
      this._logger.info(`waypoint connection ${address} low ${low} -> ${high}`);
      ...
      for (const b of validBlocks) {
        const saved = await this._engine._utxoManager.areUTXOsSavedForBlock(b.getHeight(), b.getHash());
        if (saved) {
          savedUtxos.push(b);
        } else {
          debug(`not sending block ${b.getHeight()}:${b.getHash()}`);
        }
      }

      debug(`sending payload of ${onlyBlocks.length} blocks, valid: ${validBlocks.length}, utxos: ${savedUtxos.length}`);
