...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (blocks && blocks.length > 0) {
      ...
      if (startGivenBlockHeight < waypointRequestLow - BC_MAX_DATA_RANGE * 2) {
        ...
        if (waypointRequestHigh && waypointRequestLow) {
          ...
          this._engine._emitter.emit('requestBlockRange', [waypointRequestHigh, waypointRequestLow - 2], 2);
        }
        return;
      }
            ...
            for (let req of mountBlockReq) {
              ...
              if (mountBlocks) {
                ...
                if (!foundRootBlockToMountBranch) {
                  ...
                  //TD: await this._engine.persistence.saveBlock(prevBlock)
                  ...
                  //})
                    ...
                    // TD: await this._engine.persistence.saveBlock(prevBlock)
                    // TD: await this._engine.persistence.putBlock(prevBlock, 0, BC_SUPER_COLLIDER, {
                    // TD:   asHeader: false,
                    // TD:   fromWaypoint: true,
                    // TD:   saveHeaders: true
                    // TD: })
    ...
    if (!validcb) {
      ...
      //TD: await this._engine.persistence.saveBlock(checkCoinbaseBlock)
      ...
      if (remount) {
        ...
        if (unmount && unmount < checkCoinbaseBlock.getHeight()) {
          ...
          let direction = !pendingArrivalOfLocalHas ? 1 : -1;
          ...
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
          this._engine._emitter.emit('requestBlockRange', [scanBackHeight + BC_MAX_DATA_RANGE, scanBackHeight], 1);
          return;
        }
      }
    }
