...
const _MAX_FRAME_SIZE = 18 * 1024 * 1024; // 18MB
const FRAMING_OPTS = {
  lengthSize: 4,
...
const PEER_DATA_SYNC_EXPIRE = 12602; // Peer must return a block / data / network request before time elapsed (milliseconds)
const BC_QUARANTINE_FORGIVENESS = BC_BLOODLINE ? 15000 : getRandomWithinRange(8166, 12600);
...
const loadBasedPeerExpiration = logline ? 110000 : 110000 + Math.floor(80000 / numCPUs);
...
class PeerNode {
  ...
  constructor(engine) {
    ...
    setInterval(() => {
      ...
      if (sq) {
        ...
        debug(`${this._PEER_QUARANTINE.length} waypoints in quarantine`);
      }
    ...
    async function statusInterval() {
      this._logger.info('waypoints active: ' + this._discovery.connected + ' passive: ' + this._PEER_QUARANTINE.length + ' weight: ' + this._processedBlocks);
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (blocks && blocks.length > 0) {
      ...
      if (startGivenBlockHeight > waypointRequestLow + BC_MAX_DATA_RANGE * 10) {
        ...
        this._PEER_QUARANTINE.push(addressToHost(address));
        if (currentPeer && currentPeer === addressToHost(address)) {
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
        }
        if (logline) {
          logline.write(addressToHost(address) + '\n');
      ...
      if (startGivenBlockHeight < waypointRequestLow - BC_MAX_DATA_RANGE * 8) {
      ...
      // CHECK 3 blocks are semi-continuous
      let branches = 0;
      let addToList = true;
      ...
      //  if (next) {
      //    if (b && b.getHeight && next.getHeight) {

      //      const thisHeight = parseInt(b.getHeight(), 10)
      //      const nextHeight = parseInt(next.getHeight(), 10)

      //      if (thisHeight !== nextHeight) {
      //        const df = Math.abs(thisHeight - nextHeight)
      ...
      //    } else {
      //      this._logger.warn(`waypoint sending malformed blocks`)
      //      this._PEER_QUARANTINE.push(addressToHost(address))
      //      await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`)
      //      if (logline) { logline.write(addressToHost(address) + '\n') }
      //      return
      //    }
      //  }

      //  if (addToList) {
      //    completeBlockList.push(b)
      //  }
    ...
    const lowestHeightRawBoundary = reorgFromBlock && reorgFromBlock.getHeight ? parseInt(reorgFromBlock.getHeight(), 10) - 10000 : latestHeightRaw - 10000;
    ...
    if (blockQueue.length > 0) {
      ...
      if (this._knownBlockSegments.has(segmentKey) || blockQueue.length === 1) {
        debug(`segment is already processed: ${segmentKey} of queue length ${blockQueue.length}`);
    ...
    if (newQueue.length <= 4) {
      ...
      for (let i = 0; i < newQueue.length; i++) {
        let seenBlock = await this._engine.persistence.areUTXOsSavedForBlock(newQueue[i].getHeight(), newQueue[i].getHash());
    ...
    do {
      //for (const newBlock of blockQueue) {
      ...
      if (newBlock && newBlock.getHeight) {
        checkCoinbaseBlock = newBlock;
        if (blockQueue.length === 0 && checkCoinbaseBlock.getHeight() + 10 > highestKnownHeight) {
          const bhh = await this._engine.persistence.getBlockByHeight(parseInt(newBlock.getHeight(), 10) + 1, BC_SUPER_COLLIDER);
          if (bhh && bhh.getPreviousHash && bhh.getPreviousHash() === checkCoinbaseBlock.getHash()) {
            checkCoinbaseBlock = bhh;
          }
        }
      }
      ...
      if (!this._SEEN_BLOCKS_MEMORY[newBlock.getHash()]) {
        ...
        // await this._engine.persistence.saveTxsForBlock(newBlock)
      }
      ...
      if (this._knownHashes.indexOf(newBlock.getHash()) > -1) {
        ...
        if (syncProgress > 0) {
          ...
          if (rawSyncProgress > 99.999 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
            ...
            if (blockQueue.length === 0) {
              this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} (${max(0, Math.abs(newBlock.getHeight() - this._highestRequestedRangeHeight))}) <- ${this._childBlocksSynced} <- ${newBlock.getHash().slice(0, 20)} ${newBlock.getPreviousHash().slice(0, 20)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
            }
          }
        }
      ...
      if (!prevBlock && newBlockBoundary < highestKnownHeight) {
        ...
        debug(`found but missing ${BC_SUPER_COLLIDER} block ${blockHeight} : ${newBlock.getHash()}`);
      ...
      if (newBlock !== undefined && newBlock !== null && !invalidData && prevBlock && prevBlock.getHeight && parseInt(newBlock.getHeight(), 10) > 1) {
        ...
        const isValidCoinbase = true;

        if (isValidCoinbase) {
          // remove any detected unmounts
          // await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.unmount`)
          ...
          if (prevBlock && prevBlock.getHeight() !== newBlock.getHeight()) {
            ...
            if (syncProgress > 0) {
              ...
              if (rawSyncProgress > 99.9999 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
                ...
                const childBlocks = newBlock.getBlockchainHeadersCount ? newBlock.getBlockchainHeadersCount() : 0;
                this._childBlocksSynced = this._childBlocksSynced + childBlocks;
                if (rawSyncProgress > 99.95) {
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} (${max(0, Math.abs(newBlock.getHeight() - this._highestRequestedRangeHeight))}) <- ${newBlock.getHash().slice(0, 20)}:${newBlock.getPreviousHash().slice(0, 20)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
                } else if (newBlock.getHeight() % 2 === 0) {
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} (${max(0, Math.abs(newBlock.getHeight() - this._highestRequestedRangeHeight))}) <- ${this._childBlocksSynced} ${syncProgress}% <- ${newBlock.getHash().slice(0, 20)}:${newBlock.getPreviousHash().slice(0, 20)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
                } else {
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} (${max(0, Math.abs(newBlock.getHeight() - this._highestRequestedRangeHeight))}) <- ${this._childBlocksSynced} ${syncProgress}% <- ${newBlock.getPreviousHash().slice(0, 20)}:${newBlock.getHash().slice(0, 20)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}% OL`);
                }
              }
            }
          ...
          debug(`reeval coinbase ${newBlock.getHeight()}`);
          ...
          if (opts.innerCall) {
            ...
            if (blockQueue.length === 0) {
              ...
              const stepBack = Math.floor(Math.random() * w) + Math.floor(Math.random() * Math.floor(69));
              let missingBlockHeight = true;
              const unmount = await this._engine.persistence.getUtxoUnmount();
              if (!unmount) {
                missingBlockHeight = await getCoinbaseMissingBlockHeight(newBlock, this._engine.persistence, this._engine._txHandler, 'p2p.node.missing.height');
              } else if (unmount) {
                missingBlockHeight = await getCoinbaseMissingBlockHeight(newBlock, this._engine.persistence, this._engine._txHandler, 'p2p.node.missing.height');
                await this._engine.persistence.delUtxoUnmount();
                if (parseInt(unmount, 10) > parseInt(missingBlockHeight, 10)) {} else {
                  missingBlockHeight = unmount;
                }
              }
              ...
              if (BC_ORTHOGONAL_WAYPOINTS) {
                ...
                if (missingBlockHeight && missingBlockHeight !== true) {
                  ...
                  this._logger.info(`setting data location to ${missingBlockHeight} - ${stepBack} -> search height ${step}`);
                  ...
                  // in case blocks are missing in this sequence that are not connected to failed utxo remove the mount
                  if (unmount) {
                    if (unmount && unmount > currentHeight) {
                      await this._engine.persistence.delUtxoUnmount();
                      await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.data.latest`, `${currentHeight}`);
                      continue;
                    } else if (unmount) {
                      if (blockQueue.length > 0) {
                        this._logger.info(`cleared block queue ${blockQueue.length}`);
                        blockQueue.length = 0;
                      }
                      await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.data.latest`, `${unmount}`);
                      continue;
                    } else {
                      await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.data.latest`, `${currentHeight}`);
                    }
      ...
      //}
    } while (blockQueue.length > 0 && !blocksProvidedOutsideBoundary && !brokenSegment && !segmentKnown);
    ...
    //let validcb = (checkCoinbaseBlock.getHeight() + getRandomWithinRange(50,500)) > highestKnownHeight ? await validateCoinbase(checkCoinbaseBlock, this._engine.persistence, this._engine._txHandler, 'p2p.node') : true
    let validcb = await validateCoinbase(checkCoinbaseBlock, this._engine.persistence, this._engine._txHandler, 'p2p.node');
    ...
    if (!validcb) {
      ...
      if (remount) {
        ...
        if (remount >= lowestBoundaryBlocks) {
          ...
          this._logger.warn(`${remount} block found in critical relative root position`);
          this._PEER_BLACKLIST.push(addressToHost(address));
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
          await this._engine.persistence.reorgBlockchain(BC_SUPER_COLLIDER);
          return;
        } else {
          this._logger.warn(`${remount} block found in critical root position`);
          this._PEER_BLACKLIST.push(addressToHost(address));
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
          await this._engine.persistence.reorgBlockchain(BC_SUPER_COLLIDER);
          return;
        ...
        if (unmount) {
          ...
          if (new BN(unmount).gt(new BN(checkCoinbaseBlock.getHeight()))) {
            ...
            this._logger.info(`searching for mount at: ${unmount}`);
            if (invalidBlockHeights.indexOf(unmount) < 0) {
              invalidBlockHeights.push(unmount);
            }
          }
        } else {
          this._logger.info(`unmount set on current block: ${checkCoinbaseBlock.getHeight() - 20}`);
          if (invalidBlockHeights.indexOf(checkCoinbaseBlock.getHeight() - 20) < 0) {
            invalidBlockHeights.push(checkCoinbaseBlock.getHeight() - 20);
          }
        }
      }
    }
    ...
    if (blocksLength < BC_MAX_DATA_RANGE && validBlocks > 0 && parseInt(evalBlock.getHeight(), 10) + BC_MAX_DATA_RANGE < highestKnownHeight && finalValidBlocks.length > 0 && parseInt(evalBlock.getHeight(), 10) < highestKnownHeight) {
      this._logger.info(`stale blocks from waypoint height ${evalBlock.getHeight()}`);
      ...
      if (BC_ORTHOGONAL_WAYPOINTS) {
        ...
        this._engine._emitter.emit('requestBlockRange', [sFrom + BC_MAX_DATA_RANGE, sFrom - 2]);
        return;
      ...
      if (logline) {
        this._logger.info(`logline enabled for ${invalidBlockHeights[0]}`);
        await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
        this._PEER_QUARANTINE.push(addressToHost(address));
        this._engine._emitter.emit('requestBlockRange', [invalidBlockHeights[0] + BC_MAX_DATA_RANGE, invalidBlockHeights[0] - 1]);
        return;
      }
      ...
      if (this._PEER_QUARANTINE.length >= 16 && this._PEER_BLACKLIST.length > 4) {
        ...
        if (BC_ORTHOGONAL_WAYPOINTS) {
          ...
          let movingEdge = dlh - Math.floor(Math.random() * Math.floor(128));
          if (finalInvalidBlocks.length > 0) {
            movingEdge = finalInvalidBlocks[0] - Math.floor(Math.random() * Math.floor(4));
          }
          ...
          this._engine._emitter.emit('requestBlockRange', [movingEdge + BC_MAX_DATA_RANGE - 2, movingEdge - 2], conn);
        } else {
    ...
    if (validBlocks >= minimumValidBlocks && new BN(currentHeight).gte(new BN(latestBlock.getHeight() - 1)) && !invalidData) {
      ...
      if (checkCoinbaseBlock && validcb && highestBlock.getHeight() < checkCoinbaseBlock.getHeight()) {
        debug(`block ${highestBlock.getHeight()} replaced`);
        highestBlock = checkCoinbaseBlock;
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      if (blocksLoadedFromMemory > validBlocks && validBlocks > 10 && invalidBlockHeights.length < 1) {
      ...
      //if (unseenBlocks === 0 && currentHeight + BC_MAX_DATA_RANGE < highestKnownHeight) {
      //  this._logger.info(`slip stream ahead opened: ${nextHeight} -> ${nextHeight + 256}`)
      ...
      if (currentPeer) {
        ...
        currentPeer.setExpires(Number(new Date()) + 18800);
        await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.initialpeer`, currentPeer);
      } else {
      ...
      if (finalValidBlocks.length > 0 && meanTime > 0 && BC_MAX_WAYPOINT_LATENCY < weightedTime && this._PEER_QUARANTINE.length < 6 && !BC_BLOODLINE) {
        ...
        //await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.block.reorgto`)
        //await this._engine.persistence.putLatestBlock(finalValidBlock, BC_SUPER_COLLIDER, {iterateUp: false})
        this._logger.info(`limited connection to waypoint (${meanTime})...`);
        //WAYPOINT_TIMES.length = 0
        //meanTime = 0
        //return
      ...
      if (this._requestRegistry[peerRequestKey] > 600) {
        ...
        if (sent !== undefined && sent.success) {
          // LDL
          debug(`GET_DATA sent to ${sent.address}, message: ${sent.message}`);
          //const validcblazy = getRandomWithinRange(1, 10) < 3 && checkCoinbaseBlock.getHeight() + 1000 < highestKnownHeight && validcb === true ? await validateCoinbase(checkCoinbaseBlock, this._engine.persistence, this._engine._txHandler, 'p2p.node') : true
          const validcblazy = true;
          if (!validcblazy) {
            let toHeight = data[1];
            let fromHeight = data[0];
            const unmount = await this._engine.persistence.getUtxoUnmount();
            if (unmount) {
              await this._engine.persistence.delUtxoUnmount();
              if (new BN(unmount).gt(new BN(checkCoinbaseBlock.getHeight()))) {
                ...
                this._logger.info(`searching for mount at: ${unmount}`);
                fromHeight = unmount;
                toHeight = fromHeight + 20;
              }

              this._logger.warn(`waypoint issued invalid unmount at ${checkCoinbaseBlock.getHeight()}`);
              await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
              this._PEER_QUARANTINE.unshift(addressToHost(address));
              if (this._discovery.connected && this._discovery.connected - this._PEER_QUARANTINE.length < 2) {
                this._PEER_QUARANTINE.pop();
              }
              this._engine._emitter.emit('requestBlockRange', [toHeight - 10, fromHeight - 12]);
              return;
            } else {
              ...
              if (remount) {
                ...
                await this._engine.persistence.delUtxoRemount();
                fromHeight = remount - 20;
                toHeight = fromHeight + 20;

                this._logger.warn(`waypoint issued critical sequence at ${checkCoinbaseBlock.getHeight()}`);
                this._PEER_BLACKLIST.unshift(addressToHost(address));
                await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
                await this._engine.persistence.reorgBlockchain(BC_SUPER_COLLIDER);
                this._engine._emitter.emit('requestBlockRange', [toHeight - 10, fromHeight - 12], 3);
                return;
              } else {

                this._logger.warn(`waypoint issued temporary sequence at ${checkCoinbaseBlock.getHeight()}`);
                await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
                this._PEER_QUARANTINE.unshift(addressToHost(address));
                if (this._discovery.connected && this._discovery.connected - this._PEER_QUARANTINE.length < 2) {
                  this._PEER_QUARANTINE.pop();
  ...
  async start(nodeId) {
    ...
    this._engine._emitter.on('requestBlockRange', async (range, openLink) => {
      ...
      if (difference < 8 && !givenConnection) {
        limit = 1;
      }
          ...
          if (range[2] && pass) {
            ...
            for (const remoteConnection of connections) {
              ...
              if (hosts.indexOf(host) < 0) {
                debug(`waypoint ${host} with no record`);
              }

              if (sendToPool && p < limit && this._PEER_QUARANTINE.indexOf(host) < 0 && this._PEER_BLACKLIST.indexOf(host) < 0) {
                p += 1;
                debug(`current peer handling broadcast range request to address from ${lowest} to ${highest}`);
                await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.req.range`, `${highest}:${lowest}:${now}:${host}`);
                const t = p === 1 ? 0 : getRandomWithinRange(100, 800);
                setTimeout(async () => {
                  if (limit !== 1) {
                    await this.qsend(remoteConnection, payload);
                  } else {
              ...
              } else if (!hasCurrentPeer && sendToPool && p < 1 && this._PEER_QUARANTINE.indexOf(host) < 0 && this._PEER_BLACKLIST.indexOf(host) < 0) {
                t = t + 350;
                p += 1;
    ...
    this._discovery.on('connection', async (conn, peer) => {
      ...
      try {
        ...
        conn.pipe(framer.decode(FRAMING_OPTS).once('error', e => {
          ...
          const n = Math.floor(Date.now() % 100) + data.length;
          const dataHash = crypto.blake2bl(data);
          if (this._register.length > 30) {
            this._register.shift();
          }
          this._register.push(dataHash);
          if (this._throttle === n) {
            setTimeout(async () => {
    ...
    this._discovery.join(this._discovery.hash, this._discovery.port, data => {
      ...
      this._engine._emitter.on('sendblock', msg => {
        ...
        this.qsend(msg.connection, payload).then(() => {
          this._logger.info('block uplink complete!');
        }).catch(err => {
      ...
      this._engine._asyncEmitter.on('announceblock', async msg => {
        ...
        process.nextTick(async () => {
          ...
          const tasks = shuffle(this._discovery.connections).map(conn => {
            ...
            return this.qsend(conn, payload).catch(e => {
              console.trace(e);
            });
          });
      ...
      this._engine._asyncEmitter.on('announceTx', async msg => {
        ...
        // const h = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.sync.initialsync`)

        // if (h && h === 'pending') {
        //   const p = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.sync.initialpeer`)
        //   if (p) {
        //     debug(`transaction emission halted for pending sync tx`)
        //     return
        //   }
        // }
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      process.nextTick(async () => {
        ...
        await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.block.${block.getHash()}`, block);
        await this._engine.persistence.putBlockHashAtHeight(block.getHash(), block.getHeight(), BC_SUPER_COLLIDER);
        // await this._engine.persistence.saveTxsForBlock(block)

        const exp = await this._engine.persistence.processPeerExpiration({ chainState: this._engine.chainState });
        const synced = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.sync.initialsync`);
        const ipd = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.sync.initialpeerdata`);
        if (iph === 'running' && ipd === 'pending') {
          ...
          if (!currentPeer && synced && synced === 'complete' && this._processedBlocks > 2) {
            ...
            if (currentPeer && currentPeer.getAddress() === addressToHost(address) && Number(new Date()) < parseInt(currentPeer.getExpires(), 10)) {
              ...
              debug(`yielding block ${block.getHeight()} during chain sync (r: ${remain})`);
              await this._engine.persistence.putBlock(block, 0, BC_SUPER_COLLIDER, {
                asHeader: false,
                fromWaypoint: true,
                saveHeaders: true
              ...
              debug(`boot up sequence detected ${block.getHeight()} : ${block.getHash()}`);
              await this._engine.blockFromPeer({ conn: conn, newBlock: block, options: options });
      ...
      if (high > parseInt(latestBlock.getHeight(), 10) + 3) {
        debug(`received GET_DATA request from waypoint ${address} which cannot be completely fulfilled`);
      }
      ...
      if (validDataUpdate === false) {
        ...
        //if (highestBlock && latestBlock && parseInt(highestBlock.getHeight(), 10) > parseInt(latestBlock.getHeight(), 10)) {
        //  await this._engine.persistence.putLatestBlock(highestBlock)
        //}
        const payload = encodeTypeAndData(MESSAGES.GET_DATA, data);
        const sent = await this.rsend(address, payload, conn);
        if (sent !== undefined) {
