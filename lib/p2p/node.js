      ...
      if (this._knownHashes.indexOf(newBlock.getHash()) > -1 && blockQueue.length !== 0) {
        ...
        if (syncProgress > 0) {
          ...
          if (syncProgress > 99.98 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
            ...
            if (blockQueue.length === 0) {
              this._logger.info(`NRG COLLISION #${newBlock.getHeight()} : ${this._childBlocksSynced} ${highestKnownHeight} [] ${newBlock.getHash().slice(0, 5)} ${newBlock.getPreviousHash().slice(0, 5)} ${syncProgress}% <- MINER EMB BOOST: +${this._engine._emblemPerformance}% NRG`);
            }
          }
        }
        continue;
      ...
      if (newBlock !== undefined && newBlock !== null && !invalidData && prevBlock && prevBlock.getHeight && parseInt(newBlock.getHeight(), 10) > 1) {
        ...
        if (isValidCoinbase) {
          ...
          if (prevBlock && prevBlock.getHeight() !== newBlock.getHeight()) {
            ...
            if (syncProgress > 0) {
              ...
              if (syncProgress > 99.97 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
                ...
                if (syncProgress > 99.99) {
                  this._logger.info(`NRG COLLISION #${newBlock.getHeight()} : ${this._childBlocksSynced} ${highestKnownHeight} [] ${newBlock.getHash().slice(0, 5)}:${newBlock.getPreviousHash().slice(0, 5)} <- MINER EMB BOOST: +${this._engine._emblemPerformance}% NRG`);
                } else {
                  this._logger.info(`NRG COLLISION #${newBlock.getHeight()} : ${this._childBlocksSynced} ${highestKnownHeight} [] ${newBlock.getHash().slice(0, 5)}:${newBlock.getPreviousHash().slice(0, 5)} ${syncProgress}% <- MINER EMB BOOST: +${this._engine._emblemPerformance}% NRG`);
                }
              }
            ...
            const storedBlock = await this._engine.persistence.putBlock(newBlock, 0, BC_SUPER_COLLIDER, { saveHeaders: true, fromWaypoint: true });
