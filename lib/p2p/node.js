...
const BC_QUARANTINE_FORGIVENESS = BC_BLOODLINE ? 35000 : getRandomWithinRange(30166, 49600);
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    do {
        ...
        if (syncProgress > 0) {
          ...
          if (rawSyncProgress > 99.999 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
            ...
            if (blockQueue.length === 0) {
              ...
              this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} (${max(0, Math.abs(this._highestRequestedRangeHeight - parseInt(newBlock.getHeight(), 10)))}) ${this._childBlocksSynced}:${childBlocks} <- ${newBlock.getHash().slice(0, 8)} ${newBlock.getPreviousHash().slice(0, 8)} <- EMB MINING POWER: +${this._engine._emblemPerformance}% OL (${radPower} RAD/s)`);
            }
          }
        }
      ...
      if (newBlock !== undefined && newBlock !== null && !invalidData && prevBlock && prevBlock.getHeight && parseInt(newBlock.getHeight(), 10) > 1) {
        ...
        //const isValidCoinbase = flag ? await validateCoinbase(newBlock, this._engine.persistence, this._engine, this._engine._txHandler, this._engine._txPendingPool, 'p2p.node') : true
        //const isValidCoinbase = await validateCoinbase(newBlock, this._engine.persistence, this._engine, this._engine._txHandler, this._engine._txPendingPool, 'p2p.node')
        ...
        if (isValidCoinbase) {
            ...
            if (syncProgress > 0) {
              ...
              if (rawSyncProgress > 99.9999 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
                ...
                if (rawSyncProgress > 99.99) {
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} ${this._childBlocksSynced}:${childBlocks} ${newBlock.getHash().slice(0, 8)}:${newBlock.getPreviousHash().slice(0, 8)} <- EMB MINING POWER: +${this._engine._emblemPerformance}% OL (${radPower} RAD/s)`);
                } else if (newBlock.getHeight() % 2 === 0) {
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} ${this._childBlocksSynced}:${childBlocks} ${syncProgress}% <- ${newBlock.getHash().slice(0, 8)}:${newBlock.getPreviousHash().slice(0, 8)} <- EMB MINING POWER: +${this._engine._emblemPerformance}% OL (${radPower} RAD/s)`);
                } else {
                  this._logger.info(`OL #${newBlock.getHeight()}:${highestKnownHeight} ${this._childBlocksSynced}:${childBlocks} ${syncProgress}% <- ${newBlock.getPreviousHash().slice(0, 8)}:${newBlock.getHash().slice(0, 8)} <- EMB MINING POWER: +${this._engine._emblemPerformance}% OL (${radPower} RAD/s)`);
                }
              }
            }
          ...
          if (opts.innerCall) {
            ...
            if (blockQueue.length === 0) {
              ...
              if (!unmount) {
                missingBlockHeight = await getCoinbaseMissingBlockHeight(newBlock, this._engine.persistence, this._engine._txHandler, this._engine._txPendingPool, 'p2p.node.missing.height');
              } else if (unmount) {
                missingBlockHeight = await getCoinbaseMissingBlockHeight(newBlock, this._engine.persistence, this._engine._txHandler, this._engine._txPendingPool, 'p2p.node.missing.height');
    ...
    //let validcb = (checkCoinbaseBlock.getHeight() + getRandomWithinRange(50,500)) > highestKnownHeight ? await validateCoinbase(checkCoinbaseBlock, this._engine.persistence, this._engine, this._engine._txHandler, this._engine._txPendingPool, 'p2p.node') : true
    let validcb = invalidBlockHeights.length === 0 ? await validateCoinbase(checkCoinbaseBlock, this._engine.persistence, this._engine, this._engine._txHandler, this._engine._txPendingPool, 'p2p.node') : true;
    //let validcb = true
      ...
      if (remount) {
        ...
        if (unmount) {
          ...
          if (new BN(unmount).gt(new BN(checkCoinbaseBlock.getHeight()))) {
            ...
            this._engine._emitter.emit('requestBlockRange', [unmount + BC_MAX_DATA_RANGE, unmount - 3, conn]);
            return;
          }
        } else {
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      if (this._requestRegistry[peerRequestKey] > 600) {
        ...
        if (sent !== undefined && sent.success) {
          // LDL
          debug(`GET_DATA sent to ${sent.address}, message: ${sent.message}`);
          // const validcblazy = getRandomWithinRange(1, 10) < 3 && checkCoinbaseBlock.getHeight() + 1000 < highestKnownHeight && validcb === true ? await validateCoinbase(checkCoinbaseBlock, this._engine.persistence, this._engine, this._engine._txHandler, this._engine._txPendingPool, 'p2p.node') : true
          ...
          if (!validcblazy) {
            ...
            if (unmount) {
              ...
              if (new BN(unmount).gt(new BN(checkCoinbaseBlock.getHeight()))) {
                ...
                this._engine._blockQueue.empty();
  ...
  async start(nodeId) {
    ...
    this._engine._emitter.on('requestBlockRange', async (range, openLink) => {
      ...
      if (this._engine._blockQueue.length() > 0 && openLink !== 1) {
        this._logger.info(`not requesting until block queue is drained`);
        this._engine._blockQueue.process();
        return;
      } else if (openLink === 1) {
        this._engine._blockQueue.empty();
      ...
      //let limit = openLink
      let limit = givenConnection ? 1 : openLink;
      ...
      if (givenConnection) {
        ...
        if (currentPeer !== null && parseInt(currentPeer.getExpires(), 10) >= Number(new Date()) && currentPeer.getAddress() !== addr) {
          ...
          await this._engine.persistence.del('bc.sync.initialpeer', { sync: true });
      ...
      try {
        ...
        conn.pipe(framer.decode(FRAMING_OPTS).once('error', e => {
          ...
          if (this._dataObjectCache.has(dataHash)) {
            saved++;
            debug(`data hash already in cached (${this._dataObjectCache.length}): ${dataHash}, saved: ${saved} `);
            return;
          }
          ...
          if (data) {
            ...
            //if (type === MESSAGES.TX) {
            //  setImmediate(() => {
            //    this.peerDataHandler(conn, peer, data)
            //  })
            //  return
            //}
