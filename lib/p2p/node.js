...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
      ...
      if (this._knownHashes.indexOf(newBlock.getHash()) > -1) {
        ...
        if (syncProgress > 0) {
          ...
          if (syncProgress > 99.9999 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
            ...
            //if (blockQueue.length === 0) {
            this._logger.info(`#${newBlock.getHeight()}:${highestKnownHeight} <- ${newBlock.getHash().slice(0, 20)} ${newBlock.getPreviousHash().slice(0, 20)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}%`);
            //}
          }
        }
        continue;
    ...
    this._logger.info(`loaded: ${validBlocks}, from memory: ${blocksLoadedFromMemory}, height: ${currentHeight}`);
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (!currentPeer && nodeIsSynced && synced === 'complete' && this._processedBlocks > 2) {
