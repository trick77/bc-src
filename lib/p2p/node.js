...
class PeerNode {
  ...
  qsend(conn, msg, dualSend = false) {
    return new Promise(async resolve => {
      ...
      if (!c) {
        c = conn;
        debug(`cannot locate socket in write table ${address}`);
      }
      ...
      try {
        ...
        let open = await backpressureWriteStream(c, wireData);
        clearTimeout(timeout);
        if (!open) {
          //if (!wroteDirectly) {
          timeout = setTimeout(() => {
          ...
          let secondOpen = await backpressureWriteStream(conn, wireData);
          clearTimeout(timeout);
          if (!secondOpen && !timedOut) {
            return resolve({
              address,
              success: false,
              message: err.message
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (synced !== 'complete' && initialPeer && latestBlock && block.getHeight() - 200 > latestBlock.getHeight()) {
        debug(`A: saving block ${block.getHeight()}`);
        await this._engine.persistence.saveBlock(block);
        debug(`A: check waypoint from block ${block.getHeight()}`);
        if (initialPeer && initialPeer.getAddress() !== addressToHost(address)) {
          debug(`block stored only ${block.getHeight()}`);
          return;
          //if (waypoint && waypoint.getAddress) {
          //  this.addToQuarantine(waypoint.getAddress(), `no reponse for range request`)
          //  return
          //} else {
      ...
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (highestKnownHeight && highestKnownHeight < block.getHeight()) {
          ...
          if (latestBlock.getHeight() + 60 > block.getHeight() && latestBlock.getTimestamp() + 60 > block.getTimestamp()) {
