  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
        ...
        if (isValidCoinbase) {
          ...
          if (prevBlock && prevBlock.getHeight() !== newBlock.getHeight()) {
            ...
            if (syncProgress > 0) {
              ...
              this._logger.info(`[] <- ${newBlock.getHeight()} : ${newBlock.getHash().slice(0, 32)} NRG Multichain: ${syncProgress}% | EMB Miner Reward +${this._engine._emblemPerformance}%`);
              if (syncProgress > 99.95) {
                syncThrottleMS = 1200;
                syncLookBack = 3;
              }
            }
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      if (this._requestRegistry[peerRequestKey] > 600) {
        ...
        setTimeout(async () => {
          const data = [currentHeight - syncLookBack, nextHeight];
