  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    do {
      ...
      if (!prevBlock && newBlockBoundary < highestKnownHeight) {
        ...
        const previousIndex = this._knownHashes.indexOf(newBlock.getPreviousHash());
        if (index > -1) {
          this._knownHashes.splice(index, 1);
        }
        if (previousIndex > -1) {
          this._knownHashes.splice(previousIndex, 1);
        }
        ...
        if (!found && !opts.innerCall) {
          ...
          if (blockQueue.length > 0 && blockQueue[0].getHeight() !== newBlock.getHeight() && invalidBlockHeights.indexOf(parseInt(newBlock.getHeight(), 10)) < 0) {
