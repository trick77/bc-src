...
const BC_REBUILD_TXS = parseBoolean(process.env.BC_REBUILD_TXS); // used during development to descrease size of Super Colliders
const TABLE_REQUEST_TIMEOUT = 2 * 1000; // 2 seconds used for the first request of qsend
const GIVEN_REQUEST_TIMEOUT = 5 * 1000; // 5 seconds used for the second attempt on failure OR timeout of the table request
const _MAX_FRAME_SIZE = 28 * 1024 * 1024; // 20MB
const BC_MINIMUM_BLOCK_SCHEMA = isNaN(process.env.BC_MINIMUM_BLOCK_SCHEMA) ? 2 : Number.parseInt(process.env.BC_MINIMUM_BLOCK_SCHEMA, 10);
...
// takes a block and a list of blocks
// returns the total number of connected hashes
const getLongestChainCount = (startBlock, blockList) => {
  const onlyThisHeight = blockList.filter(b => {
    if (b.getHeight() < startBlock.getHeight()) {
      return b;
    }
  });

  const smallChain = [startBlock];

  for (const block of onlyThisHeight) {
    const mount = smallChain[smallChain.length - 1];
    if (block.getHash() === mount.getPreviousHash()) {
      smallChain.push(block);
    }
  }

  return smallChain.length;
};
...
const getMissingBlock = async (host, hash, persistence) => {
  ...
  return new Promise(async resolve => {
    try {
      ...
      fetch(`http://${host}:3000/rpc`, { timeout: 3000, method: 'post', headers: headers, body: body }).then(async response => {
        if (timedOut) {
          return;
        }
...
let BC_BIND_PEER = process.env.BC_BIND_PEER && process.env.BC_BIND_PEER !== 'false' && process.env.BC_BIND_PEER.length > 4 ? process.env.BC_BIND_PEER : false;
let WAYPOINT_TIMES = [];
let logline = false;
...
const BC_EXTENDED_QUARANTINE_FORGIVENESS = BC_BLOODLINE ? 885000 : getRandomWithinRange(2090166, 9044900);
...
const loadBasedPeerExpiration = logline ? 20000 : 30000 + Math.floor(50000 / numCPUs);
...
const BC_PEER_CONSENSUS_THRESHOLD = exports.BC_PEER_CONSENSUS_THRESHOLD = Number(process.env.BC_PEER_CONSENSUS_THRESHOLD) || 2;
...
class PeerNode {
  ...
  constructor(engine) {
    ...
    this._dataObjectCache = new LRUCache({
      max: 2000,
      ttl: getRandomWithinRange(145000, 292900)
    });
    ...
    async function statusInterval() {
      ...
      if (!this._PEER_BLACKLIST || !this._PEER_BLACKLIST.length) {
        return;
      }
  ...
  getWriteSocket(addr) {
    if (!addr || addr.length < 5 || this._discovery.connections.length < 1) {
      return false;
    }
    const formattedAddress = addressToHost(addr);
    return this._discovery.connections.reduce((found, conn) => {
      if (!found) {
        const c = addressToHost(conn.remoteAddress);
        debug(`checking: ${c}`);
  ...
  addToQuarantine(addr, reason, extended) {
    ...
    debug(`added ${addr} to quarantine <- "${reason}" (extended: ${!!extended}`);
  ...
  // uses up to 2 connections prioritizing a pre-existing table connection over a given connection
  // the given connection often is a IPV6 address and docker port which is not easily accessible by firewall redirects
  // the table connection is the IPV4 address often configured by the miners to be accessible
  qsend(conn, msg, dualSend = false) {
    return new Promise(async resolve => {
      // encode the data for transmission over the wire
      const wireData = encodeMessageToWire(msg);
      // the target address to be second the wire data
      const address = `${conn.remoteAddress}:${conn.remotePort}`;
      let c = this.getWriteSocket(conn.remoteAddress);
      // by default it assumes the connection has already been established and maintained in the table (this is not always the case for IPV6 OCM clients)
      let connectionInTable = true;
      if (!c) {
        // the connection was not found in the table to the active table
        connectionInTable = false;
        c = conn;
        debug(`cannot locate socket in write table ${address} will not use failover connection`);
      }

      try {
        debug(`qsend(): about to write ${wireData.length}b`);
        let tableRequestTimedOut = false;
        let givenRequestTimedOut = false;
        let givenTimeout = false;
        let givenConnection = false;
        // create timeout for the table connection which must be cleared regardless of the success of the table connection
        let tableTimeout = setTimeout(async () => {
          tableRequestTimedOut = true;
          // the connection was in the local table but it failed so attempt to use the active connection
          if (connectionInTable) {
            try {
              givenTimeout = setTimeout(() => {
                if (givenRequestTimedOut) {
                  return;
                }
                givenRequestTimedOut = true;
                this.addToQuarantine(address, 'request timed out');
                resolve({
                  address,
                  success: false,
                  message: 'request timed out in table'
                });
              }, GIVEN_REQUEST_TIMEOUT);
              givenConnection = await backpressureWriteStream(conn, wireData);
              clearTimeout(givenTimeout);
              // if givenRequestTimedOut === true then dont return the promise as it has already returned
              if (!givenRequestTimedOut) {
                return resolve({
                  address,
                  success: true,
              ...
              // if the code is here the given request has also timed out
              return;
            } catch (e) {
              if (givenRequestTimedOut) {
                return;
            ...
            //  set the connection to timed out so the when the frame-buffer times out it does not attempt a given request
            givenRequestTimedOut = true;
            this.addToQuarantine(address, 'request timed out');
            return resolve({
              address,
              success: false,
              message: 'request timed out NOT IN TABLE'
            });
          }
        }, TABLE_REQUEST_TIMEOUT);
        // this is the table connection attempt made by the machine
        // the connection is optionally pulled if available from the peer/waypoint table
        let tableConnection = await backpressureWriteStream(c, wireData);
        // immediately clear the table timeout and optionally continue with a second request using the given connection if the table connection failed
        clearTimeout(tableTimeout);
        // the connection failed and there is no alternative to use
        if (!connectionInTable && !tableConnection && !givenRequestTimedOut) {
          this.addToQuarantine(address, 'request timed out');
          return resolve({
            address,
            success: false,
            message: 'request timed out not found in table or given connection'
          });
        }
        // the connection failed AND the table timeout did not fire AND there is no attempt to use the given connection
        if (!tableConnection && !tableRequestTimedOut && connectionInTable) {
          // create timeout which must be cleared regardless of the success of the given connection
          givenTimeout = setTimeout(() => {
            givenRequestTimedOut = true;
            this.addToQuarantine(address, 'request timed out');
            return resolve({
              address,
              success: false,
              message: 'request timed out in in table but connection found in given'
            });
          }, GIVEN_REQUEST_TIMEOUT);
          givenConnection = await backpressureWriteStream(conn, wireData);
          clearTimeout(givenTimeout);
          debug(`qsend(): wrote ${wireData.length}b to ${conn.remoteAddress}:${conn.remotePort} with direct connection`);
          if (!givenRequestTimedOut && givenConnection) {
            return resolve({
              address,
              success: true,
              message: 'success'
            });
          } else if (givenRequestTimedOut && givenConnection) {
            debug(`qsend(): the given connection stream write succeeded but a timeout was fired before the stream was closed`);
            // the second request using the given connection succeeded and did not timeout
          } else if (!givenRequestTimedOut && givenConnection) {
            debug(`qsend(): the given connection stream write succeeded`);
            return resolve({
              address,
              success: true,
              message: 'success'
          ...
          // if either timeout has fired the event loop is handling a second connection later
        } else if (tableRequestTimedOut || givenRequestTimedOut) {
          debug(`qsend(): ignoring failed stream write response due to timeout`);
        } else {
          debug(`qsend(): wrote ${wireData.length}b to ${c.remoteAddress}:${c.remotePort} with load from table`);
          return resolve({
            address,
            success: true,
            message: 'success'
        ...
        if (err) {
          debug(`connection request failure`);
          debug(err);
          return resolve({
            address,
            success: false,
      ...
      if (matched.length >= BC_USER_QUORUM) {
        ...
        const matchedSorted = matched.sort((a, b) => {
          ...
          if (new BN(a.block.getTotalDistance()).gte(new BN(b.block.getTotalDistance()))) {
            ...
            return -1;
            // given option has greater total distance than the first
          } else {
            ...
            // if given option is also higher block height put forward
            return -1;
          }
        });
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (this._PEER_EXTENDED_QUARANTINE.indexOf(addressToHost(address)) > -1 && addressToHost(address) !== BC_BIND_PEER) {
      debug(`waypoint ${address} in extended quarantine and attempted to send DATA consider adding to blacklist`);
      return;
    }
    ...
    if (!waypointRequest && !currentPeer) {
      // TD
      debug(`no range request made`);
      this.addToQuarantine(address, 'waypoint sent range when no range request made');
      if (logline) {
        logline.write(addressToHost(address) + '\n');
      }
      return;
    } else if (!waypointRequest && currentPeer && lowestBlock) {
      this._logger.info(`no range request made, setting request to match initial peer`);
      waypointRequest = `${lowestBlock.getHeight() + BC_MAX_DATA_RANGE}:${lowestBlock.getHeight() - 1}:${Date.now() / 1000}:127.0.0.1`;
    } else if (!waypointRequest) {
      this._logger.warn(`no range request made`);
      this.addToQuarantine(address, 'waypoint sent range when no range request made by local engine');
      if (logline) {
        logline.write(addressToHost(address) + '\n');
      }
      return;
    ...
    if (!currentPeer) {
      ...
      if (reorgBlock && !opts.innerCall && !BC_MINER_POOL) {
        ...
        // TD
        debug(`Unset DATA assigned to initial waypoint blocks contained lowest open req: ${waypointRequestLow}`);
        currentPeer = new InitialPeer();
        currentPeer.setAddress(addressToHost(address));
        currentPeer.setExpires(Number(new Date()) + BC_PEER_HEADER_SYNC_EXPIRE);
        await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.initialpeer`, currentPeer);
      ...
      currentPeer.setExpires(Number(new Date()) + BC_PEER_HEADER_SYNC_EXPIRE);
      await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.initialpeer`, currentPeer);
      if (blocks[0] && parseInt(blocks[0].getHeight(), 10) > 25) {
    ...
    if (currentPeer && currentPeer.getAddress && addressToHost(currentPeer.getAddress()) !== addressToHost(address)) {
      debug(`waypoint has not expired ${currentPeer.getAddress()} to allow ${addressToHost(address)} to submit data`);
    ...
    if (blocks && blocks.length > 0) {
      // LDL
      debug(`MESSAGES.DATA received [] <- ${blocks.length} blocks ${address}`);
      ...
      if (startGivenBlockHeight < waypointRequestLow - BC_MAX_DATA_RANGE * 10) {
        this._logger.info(`start given height ${startGivenBlockHeight} BELOW maximum range ${BC_MAX_DATA_RANGE * 8}`);
        this.addToQuarantine(address, `waypoint responded with block range lower than requested: ${waypointRequestLow} given by waypoint: ${startGivenBlockHeight}`);
        await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
        await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`);
        if (waypointRequestHigh && waypointRequestLow) {
          this._logger.info(`searching for high block after previous segment failed ${waypointRequestLow} -> ${waypointRequestHigh}`);
          this._engine._emitter.emit('requestBlockRange', [waypointRequestHigh, waypointRequestLow - 2], 3);
        }
        return;
      }
      ...
      if (endGivenBlockHeight > waypointRequestHigh + BC_MAX_DATA_RANGE * 10) {
        ...
        await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`);
        if (waypointRequestHigh && waypointRequestLow) {
          this._logger.info(`searching for segment after previous waypoint failed ${waypointRequestLow} -> ${waypointRequestHigh}`);
      ...
      if (endGivenBlockHeight > waypointRequestHigh + BC_MAX_DATA_RANGE * 10) {
        ...
        await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`);
        if (waypointRequestHigh && waypointRequestLow) {
          this._engine._emitter.emit('requestBlockRange', [waypointRequestHigh, waypointRequestLow], 2);
        }
        return;
      }

      // CHECK 4 time bound /////////////////////
      if (waypointRequestTime + 10000 < nowEpoch) {
        this._logger.warn(`end given height ${startGivenBlockHeight} ABOVE maximum range ${BC_MAX_DATA_RANGE * 10}`);
      ...
      // LDL
      debug(`MESSAGES.DATA received from waypoint with malformed blocks array`);
      this.addToQuarantine(address, 'MESSAGES.DATA received from waypoint without blocks');
      await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
      await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`);
      if (logline) {
        logline.write(addressToHost(address) + '\n');
    ...
    if (this._requestRegistry[peerKey] > 10000) {
    ...
    let lowestHeightRawBoundary = reorgFromBlock && reorgFromBlock.getHeight ? parseInt(reorgFromBlock.getHeight(), 10) - 500 : latestHeightRaw - 500;
    if (reqHeight && reqHeight < lowestHeightRawBoundary) {
      debug(`scan for block range detected ${lowestHeightRawBoundary} -> ${reqHeight - 200} cleared cache`);
      lowestHeightRawBoundary = reqHeight - 200;
    }

    let pendingArrivalOfLocalHash = false;
    ...
    // as blocks are evaluated they are placed in this array
    ...
    if (blockQueue.length > 0) {
      ...
      if (this._knownBlockSegments.has(segmentKey)) {
        ...
        this.addToQuarantine(address, `Waypoint sent pre-processed segment in same sync cycle ${blockQueue[0].getHeight()}`, true);
        await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`);
        await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
        this._logger.info(`searching for block segment ${blockQueue[blockQueue.length - 1].getHeight()}`);
        this._engine._emitter.emit('requestBlockRange', [blockQueue[blockQueue.length - 1].getHeight() + BC_MAX_DATA_RANGE, blockQueue[blockQueue.length - 1].getHeight()], 3);
        return;
      }
    }
    ...
    if (newQueue.length === 0 && !checkCoinbaseBlock) {
     ...
     */
    await Promise.all(blockQueue.map(block => {
      return this._engine.persistence.saveBlock(block);
    }));

    do {
      const newBlock = blockQueue.shift();
      if (!newBlock || !newBlock.getHash) {
        continue;
      }
      this._temporaryBlockStore[newBlock.getHash()] = newBlock;
      evaluatedBlocks.push(newBlock);

      if (newBlock && newBlock.getHeight) {
        if (evaluatedBlocks.length > 1) {
          const foundMount = evaluatedBlocks.slice(evaluatedBlocks.length - 7, evaluatedBlocks.length + 1).reduce((all, b) => {
            if (b.getHash() === newBlock.getPreviousHash()) {
              all = true;
            }
            return all;
          }, false);
          const branchBlock = evaluatedBlocks[evaluatedBlocks.length - 2];
          if (!foundMount && branchBlock && branchBlock.getHeight() === newBlock.getHeight()) {
            if (this._temporaryBlockStore[branchBlock.getPreviousHash()]) {
              this._logger.info(`would mount coinbase set to previous block ${branchBlock.getHash()}`);
              branchCoinbaseBlock = branchBlock;
            }
          }
        } else {
          checkCoinbaseBlock = newBlock;
      ...
      if (parseInt(newBlock.getHeight(), 10) < 2) {
        ...
        // begin checking for invalid blocks which are not on the main chain
        if (newBlock.getHeight() > 7766000) {
          validTxs = await validateTxs(this._engine._txHandler, newBlock);

          if (!validTxs) {
            this.addToQuarantine(address, `invalid txs with given ${newBlock.getHeight()}`, true);
            this._logger.info(`block submitted does not have valid txs ${newBlock.getHeight()}`);
            invalidBlockHeights.push(newBlock.getHeight());
            blocksProvidedOutsideBoundary = true;
            return;
          }
        }
      ...
      debug(`processing OL block (valid txs: ${validTxs}), ${newBlock.getHeight()} : ${newBlock.getHash()} of ${blockQueue.length}`);
      ...
      if (this._knownHashes.indexOf(newBlock.getHash()) > -1) {
        ...
        debug(`hash already seen: ${newBlock.getHeight()}:${newBlock.getHash()}`);
        ...
        // TD: await this._engine.persistence.saveBlock(newBlock)
        continue;
      ...
      if (!prevBlock) {
        ...
        prevBlock = await this._engine.persistence.getBlockByHash(newBlock.getPreviousHash(), BC_SUPER_COLLIDER);
        ...
        for (const b of evaluatedBlocks) {
          if (newBlock.getPreviousHash() === b.getHash()) {
            this._logger.info(`previous hash mount found ${b.getHash()}`);
            prevBlock = b;
          }
        }
        ...
        if (prevBlock && prevBlock.getHash() !== newBlock.getPreviousHash()) {
          ...
          if (evaluatedBlocks.length > 1) {
            if (evaluatedBlocks[evaluatedBlocks.length - 2].getHeight() === newBlock.getHeight()) {
              this._logger.info(`edge branch discovered rebased ${newBlock.getHeight()}`);
              branchCoinbaseBlock = evaluatedBlocks[evaluatedBlocks.length - 2];
              // TD: await this._engine.persistence.saveBlock(newBlock)
            ...
            // TD
            debug(`orphaned block discovered ${newBlock.getHeight()}: ${newBlock.getHash()}`);
            continue;
          } else {
            prevBlock = false;
          }
      ...
      if (!prevBlock && newBlockBoundary < highestKnownHeight) {
        ...
        if (!found) {
          ...
          if (!hb) {
            ...
            pendingArrivalOfLocalHash = true;
        ...
        debug(`found: ${found} but missing ${BC_SUPER_COLLIDER} block ${blockHeight - 1} : ${newBlock.getPreviousHash()}`);
        ...
        //  }
        //}
        if (!found && blockQueue.length > 0 && blockQueue[0].getHeight && parseInt(blockQueue[0].getHeight(), 10) !== parseInt(newBlock.getHeight(), 10)) {
          this._logger.info(`waypoint provided broken segment at ${newBlock.getHeight()}`);
          brokenSegment = true;
        }
        continue;
      }
      ...
      if (newBlock !== undefined && newBlock !== null && !invalidData && prevBlock && prevBlock.getHeight && parseInt(newBlock.getHeight(), 10) > 1) {
        ...
        if (!sv || !validTxs) {
          ...
          this.addToQuarantine(address, 'invalid block sent in response to block range request', true);
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initalpeer`);
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`);
          continue;
        }
        ...
        if (prevBlock && prevBlock.getHeight() !== newBlock.getHeight()) {
          ...
          if (!isValidSeq.valid && blockQueue[0] && blockQueue[0].getHeight() !== newBlock.getHeight()) {
            ...
            if (!optionsAt || optionsAt && optionsAt.length < 1) {
              ...
              validBlocks++;
              // TD
              debug(`alternative path already includes block: ${newBlock.getHeight()} yielding...${optionsAt.join(',')}`);
              ...
              for (const b of optionsAt) {
                if (b !== newBlock.getHash()) {
                  ...
                  if (fb) {
                    this._knownHashes.push(newBlock.getHash());
                    const d = this._knownHashes.indexOf(fb.getHash());
                    if (d < 0) {
              ...
              if (blockQueue.length === 0) {
                checkCoinbaseBlock = evaluatedBlocks[evaluatedBlocks.length - 2];
                this._logger.info(`setting check coinbase block ${checkCoinbaseBlock.getHeight()}:${checkCoinbaseBlock.getHash()}`);
              }
            }
            ...
            // TD
            debug(`isolated validation <- valid: ${isolatedValidation}, ${newBlock.getHeight()} : ${newBlock.getHash().slice(0, 8)} ${newBlock.getPreviousHash()}, connected: ${newBlock.getBlockchainHeadersCount()}`);
    ...
    if (invalidBlockHeights.length > 0) {
      for (const height of invalidBlockHeights) {
        this._logger.info(`setting edge without coinbase ${height}`);
      }
    }

    this._engine._utxoManager._activeWaypoint = currentPeer;
    /*
     * Multibranch Parsing
     * Stage 1 Test the initial edge coinbase block
     * Stage 2 If 1 fails, check blocks at that height
     * Stage 3 If 2 fails, and a branch block exists use the branch block
     */
    if (highestBlock && checkCoinbaseBlock && highestBlock.getHeight() < checkCoinbaseBlock.getHeight() && !forceBreak) {
      debug(`proposed new set block ${highestBlock.getHeight()} replaced...${checkCoinbaseBlock.getHeight()}:${checkCoinbaseBlock.getHash()}`);
      //checkCoinbaseBlock = highestBlock
    }
    let validcb = !segmentKnown && invalidBlockHeights.length === 0 ? await validateCoinbase(checkCoinbaseBlock, this._engine.persistence, this._engine, this._engine._txHandler, this._engine._txPendingPool, this._engine._utxoManager, 'p2p.node') : true;
    // Stage 1 Test the initial edge coinbase block
    let highestBlockRemount = await this._engine.persistence.getUtxoRemount();
    let highestBlockUnmount = await this._engine.persistence.getUtxoUnmount();
    let unmountRaw = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.unmount`);
    if (highestBlockRemount) {
      await this._engine.persistence.delUtxoRemount();
    }
    if (highestBlockUnmount) {
      await this._engine.persistence.delUtxoUnmount();
    }

    // hard branch check evaluates the other potential paths if there are other blocks at this height
    // Stage 2 If 1 fails, check blocks at that height
    if (!validcb) {
      const blocksAtThisHeight = await this._engine.persistence.getBlocksByHeight(checkCoinbaseBlock.getHeight(), BC_SUPER_COLLIDER, {
        asBuffer: true,
        cached: false
      });

      // there are other blocks at the height of the given coinbase block check
      if (blocksAtThisHeight) {
        const options = [];
        for (const o of blocksAtThisHeight) {
          if (o.getHash() !== checkCoinbaseBlock.getHash()) {
            options.push(o);
          }
        }
        debug(`found ${options.length} options at this height`);
        if (options.length === 1) {
          checkCoinbaseBlock = options[0];
          validcb = await validateCoinbase(checkCoinbaseBlock, this._engine.persistence, this._engine, this._engine._txHandler, this._engine._txPendingPool, this._engine._utxoManager, 'p2p.node');
          // set both of the remounts to false
          if (validcb) {
            highestBlockRemount = false;
            highestBlockUnmount = false;
          } else {
            await this._engine.persistence.delUtxoRemount();
            await this._engine.persistence.delUtxoUnmount();
          }
          ...
          const ranks = [];
          checkCoinbaseBlock = options.reduce((all, o) => {
            const count = getLongestChainCount(o, evaluatedBlocks);
            debug(`block ${o.getHeight()}:${o.getHash().slice(0, 6)} has chain of ${count} from a total of ${evaluatedBlocks.length}`);
            ranks.push([o, count]);
            if (!all) {
              all = [o, count];
            } else if (all[1] < count) {
          ...
          for (const rank of ranks) {
            ...
            this._logger.info(`checking rank ${rankIndex} of ${ranks.length} ${checkCoinbaseBlock.getHeight()}:${checkCoinbaseBlock.getHash().slice(0, 6)}`);
            validcb = await validateCoinbase(checkCoinbaseBlock, this._engine.persistence, this._engine, this._engine._txHandler, this._engine._txPendingPool, this._engine._utxoManager, 'p2p.node');
            if (validcb) {
              highestBlockRemount = false;
              highestBlockUnmount = false;
    ...
    // Stage 3 If 2 fails, and a branch block exists use the branch block
    if (!validcb) {
      if (!validcb && branchCoinbaseBlock && branchCoinbaseBlock.getHash() !== checkCoinbaseBlock.getHash()) {
        debug(`using branch block ${branchCoinbaseBlock.getHeight()}:${branchCoinbaseBlock.getHash().slice(0, 6)}`);
        validcb = await validateCoinbase(branchCoinbaseBlock, this._engine.persistence, this._engine, this._engine._txHandler, this._engine._txPendingPool, this._engine._utxoManager, 'p2p.node');
        if (validcb) {
          checkCoinbaseBlock = branchCoinbaseBlock;
        }
      }
    }

    if (validcb) {
      evalBlock = checkCoinbaseBlock;
      highestBlockRemount = false;
      highestBlockUnmount = false;
    } else {
      await this._engine.persistence.delUtxoRemount();
    ...
    if (validcb && checkCoinbaseBlock) {
      if (latestBlock && checkCoinbaseBlock.getHeight() > latestBlock.getHeight()) {
        debug(`${checkCoinbaseBlock.getHeight()} -> has valid mount from: ${latestBlock.getHeight()} ${checkCoinbaseBlock.getHeight()}`);
        //await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.block.reorgfrom`, checkCoinbaseBlock)
      } else if (!latestBlock) {
        debug(`${checkCoinbaseBlock.getHeight()} -> has valid base segment known: ${segmentKnown}`);
        //await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.block.reorgfrom`, checkCoinbaseBlock)
      }
    }

    // means all branchs have not pass the check
    if (!validcb) {
      ...
      if (highestBlockRemount) {
        await this._engine.persistence.delUtxoRemount();
        if (highestBlockRemount >= lowestBoundary) {
          if (invalidBlockHeights.indexOf(highestBlockRemount) < 0) {
            invalidBlockHeights.push(highestBlockRemount);
          }
          this._logger.warn(`${highestBlockRemount} block found in critical relative root position ${addressToHost(address)}`);
          this._PEER_BLACKLIST.push(addressToHost(address));
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
          await this._engine.persistence.reorgBlockchain();
          await this._engine.reorgTxPendingPool();
          ...
          const p = addressToHost(address);

          if (this._PEER_BLACKLIST.indexOf(p) < 0) {
            this._PEER_BLACKLIST.push(p);
          }

          this._logger.warn(`${highestBlockRemount} block found in critical root position, running large reindex commmand...`);
          ...
          if (this._PEER_CONSENSUS[consensusKey] < BC_PEER_CONSENSUS_THRESHOLD) {
            const stepBackHeight = highestBlockRemount - getRandomWithinRange(1080, 2080);
        ...
        if (highestBlockUnmount && highestBlockUnmount < checkCoinbaseBlock.getHeight()) {
          this._logger.warn(`${highestBlockUnmount} block not in position...`);

          if (new BN(highestBlockUnmount).gt(new BN(checkCoinbaseBlock.getHeight()))) {
            await this._engine.persistence.delUtxoUnmount();
            this.addToQuarantine(address, 'invalid coinbase check block from given segment');
            this._logger.info(`active sync in progress from mount ${highestBlockUnmount}`);
          } else {

            this._logger.info(`searching for mount: ${highestBlockUnmount} without penalty (${highestBlockUnmount + BC_MAX_DATA_RANGE} -> ${highestBlockUnmount - 4})`);
            ...
            if (unmountRaw) {
              ...
              await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.data.latest`, `${Number(uheight) + BC_MAX_DATA_RANGE - 100}:${Math.floor(Date.now() / 1000)}`);

              if (!block) {
                await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`, { sync: true });
                await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`, { sync: true });
                ...
                this._logger.info(`height requested directly ${Number(uheight) + BC_MAX_DATA_RANGE - 100}`);
                this._engine._emitter.emit('requestBlockRange', [Number(uheight) + BC_MAX_DATA_RANGE - 100, Number(uheight) - 100], 3);
                return;
              }
              ...
              this.addToQuarantine(address, `block provided without mount`);
              await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
              await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`);
              await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.data.latest`, `${highestBlockUnmount - 40}:${Math.floor(Date.now() / 1000)}`);
              this._engine._emitter.emit('requestBlockRange', [highestBlockUnmount + BC_MAX_DATA_RANGE - 40, highestBlockUnmount - 40], 3);
              return;
            }
          }
        } else if (!highestBlockUnmount || highestBlockUnmount > checkCoinbaseBlock.getHeight()) {
          await this._engine.persistence.delUtxoUnmount();
          forceBreak = true;

          if (highestBlockUnmount) {
            pendingArrivalOfLocalHash = false;
          }
          //this.addToQuarantine(address, 'unmount not set')
          if (pendingArrivalOfLocalHash) {
            this._logger.info(`unmount not set on current block: ${checkCoinbaseBlock.getHeight()} searching lower...`);
          } else {
            this._logger.info(`unmount unset: ${!highestBlockUnmount} <- current block: ${checkCoinbaseBlock.getHeight()} searching higher...`);
          }

          this._knownBlockSegments.clear();

          let direction = !pendingArrivalOfLocalHash ? 1 : -1;
          let scanBackHeight = checkCoinbaseBlock.getHeight() + 10 * direction;
          const prevFlaggedBlock = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.block.lock.${checkCoinbaseBlock.getHeight()}`);
          if (prevFlaggedBlock) {
            this._logger.info(`previous locked block detected ${prevFlaggedBlock} for OL BLOCK ${checkCoinbaseBlock.getHeight()}`);
            scanBackHeight = prevFlaggedBlock + 240 * direction;
          }
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`);
          await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.data.latest`, `${scanBackHeight}:${Math.floor(Date.now() / 1000)}`);
          await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.block.flag.${checkCoinbaseBlock.getHeight()}`, scanBackHeight);
    ...
    if (blocksLength < BC_MAX_DATA_RANGE && validBlocks > 0 && parseInt(evalBlock.getHeight(), 10) + BC_MAX_DATA_RANGE < highestKnownHeight && finalValidBlocks.length > 9 && finalValidBlocks.length !== 1 && parseInt(evalBlock.getHeight(), 10) < highestKnownHeight) {
      ...
      if (BC_ORTHOGONAL_WAYPOINTS) {
        ...
        this.addToQuarantine(address, 'waypoint has not provided highest edge');
        ...
        if (finalValidBlock) {
          debug(`setting latest block ${finalValidBlock.getHeight()} : ${finalValidBlock.getHash()}`);
        }
    ...
    } else if (segmentUnlinked && !segmentKnown) {
      ...
      if (this._PEER_QUARANTINE.length >= 16 && this._PEER_BLACKLIST.length > 4) {
        ...
        if (BC_ORTHOGONAL_WAYPOINTS) {
          ...
          let moveBackRange = forceBreak ? 28 : 4;
          let movingEdge = dlh - Math.floor(Math.random() * Math.floor(moveBackRange + 8));
          let numberOfWaypoints = false;
          if (finalInvalidBlocks.length > 0) {
            movingEdge = finalInvalidBlocks[0] - Math.floor(Math.random() * Math.floor(moveBackRange + 4));
          }
          ...
          Promise.all([this._engine.persistence.put(`${BC_SUPER_COLLIDER}.data.latest`, `${movingEdge}:${tn}`), this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`), this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`)]).then(() => {
            this._engine._emitter.emit('requestBlockRange', [movingEdge + BC_MAX_DATA_RANGE, movingEdge], numberOfWaypoints);
          });
          return;
        } else {
          ...
          Promise.all([this._engine.persistence.put(`${BC_SUPER_COLLIDER}.data.latest`, `${parseInt(finalValidBlock.getHeight(), 10) - 8}:${tn}`), this._engine.persistence.put(`${BC_SUPER_COLLIDER}.block.reorgfrom`, finalValidBlock), this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`), this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`)]).then(() => {
    ...
    if (validBlocks >= minimumValidBlocks && new BN(currentHeight).gte(new BN(latestBlock.getHeight() - 1)) && !invalidData) {
      ...
      if (!highestBlock && new BN(currentHeight).gt(latestBlock.getHeight())) {
        ...
        if (!highestBlock) {
          debug(`set highest block to latest ${latestBlock.getHeight()}`);
          highestBlock = latestBlock;
        }
      } else {
        debug(`default highest block to latest ${latestBlock.getHeight()}`);
        highestBlock = latestBlock;
      }

      if (checkCoinbaseBlock && validcb && highestBlock.getHeight() < checkCoinbaseBlock.getHeight() && !forceBreak) {
        debug(`would have block ${highestBlock.getHeight()} replaced...${checkCoinbaseBlock.getHeight()}:${checkCoinbaseBlock.getHash()}`);
        if (newQueue.length < 1) {
          debug(`setting check coinbase to fixed ${highestBlock.getHeight()} ${highestBlock.getHash()}`);
          // TD
          //checkCoinbaseBlock = highestBlock
        }
      }
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      const now = Date.now();
      debug(`PQ WARNING: 9 for ${addressToHost(address)}`);
      this.addToQuarantine(address, 'waypoint did not send minimum viable blocks', true);
      this._knownBlockSegments.clear();
      const syncedStatus = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.sync.initialsync`);
      await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
      ...
      debug(`setting highest height to ${currentHeight} pending sync...`);
      const t = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.data.latest`);
      if (t && t.split(':')[0] === 'currentHeight') {
        this._logger.info(`request for ${currentHeight} already exists`);
      }
      ...
      if (this._engine._blockStats) {
        for (const finalBlock of finalValidBlocks) {
          debug(`indexing ${finalBlock.getHeight()} : ${finalBlock.getHash().slice(0, 12)}`);
      ...
      if (this._requestRegistry[peerRequestKey] > 600) {
        ...
        // TD
        this._logger.info(`searching for ${data[0]} -> ${data[1]} `);
      ...
      const now = Date.now();
      debug(`multichain highest edge set <- ${currentHeight}, eval block ${evalBlock.getHeight()}`);
      if (this._engine._blockStats) {
        for (const finalBlock of finalValidBlocks) {
          const previousMiner = this._engine._loggedBcBalances.lastMiner;
          this._engine._loggedBcBalances.lastMiner = finalBlock.getMiner();
      ...
      await Promise.all([this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.edge`, parseInt(evalBlock.getHeight(), 10)), this._engine.persistence.put(`${BC_SUPER_COLLIDER}.range.lowest.height`, parseInt(evalBlock.getHeight(), 10) - 3), this._engine.persistence.put(`${BC_SUPER_COLLIDER}.data.latest`, `${parseInt(evalBlock.getHeight(), 10)}:${tn}`), this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`), this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.initialpeerheader`, 'complete'), this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.initialpeerdata`, 'complete')]);

      const rebuiltIndex = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.indexrebuilt`);
      if (!rebuiltIndex || rebuiltIndex !== 'complete') {
        debug(`running rebuilt index command`);
        const lt = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.block.latest`);
        await this._engine.persistence.rebuildChainIndexFromHeight(this._engine._utxoManager, this._engine._txHandler, 7579000, lt);
      }

      const utxoRebuilt = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.utxorebuilt`);
      if (!utxoRebuilt || utxoRebuilt !== 'complete') {
        debug(`running utxo rebuilt index command`);
        const lt = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.block.latest`);
        await this._engine.persistence.rebuildTxGraph(this._engine._utxoManager, this._engine._txHandler, this._engine._wallet, 7579000, lt);
        return;
      }
  ...
  async start(nodeId) {
    ...
    this._engine._emitter.on('requestBlockRange', async (range, openLink) => {
      ...
      if (this._engine._blockQueue && this._engine._blockQueue.length() > 0 && openLink !== 1) {
        debug(`${this._engine._blockQueue.length()} blocks in queue`);
      }
      ...
      let givenConnection = range && range.length > 2 ? range[2] : false;
      let addr = addressToHost(`${givenConnection.remoteAddress}:${givenConnection.remotePort}`);
      debug(`BLOCK RANGE REQUEST: ${range[1]} -> ${range[0]} direct: ${range.length > 2} ${addr}`);
      let currentPeer;
      //let limit = openLink
      ...
      if (givenConnection) {
        ...
        if (currentPeer !== null && parseInt(currentPeer.getExpires(), 10) < Number(new Date()) && currentPeer.getAddress() !== addr) {
          ...
          this.addToQuarantine(currentPeer.getAddress(), 'connection failed to responed for range', true);
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
          currentPeer = new InitialPeer();
          currentPeer.setAddress(addr);
          currentPeer.setExpires(Number(new Date()) + BC_PEER_HEADER_SYNC_EXPIRE);
          await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.initialpeer`, currentPeer);
          ...
          // no given peer first request
          this._logger.info(`first range request opened waypoint`);
          currentPeer = new InitialPeer();
          currentPeer.setAddress(addr);
          currentPeer.setExpires(Number(new Date()) + BC_PEER_HEADER_SYNC_EXPIRE);
          await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.initialpeer`, currentPeer);
        ...
        //currentPeer = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.sync.initialpeer`)
        //if (currentPeer !== null && parseInt(currentPeer.getExpires(), 10) >= Number(new Date()) && currentPeer.getAddress() !== addr) {
        //  this._logger.info(`pending waypoint unlock ${currentPeer.getAddress()} unlock: ${parseInt(currentPeer.getExpires(), 10) - Number(new Date())}...`)
        //  return
        //}
      }
      ...
      if (highest !== lowest) {
        ...
        if (this._discovery.connections) {
          ...
          if (range[2]) {
            ...
            const address = `${remoteConnection.remoteAddress}:${remoteConnection.remotePort}`;
            const host = addressToHost(address);
            if (this._PEER_BLACKLIST.indexOf(host) < 0 && this._PEER_QUARANTINE.indexOf(host) < 0 && this._PEER_EXTENDED_QUARANTINE.indexOf(host) < 0) {
          ...
          if (range[2] && pass) {
            ...
            if (sendToAnyPeer && p < 1 && this._PEER_EXTENDED_QUARANTINE.indexOf(address) < 0 && this._PEER_BLACKLIST.indexOf(address) < 0 && this._PEER_BLACKLIST.indexOf(host) < 0 && this._PEER_QUARANTINE.indexOf(addressToHost(address)) < 0 && sendToPool) {
              // LDL
              ...
              if (sent && sent.success) {
                ...
                this._engine._emitter.emit('requestBlockRange', [range[0], range[1]], 1);
                return;
              }
              // request is complete
              return;
          ...
          if (this._discovery.connections.length > 0) {
            // the for loop is to find a peer willing to send the range
            ...
            for (const remoteConnection of connections) {
              ...
              if (sendToPool && p < limit && this._PEER_QUARANTINE.indexOf(host) < 0 && this._PEER_BLACKLIST.indexOf(host) < 0 && this._PEER_BLACKLIST.indexOf(address) < 0) {
                p += 1;
                debug(`direct request to address ${host} from ${lowest} to ${highest}`);
                await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.req.range`, `${highest}:${lowest}:${now}:${host}`);
                ...
                p += 1;
                // TD
                debug(`broadcasting range request to address ${address} from ${lowest} to ${highest} with window ${t} and p: ${p}`);
                // continue requesting
                await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.req.range`, `${highest}:${lowest}:${now}:${host}`);
                await this.qsend(remoteConnection, payload);
              } else {
      ...
      try {
        ...
        conn.pipe(framer.decode(FRAMING_OPTS).once('error', e => {
          ...
          if (data) {
            ...
            const accepted = [MESSAGES.GET_BLOCKS, MESSAGES.GET_CONFIG, MESSAGES.GET_DATA, MESSAGES.GET_MULTIVERSE, MESSAGES.GET_BLOCK, MESSAGES.DATA];

            if (this._dataObjectCache.has(dataHash) && accepted.indexOf(type) < 0) {
              saved++;
              const readable = Object.keys(MESSAGES).filter(key => {
                if (type === MESSAGES[key]) {
                  return true;
                }
              });
              debug(`data hash already in cached (${this._dataObjectCache.length}): ${dataHash}, saved: ${saved}, conn: ${conn.remoteAddress}, type: ${readable.join('')} `);
              return;
            }
            ...
            if (type === MESSAGES.BLOCK) {
              const lock = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.indexlock`);
              if (lock) {
                debug(`index lock active`);
                return;
              }
              if (this._PEER_EXTENDED_QUARANTINE.indexOf(addressToHost(address)) > -1) {
                debug(`waypoint ${address} in extended quarantine and attempted to send BLOCK consider adding to blacklist`);
                return;
              }
            ...
            if (type === MESSAGES.TX) {
              if (this._PEER_EXTENDED_QUARANTINE.indexOf(addressToHost(address)) > -1) {
                debug(`waypoint ${address} in extended quarantine and attempted to send TX after sending invalid txs consider adding to blacklist`);
                return;
              }
              await this.peerDataHandler(conn, peer, data);
              return;
            ...
            if (type === MESSAGES.DATA) {
              if (this._PEER_EXTENDED_QUARANTINE.indexOf(addressToHost(address)) > -1) {
                debug(`waypoint ${address} in extended quarantine and attempted to send DATA after sending invalid txs consider adding to blacklist`);
                return;
              }
              await this.peerDataHandler(conn, peer, data);
              return;
        ...
        if (traditionalSync && !DISABLE_IPH_TEST) {
          ...
          try {
            ...
            await this.qsend(conn, payload, true);
          } catch (err) {
    ...
    this._discovery.join(this._discovery.hash, this._discovery.port, data => {
      ...
      this._engine._emitter.on('sendblock', msg => {
        ...
        debug(`sending payload directly to ${addr}`);
        this.qsend(msg.connection, payload).then(() => {
          debug('payload uplink complete!');
        }).catch(err => {
      ...
      this._engine._asyncEmitter.on('announceblock', async msg => {
        const payload = encodeTypeAndData(MESSAGES.BLOCK, msg.data);
        const tasks = shuffle(this._discovery.connections).map(conn => {
          //for await (const conn of shuffle(this._discovery.connections)) {
          try {
            const addr = conn.remoteAddress + ':' + conn.remotePort;
            if (msg.data.getHash !== undefined) {
              ...
              if (this._noDoubleSent.has(addressToHost(addr) + msg.data.getHash())) {
                ...
                this.qsend(conn, payload, true).then(() => {
                  debug(`block sent to ${addr}`);
                }).catch(err => {
                  debug(err);
                });
              }
        ...
        //  debug(`block broadcast ${msg.data.getHeight()} to ${this._discovery.connections.length} waypoints`)
        //}).catch((err) => {
        //  debug(err)
        //  this._logger.error(err.message)
        //})
    ...
    this._discovery._seeder.setMaxListeners(0);
    this._discovery._seeder.on('peer', peer => {
      ...
      debug(`waypoint discovered ${h}, raw ${peer}`);
      const obj = {
        host: h[0],
      ...
      this._discovery.addPeer(channel, obj);
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      const queueLength = this._engine._blockQueue.length();
      debug(`MESSAGES.BLOCK received from host ${address} while length is ${queueLength}`);

      // TD: EVM machine
      if (block.getHash() === '10d8b4587f7da6ba7cc1a92a7935070f34563f5e457061c8dc81842a5001d0a4') {
        return;
      }

      if (this._dataObjectCache.has(block.getHeight())) {
        debug(`waypoint ${address} sent BLOCK height already evaluated`);
        return;
      } else {
        this._dataObjectCache.set(block.getHeight(), true);
      }
      ...
      if (this._PEER_QUARANTINE.indexOf(addressToHost(address)) > -1 && addressToHost(address) !== BC_BIND_PEER) {
        ...
        if (this._PEER_RECORD[host]) {
          ...
          debug(`[ATTEMPT] <- OL BLOCK ${block.getHeight()} ${block.getHash()} ${block.getDifficulty()} <- ${address} <- ${block.getMiner()}`);
        }
        return;
      }
      ...
      if (this._PEER_EXTENDED_QUARANTINE.indexOf(addressToHost(address)) > -1 && addressToHost(address) !== BC_BIND_PEER) {
        ...
        if (this._PEER_RECORD[host]) {
          ...
          debug(`[EXTENDED ATTEMPT] <- OL BLOCK ${block.getHeight()} ${block.getHash()} ${block.getDifficulty()} <- ${address} <- ${block.getMiner()}`);
        }
        return;
      }

      if (this._PEER_BLACKLIST.indexOf(addressToHost(address)) > -1 && addressToHost(address) !== BC_BIND_PEER) {
        debug(`waypoint ${address} in BLACKLIST and attempted to send BLOCK`);
        return;
      }

      this._logger.info(`[] <- OL BLOCK ${block.getHeight()} ${block.getHash()}`);
      ...
      if (!BC_MINIMUM_HEADER_TEST || BC_MINIMUM_HEADER_TEST === 0) {
        ...
        debug(`_XX_ BLOCK ${block.getHeight()} ___ BC_MINIMUM_HEADER_TEST === 1, yielding block from waypoint ${block.getHeight()} ${block.getDifficulty()}`);
        return;
      ...
      if (BC_MINIMUM_BLOCK_SCHEMA < parseInt(block.getSchemaVersion()) && block.getHeight() >= highestKnownHeight) {
        this._logger.info(`block using minimum schema ${block.getHeight()} ${block.getSchemaVersion()}`);
      }

      if (new BN(block.getDistance()).lt(new BN(209578459912602)) && parseInt(block.getHeight(), 10) < 8093000) {
        this._logger.warn(`invalid sequence detected ${block.getHeight()} ${block.getMiner()}`);
        if (parseInt(block.getHeight(), 10) < 5) {
          return;
        }
        this.addToQuarantine(address, 'invalid sequence');
        return;
      }
      ...
      const blockPassesTest = highestKnownHeight <= block.getHeight();
      if (!blockPassesTest) {
        //this._PEER_RECORD[host].badBlocks++
        this.addToQuarantine(address, `waypoint sent block triggering failed rover test ${highestKnownHeight} > ${block.getHeight()}`);
        return;
      }
      ...
      if (BC_MINIMUM_HEADER_TEST === 1) {
        debug(`BC_MINIMUM_HEADER_TEST === 1 <- high risk for waypoint sync failure`);
      }
      ...
      if (!this._SEEN_BLOCKS_MEMORY[block.getHash()] && parseInt(block.getHeight(), 10) !== 1) {
        ...
        debug(`new block ${block.getHeight()}, from waypoint: ${address}, processed blocks ${this._processedBlocks} miner: ${block.getMiner()}, hash: ${dataHash}`);
        ...
        debug(`block seen ${block.getHeight()}, from waypoint: ${address}, processed blocks ${this._processedBlocks} miner: ${block.getMiner()}`);
        return;
      }

      // check if the current waypoint sync has expired
      // check if the block received is less than 200 blocks from the last known latest block
      ...
      if (synced !== 'complete' && initialPeer && latestBlock && block.getHeight() - 30 > latestBlock.getHeight() && Number(new Date()) < parseInt(initialPeer.getExpires(), 10)) {
        debug(`A: saving block ${block.getHeight()} peer has not expired`);
        await this._engine.persistence.saveBlock(block);
        debug(`A: check waypoint from block ${block.getHeight()}`);
        return;
      } else if (synced !== 'complete' && initialPeer && latestBlock && block.getHeight() - 30 > latestBlock.getHeight() && Number(new Date()) > parseInt(initialPeer.getExpires(), 10)) {
        await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
        if (initialPeer.getAddress() !== addressToHost(address)) {
          debug(`waypoint has not responded, consider reorg blockchain function`);
          this.addToQuarantine(initialPeer.getAddress(), `no response for range request`);
        } else {
          // ignore block from the expired peer
          return;
        }
      }
      ...
      let latestBlockChildSum = false;
      let newBlockChildSum = false;
      const hold = false;
      const ipd = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.sync.initialpeerdata`);
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (latestBlock && latestBlock.getHeight() > 1 && block.getPreviousHash() === block.getHash()) {
          ...
          latestBlockChildSum = childrenHeightSum(latestBlock);
          newBlockChildSum = childrenHeightSum(block);
        ...
        this._engine.pubsub.publish('block.peer', {
          type: 'block.peer',
          data: block
        });

        if (block.getHeight() > 7766000) {
          const validTxs = await validateTxs(this._engine._txHandler, block);
          if (!validTxs) {
            this.addToQuarantine(address, `invalid txs with given ${block.getHeight()}`, true);
            this._logger.info(`block submitted does not have valid txs ${block.getHeight()}`);
            return;
          }
        }
        ...
        if (latestBlock && latestBlock.getHeight() - 20 > block.getHeight()) {
          this._logger.info(`weight is low <- ${highestKnownHeight} <> ${block.getHeight()} yielding block...`);
          // if the latest block has a higher mutatation, is less than 180 seconds old and the state of the local miner is synced
          if (latestBlockChildSum - 3 > newBlockChildSum && latestBlock.getTimestamp() + 180 > currentTimeSeconds && synced === 'complete') {
            const waypointKey = conn.remoteAddress + block.getHash();
            this._engine._emitter.emit(`sendblock`, { data: block, connection: conn });
            return;
            // if the current latest block is less than 60 seconds old and the synced status is complete send a challenge block back
          } else if (latestBlock.getTimestamp() + 60 > currentTimeSeconds && synced === 'complete') {
            const waypointKey = conn.remoteAddress + block.getHash();
            this._engine._emitter.emit(`sendblock`, { data: block, connection: conn });
            return;
          }
        ...
        if (highestKnownHeight && highestKnownHeight < block.getHeight()) {
          // ensure that the latest block is at least 60 blocks ahead of the highest known height
          if (latestBlock.getHeight() + 20 > block.getHeight() && latestBlock.getTimestamp() + 20 > block.getTimestamp()) {
        ...
        if (!currentPeer && synced && synced === 'complete' && this._processedBlocks > 2) {
          ...
          if (currentPeer && currentPeer.getAddress() === addressToHost(address) && Number(new Date()) < parseInt(currentPeer.getExpires(), 10)) {
          ...
          } else if (this._processedBlocks < 9 && !currentPeer) {
            this._processedBlocks++;
            ...
            if (l && !p && parseInt(block.getHeight(), 10) > parseInt(l.getHeight(), 10) && this._PEER_QUARANTINE.indexOf(addressToHost(address)) < 0) {
              ...
              // set the initial sync to complete
              await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.initialsync`, 'complete');
              ...
              if (d) {
                ...
                if (block.getHeight() - 3 < currentHeight || currentTime < 26000) {
              ...
              // TD
              debug(`block to save block ${block.getHeight()}`);
              await this._engine.persistence.saveBlock(block);
              debug(`block to put block ${block.getHeight()}`);
              //await this._engine.persistence.putBlock(block, 0, BC_SUPER_COLLIDER, { asHeader: false, fromWaypoint: true, saveHeaders: true })
              debug(`block to peer ${block.getHeight()}`);
              //await this._engine.blockFromPeerQueue({conn: conn, newBlock: block, options: options})
              await this._engine.blockFromPeer({ conn: conn, newBlock: block, options: options });
            }
          } else {
        ...
        debug(options);
        // EMIT TO BLOCK FROM PEER
        debug(`C: block to save block ${block.getHeight()}`);
        await this._engine.persistence.saveBlock(block);
        debug(`C: block peer block ${block.getHeight()}`);
        //await this._engine.blockFromPeerQueue({conn: conn, newBlock: block, options: options})
        await this._engine.blockFromPeer({ conn: conn, newBlock: block, options: options });
      }
      ...
      const latestBlock = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.block.latest`);
      const payload = encodeTypeAndData(MESSAGES.BLOCK, latestBlock);
      const result = await this.qsend(conn, payload, true);
      if (result.success) {
      ...
      if (currentPeer && passthrough === 1 && addressToHost(currentPeer.getAddress()) === addressToHost(address)) {
        ...
        if (parseInt(latestBlock.getHeight(), 10) < parseInt(highestHeader.getHeight(), 10) && headers.length >= MAX_HEADER_RANGE) {
          ...
          const result = await this.qsend(conn, msg, true);
          if (result.success) {
          ...
          this.qsend(conn, payload, true).then(result => {
      ...
      const address = conn.remoteAddress + ':' + conn.remotePort;
      debug(`MESSAGES.GET_DATA received from waypoint ${address}`);
      const currentPeer = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.sync.initialpeer`);

      if (currentPeer && currentPeer.getAddress() === addressToHost(address)) {
        debug(`received GET_DATA request from paired waypoint ${address} during sync <- yielding...`);
        return;
      }
      ...
      if (highestKnownHeight > parseInt(latestBlock.getHeight(), 10) + 200 && synced !== 'complete' || from - 200 > parseInt(latestBlock.getHeight(), 10) && synced !== 'complete') {
        debug(`received request from waypoint which cannot be completely fulfilled...${address} from: ${from}, to: ${to}, highest known: ${highestKnownHeight}, latest edge: ${latestBlock.getHeight()}`);
        //return
      }
      ...
      // TD
      debug(`waypoint connection ${address.slice(0, 6)} low ${low} -> ${high}`);
      ...
      validBlocks = onlyBlocks.filter(async b => {
        if (b && b.getTxsList) {
          const saved = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.block.${b.getHeight()}.utxoSaved`);
          if (saved) {
            if (saved === b.getHash()) {
              return b;
            }
      ...
      debug(`sending payload of ${onlyBlocks.length} blocks, valid: ${validBlocks.length}`);
      ...
      for (const b of blocks) {
        ...
        const localBlock = await this._engine.persistence.getBlockByHash(block.getHash(), BC_SUPER_COLLIDER);
        if (!localBlock) {
          await this._engine.persistence.saveBlock(block);
        }
      }
  ...
  async broadcastNewBlock(block, latestHeight) {
    debug(`broadcasting msg to peers, ${inspect(block.toObject())}`);
    let filters = [];
    if (!latestHeight) {
      latestHeight = 0;
    }
