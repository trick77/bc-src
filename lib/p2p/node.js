...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    do {
      ...
      let prevBlock = temporaryBlockStore[newBlock.getPreviousHash()] ? temporaryBlockStore[newBlock.getPreviousHash()] : await this._engine.persistence.getBlockByHash(newBlock.getPreviousHash(), BC_SUPER_COLLIDER, { asHeader: false, cached: false });
      ...
      if (!prevBlock) {
        const prevBlockOpts = await this._engine.persistence.getBlocksByHeight(parseInt(newBlock.getHeight(), 10) - 1, BC_SUPER_COLLIDER, { asHeader: false, cached: false });
        if (prevBlockOpts) {
      ...
      if (!prevBlock && newBlockBoundary < highestKnownHeight) {
        this._logger.info(`pending arrival of local hash: ${newBlock.getPreviousHash()} to mount ${parseInt(newBlock.getHeight(), 10) - 1} : ${newBlock.getHash()}`);
      ...
      if (newBlock !== undefined && newBlock !== null && !invalidData && prevBlock && prevBlock.getHeight && parseInt(newBlock.getHeight(), 10) > 1) {
        ...
        if (!sv) {
          ...
          // LDL
          this._logger.info(`pending block ${newBlock.getHeight()} after isolated block assertion`);
        ...
        if (isValidCoinbase) {
          ...
          if (prevBlock && prevBlock.getHeight() !== newBlock.getHeight()) {
            ...
            if (mountBlockReq) {
              ...
              await this._engine.persistence.putBlock(prevBlock, 0, BC_SUPER_COLLIDER, { asHeader: false, force: true });
            ...
            if (!isValidSeq.valid && parseInt(newBlock.getHeight(), 10) === parseInt(prevBlock.getHeight(), 10) + 1) {
              ...
              temporaryBlockStore[newBlock.getHash()] = newBlock;
              await this._engine.persistence.putBlock(newBlock, 0, BC_SUPER_COLLIDER, { asHeader: false, force: true });
              continue;
            ...
            if (syncProgress > 0) {
              ...
              if (syncProgress > 99.97) {
                ...
                //let set = [' ',' ',' ',' ',' ',' ',' ',' ',' ',' ']
                //let segmentsTraversed = Math.abs(syncProgress / 10)
                //if (segmentsTraversed === 10) {
                //  segmentsTraversed = 9
                //}
                //set = set.map((s, i) => {
                //  if (i < segmentsTraversed) {
                //    return "="
                //  }
                //  return " "
                //})

                this._logger.info(`COLLIDER ${newBlock.getHash()} NRG Multichain: ${syncProgress}% <- Miner EMB Boost: +${this._engine._emblemPerformance}%`);
              }
            }
            finalValidBlocks.push(newBlock);
            validBlockHeights.push(parseInt(newBlock.getHeight(), 10));
          ...
          if (opts.innerCall && blockQueue.length > 0 && blockQueue[0].getHash() !== newBlock.getHash()) {
            ...
            if (blockQueue.length > 0 && blockQueue[0].getHeight() !== newBlock.getHeight() && validBlockHeights.indexOf(blockHeight) > -1) {
              if (prevBlock) {
                // LDL
                //blockQueue.unshift(prevBlock)
                blockQueue.push(prevBlock);
              }
              // test block next and at the end
              blockQueue.unshift(newBlock);
              blockQueue.push(newBlock);
            }
    ...
    if (blocksLength < MAX_DATA_RANGE && validBlocks > 0 && parseInt(evalBlock.getHeight()) < highestKnownHeight) {
      ...
      this._logger.warn(`waypoint unable to provide mountable block, all other blocks have been stored pending future evaluation...synced status: ${synced}`);
      const reorgBlock = await this._engine.persistence.get(`${BC_SUPER_COLLIDER}.block.reorgfrom`);
      await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.sync.initialpeer`);
      if (PEER_QUARANTINE.length <= 16 && synced === 'pending' && finalValidBlocks.length > 0 && finalValidBlocks[0].getHash) {
        PEER_QUARANTINE.push(addressToHost(address));
      ...
      if (PEER_QUARANTINE.length > 16 && PEER_BLACKLIST.length < 16) {
        for (let p of PEER_QUARANTINE) {
          if (PEER_BLACKLIST.indexOf(p) < 0) {
            PEER_BLACKLIST.push(p);
          }
        }

        if (PEER_BLACKLIST.length > 36) {
        ...
        return;
      } else if (PEER_QUARANTINE.length <= 16 && synced === 'pending' && finalValidBlocks.length > 0 && finalValidBlocks[0].getHash && PEER_BLACKLIST.length < 16) {
        PEER_QUARANTINE.push(addressToHost(address));
        const finalValidBlock = finalValidBlocks[0];
        await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.data.latest`, `${parseInt(finalValidBlock.getHeight(), 10)}:${tn}`);
        await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.edge`, parseInt(finalValidBlock.getHeight(), 10));
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      if (this._requestRegistry[peerRequestKey] > 600) {
        ...
        if (syncLookAhead > 0) {
          ...
          if (sent !== undefined && sent.success) {
            ...
            PEER_QUARANTINE.unshift(addressToHost(address));
          }
        } else {
          const data = [currentHeight, nextHeight];
          const payload = encodeTypeAndData(MESSAGES.GET_DATA, data);
      ...
      PEER_BLACKLIST = PEER_BLACKLIST.slice(0, 10);
      const now = Date.now();
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (!SEEN_BLOCKS_MEMORY[block.getHash()] && parseInt(block.getHeight(), 10) !== 1) {
        ...
        if (Object.keys(SEEN_BLOCKS_MEMORY).length > 5000) {
          for (let member in SEEN_BLOCKS_MEMORY) delete SEEN_BLOCKS_MEMORY[member];
        }
      } else {
        return;
      }
      ...
      // if the node is already busy synced ignore
      if (synced === 'pending' && reorgBlock && !this._syncComplete) {
        // LDL
        debug(`indexed block ${block.getHeight()} : ${block.getHash()}`);
        return;
      }
      ...
      if (!isolatedValidation) {
        // LDL
        this._logger.info(`processed block ${block.getHeight()} : ${block.getHash()}`);
        return;
      }
      ...
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (synced && synced === 'complete') {
          if (SEEN_BLOCKS_MEMORY[parseInt(block.getHeight(), 10) + 1] || SEEN_BLOCKS_MEMORY[parseInt(block.getHeight(), 10)]) {
