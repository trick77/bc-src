    ...
    do {
      ...
      if (newBlock && newBlock.getHeight) {
        if (evaluatedBlocks.length > 1) {
          ...
          if (!foundMount && evaluatedBlocks[evaluatedBlocks.length - 1].getHeight() === newBlock.getHeight()) {
            this._logger.info(`mounted coinbase set to previous block ${evaluatedBlocks[evaluatedBlocks.length - 1].getHash()}`);
            checkCoinbaseBlock = evaluatedBlocks[evaluatedBlocks.length - 1];
          }
        } else {
          checkCoinbaseBlock = newBlock;
        }
    ...
    if (!validcb) {
      ...
      if (remount) {
        ...
        if (unmount) {
          ...
          let scanBackHeight = checkCoinbaseBlock.getHeight() + 2040 * direction;
          ...
          if (prevFlaggedBlock) {
            ...
            scanBackHeight = prevFlaggedBlock + 2040 * direction;
          }
          await this._engine.persistence.del(`${BC_SUPER_COLLIDER}.req.range`);
