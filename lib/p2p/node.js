...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    do {
      ...
      if (this._knownHashes.indexOf(newBlock.getHash()) > -1 && blockQueue.length !== 0) {
        ...
        if (syncProgress > 0) {
          ...
          if (syncProgress > 99.99 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
            ...
            if (blockQueue.length === 0) {
              this._logger.info(`DOWNLINK #${newBlock.getHeight()}:${highestKnownHeight} ${syncProgress}% <- ${newBlock.getHash().slice(0, 20)} ${newBlock.getPreviousHash().slice(0, 20)} <- Emblem Miner Boost: +${this._engine._emblemPerformance}%`);
            }
          }
        }
        continue;
      ...
      if (!prevBlock) {
        ...
        if (prevBlock && prevBlock.getHash() !== newBlock.getPreviousHash()) {
          this._logger.info(`cannot find prev block ${newBlock.getPreviousHash()}`);
          prevBlock = false;
        }
      }
            ...
            if (syncProgress > 0) {
              ...
              if (syncProgress > 99.98 && syncedBeforeData !== 'pending' && BC_USER_QUORUM === DEFAULT_QUORUM) {
                ...
                if (syncProgress > 99.99) {
                  this._logger.info(`DOWNLINK #${newBlock.getHeight()}:${highestKnownHeight} <- ${newBlock.getHash().slice(0, 20)}:${newBlock.getPreviousHash().slice(0, 20)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}%`);
                } else if (newBlock.getHeight() % 2 === 0) {
                  this._logger.info(`DOWNLINK #${newBlock.getHeight()}:${highestKnownHeight} ${syncProgress}% <- ${newBlock.getHash().slice(0, 20)}:${newBlock.getPreviousHash().slice(0, 20)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}%`);
                } else {
                  this._logger.info(`DOWNLINK #${newBlock.getHeight()}:${highestKnownHeight} ${syncProgress}% <- ${newBlock.getPreviousHash().slice(0, 20)}:${newBlock.getHash().slice(0, 20)} <- EMBLEM MINER BOOST: +${this._engine._emblemPerformance}%`);
                }
              }
            }
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      debug(`[] -> [] Overline confirmed global state`);
      if (BC_BIND_PEER) {
        ...
        if (this._discovery.connections) {
          ...
          if (range[2]) {
            ...
            if (hasCurrentPeer && sendToPool) {
              ...
              this._logger.warn(`unable to make range request for ${lowest} to ${highest}`);
            }
          }
        } else {
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (!this._SEEN_BLOCKS_MEMORY[block.getHash()] && parseInt(block.getHeight(), 10) !== 1) {
        ...
        this._logger.info(`COLLATERAL BLOCK ${block.getHeight()} : ${block.getHash()} ${block.getDifficulty()}`);
