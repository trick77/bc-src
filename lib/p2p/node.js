...
const BC_MINIMUM_HEADER_TEST = isNaN(process.env.BC_MINIMUM_HEADER_TEST) ? 3 : process.env.BC_MINIMUM_HEADER_TEST;
...
const loadBasedPeerExpiration = 33000 + Math.floor(30000 / numCPUs);
/* OVERLINE
...
const MAX_DATA_RANGE = exports.MAX_DATA_RANGE = Number(process.env.MAX_DATA_RANGE) || 12;
        ...
        if (isValidCoinbase) {
          ...
          if (prevBlock && prevBlock.getHeight() !== newBlock.getHeight()) {
            ...
            if (!isValidSeq.valid && parseInt(newBlock.getHeight(), 10) === parseInt(prevBlock.getHeight(), 10) + 1) {
              ...
              this._logger.warn(`unconfirmed sequence at ${newBlock.getHeight()} : ${newBlock.getHash().slice(0, 8)} ${newBlock.getPreviousHash().slice(0, 8)}`);
              if (blockQueue.length === 0) {
                // shift window back
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (this._blockHeaderTest) {
        ...
        if (parseInt(block.getHeight(), 10) < parseInt(highestTestBlock.getHeight(), 10) - 5) {
          this._PEER_BLACKLIST.push(addressToHost(address));
          return;
        }
