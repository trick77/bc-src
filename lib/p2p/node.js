...
const TABLE_REQUEST_TIMEOUT = 3 * 1000; // 3 seconds used for the first request of qsend
...
class PeerNode {
  ...
  addToQuarantine(addr, reason, extended) {
    ...
    this._logger.info(`added ${addr} to quarantine <- "${reason}" (extended: ${!!extended}`);
      ...
      try {
        ...
        let tableTimeout = setTimeout(async () => {
          ...
          if (connectionInTable) {
            ...
            return;
            // the connection was not in the table and so dont attempt a given connection but fail the request
          } else {
    ...
    this._discovery.join(this._discovery.hash, this._discovery.port, data => {
      ...
      this._engine._asyncEmitter.on('announceblock', async msg => {
        ...
        Promise.all(tasks).then(a => {
          this._logger.info(`block broadcast ${msg.data.getHeight()} to ${this._discovery.connections.length} waypoints`);
        }).catch(err => {
          debug(err);
          this._logger.error(err.message);
        });
