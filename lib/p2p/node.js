...
const BC_QUARANTINE_FORGIVENESS = BC_BLOODLINE ? 5000 : getRandomWithinRange(63000, 128000);
...
const loadBasedPeerExpiration = 109000 + Math.floor(80000 / numCPUs);
...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (blocks && blocks.length > 0) {
      ...
      if (blocks.length > 55) {
        this._logger.warn(`waypoint sent block range above maximum 55`);
      }
  ...
  async peerDataHandler(conn, info, str) {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (!currentPeer && this._processedBlocks > 9 || synced && synced === 'complete' && this._processedBlocks > 9) {
          ...
          if (currentPeer && currentPeer.getAddress() === addressToHost(address) && Number(new Date()) < parseInt(currentPeer.getExpires(), 10)) {
            ...
            debug(`yielding block ${block.getHeight()} during chain sync`);
            return;
