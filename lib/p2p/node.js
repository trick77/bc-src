...
const loadBasedPeerExpiration = 65000 + Math.floor(30000 / numCPUs);
...
class PeerNode {
  ...
  constructor(engine) {
    ...
    }, getRandomWithinRange(63000, 128000));
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (blocks && blocks.length > 0) {
      ...
      // CHECK 1 blocks sent //////////////////
      if (blocks.length > BC_MAX_DATA_RANGE * 10) {
        this._logger.warn(`waypoint sent block range above maximum ${BC_MAX_DATA_RANGE * 10} <- compress to 20`);
    ...
    do {
      ...
      if (newBlock !== undefined && newBlock !== null && !invalidData && prevBlock && prevBlock.getHeight && parseInt(newBlock.getHeight(), 10) > 1) {
        ...
        flag = blockQueue.length === 0 || blockQueue.length === 8 || blockQueue.length === 12;
        ...
        if (isValidCoinbase) {
          ...
          if (prevBlock && prevBlock.getHeight() !== newBlock.getHeight()) {
            ...
            if (!isValidSeq.valid && blockQueue[0] && blockQueue[0].getHeight() !== newBlock.getHeight()) {
              ...
              debug(`isolated validation ${newBlock.getHeight()} : ${newBlock.getHash().slice(0, 8)} ${newBlock.getPreviousHash()} <- ${isolatedValidation}`);
              await this._engine.persistence.putBlock(prevBlock, 0, BC_SUPER_COLLIDER, { asHeader: false, fromWaypoint: true, saveHeaders: false });
    ...
    if (blocksLength < BC_MAX_DATA_RANGE && validBlocks > 0 && parseInt(evalBlock.getHeight(), 10) + BC_MAX_DATA_RANGE < highestKnownHeight && finalValidBlocks.length > 0 && parseInt(evalBlock.getHeight(), 10) < highestKnownHeight) {
      ...
      if (this._PEER_QUARANTINE.length >= 6 && this._PEER_BLACKLIST.length > 4) {
        ...
        return;
      } else if (this._PEER_QUARANTINE.length < 8) {
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      if (this._requestRegistry[peerRequestKey] > 600) {
        ...
        await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.req.range`, `${nextHeight}:${currentHeight + 1}:${Math.floor(now / 1000)}:na`);
        const payload = encodeTypeAndData(MESSAGES.GET_DATA, data);
        const sent = await this.qsend(conn, payload);
        if (sent !== undefined && sent.success) {
          // LDL
