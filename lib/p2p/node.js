...
const loadBasedPeerExpiration = logline ? 30000 : 50000 + Math.floor(80000 / numCPUs);
...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    const newQueue = blockQueue.reduce((all, b) => {
    ...
    if (newQueue.length === 0) {
      blocksLoadedFromMemory = blockQueue.length;
      ...
      if (this._PEER_RECORD[addressToHost(address)]) {
        if (finalValidBlocks.length > 0) {
          ...
          if (lbkh === lastSeenHash) {
            ...
            if (finalValidBlocks.length < 9 || blocksLoadedFromMemory === validBlocks) {
    ...
    if (blocksLength < BC_MAX_DATA_RANGE && validBlocks > 0 && parseInt(evalBlock.getHeight(), 10) + BC_MAX_DATA_RANGE < highestKnownHeight && finalValidBlocks.length > 0 && parseInt(evalBlock.getHeight(), 10) < highestKnownHeight) {
      ...
      if (lowestBoundary && invalidBlockHeights[0] < lowestBoundary) {
        ...
        this._engine._emitter.emit('requestBlockRange', [invalidBlockHeights[0] + BC_MAX_DATA_RANGE - 2, invalidBlockHeights[0] - 2]);
        return;
      }
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      if (currentPeer) {
        ...
        currentPeer.setExpires(Number(new Date()) + 8800);
        await this._engine.persistence.put(`${BC_SUPER_COLLIDER}.sync.initialpeer`, currentPeer);
      } else {
    ...
    if (type === MESSAGES.BLOCK) {
      ...
      if (iph === 'running' && ipd === 'pending') {
        ...
        if (!currentPeer && this._processedBlocks > 2 || synced && synced === 'complete' && this._processedBlocks > 2) {
          ...
          const fastSend = getRandomWithinRange(2, 8);
      ...
      for (let i = 0; i < blocks.length; i++) {
        ...
        if (block === null || newBlock.getHash() !== block.getHash()) {
          ...
          if (block !== null && block !== undefined) {
            ...
            this._logger.info(`uplink complete ${newBlock.getHeight()} is an update from waypoint`);
            continue;
          }
        }
