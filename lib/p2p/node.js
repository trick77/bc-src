...
const loadBasedPeerExpiration = 95000 + Math.floor(40000 / numCPUs);
...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (blocks && blocks.length > 0) {
      ...
      if (blocks.length > 310) {
        this._logger.warn(`waypoint sent block range above maximum 31 <- compress to 40`);
        blocks = blocks.slice(0, 310);
      }
      ...
      if (startGivenBlockHeight < waypointRequestLow - BC_MAX_DATA_RANGE) {
