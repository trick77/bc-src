...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    let validcb = checkCoinbaseBlock.getHeight() + 1000 > highestKnownHeight ? await validateCoinbase(checkCoinbaseBlock, this._engine.persistence, this._engine._txHandler, 'p2p.node') : true;
    //const validcb = await validateCoinbase(checkCoinbaseBlock, this._engine.persistence, this._engine._txHandler, 'p2p.node')
    ...
    if (sequenceFailed) {
      debug(`sequence failed clearing invalid block heights and final invalid blocks`);
