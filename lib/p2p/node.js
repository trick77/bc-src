...
const BC_QUARANTINE_FORGIVENESS = BC_BLOODLINE ? 15000 : getRandomWithinRange(8000, 12000);
...
class PeerNode {
  ...
  async processDataMessage(conn, blocks, opts = { innerCall: false }) {
    ...
    if (validBlocks < minimumValidBlocks && validBlocks < 1) {
      ...
      this._logger.info(`[] -> [] completed traverse to edge of global state -> ${currentHeight} <- monitoring ${this._PEER_QUARANTINE.length} sessions`);
