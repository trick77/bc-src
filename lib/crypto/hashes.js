...
const strings = {
  ...
  blake2bb(str) {
    return strings.blake2b_base(str);
  },
  blake2b(str) {
    return strings.blake2b_base(str, 'hex');
  },
  blake2bl(str) {
    return strings.blake2b_base(str, 'lite');
  },
  blake2bls(str) {
    return strings.blake2b_base(str, 'lite-sha1');
  },
  ...
  blake2s(str) {
    ...
    return stringError(str, 'failed to create blake2s of string') || avon.sumBuffer(new Buffer(str), avon.ALGORITHMS.SP).toString('hex');
  },
  ...
  privateKeyToAddress(priv) {
    ...
    publicKey = secp256k1.publicKeyCreate(new Uint8Array(privateKeyBuffer), false).slice(1);
    return '0x' + keccak('keccak256').update(Buffer.from(publicKey)).digest().slice(-20).toString('hex');
  },

  addressToIcap: ICAP.fromAddress,

  keccak(str) {
    return stringError(str, 'failed to create keccak256 of string') || keccak('keccak256').update(str).digest('hex');
  },

  doubleSha(str) {
    return stringError(str, 'failed to create sha of string') || crypto.createHash('sha256').update(crypto.createHash('sha256').update(str).digest('hex')).digest('hex');
  },

  sha(str) {
    return stringError(str, 'failed to create sha of string') || crypto.createHash('sha1').update(str).digest('hex');
  },

  sha256(str) {
    return stringError(str, 'failed to create sha of string') || crypto.createHash('sha256').update(str).digest('hex');
  },
